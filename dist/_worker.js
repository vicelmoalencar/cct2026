...
TRANSCRIÇÃO:
${T}`,N=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(N.ok){const M=((n=(s=(r=(await N.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(P=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,P,i]),e.json({transcript:T,description:P,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});I.delete("/api/admin/lessons/:id",L,async e=>{try{const t=e.req.param("id");return await q(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",Y,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=q(e);await zr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=ri(e),f=await Br(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await Po(l,s,a)){const p=await Br(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       ...
       ORDER BY lr.expires_at DESC`,[t.email]);return e.json({rentals:s})}catch(t){return console.error("Get rentals error:",t),e.json({error:t.message||"Erro ao buscar aluguéis"},500)}});async function Pe(e){await e.sql(`
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",Y,async e=>{try{const t=e.get("user"),r=q(e);await Pe(r);const s=await r.sql(`
      SELECT t.*,
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});I.get("/api/courses/:id",async e=>{try{const t=e.req.param("id"),r=q(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.query("modules",{select:"*",filters:{course_id:t},order:"order_index"}),i=await r.sql(`SELECT l.* FROM lessons l
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const a of i){const c=o.get(a.module_id)||[];c.push(a),o.set(a.module_id,c)}for(const a of n)a.lessons=o.get(a.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});I.get("/api/lessons/:id",async e=>{var t,r,s;try{const n=e.req.param("id"),i=$(e,"sb-access-token");let o=null;if(i){const v=await J(i,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);v&&(o=v.email)}const a=q(e);let c=!1,u=!1;if(o)try{const v=await a.rpc("user_has_lesson_access",{email_usuario:o,lesson_id:parseInt(n)});if(console.log("Access check result:",v),Array.isArray(v)&&v.length>0?c=v[0].user_has_lesson_access||v[0]===!0||!!v[0]:typeof v=="boolean"?c=v:v&&typeof v=="object"&&(c=!!v.user_has_lesson_access),console.log("Has access:",c,"User:",o,"Lesson:",n),!c)if((await a.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[o,parseInt(n)])).length>0)c=!0;else{const y=await a.sql("SELECT rentable, rental_credits, title FROM lessons WHERE id = $1",[parseInt(n)]);return console.log("❌ Access denied for user:",o,"lesson:",n),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((t=y[0])==null?void 0:t.rentable)||!1,rental_credits:((r=y[0])==null?void 0:r.rental_credits)||0,lesson_title:((s=y[0])==null?void 0:s.title)||""},403)}console.log("✅ Access granted for user:",o,"lesson:",n)}catch(v){console.error("❌ Error checking access via RPC:",v),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),u=!0,c=!0}if(!o||!c&&!u){const v=await a.query("lessons",{select:"teste_gratis",filters:{id:n},single:!0});if(!(v!=null&&v.teste_gratis))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}const f=await a.sql(`
      SELECT l.*, m.title as module_title, c.title as course_title, c.id as course_id
      FROM lessons l
      LEFT JOIN modules m ON l.module_id = m.id
      LEFT JOIN courses c ON m.course_id = c.id
      WHERE l.id = $1
    `,[parseInt(n)]);if(!f||f.length===0)return e.json({error:"Lesson not found"},404);const d=f[0],p=await a.query("comments",{select:"*",filters:{lesson_id:n},order:"created_at DESC"}),b=await a.sql(`
      SELECT
        t.id as trail_id,
