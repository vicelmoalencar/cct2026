...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id/modules",async e=>{try{const t=e.req.param("id"),r=P(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.sql(`SELECT m.*,
       ...
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const l of i){const u=o.get(l.module_id)||[];u.push(l),o.set(l.module_id,u)}for(const l of n)l.lessons=o.get(l.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});R.get("/api/search/lessons",K,async e=>{try{const t=e.get("user"),r=P(e);await Et(r);const s=(e.req.query("q")||"").trim(),n=e.req.query("course_id"),i=e.req.query("type")||"all",o=Math.max(parseInt(e.req.query("min_duration")||"0",10),0),l=Math.min(Math.max(parseInt(e.req.query("max_duration")||"999",10),o),999),u=Math.min(Math.max(parseInt(e.req.query("limit")||"80",10),1),120),d=e.req.query("sort")||"relevance",a=[t.email],f=["1=1"];let c=a.length+1;n&&(f.push(`co.id = $${c++}`),a.push(parseInt(n,10))),f.push(`COALESCE(l.duration_minutes, 0) BETWEEN $${c++} AND $${c++}`),a.push(o,l),i==="free"?f.push("COALESCE(l.teste_gratis, false) = true"):i==="premium"?f.push("COALESCE(l.teste_gratis, false) = false"):i==="rented"&&f.push("lr.lesson_id IS NOT NULL");const p=s.length>=2?s.split(/\s+/).filter(Boolean).slice(0,6):[];for(const S of p)f.push(`(
        l.title ILIKE $${c}
        OR COALESCE(l.description, '') ILIKE $${c}
        OR COALESCE(l.transcript, '') ILIKE $${c}
        OR m.title ILIKE $${c}
        OR co.title ILIKE $${c}
      )`),a.push(`%${S}%`),c++;const h=p[0]||"";a.push(h);const b=`$${c++}`;a.push(`%${s}%`);const v=`$${c++}`;a.push(u);const m=`$${c++}`;let y="relevance_score DESC, co.title ASC, m.order_index ASC, l.order_index ASC";d==="title"&&(y="l.title ASC"),d==="duration"&&(y="COALESCE(l.duration_minutes, 0) ASC, l.title ASC"),d==="date"&&(y="l.created_at DESC NULLS LAST, l.id DESC");const w=await r.sql(`
      SELECT l.id,
             l.title,
             l.description,
             l.duration_minutes,
      ...
      ORDER BY ${y}
      LIMIT ${m}
    `,a);return e.json({lessons:w,limit:u})}catch(t){return console.error("❌ /api/search/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to search lessons"},500)}});R.get("/api/modules/:id/lessons",async e=>{try{const t=e.req.param("id"),s=await P(e).sql(`SELECT *
       FROM lessons
       WHERE module_id = $1
       ORDER BY order_index`,[t]);return e.json({lessons:s||[]})}catch(t){return console.error("❌ /api/modules/:id/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch module lessons"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,n;try{const i=e.req.param("id"),o=V(e,"sb-access-token");let l=null;if(o){const m=await G(o,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);m&&(l=m.email)}const u=P(e);let d=!1,a=!1;if(l)try{if(d=!!((t=(await u.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[l,parseInt(i)]))[0])!=null&&t.has_access),console.log("Has access:",d,"User:",l,"Lesson:",i),!d){const y=e.env.DATABASE_SUITEPLUS,[w,S,I]=await Promise.all([u.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[l,parseInt(i)]),u.sql(`SELECT id FROM member_subscriptions
               WHERE lower(email_membro) = lower($1)
                 AND data_expiracao > NOW()
                 AND COALESCE(ativo, true) = true
                 AND COALESCE(teste_gratis, false) = false
               LIMIT 1`,[l]),y?xt(l,y):Promise.resolve(null)]);if((w.length>0||S.length>0||I&&I>new Date)&&(d=!0),!d){const A=await u.sql("SELECT rentable, rental_credits, title FROM lessons WHERE id = $1",[parseInt(i)]);return console.log("❌ Access denied for user:",l,"lesson:",i),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((r=A[0])==null?void 0:r.rentable)||!1,rental_credits:((s=A[0])==null?void 0:s.rental_credits)||0,lesson_title:((n=A[0])==null?void 0:n.title)||""},403)}}console.log("✅ Access granted for user:",l,"lesson:",i)}catch(m){console.error("❌ Error checking access via RPC:",m),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),a=!0,d=!0}if(!l||!d&&!a){const m=await u.query("lessons",{select:"teste_gratis",filters:{id:i},single:!0});if(!(m!=null&&m.teste_gratis))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await _t(u);const f=`
