...
TRANSCRIÇÃO:
${R}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const B=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(B){R=B;const F=B.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(N=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[R,N,n]),e.json({transcript:R,description:N,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});T.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});T.post("/api/lessons/:id/rent",X,async e=>{var t;try{const r=parseInt(e.req.param("id")),i=e.get("user").email,n=L(e);await St(n);const o=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[r]);if(!o.length||!o[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const l=o[0],u=parseInt(l.rental_credits);if(!Number.isFinite(u)||u<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const d=await n.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[i,r]);if(d.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:d[0].expires_at},400);let f=!!((t=(await n.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[i,r]))[0])!=null&&t.has_access);if(!f){const m=e.env.DATABASE_SUITEPLUS,[y,_]=await Promise.all([n.sql(`SELECT id FROM member_subscriptions
           WHERE lower(email_membro) = lower($1)
             AND data_expiracao > NOW()
             AND COALESCE(ativo, true) = true
             AND COALESCE(teste_gratis, false) = false
           LIMIT 1`,[i]),m?ct(i,m):Promise.resolve(null)]);f=y.length>0||_!==null&&_>new Date}if(f)return e.json({success:!0,alreadyHasAccess:!0,message:"Você já tem acesso a esta aula pela sua assinatura ativa — não foi cobrado nenhum crédito."});const c=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!c,"| prefix:",c==null?void 0:c.substring(0,30));const p=Xr(e),h=await zr(p,i);if(h<u)return e.json({error:"Créditos insuficientes",available:h,required:u},400);if(!await Po(p,i,u)){const m=await zr(p,i);return e.json({error:"Créditos insuficientes",available:m,required:u},400)}await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[i,r,u]);const v=new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"});return await qo(e.env,["🎬 *Aula alugada!*","",`📧 *Email:* ${i}`,`📖 *Aula:* ${l.title}`,`💳 *Créditos usados:* ${u}`,`🕐 *Data:* ${v}`].join(`
