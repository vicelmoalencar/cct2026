...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      `):e.redirect(`/?error=${r}`);const s=t.searchParams.get("access_token")||((c=t.hash.match(/access_token=([^&]+)/))==null?void 0:c[1]),n=t.searchParams.get("refresh_token")||((u=t.hash.match(/refresh_token=([^&]+)/))==null?void 0:u[1]),i=t.searchParams.get("type")||((l=t.hash.match(/type=([^&]+)/))==null?void 0:l[1]);if(!s)return e.redirect("/?error=no_token");try{const p=(f=JSON.parse(atob(s.split(".")[1])).amr)==null?void 0:f.some(g=>g.method==="otp");if(i==="recovery"||p)return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}catch{if(i==="recovery")return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}return se(e,"sb-access-token",s,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),n&&se(e,"sb-refresh-token",n,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.redirect("/?auth=success")}I.post("/api/auth/callback",async e=>{try{const{access_token:t,refresh_token:r}=await e.req.json();return t?(se(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),r&&se(e,"sb-refresh-token",r,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0})):e.json({error:"No access token"},400)}catch{return e.json({error:"Callback failed"},500)}});I.post("/api/auth/forgot-password",async e=>{try{const{email:t}=await e.req.json();if(!t)return e.json({error:"Email is required"},400);const r=e.req.header("host")||"localhost:3000",n=`${r.includes("localhost")?"http":"https"}://${r}/auth/callback`,i=await fetch(`${e.env.SUPABASE_URL}/auth/v1/recover`,{method:"POST",headers:{"Content-Type":"application/json",apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({email:t,options:{redirectTo:n}})});if(i.ok)return e.json({success:!0,message:"Se o email estiver cadastrado, você receberá um link de recuperação. O link é válido por 1 hora."});const o=await i.json();return e.json({error:o.error_description||"Failed to send reset email"},400)}catch{return e.json({error:"Failed to process request"},500)}});I.post("/api/auth/reset-password",async e=>{try{const t=await e.req.json(),{token:r,password:s}=t;if(console.log("🔐 Password reset attempt"),console.log("   Token present:",!!r),console.log("   Token length:",r==null?void 0:r.length),console.log("   Password length:",s==null?void 0:s.length),!r||!s)return console.error("❌ Missing token or password"),e.json({error:"Token e senha são obrigatórios"},400);if(s.length<6)return console.error("❌ Password too short"),e.json({error:"A senha deve ter pelo menos 6 caracteres"},400);console.log("📨 Calling Supabase to update password...");const n=await fetch(`${e.env.SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({password:s})});if(console.log("📨 Supabase response:",{status:n.status,ok:n.ok}),!n.ok){const o=await n.json();console.error("❌ Password reset failed:",o);let a="Falha ao redefinir senha";return o.error_code==="same_password"?a="A nova senha deve ser diferente da senha atual":o.msg?a=o.msg:o.error_description?a=o.error_description:o.message&&(a=o.message),e.json({error:a},400)}const i=await n.json();return console.log("✅ Password reset successful"),i.access_token&&se(e,"sb-access-token",i.access_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),i.refresh_token&&se(e,"sb-refresh-token",i.refresh_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0,message:"Senha alterada com sucesso!"})}catch{return e.json({error:"Failed to reset password"},500)}});async function Fr(e,t,r,s){try{return await new Zi(t,r).query("users",{select:"id, email, isadmin",filters:{email:e,isadmin:!0},single:!0},s)!==null}catch(n){return console.error("Error checking admin access in Supabase users:",n),!1}}async function q(e,t){const r=G(e,"sb-access-token");if(!r)return e.json({error:"Unauthorized"},401);const s=await J(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!s)return e.json({error:"Invalid token"},401);if(!await Fr(s.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))return e.json({error:"Forbidden - Admin only"},403);e.set("user",s),await t()}I.get("/api/admin/check",async e=>{const t=G(e,"sb-access-token");if(!t)return e.json({isAdmin:!1});const r=await J(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({isAdmin:!1});const s=await Fr(r.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,t);return e.json({isAdmin:s})});I.post("/api/admin/impersonate",q,async e=>{try{const{user_email:t}=await e.req.json();if(!t)return e.json({error:"user_email is required"},400);console.log(`🎭 Admin impersonating user: ${t}`);const s=await D(e).query("users",{select:"*",filters:{email:t}});if(!s||s.length===0)return e.json({error:"User not found"},404);const n=s[0],i={email:t,nome:n.nome||"Usuário",impersonated:!0,impersonated_at:new Date().toISOString(),user_id:n.id,signature:Buffer.from(`${t}:${e.env.SUPABASE_ANON_KEY}`).toString("base64")},o=`IMPERSONATE:${Buffer.from(JSON.stringify(i)).toString("base64")}`;return console.log(`✅ Impersonation token created for ${t}`),e.json({token:o,user_email:t,user_name:n.nome})}catch(t){return console.error("Impersonation error:",t),e.json({error:t.message||"Failed to impersonate user"},500)}});I.post("/api/admin/courses",q,async e=>{try{const{title:t,description:r,duration_hours:s,instructor:n,offers_certificate:i,is_published:o}=await e.req.json(),c=await D(e).insert("courses",{title:t,description:r||null,duration_hours:s||0,instructor:n||"Vicelmo",offers_certificate:i!==void 0?i:!0,is_published:o!==void 0?o:!0});return e.json({success:!0,course_id:c[0].id})}catch(t){return console.error("Create course error:",t),e.json({error:t.message||"Failed to create course"},500)}});I.put("/api/admin/courses/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,duration_hours:n,instructor:i,offers_certificate:o,is_published:a}=await e.req.json();return await D(e).update("courses",{id:t},{title:r,description:s||null,duration_hours:n||0,instructor:i||"Vicelmo",offers_certificate:o!==void 0?o:!0,is_published:a!==void 0?a:!0}),e.json({success:!0})}catch{return e.json({error:"Failed to update course"},500)}});I.delete("/api/admin/courses/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("courses",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete course"},500)}});I.get("/api/admin/courses/find",q,async e=>{try{const t=e.req.query("title");if(!t)return e.json({error:"Title is required"},400);const s=await D(e).query("courses",{select:"*",filters:{title:t},limit:1});return s&&s.length>0?e.json({course:s[0]}):e.json({course:null})}catch(t){return console.error("Find course error:",t),e.json({error:t.message||"Failed to find course"},500)}});I.post("/api/admin/modules",q,async e=>{try{const{course_id:t,title:r,description:s,order_index:n}=await e.req.json(),o=await D(e).insert("modules",{course_id:t,title:r,description:s||null,order_index:n||0});return e.json({success:!0,module_id:o[0].id})}catch(t){return console.error("Create module error:",t),e.json({error:t.message||"Failed to create module"},500)}});I.put("/api/admin/modules/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,order_index:n}=await e.req.json();return await D(e).update("modules",{id:t},{title:r,description:s||null,order_index:n}),e.json({success:!0})}catch{return e.json({error:"Failed to update module"},500)}});I.delete("/api/admin/modules/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("modules",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete module"},500)}});I.get("/api/admin/modules/find",q,async e=>{try{const t=e.req.query("course_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"course_id and title are required"},400);const n=await D(e).query("modules",{select:"*",filters:{course_id:t,title:r},limit:1});return n&&n.length>0?e.json({module:n[0]}):e.json({module:null})}catch(t){return console.error("Find module error:",t),e.json({error:t.message||"Failed to find module"},500)}});I.post("/api/admin/lessons",q,async e=>{try{const{module_id:t,title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:a,free_trial:c,support_text:u,transcript:l,attachments:f,rentable:d,rental_credits:p}=await e.req.json();let g=null;n&&i&&(n==="youtube"?g=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?g=`https://vimeo.com/${i}`:g=i);const v=await D(e).insert("lessons",{module_id:t,title:r,description:s||null,video_url:g,video_provider:n||null,video_id:i||null,duration_minutes:o||0,order_index:a||0,teste_gratis:c||!1,support_text:u||null,transcript:l||null,attachments:JSON.stringify(f||[]),rentable:d||!1,rental_credits:p||0});return e.json({success:!0,lesson_id:v[0].id})}catch(t){return console.error("Create lesson error:",t),e.json({error:t.message||"Failed to create lesson"},500)}});I.post("/api/admin/lessons-reorder",q,async e=>{try{const{lessons:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"lessons must be an array"},400);const r=D(e);for(const{id:s,order_index:n}of t)await r.update("lessons",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder lessons error:",t),e.json({error:t.message||"Failed to reorder lessons"},500)}});I.put("/api/admin/lessons/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:a,free_trial:c,support_text:u,transcript:l,attachments:f,rentable:d,rental_credits:p}=await e.req.json();let g=null;n&&i&&(n==="youtube"?g=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?g=`https://vimeo.com/${i}`:g=i);const b=D(e),v=parseInt(t);await b.update("lessons",{id:v},{title:r,description:s||null,video_url:g,video_provider:n||null,video_id:i||null,duration_minutes:parseInt(o)||0,order_index:parseInt(a)||0,teste_gratis:c===!0||c==="true"});try{await b.sql("UPDATE lessons SET support_text = $1, transcript = $2, attachments = $3::jsonb WHERE id = $4",[u||null,l||null,JSON.stringify(f||[]),v])}catch(m){console.warn("support_text/transcript/attachments columns may not exist:",m.message)}return await b.sql("UPDATE lessons SET rentable = $1, rental_credits = $2 WHERE id = $3",[d===!0||d==="true",parseInt(p)||0,v]),e.json({success:!0})}catch(t){return console.error("Update lesson error:",t),e.json({error:t.message||"Failed to update lesson"},500)}});I.post("/api/admin/lessons/:id/generate-transcript",q,async e=>{var t,r,s;try{const n=parseInt(e.req.param("id")),{context:i}=await e.req.json(),a=await D(e).sql("SELECT title, transcript FROM lessons WHERE id = $1",[n]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(!c.transcript)return e.json({error:"Esta aula não possui transcrição para estruturar."},400);const u=e.env.VITE_OPENROUTER_API_KEY;if(!u)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada no ambiente"},500);const l="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",f=`Abaixo está a transcrição bruta de uma aula chamada "${c.title}".${i?`
...
---
${i}
---`,l=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:u}]})});if(!l.ok)return e.json({error:`OpenRouter: ${await l.text()}`},500);const f=await l.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});I.post("/api/admin/vimeo-transcript",q,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((g,b)=>{const v=m=>{let y=0;return String(m.language||"").toLowerCase().startsWith("pt")&&(y+=40),m.active!==!1&&(y+=20),m.type==="captions"&&(y+=10),y};return v(b)-v(g)}).find(g=>g.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const u=await fetch(c.link);if(!u.ok)return e.json({error:`Download da legenda falhou: ${u.status}`},502);const f=(await u.text()).replace(/^﻿/,"").split(/\r?\n/).map(g=>g.trim()),d=[];for(const g of f){if(!g||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(g)||g.includes("-->"))continue;const b=g.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();b&&d[d.length-1]!==b&&d.push(b)}const p=d.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/lessons/:id/whisper-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=D(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.OPENAI_API_KEY||u.OPENAI_API_KEY||u["OPENAI_API-KEY"],d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const E=Object.keys(u).filter(O=>O.toLowerCase().includes("openai"));return e.json({error:`OPENAI_API_KEY não configurado. Chaves OPENAI encontradas: [${E.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(E=>E.link&&E.size).sort((E,O)=>E.size-O.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],m=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let x=Buffer.from(await y.arrayBuffer()),A="video.mp4";const N=24*1024*1024;if(x.length>N){try{const E=await import("os"),O=await import("path"),j=await import("fs"),M=await import("child_process"),H=O.join(E.tmpdir(),`cct_w_${i}.mp4`),z=O.join(E.tmpdir(),`cct_w_${i}.mp3`);j.writeFileSync(H,x),M.spawnSync("ffmpeg",["-y","-i",H,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&j.existsSync(z)&&(x=j.readFileSync(z),A="audio.mp3",j.unlinkSync(z)),j.unlinkSync(H)}catch{}if(x.length>N)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const _="----WhisperBoundary"+Date.now().toString(16),h=`\r
`,S=A.endsWith(".mp3")?"audio/mpeg":"video/mp4",w=Buffer.concat([Buffer.from(`--${_}${h}Content-Disposition: form-data; name="file"; filename="${A}"${h}Content-Type: ${S}${h}${h}`),x,Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="model"${h}${h}whisper-1`),Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="language"${h}${h}pt`),Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="response_format"${h}${h}text`),Buffer.from(`${h}--${_}--${h}`)]),C=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${_}`,"Content-Length":String(w.length)},body:w});if(!C.ok)return e.json({error:`OpenAI Whisper ${C.status}: ${await C.text()}`},502);let T=(await C.text()).trim(),R=null;if(d&&T){const E=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${T}`,O=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(O.ok){const M=((n=(s=(r=(await O.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const H=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);H&&(R=H[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,R,i]),e.json({transcript:T,description:R,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});I.post("/api/admin/lessons/:id/groq-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=D(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||u.GROQ_API_KEY,d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const E=Object.keys(u).filter(O=>O.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${E.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(E=>E.link&&E.size).sort((E,O)=>E.size-O.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],m=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let x=Buffer.from(await y.arrayBuffer()),A="video.mp4";const N=24*1024*1024;if(x.length>N){try{const E=await import("os"),O=await import("path"),j=await import("fs"),M=await import("child_process"),H=O.join(E.tmpdir(),`cct_g_${i}.mp4`),z=O.join(E.tmpdir(),`cct_g_${i}.mp3`);j.writeFileSync(H,x),M.spawnSync("ffmpeg",["-y","-i",H,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&j.existsSync(z)&&(x=j.readFileSync(z),A="audio.mp3",j.unlinkSync(z)),j.unlinkSync(H)}catch{}if(x.length>N)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const _="----GroqBoundary"+Date.now().toString(16),h=`\r
`,S=A.endsWith(".mp3")?"audio/mpeg":"video/mp4",w=Buffer.concat([Buffer.from(`--${_}${h}Content-Disposition: form-data; name="file"; filename="${A}"${h}Content-Type: ${S}${h}${h}`),x,Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="model"${h}${h}whisper-large-v3-turbo`),Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="language"${h}${h}pt`),Buffer.from(`${h}--${_}${h}Content-Disposition: form-data; name="response_format"${h}${h}text`),Buffer.from(`${h}--${_}--${h}`)]),C=await fetch("https://api.groq.com/openai/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${_}`,"Content-Length":String(w.length)},body:w});if(!C.ok)return e.json({error:`Groq Whisper ${C.status}: ${await C.text()}`},502);let T=(await C.text()).trim(),R=null;if(d&&T){const E=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${T}`,O=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(O.ok){const M=((n=(s=(r=(await O.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const H=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);H&&(R=H[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,R,i]),e.json({transcript:T,description:R,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});I.delete("/api/admin/lessons/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=D(e);await Ur(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=$n(e),f=await kr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await _o(l,s,a)){const p=await kr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[s,t,a]),e.json({success:!0,message:"Aula alugada com sucesso! Acesso liberado por 30 dias."})}catch(t){return console.error("Rent lesson error:",t),e.json({error:t.message||"Erro ao processar aluguel"},500)}});I.get("/api/user/rentals",K,async e=>{try{const t=e.get("user"),r=D(e);await Ur(r);const s=await r.sql(`SELECT lr.id, lr.lesson_id, lr.credits_paid, lr.rented_at, lr.expires_at,
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=D(e);await Pe(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});I.get("/api/admin/trails",q,async e=>{try{const t=D(e);await Pe(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails",q,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=D(e);await Pe(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});I.put("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=D(e);await Pe(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.delete("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=D(e);return await Pe(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/lessons",q,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=D(e);await Pe(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});I.delete("/api/admin/trails/:id/lessons/:lessonId",q,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await D(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/reorder",q,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=D(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/trails/search-lessons",q,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=D(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
      ...
      LIMIT 50
    `,o);return e.json({lessons:a})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/lessons/find",q,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const n=await D(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return n&&n.length>0?e.json({lesson:n[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});I.post("/api/admin/run-migration-lesson-fields",q,async e=>{try{const t=D(e);return await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS support_text TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS transcript TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'::jsonb"),await Ur(t),e.json({success:!0,message:"Migration applied successfully"})}catch(t){return console.error("Migration error:",t),e.json({error:t.message},500)}});I.get("/api/admin/users",q,async e=>{try{const r=await D(e).query("users",{select:"*",order:"created_at DESC"});return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});I.get("/api/admin/users/find",q,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await D(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});I.post("/api/admin/users",q,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await D(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});I.put("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await D(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});I.delete("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});I.get("/api/admin/certificates",q,async e=>{try{const r=await D(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});I.get("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),s=await D(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});I.get("/api/admin/certificates/find",q,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await D(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});I.post("/api/admin/certificates",q,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=D(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});I.put("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=D(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});I.delete("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function To(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const u=s?e.modules.map((f,d)=>`<div class="mod-item"><span class="mod-num">${String(d+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",l=r?`
...
<body>
  ...
  <div class="page">
    ...
    <div class="logo-section">
      <img class="logo-img" src="${$r}" alt="Ensino Plus"/>
    </div>
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `)}catch(t){return console.error("Verify certificate error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});I.get("/api/verify/:code",async e=>{try{const t=e.req.param("code"),r=D(e),s=await r.query("certificates",{select:"*",filters:{certificate_code:t}});if(!s||s.length===0)return e.json({valid:!1,message:"Certificate not found"},404);const n=s[0];try{await r.update("certificates",{id:n.id},{verification_count:(n.verification_count||0)+1})}catch{}return e.json({valid:!0,certificate:{student_name:n.user_name,course_title:n.course_title,workload:n.carga_horaria,completion_date:n.completion_date,issued_at:n.issued_at,certificate_code:n.certificate_code,verification_count:(n.verification_count||0)+1}})}catch(t){return console.error("Verify certificate API error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});I.get("/api/admin/member-subscriptions",q,async e=>{try{const r=await D(e).query("member_subscriptions",{select:"*",order:"created_at DESC"});return e.json({subscriptions:r||[]})}catch(t){return console.error("List member subscriptions error:",t),e.json({error:t.message||"Failed to list member subscriptions"},500)}});I.get("/api/admin/suiteplus-subscriptions",q,async e=>{try{const t=e.env.DATABASE_SUITEPLUS;if(!t)return e.json({error:"DATABASE_SUITEPLUS não configurada"},500);const s=await new vt(t).sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
       FROM user_subscriptions
       ORDER BY expires_at DESC`);return e.json({subscriptions:s})}catch(t){return console.error("SuitePlus subscriptions error:",t),e.json({error:t.message||"Erro ao buscar assinaturas SuitePlus"},500)}});I.get("/api/admin/member-subscriptions/find",q,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email parameter is required"},400);const s=await D(e).query("member_subscriptions",{select:"*",filters:{email_membro:t}});return e.json({subscriptions:s||[]})}catch(t){return console.error("Find member subscription error:",t),e.json({error:t.message||"Failed to find member subscription"},500)}});I.post("/api/admin/member-subscriptions",q,async e=>{try{const t=await e.req.json();if(!t.email_membro)return e.json({error:"Email is required"},400);const s=await D(e).insert("member_subscriptions",{email_membro:t.email_membro,data_expiracao:t.data_expiracao||null,detalhe:t.detalhe||null,origem:t.origem||null,teste_gratis:t.teste_gratis||!1,ativo:t.ativo!==void 0?t.ativo:!0});return e.json({success:!0,subscription_id:s&&s.length>0?s[0].id:null})}catch(t){return console.error("Create member subscription error:",t),e.json({error:t.message||"Failed to create member subscription"},500)}});I.put("/api/admin/member-subscriptions/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await D(e).update("member_subscriptions",{id:t},{email_membro:r.email_membro,data_expiracao:r.data_expiracao,detalhe:r.detalhe,origem:r.origem,teste_gratis:r.teste_gratis,ativo:r.ativo,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update member subscription error:",t),e.json({error:t.message||"Failed to update member subscription"},500)}});I.delete("/api/admin/member-subscriptions/:id",q,async e=>{try{const t=e.req.param("id");return await D(e).delete("member_subscriptions",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});I.get("/api/courses",async e=>{try{const t=D(e),r=G(e,"sb-access-token");let s=!1;if(r){const a=await J(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);a&&(s=await Fr(a.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))}const n=s?{}:{is_published:!0},i=s?"":"WHERE c.is_published = true",o=await t.sql(`
      SELECT c.*,
...
<p>Abrindo aula...</p>
...
</html>`)});I.get("/api/favorites",K,async e=>{const t=e.get("user"),r=D(e);try{const s=await r.sql(`
    ...
    `,[t.email]);return e.json(s)}finally{await r.end()}});I.post("/api/favorites",K,async e=>{const t=e.get("user"),r=await e.req.json();if(!r.lesson_id)return e.json({error:"lesson_id required"},400);const s=D(e);try{return await s.sql("INSERT INTO user_favorites (user_email, lesson_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",[t.email,r.lesson_id]),e.json({ok:!0})}finally{await s.end()}});I.delete("/api/favorites/:lessonId",K,async e=>{const t=e.get("user"),r=parseInt(e.req.param("lessonId")),s=D(e);try{return await s.sql("DELETE FROM user_favorites WHERE user_email = $1 AND lesson_id = $2",[t.email,r]),e.json({ok:!0})}finally{await s.end()}});I.get("/api/favorites/check/:lessonId",K,async e=>{const t=e.get("user"),r=parseInt(e.req.param("lessonId")),s=D(e);try{const n=await s.sql("SELECT id FROM user_favorites WHERE user_email = $1 AND lesson_id = $2 LIMIT 1",[t.email,r]);return e.json({favorite:n.length>0})}finally{await s.end()}});I.get("/favorites",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-50">
  ...
  `));const Ts=new Xn,Po=Object.assign({"/src/index.tsx":I});let ei=!1;for(const[,e]of Object.entries(Po))e&&(Ts.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ts.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),ei=!0);if(!ei)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var rr={exports:{}},sr={exports:{}},Ne={},nr={},Ps;function ti(){if(Ps)return nr;Ps=1,nr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return nr}var ir,Rs;function ri(){if(Rs)return ir;Rs=1;var e=ti();return ir={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},ir}var or,Is;function Ro(){if(Is)return or;Is=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;or=function(u){if(s.test(u))return Number(u.replace("i","I"));var l=e.exec(u);if(!l)return n(u)||null;var f=!!l[8],d=parseInt(l[1],10);f&&(d=o(d));var p=parseInt(l[2],10)-1,g=l[3],b=parseInt(l[4],10),v=parseInt(l[5],10),m=parseInt(l[6],10),y=l[7];y=y?1e3*parseFloat(y):0;var x,A=i(u);return A!=null?(x=new Date(Date.UTC(d,p,g,b,v,m,y)),a(d)&&x.setUTCFullYear(d),A!==0&&x.setTime(x.getTime()-A)):(x=new Date(d,p,g,b,v,m,y),a(d)&&x.setFullYear(d)),x};function n(c){var u=t.exec(c);if(u){var l=parseInt(u[1],10),f=!!u[4];f&&(l=o(l));var d=parseInt(u[2],10)-1,p=u[3],g=new Date(l,d,p);return a(l)&&g.setFullYear(l),g}}function i(c){if(c.endsWith("+00"))return 0;var u=r.exec(c.split(" ")[1]);if(u){var l=u[1];if(l==="Z")return 0;var f=l==="-"?-1:1,d=parseInt(u[2],10)*3600+parseInt(u[3]||0,10)*60+parseInt(u[4]||0,10);return d*f*1e3}}function o(c){return-(c-1)}function a(c){return c>=0&&c<100}return or}var ar,Os;function Io(){if(Os)return ar;Os=1,ar=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return ar}var lr,Ns;function Oo(){if(Ns)return lr;Ns=1;var e=Io();lr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,b(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(m){var y=this[m]||0;return m==="seconds"&&this.milliseconds&&(y=(y+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),y+" "+m},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(y,this).join(""),m=i.map(y,this).join("");return"P"+v+"T"+m;function y(x){var A=this[x]||0;return x==="seconds"&&this.milliseconds&&(A=(A+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),A+s[x]}};var o="([+-]?\\d+)",a=o+"\\s+years?",c=o+"\\s+mons?",u=o+"\\s+days?",l="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([a,c,u,l].map(function(v){return"("+v+")?"}).join("\\s*")),d={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function g(v){var m=v+"000000".slice(v.length);return parseInt(m,10)/1e3}function b(v){if(!v)return{};var m=f.exec(v),y=m[8]==="-";return Object.keys(d).reduce(function(x,A){var N=d[A],_=m[N];return!_||(_=A==="milliseconds"?g(_):parseInt(_,10),!_)||(y&&~p.indexOf(A)&&(_*=-1),x[A]=_),x},{})}return lr}var cr,Ds;function No(){if(Ds)return cr;Ds=1;var e=Buffer.from||Buffer;return cr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},cr}var ur,js;function Do(){if(js)return ur;js=1;var e=ti(),t=ri(),r=Ro(),s=Oo(),n=No();function i(h){return function(w){return w===null?w:h(w)}}function o(h){return h===null?h:h==="TRUE"||h==="t"||h==="true"||h==="y"||h==="yes"||h==="on"||h==="1"}function a(h){return h?e.parse(h,o):null}function c(h){return parseInt(h,10)}function u(h){return h?e.parse(h,i(c)):null}function l(h){return h?e.parse(h,i(function(S){return y(S).trim()})):null}var f=function(h){if(!h)return null;var S=t.create(h,function(w){return w!==null&&(w=A(w)),w});return S.parse()},d=function(h){if(!h)return null;var S=t.create(h,function(w){return w!==null&&(w=parseFloat(w)),w});return S.parse()},p=function(h){if(!h)return null;var S=t.create(h);return S.parse()},g=function(h){if(!h)return null;var S=t.create(h,function(w){return w!==null&&(w=r(w)),w});return S.parse()},b=function(h){if(!h)return null;var S=t.create(h,function(w){return w!==null&&(w=s(w)),w});return S.parse()},v=function(h){return h?e.parse(h,i(n)):null},m=function(h){return parseInt(h,10)},y=function(h){var S=String(h);return/^\d+$/.test(S)?S:h},x=function(h){return h?e.parse(h,i(JSON.parse)):null},A=function(h){return h[0]!=="("?null:(h=h.substring(1,h.length-1).split(","),{x:parseFloat(h[0]),y:parseFloat(h[1])})},N=function(h){if(h[0]!=="<"&&h[1]!=="(")return null;for(var S="(",w="",C=!1,T=2;T<h.length-1;T++){if(C||(S+=h[T]),h[T]===")"){C=!0;continue}else if(!C)continue;h[T]!==","&&(w+=h[T])}var R=A(S);return R.radius=parseFloat(w),R},_=function(h){h(20,y),h(21,m),h(23,m),h(26,m),h(700,parseFloat),h(701,parseFloat),h(16,o),h(1082,r),h(1114,r),h(1184,r),h(600,A),h(651,p),h(718,N),h(1e3,a),h(1001,v),h(1005,u),h(1007,u),h(1028,u),h(1016,l),h(1017,f),h(1021,d),h(1022,d),h(1231,d),h(1014,p),h(1015,p),h(1008,p),h(1009,p),h(1040,p),h(1041,p),h(1115,g),h(1182,g),h(1185,g),h(1186,s),h(1187,b),h(17,n),h(114,JSON.parse.bind(JSON)),h(3802,JSON.parse.bind(JSON)),h(199,x),h(3807,x),h(3907,p),h(2951,p),h(791,p),h(1183,p),h(1270,p)};return ur={init:_},ur}var dr,qs;function jo(){if(qs)return dr;qs=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",a,c,u,l,f,d;{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}return a=s%e,c=4294967296*a+n,u=""+c%e,i+u+o}return dr=t,dr}var fr,ks;function qo(){if(ks)return fr;ks=1;var e=jo(),t=function(p,g,b,v,m){b=b||0,v=v||!1,m=m||function(C,T,R){return C*Math.pow(2,R)+T};var y=b>>3,x=function(C){return v?~C&255:C},A=255,N=8-b%8;g<N&&(A=255<<8-g&255,N=g),b&&(A=A>>b%8);var _=0;b%8+g>=8&&(_=m(0,x(p[y])&A,N));for(var h=g+b>>3,S=y+1;S<h;S++)_=m(_,x(p[S]),8);var w=(g+b)%8;return w>0&&(_=m(_,x(p[h])>>8-w,w)),_},r=function(p,g,b){var v=Math.pow(2,b-1)-1,m=t(p,1),y=t(p,b,1);if(y===0)return 0;var x=1,A=function(_,h,S){_===0&&(_=1);for(var w=1;w<=S;w++)x/=2,(h&1<<S-w)>0&&(_+=x);return _},N=t(p,g,b+1,!1,A);return y==Math.pow(2,b+1)-1?N===0?m===0?1/0:-1/0:NaN:(m===0?1:-1)*Math.pow(2,y-v)*N},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},a=function(p){var g=t(p,16,32);if(g==49152)return NaN;for(var b=Math.pow(1e4,t(p,16,16)),v=0,m=t(p,16),y=0;y<m;y++)v+=t(p,16,64+16*y)*b,b/=1e4;var x=Math.pow(10,t(p,16,48));return(g===0?1:-1)*Math.round(v*x)/x},c=function(p,g){var b=t(g,1),v=t(g,63,1),m=new Date((b===0?1:-1)*v/1e3+9466848e5);return p||m.setTime(m.getTime()+m.getTimezoneOffset()*6e4),m.usec=v%1e3,m.getMicroSeconds=function(){return this.usec},m.setMicroSeconds=function(y){this.usec=y},m.getUTCMicroSeconds=function(){return this.usec},m},u=function(p){var g=t(p,32);t(p,32,32);for(var b=t(p,32,64),v=96,m=[],y=0;y<g;y++)m[y]=t(p,32,v),v+=32,v+=32;var x=function(N){var _=t(p,32,v);if(v+=32,_==4294967295)return null;var h;if(N==23||N==20)return h=t(p,_*8,v),v+=_*8,h;if(N==25)return h=p.toString(this.encoding,v>>3,(v+=_<<3)>>3),h;console.log("ERROR: ElementType not implemented: "+N)},A=function(N,_){var h=[],S;if(N.length>1){var w=N.shift();for(S=0;S<w;S++)h[S]=A(N,_);N.unshift(w)}else for(S=0;S<N[0];S++)h[S]=x(_);return h};return A(m,b)},l=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},d=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,a),p(700,i),p(701,o),p(16,f),p(1114,c.bind(null,!1)),p(1184,c.bind(null,!0)),p(1e3,u),p(1007,u),p(1016,u),p(1008,u),p(1009,u),p(25,l)};return fr={init:d},fr}var pr,Ls;function ko(){return Ls||(Ls=1,pr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),pr}var Bs;function wt(){if(Bs)return Ne;Bs=1;var e=Do(),t=qo(),r=ri(),s=ko();Ne.getTypeParser=o,Ne.setTypeParser=a,Ne.arrayParser=r,Ne.builtins=s;var n={text:{},binary:{}};function i(c){return String(c)}function o(c,u){return u=u||"text",n[u]&&n[u][c]||i}function a(c,u,l){typeof u=="function"&&(l=u,u="text"),n[u][c]=l}return e.init(function(c,u){n.text[c]=u}),t.init(function(c,u){n.binary[c]=u}),Ne}var Ms;function xt(){return Ms||(Ms=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=wt(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(sr)),sr.exports}var hr,Us;function $e(){if(Us)return hr;Us=1;const e=xt(),t=He,{isDate:r}=t.types||t;function s(d){return'"'+d.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(d){let p="{";for(let g=0;g<d.length;g++)if(g>0&&(p=p+","),d[g]===null||typeof d[g]>"u")p=p+"NULL";else if(Array.isArray(d[g]))p=p+n(d[g]);else if(ArrayBuffer.isView(d[g])){let b=d[g];if(!(b instanceof Buffer)){const v=Buffer.from(b.buffer,b.byteOffset,b.byteLength);v.length===b.byteLength?b=v:b=v.slice(b.byteOffset,b.byteOffset+b.byteLength)}p+="\\\\x"+b.toString("hex")}else p+=s(i(d[g]));return p=p+"}",p}const i=function(d,p){if(d==null)return null;if(typeof d=="object"){if(d instanceof Buffer)return d;if(ArrayBuffer.isView(d)){const g=Buffer.from(d.buffer,d.byteOffset,d.byteLength);return g.length===d.byteLength?g:g.slice(d.byteOffset,d.byteOffset+d.byteLength)}return r(d)?e.parseInputDatesAsUTC?c(d):a(d):Array.isArray(d)?n(d):o(d,p)}return d.toString()};function o(d,p){if(d&&typeof d.toPostgres=="function"){if(p=p||[],p.indexOf(d)!==-1)throw new Error('circular reference detected while preparing "'+d+'" for query');return p.push(d),i(d.toPostgres(i),p)}return JSON.stringify(d)}function a(d){let p=-d.getTimezoneOffset(),g=d.getFullYear();const b=g<1;b&&(g=Math.abs(g)+1);let v=String(g).padStart(4,"0")+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0")+"T"+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")+":"+String(d.getSeconds()).padStart(2,"0")+"."+String(d.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),b&&(v+=" BC"),v}function c(d){let p=d.getUTCFullYear();const g=p<1;g&&(p=Math.abs(p)+1);let b=String(p).padStart(4,"0")+"-"+String(d.getUTCMonth()+1).padStart(2,"0")+"-"+String(d.getUTCDate()).padStart(2,"0")+"T"+String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")+":"+String(d.getUTCSeconds()).padStart(2,"0")+"."+String(d.getUTCMilliseconds()).padStart(3,"0");return b+="+00:00",g&&(b+=" BC"),b}function u(d,p,g){return d=typeof d=="string"?{text:d}:d,p&&(typeof p=="function"?d.callback=p:d.values=p),g&&(d.callback=g),d}return hr={prepareValue:function(p){return i(p)},normalizeQueryConfig:u,escapeIdentifier:function(d){return'"'+d.replace(/"/g,'""')+'"'},escapeLiteral:function(d){let p=!1,g="'";if(d==null||typeof d!="string")return"''";for(let b=0;b<d.length;b++){const v=d[b];v==="'"?g+=v+v:v==="\\"?(g+=v+v,p=!0):g+=v}return g+="'",p===!0&&(g=" E"+g),g}},hr}var lt={exports:{}},mr,Hs;function Lo(){if(Hs)return mr;Hs=1;const e=Cn;function t(a){return e.createHash("md5").update(a,"utf-8").digest("hex")}function r(a,c,u){const l=t(c+a);return"md5"+t(Buffer.concat([Buffer.from(l),u]))}function s(a){return e.createHash("sha256").update(a).digest()}function n(a,c){return a=a.replace(/(\D)-/,"$1"),e.createHash(a).update(c).digest()}function i(a,c){return e.createHmac("sha256",a).update(c).digest()}async function o(a,c,u){return e.pbkdf2Sync(a,c,u,32,"sha256")}return mr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},mr}var gr,Fs;function Bo(){if(Fs)return gr;Fs=1;const e=Cn;gr={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:l,sha256:a,hashByName:c,hmacSha256:u,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,g=await r.digest("MD5",p);return Array.from(new Uint8Array(g)).map(b=>b.toString(16).padStart(2,"0")).join("")}}async function o(f,d,p){const g=await i(d+f);return"md5"+await i(Buffer.concat([Buffer.from(g),p]))}async function a(f){return await r.digest("SHA-256",f)}async function c(f,d){return await r.digest(f,d)}async function u(f,d){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(d))}async function l(f,d,p){const g=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),b={name:"PBKDF2",hash:"SHA-256",salt:d,iterations:p};return await r.deriveBits(b,g,256,["deriveBits"])}return gr}var zs;function si(){return zs||(zs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?lt.exports=Lo():lt.exports=Bo()),lt.exports}var br,Qs;function Mo(){if(Qs)return br;Qs=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let a=i[o++];if(a<128)return{length:a,index:o};const c=a&127;if(c>4)throw e("bad length",i);a=0;for(let u=0;u<c;u++)a=a<<8|i[o++];return{length:a,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:a,index:c}=t(i,o);o=c;const u=o+a,l=i[o++];let f=(l/40>>0)+"."+l%40;for(;o<u;){let d=0;for(;o<u;){const p=i[o++];if(d=d<<7|p&127,p<128)break}f+="."+d}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:a,index:c}=s(i,o);o=c+a,o=s(i,o).index;const{oid:u,index:l}=r(i,o);switch(u){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=l,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+u,i)}return br={signatureAlgorithmHashFromCertificate:n},br}var yr,Vs;function Uo(){if(Vs)return yr;Vs=1;const e=si(),{signatureAlgorithmHashFromCertificate:t}=Mo();function r(f,d){const p=["SCRAM-SHA-256"];d&&p.unshift("SCRAM-SHA-256-PLUS");const g=p.find(m=>f.includes(m));if(!g)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(g==="SCRAM-SHA-256-PLUS"&&typeof d.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const b=e.randomBytes(18).toString("base64");return{mechanism:g,clientNonce:b,response:(g==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":d?"y":"n")+",,n=*,r="+b,message:"SASLInitialResponse"}}async function s(f,d,p,g){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(d==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const b=c(p);if(b.nonce.startsWith(f.clientNonce)){if(b.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,m="r="+b.nonce+",s="+b.salt+",i="+b.iteration;let y=g?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const E=g.getPeerCertificate().raw;let O=t(E);(O==="MD5"||O==="SHA-1")&&(O="SHA-256");const j=await e.hashByName(O,E);y=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(j)]).toString("base64")}const x="c="+y+",r="+b.nonce,A=v+","+m+","+x,N=Buffer.from(b.salt,"base64"),_=await e.deriveKey(d,N,b.iteration),h=await e.hmacSha256(_,"Client Key"),S=await e.sha256(h),w=await e.hmacSha256(S,A),C=l(Buffer.from(h),Buffer.from(w)).toString("base64"),T=await e.hmacSha256(_,"Server Key"),R=await e.hmacSha256(T,A);f.message="SASLResponse",f.serverSignature=Buffer.from(R).toString("base64"),f.response=x+",p="+C}function n(f,d){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=u(d);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((d,p)=>f.charCodeAt(p)).every(d=>d>=33&&d<=43||d>=45&&d<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function a(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(d=>{if(!/^.=/.test(d))throw new Error("SASL: Invalid attribute pair entry");const p=d[0],g=d.substring(2);return[p,g]}))}function c(f){const d=a(f),p=d.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const g=d.get("s");if(g){if(!o(g))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const b=d.get("i");if(b){if(!/^[1-9][0-9]*$/.test(b))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(b,10);return{nonce:p,salt:g,iteration:v}}function u(f){const p=a(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function l(f,d){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(d))throw new TypeError("second argument must be a Buffer");if(f.length!==d.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,g)=>f[g]^d[g]))}return yr={startSession:r,continueSession:s,finalizeSession:n},yr}var vr,Xs;function zr(){if(Xs)return vr;Xs=1;const e=wt();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},vr=t,vr}var wr,Ws;function Ho(){if(Ws)return wr;Ws=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const a={};let c,u=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(i,"postgres://base")}catch{c=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),u=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of c.searchParams.entries())a[p[0]]=p[1];if(a.user=a.user||decodeURIComponent(c.username),a.password=a.password||decodeURIComponent(c.password),c.protocol=="socket:")return a.host=decodeURI(c.pathname),a.database=c.searchParams.get("db"),a.client_encoding=c.searchParams.get("encoding"),a;const l=u?"":c.hostname;a.host?l&&/^%2f/i.test(l)&&(c.pathname=l+c.pathname):a.host=decodeURIComponent(l),a.port||(a.port=c.port);const f=c.pathname.slice(1)||null;a.database=f?decodeURI(f):null,(a.ssl==="true"||a.ssl==="1")&&(a.ssl=!0),a.ssl==="0"&&(a.ssl=!1),(a.sslcert||a.sslkey||a.sslrootcert||a.sslmode)&&(a.ssl={});const d=a.sslcert||a.sslkey||a.sslrootcert?Tn:null;if(a.sslcert&&(a.ssl.cert=d.readFileSync(a.sslcert).toString()),a.sslkey&&(a.ssl.key=d.readFileSync(a.sslkey).toString()),a.sslrootcert&&(a.ssl.ca=d.readFileSync(a.sslrootcert).toString()),o.useLibpqCompat&&a.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(a.uselibpqcompat==="true"||o.useLibpqCompat)switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":{a.ssl.rejectUnauthorized=!1;break}case"require":{a.sslrootcert?a.ssl.checkServerIdentity=function(){}:a.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!a.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");a.ssl.checkServerIdentity=function(){};break}}else switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{a.sslmode!=="verify-full"&&n(a.sslmode);break}case"no-verify":{a.ssl.rejectUnauthorized=!1;break}}return a}function t(i){return Object.entries(i).reduce((a,[c,u])=>(u!=null&&(a[c]=u),a),{})}function r(i){return Object.entries(i).reduce((a,[c,u])=>{if(c==="ssl"){const l=u;typeof l=="boolean"&&(a[c]=l),typeof l=="object"&&(a[c]=t(l))}else if(u!=null)if(c==="port"){if(u!==""){const l=parseInt(u,10);if(isNaN(l))throw new Error(`Invalid ${c}: ${u}`);a[c]=l}}else a[c]=u;return a},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
