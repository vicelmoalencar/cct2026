...
TRANSCRIÇÃO:
${T}`,N=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(N.ok){const M=((n=(s=(r=(await N.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(R=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,R,i]),e.json({transcript:T,description:R,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});I.delete("/api/admin/lessons/:id",L,async e=>{try{const t=e.req.param("id");return await q(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",Y,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=q(e);await zr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=ri(e),f=await Br(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await Ro(l,s,a)){const p=await Br(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
...
<body>
  ...
  <div class="page">
    ...
    <div class="logo-section">
      <img class="logo-img" src="${rs}" alt="Ensino Plus"/>
    </div>
