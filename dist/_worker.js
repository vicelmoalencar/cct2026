...
TRANSCRIÇÃO:
${T}`,O=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(O.ok){const M=((n=(s=(r=(await O.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const H=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);H&&(R=H[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,R,i]),e.json({transcript:T,description:R,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});I.delete("/api/admin/lessons/:id",j,async e=>{try{const t=e.req.param("id");return await D(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=D(e);await Ur(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=$n(e),f=await jr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await _o(l,s,a)){const p=await jr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=D(e);await Pe(r);const s=await r.sql(`
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});I.get("/api/admin/trails",j,async e=>{try{const t=D(e);await Pe(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails",j,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=D(e);await Pe(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});I.put("/api/admin/trails/:id",j,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=D(e);await Pe(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.delete("/api/admin/trails/:id",j,async e=>{try{const t=parseInt(e.req.param("id")),r=D(e);return await Pe(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/lessons",j,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=D(e);await Pe(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});I.delete("/api/admin/trails/:id/lessons/:lessonId",j,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await D(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/reorder",j,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=D(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/trails/search-lessons",j,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=D(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
