...
${w}`,pendingAction:{tool:I,args:A,description:w}})}const g=await jo(I,A,u,e);p.push({role:"assistant",content:b.content||null,tool_calls:_}),p.push({role:"tool",content:JSON.stringify(g),tool_call_id:x.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(i){return console.error("Agent error:",i),e.json({error:i.message||"Erro no agente"},500)}});const Bo=[{type:"function",function:{name:"list_courses",description:"Lista todos os cursos publicados disponíveis na plataforma, com id, título e carga horária.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"get_course_details",description:"Retorna os módulos e a quantidade de aulas de um curso publicado.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso"}},required:["course_id"]}}},{type:"function",function:{name:"get_my_progress",description:"Retorna o progresso do aluno autenticado em todos os cursos que ele já iniciou: % concluído, aulas completadas e aulas totais por curso.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"recommend_next_lessons",description:"Recomenda as próximas aulas que o aluno autenticado ainda não assistiu. Se course_id for informado, recomenda dentro daquele curso; caso contrário, considera todos os cursos que o aluno já iniciou.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso (opcional)"},limit:{type:"number",description:"Máximo de aulas a recomendar (padrão 5)"}}}}},{type:"function",function:{name:"search_lessons",description:"Busca aulas por título/palavra-chave em cursos publicados.",parameters:{type:"object",properties:{query:{type:"string",description:"Termo de busca no título da aula"},limit:{type:"number",description:"Máximo de resultados (padrão 10)"}},required:["query"]}}},{type:"function",function:{name:"get_my_certificates",description:"Lista os certificados já emitidos para o aluno autenticado.",parameters:{type:"object",properties:{}}}}];async function Mo(e,t,r,s){switch(e){case"list_courses":return{courses:await r.sql("SELECT id, title, instructor, duration_hours FROM courses WHERE is_published = true ORDER BY title")};case"get_course_details":{const[i,n]=await Promise.all([r.sql("SELECT id, title, description, instructor, duration_hours FROM courses WHERE id = $1 AND is_published = true",[t.course_id]),r.sql(`SELECT m.id, m.title, m.order_index, COUNT(l.id)::int as lesson_count
         ...
         GROUP BY c.id, c.title
         HAVING COUNT(up.lesson_id) FILTER (WHERE up.completed = true) > 0
         ORDER BY completed_lessons DESC`,[s])).map(o=>({...o,progress_pct:o.total_lessons>0?Math.round(o.completed_lessons/o.total_lessons*100):0}));return{courses_with_progress:n,total_courses:n.length}}case"recommend_next_lessons":{const i=Math.min(t.limit||5,20),n=t.course_id?"AND c.id = $2":"",o=t.course_id?[s,t.course_id]:[s],l=await r.sql(`SELECT l.id as lesson_id, l.title as lesson_title, m.title as module_title,
         ...
         LIMIT ${i}`,o);return{recommended_lessons:l,count:l.length}}case"search_lessons":{const i=Math.min(t.limit||10,50),n=await r.sql(`SELECT l.id, l.title, m.title as module_title, c.title as course_title, c.id as course_id
         FROM lessons l JOIN modules m ON m.id = l.module_id JOIN courses c ON c.id = m.course_id
         WHERE l.title ILIKE $1 AND c.is_published = true
         ORDER BY l.title LIMIT $2`,[`%${t.query}%`,i]);return{lessons:n,count:n.length}}case"get_my_certificates":return{certificates:await r.sql(`SELECT id, course_title, carga_horaria, certificate_code, completion_date FROM certificates
         WHERE lower(user_email) = lower($1) ORDER BY created_at DESC`,[s])};default:return{error:`Ferramenta desconhecida: ${e}`}}}R.post("/api/student-agent",J,async e=>{var t,r;try{const s=e.get("user"),i=s.email,n=await e.req.json(),{message:o,history:l=[]}=n,u=L(e),d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);let f=[{role:"system",content:`Você é o assistente virtual do CCT (Clube de Cálculo Trabalhista), atendendo o aluno "${((t=s.user_metadata)==null?void 0:t.name)||s.email}".
...
Se o aluno perguntar sobre cancelamento, reembolso, cobrança ou alteração de plano, explique gentilmente que você ainda não pode resolver isso diretamente e oriente a falar com o suporte pelo WhatsApp.`},...l.slice(-20),{role:"user",content:o}];for(let c=0;c<5;c++){const p=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Student Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:f,tools:Bo,tool_choice:"auto"})});if(!p.ok)return e.json({error:`OpenRouter: ${await p.text()}`},500);const y=(r=(await p.json()).choices)==null?void 0:r[0];if(!y)return e.json({error:"Resposta vazia do modelo"},500);const v=y.message,m=v.tool_calls;if(!m||m.length===0)return e.json({reply:v.content||""});const b=m[0],_=b.function.name;let x={};try{x=JSON.parse(b.function.arguments)}catch{}const I=await Mo(_,x,u,i);f.push({role:"assistant",content:v.content||null,tool_calls:m}),f.push({role:"tool",content:JSON.stringify(I),tool_call_id:b.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(s){return console.error("Student agent error:",s),e.json({error:s.message||"Erro no assistente"},500)}});R.get("/api/admin/users",q,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `,[r,t]))[0]||{};return e.json({start_date:n.start_date||null,completion_date:n.completion_date||null})}catch(t){return console.error("Suggest certificate dates error:",t),e.json({error:t.message||"Failed to suggest dates"},500)}});R.post("/api/admin/certificates",q,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),i=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,start_date:t.start_date||null,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:i&&i.length>0?i[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],i=L(e),n=[];for(const l of s){const u=m=>{const b=String(m??"").trim();return!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined"?"":b},d=u(l.user_email||l.email).toLowerCase(),a=u(l.user_name||l.nome)||null,f=u(l.course_title||l.curso);let c=l.carga_horaria?parseInt(String(l.carga_horaria)):null;if(c!==null&&!Number.isFinite(c)&&(c=null),!d.includes("@")||!f){n.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const b=String(m).trim();if(!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(b)){const[_,x,I]=b.split("/");return`${I}-${x}-${_}T12:00:00Z`}return isNaN(new Date(b).getTime())?null:new Date(b).toISOString()},h=p(l.data_final||l.completion_date),y=p(l.data_inicio||l.data_inicial||l.start_date);if(!c){const m=await i.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)c=parseInt(m[0].duration_hours);else{const b=x=>x.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),_=b(f);if(_.length>0){const I=(await i.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=b(A.title).join(" ");return _.every(w=>g.includes(w))});I&&(c=parseInt(I.duration_hours))}}}const v=await i.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await i.sql(`UPDATE certificates
           SET user_name = COALESCE($1, user_name),
               ...
               updated_at = NOW()
           WHERE id = $5`,[a,c,h,y,v[0].id]),n.push({user_email:d,course_title:f,status:"updated",certificate_id:v[0].id});else{const m=new Uint8Array(4);crypto.getRandomValues(m);const b="CCT-"+new Date().getFullYear()+"-"+Array.from(m).map(I=>I.toString(16).padStart(2,"0")).join("").toUpperCase(),_=new Date().toISOString(),x=await i.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, start_date, created_at)
           VALUES ($1, $2, $3, $4, $5, $5, $6, COALESCE($7, $6::timestamp), $8::date, $6)
           RETURNING id`,[d,a||"Aluno",f,c,b,_,h,y]);n.push({user_email:d,course_title:f,status:"created",certificate_id:(t=x[0])==null?void 0:t.id,verification_code:b})}}const o=n.filter(l=>l.status==="error").length;return e.json({success:o===0,total:s.length,results:n})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let i=r.course_title;if(r.course_id){const n=await s.query("courses",{select:"title",filters:{id:r.course_id}});n&&n.length>0&&(i=n[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:i,carga_horaria:r.carga_horaria,start_date:r.start_date||null,completion_date:r.completion_date||null,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Uo(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,c)=>`<div class="mod-item"><span class="mod-num">${String(c+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=r?`
