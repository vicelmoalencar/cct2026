...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      `):e.redirect(`/?error=${r}`);const s=t.searchParams.get("access_token")||((u=t.hash.match(/access_token=([^&]+)/))==null?void 0:u[1]),i=t.searchParams.get("refresh_token")||((d=t.hash.match(/refresh_token=([^&]+)/))==null?void 0:d[1]),n=t.searchParams.get("type")||((a=t.hash.match(/type=([^&]+)/))==null?void 0:a[1]);if(!s)return e.redirect("/?error=no_token");try{const p=(f=JSON.parse(atob(s.split(".")[1])).amr)==null?void 0:f.some(h=>h.method==="otp");if(n==="recovery"||p)return e.redirect(`/reset-password#access_token=${s}&refresh_token=${i||""}&type=recovery`)}catch{if(n==="recovery")return e.redirect(`/reset-password#access_token=${s}&refresh_token=${i||""}&type=recovery`)}return G(e,"sb-access-token",s,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),i&&G(e,"sb-refresh-token",i,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.redirect("/?auth=success")}R.post("/api/auth/callback",async e=>{try{const{access_token:t,refresh_token:r}=await e.req.json();return t?(G(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),r&&G(e,"sb-refresh-token",r,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0})):e.json({error:"No access token"},400)}catch{return e.json({error:"Callback failed"},500)}});R.post("/api/auth/forgot-password",async e=>{try{const{email:t}=await e.req.json();if(!t)return e.json({error:"Email is required"},400);const r=e.req.header("host")||"localhost:3000",i=`${r.includes("localhost")?"http":"https"}://${r}/auth/callback`,n=await fetch(`${e.env.SUPABASE_URL}/auth/v1/recover`,{method:"POST",headers:{"Content-Type":"application/json",apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({email:t,options:{redirectTo:i}})});if(n.ok)return e.json({success:!0,message:"Se o email estiver cadastrado, você receberá um link de recuperação. O link é válido por 1 hora."});const o=await n.json();return e.json({error:o.error_description||"Failed to send reset email"},400)}catch{return e.json({error:"Failed to process request"},500)}});R.post("/api/auth/reset-password",async e=>{try{const t=await e.req.json(),{token:r,password:s}=t;if(console.log("🔐 Password reset attempt"),console.log("   Token present:",!!r),console.log("   Token length:",r==null?void 0:r.length),console.log("   Password length:",s==null?void 0:s.length),!r||!s)return console.error("❌ Missing token or password"),e.json({error:"Token e senha são obrigatórios"},400);if(s.length<6)return console.error("❌ Password too short"),e.json({error:"A senha deve ter pelo menos 6 caracteres"},400);console.log("📨 Calling Supabase to update password...");const i=await fetch(`${e.env.SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({password:s})});if(console.log("📨 Supabase response:",{status:i.status,ok:i.ok}),!i.ok){const o=await i.json();console.error("❌ Password reset failed:",o);let l="Falha ao redefinir senha";return o.error_code==="same_password"?l="A nova senha deve ser diferente da senha atual":o.msg?l=o.msg:o.error_description?l=o.error_description:o.message&&(l=o.message),e.json({error:l},400)}const n=await i.json();return console.log("✅ Password reset successful"),n.access_token&&G(e,"sb-access-token",n.access_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),n.refresh_token&&G(e,"sb-refresh-token",n.refresh_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0,message:"Senha alterada com sucesso!"})}catch{return e.json({error:"Failed to reset password"},500)}});async function Jr(e,t,r,s){try{return await new no(t,r).query("users",{select:"id, email, isadmin",filters:{email:e,isadmin:!0},single:!0},s)!==null}catch(i){return console.error("Error checking admin access in Supabase users:",i),!1}}async function P(e,t){const r=V(e,"sb-access-token");if(!r)return e.json({error:"Unauthorized"},401);const s=await K(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!s)return e.json({error:"Invalid token"},401);if(!await Jr(s.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))return e.json({error:"Forbidden - Admin only"},403);e.set("user",s),await t()}R.get("/api/admin/check",async e=>{const t=V(e,"sb-access-token");if(!t)return e.json({isAdmin:!1});const r=await K(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({isAdmin:!1});const s=await Jr(r.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,t);return e.json({isAdmin:s})});R.post("/api/admin/impersonate",P,async e=>{try{const{user_email:t}=await e.req.json();if(!t)return e.json({error:"user_email is required"},400);console.log(`🎭 Admin impersonating user: ${t}`);const r=L(e);let s=await r.sql("SELECT id, email, nome FROM users WHERE lower(email) = lower($1) LIMIT 1",[t]);if((!s||s.length===0)&&(s=await r.sql(`SELECT NULL::integer AS id, email_membro AS email, NULL::text AS nome
         FROM member_subscriptions
         WHERE lower(email_membro) = lower($1)
         ORDER BY data_expiracao DESC NULLS LAST
         LIMIT 1`,[t])),!s||s.length===0)return e.json({error:"User not found"},404);const i=s[0],n={email:t,nome:i.nome||"Usuário",impersonated:!0,impersonated_at:new Date().toISOString(),user_id:i.id,signature:Buffer.from(`${t}:${e.env.SUPABASE_ANON_KEY}`).toString("base64")},o=`IMPERSONATE:${Buffer.from(JSON.stringify(n)).toString("base64")}`,l=V(e,"sb-access-token");return l&&G(e,"sb-admin-backup-token",l,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),G(e,"sb-access-token",o,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),console.log(`✅ Impersonation session set for ${t}`),e.json({success:!0,user_email:t,user_name:i.nome})}catch(t){return console.error("Impersonation error:",t),e.json({error:t.message||"Failed to impersonate user"},500)}});R.post("/api/admin/exit-impersonation",async e=>{try{const t=V(e,"sb-admin-backup-token");return t?(G(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),et(e,"sb-admin-backup-token"),e.json({success:!0})):e.json({error:"No admin session to restore"},400)}catch(t){return console.error("Exit impersonation error:",t),e.json({error:t.message||"Failed to exit impersonation"},500)}});R.post("/api/admin/courses",P,async e=>{try{const{title:t,description:r,duration_hours:s,instructor:i,offers_certificate:n,is_published:o,min_completion_days:l}=await e.req.json(),d=await L(e).insert("courses",{title:t,description:r||null,duration_hours:s||0,instructor:i||"Vicelmo",offers_certificate:n!==void 0?n:!0,is_published:o!==void 0?o:!0,min_completion_days:l||null});return e.json({success:!0,course_id:d[0].id})}catch(t){return console.error("Create course error:",t),e.json({error:t.message||"Failed to create course"},500)}});R.put("/api/admin/courses/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,duration_hours:i,instructor:n,offers_certificate:o,is_published:l,min_completion_days:u}=await e.req.json();return await L(e).update("courses",{id:t},{title:r,description:s||null,duration_hours:i||0,instructor:n||"Vicelmo",offers_certificate:o!==void 0?o:!0,is_published:l!==void 0?l:!0,min_completion_days:u||null}),e.json({success:!0})}catch{return e.json({error:"Failed to update course"},500)}});R.delete("/api/admin/courses/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("courses",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete course"},500)}});R.get("/api/admin/courses/find",P,async e=>{try{const t=e.req.query("title");if(!t)return e.json({error:"Title is required"},400);const r=L(e),s=await r.sql("SELECT * FROM courses WHERE lower(title) = lower($1) LIMIT 1",[t]);if(s&&s.length>0)return e.json({course:s[0]});const i=await r.sql("SELECT * FROM courses WHERE lower(title) ILIKE lower($1) LIMIT 1",[`%${t}%`]);return e.json({course:i&&i.length>0?i[0]:null})}catch(t){return console.error("Find course error:",t),e.json({error:t.message||"Failed to find course"},500)}});R.post("/api/admin/modules",P,async e=>{try{const{course_id:t,title:r,description:s,order_index:i}=await e.req.json(),o=await L(e).insert("modules",{course_id:t,title:r,description:s||null,order_index:i||0});return e.json({success:!0,module_id:o[0].id})}catch(t){return console.error("Create module error:",t),e.json({error:t.message||"Failed to create module"},500)}});R.post("/api/admin/modules-reorder",P,async e=>{try{const{modules:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"modules must be an array"},400);const r=L(e);for(const{id:s,order_index:i}of t)await r.update("modules",{id:Number(s)},{order_index:Number(i)});return e.json({success:!0})}catch(t){return console.error("Reorder modules error:",t),e.json({error:t.message||"Failed to reorder modules"},500)}});R.put("/api/admin/modules/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,order_index:i}=await e.req.json();return await L(e).update("modules",{id:t},{title:r,description:s||null,order_index:i}),e.json({success:!0})}catch{return e.json({error:"Failed to update module"},500)}});R.delete("/api/admin/modules/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("modules",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete module"},500)}});R.get("/api/admin/modules/find",P,async e=>{try{const t=e.req.query("course_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"course_id and title are required"},400);const i=await L(e).query("modules",{select:"*",filters:{course_id:t,title:r},limit:1});return i&&i.length>0?e.json({module:i[0]}):e.json({module:null})}catch(t){return console.error("Find module error:",t),e.json({error:t.message||"Failed to find module"},500)}});R.post("/api/admin/lessons",P,async e=>{try{const{module_id:t,title:r,description:s,video_provider:i,video_id:n,duration_minutes:o,order_index:l,free_trial:u,support_text:d,transcript:a,attachments:f,rentable:c,rental_credits:p}=await e.req.json();let h=null;i&&n&&(i==="youtube"?h=`https://www.youtube.com/watch?v=${n}`:i==="vimeo"?h=`https://vimeo.com/${n}`:h=n);const v=await L(e).insert("lessons",{module_id:t,title:r,description:s||null,video_url:h,video_provider:i||null,video_id:n||null,duration_minutes:o||0,order_index:l||0,teste_gratis:u||!1,support_text:d||null,transcript:a||null,attachments:JSON.stringify(f||[]),rentable:c||!1,rental_credits:p||0});return e.json({success:!0,lesson_id:v[0].id})}catch(t){return console.error("Create lesson error:",t),e.json({error:t.message||"Failed to create lesson"},500)}});R.post("/api/admin/lessons-reorder",P,async e=>{try{const{lessons:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"lessons must be an array"},400);const r=L(e);for(const{id:s,order_index:i}of t)await r.update("lessons",{id:Number(s)},{order_index:Number(i)});return e.json({success:!0})}catch(t){return console.error("Reorder lessons error:",t),e.json({error:t.message||"Failed to reorder lessons"},500)}});R.put("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,video_provider:i,video_id:n,duration_minutes:o,order_index:l,free_trial:u,support_text:d,transcript:a,attachments:f,rentable:c,rental_credits:p}=await e.req.json();let h=null;i&&n&&(i==="youtube"?h=`https://www.youtube.com/watch?v=${n}`:i==="vimeo"?h=`https://vimeo.com/${n}`:h=n);const y=L(e),v=parseInt(t);await y.update("lessons",{id:v},{title:r,description:s||null,video_url:h,video_provider:i||null,video_id:n||null,duration_minutes:parseInt(o)||0,order_index:parseInt(l)||0,teste_gratis:u===!0||u==="true"});try{await y.sql("UPDATE lessons SET support_text = $1, transcript = $2, attachments = $3::jsonb WHERE id = $4",[d||null,a||null,JSON.stringify(f||[]),v])}catch(m){console.warn("support_text/transcript/attachments columns may not exist:",m.message)}return await y.sql("UPDATE lessons SET rentable = $1, rental_credits = $2 WHERE id = $3",[c===!0||c==="true",parseInt(p)||0,v]),e.json({success:!0})}catch(t){return console.error("Update lesson error:",t),e.json({error:t.message||"Failed to update lesson"},500)}});R.post("/api/admin/lessons/:id/generate-transcript",P,async e=>{var t,r,s;try{const i=parseInt(e.req.param("id")),{context:n}=await e.req.json(),l=await L(e).sql("SELECT title, transcript FROM lessons WHERE id = $1",[i]);if(!l.length)return e.json({error:"Aula não encontrada"},404);const u=l[0];if(!u.transcript)return e.json({error:"Esta aula não possui transcrição para estruturar."},400);const d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada no ambiente"},500);const a="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",f=`Abaixo está a transcrição bruta de uma aula chamada "${u.title}".${n?`
...
TRANSCRIÇÃO:
${C}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const M=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,n]),e.json({transcript:C,description:O,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});R.post("/api/admin/lessons/:id/groq-transcribe",P,async e=>{var t,r,s,i;try{const n=parseInt(e.req.param("id")),o=L(e),l=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[n]);if(!l.length)return e.json({error:"Aula não encontrada"},404);const u=l[0];if(u.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!u.video_id||!/^\d+$/.test(u.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const d=((t=globalThis.process)==null?void 0:t.env)||{},a=e.env.VIMEO_ACCESS_TOKEN||d.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||d.GROQ_API_KEY,c=e.env.VITE_OPENROUTER_API_KEY||d.VITE_OPENROUTER_API_KEY;if(!a)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const S=Object.keys(d).filter(q=>q.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${S.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${u.video_id}?fields=download`,{headers:{Authorization:`Bearer ${a}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const y=((await p.json()).download||[]).filter(S=>S.link&&S.size).sort((S,q)=>S.size-q.size);if(!y.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=y[0],m=(v.size/1024/1024).toFixed(1),b=await fetch(v.link);if(!b.ok)return e.json({error:`Download falhou: HTTP ${b.status}`},502);let _=Buffer.from(await b.arrayBuffer()),E="video.mp4";const I=24*1024*1024;if(_.length>I){try{const S=await import("os"),q=await import("path"),D=await import("fs"),M=await import("child_process"),F=q.join(S.tmpdir(),`cct_g_${n}.mp4`),$=q.join(S.tmpdir(),`cct_g_${n}.mp3`);D.writeFileSync(F,_),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",$],{stdio:"pipe"}).status===0&&D.existsSync($)&&(_=D.readFileSync($),E="audio.mp3",D.unlinkSync($)),D.unlinkSync(F)}catch{}if(_.length>I)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const A="----GroqBoundary"+Date.now().toString(16),g=`\r
`,w=E.endsWith(".mp3")?"audio/mpeg":"video/mp4",x=Buffer.concat([Buffer.from(`--${A}${g}Content-Disposition: form-data; name="file"; filename="${E}"${g}Content-Type: ${w}${g}${g}`),_,Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-large-v3-turbo`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${A}--${g}`)]),T=await fetch("https://api.groq.com/openai/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(x.length)},body:x});if(!T.ok)return e.json({error:`Groq Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),O=null;if(c&&C){const S=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const M=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,n]),e.json({transcript:C,description:O,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});R.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});R.post("/api/lessons/:id/rent",J,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,i=L(e);await Et(i);const n=await i.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!n.length||!n[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=n[0],l=parseInt(o.rental_credits);if(!Number.isFinite(l)||l<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const u=await i.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(u.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:u[0].expires_at},400);const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const a=Qr(e),f=await Hr(a,s);if(f<l)return e.json({error:"Créditos insuficientes",available:f,required:l},400);if(!await qo(a,s,l)){const h=await Hr(a,s);return e.json({error:"Créditos insuficientes",available:h,required:l},400)}await i.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[s,t,l]);const p=new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"});return await Lo(e.env,["🎬 *Aula alugada!*","",`📧 *Email:* ${s}`,`📖 *Aula:* ${o.title}`,`💳 *Créditos usados:* ${l}`,`🕐 *Data:* ${p}`].join(`
`)),e.json({success:!0,message:"Aula alugada com sucesso! Acesso liberado por 30 dias."})}catch(t){return console.error("Rent lesson error:",t),e.json({error:t.message||"Erro ao processar aluguel"},500)}});R.get("/api/user/rentals",J,async e=>{try{const t=e.get("user"),r=L(e);await Et(r);const s=await r.sql(`SELECT lr.id, lr.lesson_id, lr.credits_paid, lr.rented_at, lr.expires_at,
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}R.get("/api/trails",J,async e=>{try{const t=e.get("user"),r=L(e);await Ne(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,o);return e.json({lessons:l})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/lessons/find",P,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const i=await L(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return i&&i.length>0?e.json({lesson:i[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});function Yr(e,t){const r=Array.isArray(e.alternatives)?e.alternatives:[],s=Array.isArray(e.tags)?e.tags:String(e.tags||"").split(",").map(i=>i.trim()).filter(Boolean);return{title:e.title||null,statement_html:e.statement_html||"",question_type:e.question_type||"multiple_choice",alternatives:JSON.stringify(r),answer_key:JSON.stringify(e.answer_key||{}),technical_comment_html:e.technical_comment_html||null,difficulty:e.difficulty||"medio",theme:e.theme||null,subtheme:e.subtheme||null,legal_basis:e.legal_basis||null,weight:Number(e.weight||1),estimated_minutes:parseInt(e.estimated_minutes)||5,tags:JSON.stringify(s),status:e.status||"draft",professor:e.professor||null,course_id:e.course_id?parseInt(e.course_id):null,lesson_id:e.lesson_id?parseInt(e.lesson_id):null,source_transcript:e.source_transcript||null,ai_generated:e.ai_generated===!0||e.ai_generated==="true",order_index:parseInt(e.order_index)||0,usage_count:parseInt(e.usage_count)||0,attempts_count:parseInt(e.attempts_count)||0,correct_count:parseInt(e.correct_count)||0,wrong_count:parseInt(e.wrong_count)||0,created_by:t||e.created_by||null}}function $r(e){if(typeof e=="string")return e.replace(/\b(conforme|segundo|de acordo com|com base na|a partir da)\s+a\s+transcri[cç][aã]o\b/gi,"").replace(/\bna\s+transcri[cç][aã]o\b/gi,"no material de estudo").replace(/\bda\s+transcri[cç][aã]o\b/gi,"do material de estudo").replace(/\btranscri[cç][aã]o\b/gi,"material de estudo").replace(/\s{2,}/g," ").replace(/\s+([,.;:!?])/g,"$1").trim();if(Array.isArray(e))return e.map($r).filter(t=>!(typeof t=="string"&&/transcri[cç][aã]o/i.test(t)));if(e&&typeof e=="object"){const t={};for(const[r,s]of Object.entries(e))t[r]=$r(s);return t}return e}R.get("/api/admin/questions",P,async e=>{try{const t=L(e);await ne(t);const r=[],s=[];let i=1;const n=(m,b)=>{r.push(m.replace("?",`$${i++}`)),s.push(b)},o=e.req.query("q"),l=e.req.query("theme"),u=e.req.query("professor"),d=e.req.query("course_id"),a=e.req.query("difficulty"),f=e.req.query("question_type"),c=e.req.query("status"),p=e.req.query("from"),h=e.req.query("to");o&&(r.push(`(title ILIKE $${i} OR statement_html ILIKE $${i+1} OR legal_basis ILIKE $${i+2})`),s.push(`%${o}%`,`%${o}%`,`%${o}%`),i+=3),l&&n("theme = ?",l),u&&n("professor = ?",u),d&&n("course_id = ?",parseInt(d)),a&&n("difficulty = ?",a),f&&n("question_type = ?",f),c&&n("status = ?",c),p&&n("created_at >= ?",p),h&&n("created_at <= ?",h);const y=r.length?`WHERE ${r.join(" AND ")}`:"",v=await t.sql(`
      ...
      LIMIT 500
    `,s);return e.json({questions:v})}catch(t){return console.error("List questions error:",t),e.json({error:t.message||"Failed to list questions"},500)}});R.get("/api/admin/questions/stats",P,async e=>{try{const t=L(e);await ne(t);const r=await t.sql(`
      SELECT
        COUNT(*)::int AS total,
      ...
      FROM question_bank
    `),s=await t.sql("SELECT id, title, wrong_count, attempts_count FROM question_bank ORDER BY wrong_count DESC, attempts_count DESC LIMIT 5"),i=await t.sql("SELECT id, title, usage_count FROM question_bank ORDER BY usage_count DESC LIMIT 5");return e.json({stats:r[0]||{},mostWrong:s,mostUsed:i})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/questions/:id/versions",P,async e=>{try{const t=L(e);await ne(t);const r=await t.sql("SELECT * FROM question_bank_versions WHERE question_id = $1 ORDER BY version DESC, created_at DESC",[parseInt(e.req.param("id"))]);return e.json({versions:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions",P,async e=>{try{const t=e.get("user"),r=L(e);await ne(r);const s=Yr(await e.req.json(),t==null?void 0:t.email),n=(await r.sql(`
    ...
    `,[`${n.title||"Questao"} (copia)`,n.statement_html,n.question_type,JSON.stringify(n.alternatives||[]),JSON.stringify(n.answer_key||{}),n.technical_comment_html,n.difficulty,n.theme,n.subtheme,n.legal_basis,n.weight,n.estimated_minutes,JSON.stringify(n.tags||[]),n.professor,n.course_id,n.lesson_id,n.source_transcript,n.ai_generated,n.order_index+1,(t==null?void 0:t.email)||null]);return e.json({success:!0,question:o[0]})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/questions/:id",P,async e=>{try{const t=L(e);return await ne(t),await t.sql("DELETE FROM question_bank WHERE id = $1",[parseInt(e.req.param("id"))]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions-reorder",P,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions must be an array"},400);const r=L(e);await ne(r);for(const s of t)await r.sql("UPDATE question_bank SET order_index = $1, updated_at = NOW() WHERE id = $2",[parseInt(s.order_index),parseInt(s.id)]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/import",P,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions array required"},400);const r=e.get("user"),s=L(e);await ne(s);let i=0;for(const n of t){const o=Yr(n,r==null?void 0:r.email);await s.sql(`
      ...
      `,[o.title,o.statement_html,o.question_type,o.alternatives,o.answer_key,o.technical_comment_html,o.difficulty,o.theme,o.subtheme,o.legal_basis,o.weight,o.estimated_minutes,o.tags,o.status,o.professor,o.course_id,o.lesson_id,o.source_transcript,o.ai_generated,o.order_index,o.created_by]),i++}return e.json({success:!0,created:i})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/generate-ai",P,async e=>{var t,r,s;try{const{lesson_id:i,transcript:n,count:o=5,types:l=["multiple_choice","true_false","discursive"],difficulty:u="misto",context:d=""}=await e.req.json(),a=L(e);await ne(a);let f=n||"",c=null;if(i&&(c=(await a.sql(`
...
${(t.emails||[]).join(`
`)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function Bo(e,t,r,s){var i,n,o,l,u,d,a;switch(e){case"search_users":{const f=`%${t.query}%`,c=Math.min(t.limit||10,50),p=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const f=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,f]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[f])}}case"dashboard_stats":{const[f,c,p,h,y,v,m]=await Promise.all([r.sql("SELECT COUNT(*)::int as total FROM users"),r.sql("SELECT COUNT(*)::int as total FROM member_subscriptions WHERE data_expiracao > NOW() AND ativo = true"),r.sql("SELECT COUNT(*)::int as total FROM certificates"),r.sql("SELECT COUNT(*)::int as total FROM comments WHERE admin_reply IS NULL"),r.sql("SELECT COUNT(*)::int as total FROM courses"),r.sql("SELECT COUNT(*)::int as total FROM lessons"),(()=>{const b=s.env.DATABASE_SUITEPLUS;return b?new $e(b).sql("SELECT COUNT(*)::int as total FROM user_subscriptions WHERE status = 'active' AND product_id = 4 AND expires_at > NOW()").catch(()=>[{total:null}]):Promise.resolve([{total:null}])})()]);return{total_users:(i=f[0])==null?void 0:i.total,active_subscriptions_cct:(n=c[0])==null?void 0:n.total,active_subscriptions_suiteplus:(o=m[0])==null?void 0:o.total,total_certificates:(l=p[0])==null?void 0:l.total,pending_comments:(u=h[0])==null?void 0:u.total,total_courses:(d=y[0])==null?void 0:d.total,total_lessons:(a=v[0])==null?void 0:a.total}}case"get_user_progress":{const c=(await r.sql(`SELECT c.id as course_id, c.title as course_title,
       ...
       ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`,[t||""]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/agent",P,async e=>{var t,r,s;try{const i=await e.req.json(),{message:n,history:o=[],pendingAction:l}=i,u=L(e),d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);if(l){const h=await Mo(l.tool,l.args,u,e);return h.error?e.json({reply:`❌ Erro ao executar: ${h.error}`}):e.json({reply:`✅ Feito! Resultado:
\`\`\`json
...
${a}`:""}`},...o.slice(-20),{role:"user",content:n}];for(let h=0;h<6;h++){const y=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Admin Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:p,tools:jo,tool_choice:"auto"})});if(!y.ok)return e.json({error:`OpenRouter: ${await y.text()}`},500);const m=(s=(await y.json()).choices)==null?void 0:s[0];if(!m)return e.json({error:"Resposta vazia do modelo"},500);const b=m.message,_=b.tool_calls;if(!_||_.length===0)return e.json({reply:b.content||""});const E=_[0],I=E.function.name;let A={};try{A=JSON.parse(E.function.arguments)}catch{}if(Do.has(I)){const w=ko(I,A);return e.json({reply:`⚠️ **Confirmação necessária**

${w}`,pendingAction:{tool:I,args:A,description:w}})}const g=await Bo(I,A,u,e);p.push({role:"assistant",content:b.content||null,tool_calls:_}),p.push({role:"tool",content:JSON.stringify(g),tool_call_id:E.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(i){return console.error("Agent error:",i),e.json({error:i.message||"Erro no agente"},500)}});const Uo=[{type:"function",function:{name:"list_courses",description:"Lista todos os cursos publicados disponíveis na plataforma, com id, título e carga horária.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"get_course_details",description:"Retorna os módulos e a quantidade de aulas de um curso publicado.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso"}},required:["course_id"]}}},{type:"function",function:{name:"get_my_progress",description:"Retorna o progresso do aluno autenticado em todos os cursos que ele já iniciou: % concluído, aulas completadas e aulas totais por curso.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"recommend_next_lessons",description:"Recomenda as próximas aulas que o aluno autenticado ainda não assistiu. Se course_id for informado, recomenda dentro daquele curso; caso contrário, considera todos os cursos que o aluno já iniciou.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso (opcional)"},limit:{type:"number",description:"Máximo de aulas a recomendar (padrão 5)"}}}}},{type:"function",function:{name:"search_lessons",description:"Busca aulas por título/palavra-chave em cursos publicados.",parameters:{type:"object",properties:{query:{type:"string",description:"Termo de busca no título da aula"},limit:{type:"number",description:"Máximo de resultados (padrão 10)"}},required:["query"]}}},{type:"function",function:{name:"get_my_certificates",description:"Lista os certificados já emitidos para o aluno autenticado.",parameters:{type:"object",properties:{}}}}];async function Fo(e,t,r,s,i){switch(e){case"list_courses":return{courses:await r.sql("SELECT id, title, instructor, duration_hours FROM courses WHERE is_published = true ORDER BY title")};case"get_course_details":{const[n,o]=await Promise.all([r.sql("SELECT id, title, description, instructor, duration_hours FROM courses WHERE id = $1 AND is_published = true",[t.course_id]),r.sql(`SELECT m.id, m.title, m.order_index, COUNT(l.id)::int as lesson_count
           FROM modules m LEFT JOIN lessons l ON l.module_id = m.id
         ...
         LIMIT ${n}`,l)).map(a=>({...a,lesson_url:`${i}/aula/${a.lesson_id}`}));return{recommended_lessons:d,count:d.length}}case"search_lessons":{const n=Math.min(t.limit||10,50),l=(await r.sql(`SELECT l.id, l.title, m.title as module_title, c.title as course_title, c.id as course_id
         FROM lessons l JOIN modules m ON m.id = l.module_id JOIN courses c ON c.id = m.course_id
         WHERE l.title ILIKE $1 AND c.is_published = true
         ORDER BY l.title LIMIT $2`,[`%${t.query}%`,n])).map(u=>({...u,lesson_url:`${i}/aula/${u.id}`}));return{lessons:l,count:l.length}}case"get_my_certificates":return{certificates:await r.sql(`SELECT id, course_title, carga_horaria, certificate_code, completion_date FROM certificates
         WHERE lower(user_email) = lower($1) ORDER BY created_at DESC`,[s])};default:return{error:`Ferramenta desconhecida: ${e}`}}}R.post("/api/student-agent",J,async e=>{var t,r;try{const s=e.get("user"),i=s.email,n=await e.req.json(),{message:o,history:l=[]}=n,u=L(e),d=new URL(e.req.url).origin,a=e.env.VITE_OPENROUTER_API_KEY;if(!a)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);let c=[{role:"system",content:`Você é o assistente virtual do CCT (Clube de Cálculo Trabalhista), atendendo o aluno "${((t=s.user_metadata)==null?void 0:t.name)||s.email}".
...
- Se o aluno perguntar sobre cancelamento, reembolso, cobrança ou alteração de plano, explique gentilmente que você ainda não pode resolver isso diretamente e oriente a falar com o suporte pelo WhatsApp.`},...l.slice(-20),{role:"user",content:o}];for(let p=0;p<5;p++){const h=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Student Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:c,tools:Uo,tool_choice:"auto"})});if(!h.ok)return e.json({error:`OpenRouter: ${await h.text()}`},500);const v=(r=(await h.json()).choices)==null?void 0:r[0];if(!v)return e.json({error:"Resposta vazia do modelo"},500);const m=v.message,b=m.tool_calls;if(!b||b.length===0)return e.json({reply:m.content||""});const _=b[0],E=_.function.name;let I={};try{I=JSON.parse(_.function.arguments)}catch{}const A=await Fo(E,I,u,i,d);c.push({role:"assistant",content:m.content||null,tool_calls:b}),c.push({role:"tool",content:JSON.stringify(A),tool_call_id:_.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(s){return console.error("Student agent error:",s),e.json({error:s.message||"Erro no assistente"},500)}});R.get("/api/admin/users",P,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `,[r,t]))[0]||{};return e.json({start_date:n.start_date||null,completion_date:n.completion_date||null})}catch(t){return console.error("Suggest certificate dates error:",t),e.json({error:t.message||"Failed to suggest dates"},500)}});R.post("/api/admin/certificates",P,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),i=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,start_date:t.start_date||null,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:i&&i.length>0?i[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],i=L(e),n=[];for(const l of s){const u=m=>{const b=String(m??"").trim();return!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined"?"":b},d=u(l.user_email||l.email).toLowerCase(),a=u(l.user_name||l.nome)||null,f=u(l.course_title||l.curso);let c=l.carga_horaria?parseInt(String(l.carga_horaria)):null;if(c!==null&&!Number.isFinite(c)&&(c=null),!d.includes("@")||!f){n.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const b=String(m).trim();if(!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(b)){const[_,E,I]=b.split("/");return`${I}-${E}-${_}T12:00:00Z`}return isNaN(new Date(b).getTime())?null:new Date(b).toISOString()},h=p(l.data_final||l.completion_date),y=p(l.data_inicio||l.data_inicial||l.start_date);if(!c){const m=await i.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)c=parseInt(m[0].duration_hours);else{const b=E=>E.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),_=b(f);if(_.length>0){const I=(await i.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=b(A.title).join(" ");return _.every(w=>g.includes(w))});I&&(c=parseInt(I.duration_hours))}}}const v=await i.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await i.sql(`UPDATE certificates
           ...
           RETURNING id`,[d,a||"Aluno",f,c,b,_,h,y]);n.push({user_email:d,course_title:f,status:"created",certificate_id:(t=E[0])==null?void 0:t.id,verification_code:b})}}const o=n.filter(l=>l.status==="error").length;return e.json({success:o===0,total:s.length,results:n})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let i=r.course_title;if(r.course_id){const n=await s.query("courses",{select:"title",filters:{id:r.course_id}});n&&n.length>0&&(i=n[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:i,carga_horaria:r.carga_horaria,start_date:r.start_date||null,completion_date:r.completion_date||null,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Ho(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,c)=>`<div class="mod-item"><span class="mod-num">${String(c+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=s?`
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
...
</html>`}R.get("/api/my-courses-progress",J,async e=>{try{const r=e.get("user").email;if(!r)return e.json({error:"User email not found"},400);const i=await L(e).sql(`
      SELECT
        c.id AS course_id,
        c.title AS course_title,
        c.duration_hours,
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id/modules",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const i=await r.sql(`SELECT m.*,
      ...
      ORDER BY ${b}
      LIMIT ${m}
    `,a);return e.json({lessons:_,limit:u})}catch(t){return console.error("❌ /api/search/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to search lessons"},500)}});R.get("/api/modules/:id/lessons",async e=>{try{const t=e.req.param("id"),s=await L(e).sql(`SELECT *
       FROM lessons
       WHERE module_id = $1
       ORDER BY order_index`,[t]);return e.json({lessons:s||[]})}catch(t){return console.error("❌ /api/modules/:id/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch module lessons"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,i,n,o;try{const l=e.req.param("id"),u=V(e,"sb-access-token");let d=null;if(u){const _=await K(u,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);_&&(d=_.email)}const a=L(e);let f=!1,c=!1;if(d)try{if(f=!!((t=(await a.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[d,parseInt(l)]))[0])!=null&&t.has_access),console.log("Has access:",f,"User:",d,"Lesson:",l),!f){const E=e.env.DATABASE_SUITEPLUS,[I,A,g]=await Promise.all([a.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[d,parseInt(l)]),a.sql(`SELECT id FROM member_subscriptions
               WHERE lower(email_membro) = lower($1)
                 AND data_expiracao > NOW()
                 AND COALESCE(ativo, true) = true
                 AND COALESCE(teste_gratis, false) = false
               LIMIT 1`,[d]),E?xt(d,E):Promise.resolve(null)]);if((I.length>0||A.length>0||g&&g>new Date)&&(f=!0),!f){const w=await a.sql("SELECT rentable, rental_credits, title, COALESCE(teste_gratis, false) AS teste_gratis, COALESCE(free_trial, false) AS free_trial FROM lessons WHERE id = $1",[parseInt(l)]);if((r=w[0])!=null&&r.teste_gratis||(s=w[0])!=null&&s.free_trial)f=!0;else return console.log("❌ Access denied for user:",d,"lesson:",l),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((i=w[0])==null?void 0:i.rentable)||!1,rental_credits:((n=w[0])==null?void 0:n.rental_credits)||0,lesson_title:((o=w[0])==null?void 0:o.title)||""},403)}}console.log("✅ Access granted for user:",d,"lesson:",l)}catch(_){console.error("❌ Error checking access via RPC:",_),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),c=!0,f=!0}if(!d||!f&&!c){const _=await a.query("lessons",{select:"teste_gratis, free_trial",filters:{id:l},single:!0});if(!(_!=null&&_.teste_gratis)&&!(_!=null&&_.free_trial))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await wt(a);const p=`
...
</html>`)});R.get("/api/favorites",J,async e=>{const t=e.get("user"),r=L(e);try{const s=await r.sql(`
...
<body class="bg-gray-50">
  ...
  `));const Ds=new Zi,$o=Object.assign({"/src/index.tsx":R});let on=!1;for(const[,e]of Object.entries($o))e&&(Ds.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ds.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),on=!0);if(!on)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var lr={exports:{}},cr={exports:{}},qe={},ur={},js;function an(){if(js)return ur;js=1,ur.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,i){this.source=s,this.transform=i||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var i;(this.recorded.length>0||s)&&(i=this.recorded.join(""),i==="NULL"&&!s&&(i=null),i!==null&&(i=this.transform(i)),this.entries.push(i),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var i,n,o;for(this.consumeDimensions();!this.isEof();)if(i=this.nextCharacter(),i.value==="{"&&!o)this.dimension++,this.dimension>1&&(n=new e(this.source.substr(this.position-1),this.transform),this.entries.push(n.parse(!0)),this.position+=n.position-2);else if(i.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else i.value==='"'&&!i.escaped?(o&&this.newEntry(!0),o=!o):i.value===","&&!o?this.newEntry():this.record(i.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return ur}var dr,ks;function ln(){if(ks)return dr;ks=1;var e=an();return dr={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},dr}var fr,Bs;function zo(){if(Bs)return fr;Bs=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;fr=function(d){if(s.test(d))return Number(d.replace("i","I"));var a=e.exec(d);if(!a)return i(d)||null;var f=!!a[8],c=parseInt(a[1],10);f&&(c=o(c));var p=parseInt(a[2],10)-1,h=a[3],y=parseInt(a[4],10),v=parseInt(a[5],10),m=parseInt(a[6],10),b=a[7];b=b?1e3*parseFloat(b):0;var _,E=n(d);return E!=null?(_=new Date(Date.UTC(c,p,h,y,v,m,b)),l(c)&&_.setUTCFullYear(c),E!==0&&_.setTime(_.getTime()-E)):(_=new Date(c,p,h,y,v,m,b),l(c)&&_.setFullYear(c)),_};function i(u){var d=t.exec(u);if(d){var a=parseInt(d[1],10),f=!!d[4];f&&(a=o(a));var c=parseInt(d[2],10)-1,p=d[3],h=new Date(a,c,p);return l(a)&&h.setFullYear(a),h}}function n(u){if(u.endsWith("+00"))return 0;var d=r.exec(u.split(" ")[1]);if(d){var a=d[1];if(a==="Z")return 0;var f=a==="-"?-1:1,c=parseInt(d[2],10)*3600+parseInt(d[3]||0,10)*60+parseInt(d[4]||0,10);return c*f*1e3}}function o(u){return-(u-1)}function l(u){return u>=0&&u<100}return fr}var pr,Ms;function Wo(){if(Ms)return pr;Ms=1,pr=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var i=arguments[s];for(var n in i)e.call(i,n)&&(r[n]=i[n])}return r}return pr}var mr,Us;function Vo(){if(Us)return mr;Us=1;var e=Wo();mr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,y(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(m){var b=this[m]||0;return m==="seconds"&&this.milliseconds&&(b=(b+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),b+" "+m},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},i=["years","months","days"],n=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=i.map(b,this).join(""),m=n.map(b,this).join("");return"P"+v+"T"+m;function b(_){var E=this[_]||0;return _==="seconds"&&this.milliseconds&&(E=(E+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),E+s[_]}};var o="([+-]?\\d+)",l=o+"\\s+years?",u=o+"\\s+mons?",d=o+"\\s+days?",a="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([l,u,d,a].map(function(v){return"("+v+")?"}).join("\\s*")),c={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function h(v){var m=v+"000000".slice(v.length);return parseInt(m,10)/1e3}function y(v){if(!v)return{};var m=f.exec(v),b=m[8]==="-";return Object.keys(c).reduce(function(_,E){var I=c[E],A=m[I];return!A||(A=E==="milliseconds"?h(A):parseInt(A,10),!A)||(b&&~p.indexOf(E)&&(A*=-1),_[E]=A),_},{})}return mr}var hr,Fs;function Qo(){if(Fs)return hr;Fs=1;var e=Buffer.from||Buffer;return hr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",i=0;i<r.length;)if(r[i]!=="\\")s+=r[i],++i;else if(/[0-7]{3}/.test(r.substr(i+1,3)))s+=String.fromCharCode(parseInt(r.substr(i+1,3),8)),i+=4;else{for(var n=1;i+n<r.length&&r[i+n]==="\\";)n++;for(var o=0;o<Math.floor(n/2);++o)s+="\\";i+=Math.floor(n/2)*2}return e(s,"binary")},hr}var gr,Hs;function Xo(){if(Hs)return gr;Hs=1;var e=an(),t=ln(),r=zo(),s=Vo(),i=Qo();function n(g){return function(x){return x===null?x:g(x)}}function o(g){return g===null?g:g==="TRUE"||g==="t"||g==="true"||g==="y"||g==="yes"||g==="on"||g==="1"}function l(g){return g?e.parse(g,o):null}function u(g){return parseInt(g,10)}function d(g){return g?e.parse(g,n(u)):null}function a(g){return g?e.parse(g,n(function(w){return b(w).trim()})):null}var f=function(g){if(!g)return null;var w=t.create(g,function(x){return x!==null&&(x=E(x)),x});return w.parse()},c=function(g){if(!g)return null;var w=t.create(g,function(x){return x!==null&&(x=parseFloat(x)),x});return w.parse()},p=function(g){if(!g)return null;var w=t.create(g);return w.parse()},h=function(g){if(!g)return null;var w=t.create(g,function(x){return x!==null&&(x=r(x)),x});return w.parse()},y=function(g){if(!g)return null;var w=t.create(g,function(x){return x!==null&&(x=s(x)),x});return w.parse()},v=function(g){return g?e.parse(g,n(i)):null},m=function(g){return parseInt(g,10)},b=function(g){var w=String(g);return/^\d+$/.test(w)?w:g},_=function(g){return g?e.parse(g,n(JSON.parse)):null},E=function(g){return g[0]!=="("?null:(g=g.substring(1,g.length-1).split(","),{x:parseFloat(g[0]),y:parseFloat(g[1])})},I=function(g){if(g[0]!=="<"&&g[1]!=="(")return null;for(var w="(",x="",T=!1,C=2;C<g.length-1;C++){if(T||(w+=g[C]),g[C]===")"){T=!0;continue}else if(!T)continue;g[C]!==","&&(x+=g[C])}var O=E(w);return O.radius=parseFloat(x),O},A=function(g){g(20,b),g(21,m),g(23,m),g(26,m),g(700,parseFloat),g(701,parseFloat),g(16,o),g(1082,r),g(1114,r),g(1184,r),g(600,E),g(651,p),g(718,I),g(1e3,l),g(1001,v),g(1005,d),g(1007,d),g(1028,d),g(1016,a),g(1017,f),g(1021,c),g(1022,c),g(1231,c),g(1014,p),g(1015,p),g(1008,p),g(1009,p),g(1040,p),g(1041,p),g(1115,h),g(1182,h),g(1185,h),g(1186,s),g(1187,y),g(17,i),g(114,JSON.parse.bind(JSON)),g(3802,JSON.parse.bind(JSON)),g(199,_),g(3807,_),g(3907,p),g(2951,p),g(791,p),g(1183,p),g(1270,p)};return gr={init:A},gr}var yr,$s;function Jo(){if($s)return yr;$s=1;var e=1e6;function t(r){var s=r.readInt32BE(0),i=r.readUInt32BE(4),n="";s<0&&(s=~s+(i===0),i=~i+1>>>0,n="-");var o="",l,u,d,a,f,c;{if(l=s%e,s=s/e>>>0,u=4294967296*l+i,i=u/e>>>0,d=""+(u-e*i),i===0&&s===0)return n+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}{if(l=s%e,s=s/e>>>0,u=4294967296*l+i,i=u/e>>>0,d=""+(u-e*i),i===0&&s===0)return n+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}{if(l=s%e,s=s/e>>>0,u=4294967296*l+i,i=u/e>>>0,d=""+(u-e*i),i===0&&s===0)return n+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}return l=s%e,u=4294967296*l+i,d=""+u%e,n+d+o}return yr=t,yr}var br,zs;function Yo(){if(zs)return br;zs=1;var e=Jo(),t=function(p,h,y,v,m){y=y||0,v=v||!1,m=m||function(T,C,O){return T*Math.pow(2,O)+C};var b=y>>3,_=function(T){return v?~T&255:T},E=255,I=8-y%8;h<I&&(E=255<<8-h&255,I=h),y&&(E=E>>y%8);var A=0;y%8+h>=8&&(A=m(0,_(p[b])&E,I));for(var g=h+y>>3,w=b+1;w<g;w++)A=m(A,_(p[w]),8);var x=(h+y)%8;return x>0&&(A=m(A,_(p[g])>>8-x,x)),A},r=function(p,h,y){var v=Math.pow(2,y-1)-1,m=t(p,1),b=t(p,y,1);if(b===0)return 0;var _=1,E=function(A,g,w){A===0&&(A=1);for(var x=1;x<=w;x++)_/=2,(g&1<<w-x)>0&&(A+=_);return A},I=t(p,h,y+1,!1,E);return b==Math.pow(2,y+1)-1?I===0?m===0?1/0:-1/0:NaN:(m===0?1:-1)*Math.pow(2,b-v)*I},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},i=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},n=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},l=function(p){var h=t(p,16,32);if(h==49152)return NaN;for(var y=Math.pow(1e4,t(p,16,16)),v=0,m=t(p,16),b=0;b<m;b++)v+=t(p,16,64+16*b)*y,y/=1e4;var _=Math.pow(10,t(p,16,48));return(h===0?1:-1)*Math.round(v*_)/_},u=function(p,h){var y=t(h,1),v=t(h,63,1),m=new Date((y===0?1:-1)*v/1e3+9466848e5);return p||m.setTime(m.getTime()+m.getTimezoneOffset()*6e4),m.usec=v%1e3,m.getMicroSeconds=function(){return this.usec},m.setMicroSeconds=function(b){this.usec=b},m.getUTCMicroSeconds=function(){return this.usec},m},d=function(p){var h=t(p,32);t(p,32,32);for(var y=t(p,32,64),v=96,m=[],b=0;b<h;b++)m[b]=t(p,32,v),v+=32,v+=32;var _=function(I){var A=t(p,32,v);if(v+=32,A==4294967295)return null;var g;if(I==23||I==20)return g=t(p,A*8,v),v+=A*8,g;if(I==25)return g=p.toString(this.encoding,v>>3,(v+=A<<3)>>3),g;console.log("ERROR: ElementType not implemented: "+I)},E=function(I,A){var g=[],w;if(I.length>1){var x=I.shift();for(w=0;w<x;w++)g[w]=E(I,A);I.unshift(x)}else for(w=0;w<I[0];w++)g[w]=_(A);return g};return E(m,y)},a=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},c=function(p){p(20,e),p(21,s),p(23,i),p(26,i),p(1700,l),p(700,n),p(701,o),p(16,f),p(1114,u.bind(null,!1)),p(1184,u.bind(null,!0)),p(1e3,d),p(1007,d),p(1016,d),p(1008,d),p(1009,d),p(25,a)};return br={init:c},br}var vr,Ws;function Go(){return Ws||(Ws=1,vr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),vr}var Vs;function St(){if(Vs)return qe;Vs=1;var e=Xo(),t=Yo(),r=ln(),s=Go();qe.getTypeParser=o,qe.setTypeParser=l,qe.arrayParser=r,qe.builtins=s;var i={text:{},binary:{}};function n(u){return String(u)}function o(u,d){return d=d||"text",i[d]&&i[d][u]||n}function l(u,d,a){typeof d=="function"&&(a=d,d="text"),i[d][u]=a}return e.init(function(u,d){i.text[u]=d}),t.init(function(u,d){i.binary[u]=d}),qe}var Qs;function At(){return Qs||(Qs=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=St(),i=s.getTypeParser(20,"text"),n=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):i),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):n)})})(cr)),cr.exports}var _r,Xs;function tt(){if(Xs)return _r;Xs=1;const e=At(),t=He,{isDate:r}=t.types||t;function s(c){return'"'+c.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function i(c){let p="{";for(let h=0;h<c.length;h++)if(h>0&&(p=p+","),c[h]===null||typeof c[h]>"u")p=p+"NULL";else if(Array.isArray(c[h]))p=p+i(c[h]);else if(ArrayBuffer.isView(c[h])){let y=c[h];if(!(y instanceof Buffer)){const v=Buffer.from(y.buffer,y.byteOffset,y.byteLength);v.length===y.byteLength?y=v:y=v.slice(y.byteOffset,y.byteOffset+y.byteLength)}p+="\\\\x"+y.toString("hex")}else p+=s(n(c[h]));return p=p+"}",p}const n=function(c,p){if(c==null)return null;if(typeof c=="object"){if(c instanceof Buffer)return c;if(ArrayBuffer.isView(c)){const h=Buffer.from(c.buffer,c.byteOffset,c.byteLength);return h.length===c.byteLength?h:h.slice(c.byteOffset,c.byteOffset+c.byteLength)}return r(c)?e.parseInputDatesAsUTC?u(c):l(c):Array.isArray(c)?i(c):o(c,p)}return c.toString()};function o(c,p){if(c&&typeof c.toPostgres=="function"){if(p=p||[],p.indexOf(c)!==-1)throw new Error('circular reference detected while preparing "'+c+'" for query');return p.push(c),n(c.toPostgres(n),p)}return JSON.stringify(c)}function l(c){let p=-c.getTimezoneOffset(),h=c.getFullYear();const y=h<1;y&&(h=Math.abs(h)+1);let v=String(h).padStart(4,"0")+"-"+String(c.getMonth()+1).padStart(2,"0")+"-"+String(c.getDate()).padStart(2,"0")+"T"+String(c.getHours()).padStart(2,"0")+":"+String(c.getMinutes()).padStart(2,"0")+":"+String(c.getSeconds()).padStart(2,"0")+"."+String(c.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),y&&(v+=" BC"),v}function u(c){let p=c.getUTCFullYear();const h=p<1;h&&(p=Math.abs(p)+1);let y=String(p).padStart(4,"0")+"-"+String(c.getUTCMonth()+1).padStart(2,"0")+"-"+String(c.getUTCDate()).padStart(2,"0")+"T"+String(c.getUTCHours()).padStart(2,"0")+":"+String(c.getUTCMinutes()).padStart(2,"0")+":"+String(c.getUTCSeconds()).padStart(2,"0")+"."+String(c.getUTCMilliseconds()).padStart(3,"0");return y+="+00:00",h&&(y+=" BC"),y}function d(c,p,h){return c=typeof c=="string"?{text:c}:c,p&&(typeof p=="function"?c.callback=p:c.values=p),h&&(c.callback=h),c}return _r={prepareValue:function(p){return n(p)},normalizeQueryConfig:d,escapeIdentifier:function(c){return'"'+c.replace(/"/g,'""')+'"'},escapeLiteral:function(c){let p=!1,h="'";if(c==null||typeof c!="string")return"''";for(let y=0;y<c.length;y++){const v=c[y];v==="'"?h+=v+v:v==="\\"?(h+=v+v,p=!0):h+=v}return h+="'",p===!0&&(h=" E"+h),h}},_r}var dt={exports:{}},Er,Js;function Ko(){if(Js)return Er;Js=1;const e=qi;function t(l){return e.createHash("md5").update(l,"utf-8").digest("hex")}function r(l,u,d){const a=t(u+l);return"md5"+t(Buffer.concat([Buffer.from(a),d]))}function s(l){return e.createHash("sha256").update(l).digest()}function i(l,u){return l=l.replace(/(\D)-/,"$1"),e.createHash(l).update(u).digest()}function n(l,u){return e.createHmac("sha256",l).update(u).digest()}async function o(l,u,d){return e.pbkdf2Sync(l,u,d,32,"sha256")}return Er={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:i,hmacSha256:n,md5:t},Er}var wr,Ys;function Zo(){if(Ys)return wr;Ys=1;const e=qi;wr={postgresMd5PasswordHash:o,randomBytes:i,deriveKey:a,sha256:l,hashByName:u,hmacSha256:d,md5:n};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function i(f){return t.getRandomValues(Buffer.alloc(f))}async function n(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,h=await r.digest("MD5",p);return Array.from(new Uint8Array(h)).map(y=>y.toString(16).padStart(2,"0")).join("")}}async function o(f,c,p){const h=await n(c+f);return"md5"+await n(Buffer.concat([Buffer.from(h),p]))}async function l(f){return await r.digest("SHA-256",f)}async function u(f,c){return await r.digest(f,c)}async function d(f,c){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(c))}async function a(f,c,p){const h=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),y={name:"PBKDF2",hash:"SHA-256",salt:c,iterations:p};return await r.deriveBits(y,h,256,["deriveBits"])}return wr}var Gs;function cn(){return Gs||(Gs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?dt.exports=Ko():dt.exports=Zo()),dt.exports}var xr,Ks;function ea(){if(Ks)return xr;Ks=1;function e(n,o){return new Error("SASL channel binding: "+n+" when parsing public certificate "+o.toString("base64"))}function t(n,o){let l=n[o++];if(l<128)return{length:l,index:o};const u=l&127;if(u>4)throw e("bad length",n);l=0;for(let d=0;d<u;d++)l=l<<8|n[o++];return{length:l,index:o}}function r(n,o){if(n[o++]!==6)throw e("non-OID data",n);const{length:l,index:u}=t(n,o);o=u;const d=o+l,a=n[o++];let f=(a/40>>0)+"."+a%40;for(;o<d;){let c=0;for(;o<d;){const p=n[o++];if(c=c<<7|p&127,p<128)break}f+="."+c}return{oid:f,index:o}}function s(n,o){if(n[o++]!==48)throw e("non-sequence data",n);return t(n,o)}function i(n,o){o===void 0&&(o=0),o=s(n,o).index;const{length:l,index:u}=s(n,o);o=u+l,o=s(n,o).index;const{oid:d,index:a}=r(n,o);switch(d){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=a,o=s(n,o).index,n[o++]!==160)throw e("non-tag data",n);o=t(n,o).index,o=s(n,o).index;const{oid:f}=r(n,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,n)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+d,n)}return xr={signatureAlgorithmHashFromCertificate:i},xr}var Sr,Zs;function ta(){if(Zs)return Sr;Zs=1;const e=cn(),{signatureAlgorithmHashFromCertificate:t}=ea();function r(f,c){const p=["SCRAM-SHA-256"];c&&p.unshift("SCRAM-SHA-256-PLUS");const h=p.find(m=>f.includes(m));if(!h)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(h==="SCRAM-SHA-256-PLUS"&&typeof c.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const y=e.randomBytes(18).toString("base64");return{mechanism:h,clientNonce:y,response:(h==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":c?"y":"n")+",,n=*,r="+y,message:"SASLInitialResponse"}}async function s(f,c,p,h){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof c!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(c==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const y=u(p);if(y.nonce.startsWith(f.clientNonce)){if(y.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,m="r="+y.nonce+",s="+y.salt+",i="+y.iteration;let b=h?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const S=h.getPeerCertificate().raw;let q=t(S);(q==="MD5"||q==="SHA-1")&&(q="SHA-256");const D=await e.hashByName(q,S);b=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(D)]).toString("base64")}const _="c="+b+",r="+y.nonce,E=v+","+m+","+_,I=Buffer.from(y.salt,"base64"),A=await e.deriveKey(c,I,y.iteration),g=await e.hmacSha256(A,"Client Key"),w=await e.sha256(g),x=await e.hmacSha256(w,E),T=a(Buffer.from(g),Buffer.from(x)).toString("base64"),C=await e.hmacSha256(A,"Server Key"),O=await e.hmacSha256(C,E);f.message="SASLResponse",f.serverSignature=Buffer.from(O).toString("base64"),f.response=_+",p="+T}function i(f,c){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof c!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=d(c);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function n(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((c,p)=>f.charCodeAt(p)).every(c=>c>=33&&c<=43||c>=45&&c<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function l(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(c=>{if(!/^.=/.test(c))throw new Error("SASL: Invalid attribute pair entry");const p=c[0],h=c.substring(2);return[p,h]}))}function u(f){const c=l(f),p=c.get("r");if(p){if(!n(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const h=c.get("s");if(h){if(!o(h))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const y=c.get("i");if(y){if(!/^[1-9][0-9]*$/.test(y))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(y,10);return{nonce:p,salt:h,iteration:v}}function d(f){const p=l(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function a(f,c){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(c))throw new TypeError("second argument must be a Buffer");if(f.length!==c.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,h)=>f[h]^c[h]))}return Sr={startSession:r,continueSession:s,finalizeSession:i},Sr}var Ar,ei;function Gr(){if(ei)return Ar;ei=1;const e=St();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,i){typeof s=="function"&&(i=s,s="text"),this.getOverrides(s)[r]=i},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},Ar=t,Ar}var Tr,ti;function ra(){if(ti)return Tr;ti=1;function e(n,o={}){if(n.charAt(0)==="/"){const p=n.split(" ");return{host:p[0],database:p[1]}}const l={};let u,d=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(n)&&(n=encodeURI(n).replace(/%25(\d\d)/g,"%$1"));try{try{u=new URL(n,"postgres://base")}catch{u=new URL(n.replace("@/","@___DUMMY___/"),"postgres://base"),d=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of u.searchParams.entries())l[p[0]]=p[1];if(l.user=l.user||decodeURIComponent(u.username),l.password=l.password||decodeURIComponent(u.password),u.protocol=="socket:")return l.host=decodeURI(u.pathname),l.database=u.searchParams.get("db"),l.client_encoding=u.searchParams.get("encoding"),l;const a=d?"":u.hostname;l.host?a&&/^%2f/i.test(a)&&(u.pathname=a+u.pathname):l.host=decodeURIComponent(a),l.port||(l.port=u.port);const f=u.pathname.slice(1)||null;l.database=f?decodeURI(f):null,(l.ssl==="true"||l.ssl==="1")&&(l.ssl=!0),l.ssl==="0"&&(l.ssl=!1),(l.sslcert||l.sslkey||l.sslrootcert||l.sslmode)&&(l.ssl={});const c=l.sslcert||l.sslkey||l.sslrootcert?Pi:null;if(l.sslcert&&(l.ssl.cert=c.readFileSync(l.sslcert).toString()),l.sslkey&&(l.ssl.key=c.readFileSync(l.sslkey).toString()),l.sslrootcert&&(l.ssl.ca=c.readFileSync(l.sslrootcert).toString()),o.useLibpqCompat&&l.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(l.uselibpqcompat==="true"||o.useLibpqCompat)switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":{l.ssl.rejectUnauthorized=!1;break}case"require":{l.sslrootcert?l.ssl.checkServerIdentity=function(){}:l.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!l.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");l.ssl.checkServerIdentity=function(){};break}}else switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{l.sslmode!=="verify-full"&&i(l.sslmode);break}case"no-verify":{l.ssl.rejectUnauthorized=!1;break}}return l}function t(n){return Object.entries(n).reduce((l,[u,d])=>(d!=null&&(l[u]=d),l),{})}function r(n){return Object.entries(n).reduce((l,[u,d])=>{if(u==="ssl"){const a=d;typeof a=="boolean"&&(l[u]=a),typeof a=="object"&&(l[u]=t(a))}else if(d!=null)if(u==="port"){if(d!==""){const a=parseInt(d,10);if(isNaN(a))throw new Error(`Invalid ${u}: ${d}`);l[u]=a}}else l[u]=d;return l},{})}function s(n){return r(e(n))}function i(n){!i.warned&&typeof process<"u"&&process.emitWarning&&(i.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
