...
TRANSCRIÇÃO:
${T}`,I=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(I.ok){const M=((n=(s=(r=(await I.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(P=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,P,i]),e.json({transcript:T,description:P,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});N.post("/api/admin/lessons/:id/groq-transcribe",L,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=O(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||u.GROQ_API_KEY,d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const E=Object.keys(u).filter(I=>I.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${E.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(E=>E.link&&E.size).sort((E,I)=>E.size-I.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],h=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let x=Buffer.from(await y.arrayBuffer()),S="video.mp4";const q=24*1024*1024;if(x.length>q){try{const E=await import("os"),I=await import("path"),D=await import("fs"),M=await import("child_process"),F=I.join(E.tmpdir(),`cct_g_${i}.mp4`),z=I.join(E.tmpdir(),`cct_g_${i}.mp3`);D.writeFileSync(F,x),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&D.existsSync(z)&&(x=D.readFileSync(z),S="audio.mp3",D.unlinkSync(z)),D.unlinkSync(F)}catch{}if(x.length>q)return e.json({error:`Arquivo muito grande (${h} MB). Use o script de transcrição em lote.`},400)}const A="----GroqBoundary"+Date.now().toString(16),m=`\r
...
${T}`,I=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:E}]})});if(I.ok){const M=((n=(s=(r=(await I.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){T=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(P=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[T,P,i]),e.json({transcript:T,description:P,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});N.delete("/api/admin/lessons/:id",L,async e=>{try{const t=e.req.param("id");return await O(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});N.post("/api/lessons/:id/rent",Y,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=O(e);await zr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=ri(e),f=await Br(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await Po(l,s,a)){const p=await Br(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}N.get("/api/trails",Y,async e=>{try{const t=e.get("user"),r=O(e);await Pe(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});N.get("/api/admin/trails",L,async e=>{try{const t=O(e);await Pe(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/trails",L,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=O(e);await Pe(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});N.put("/api/admin/trails/:id",L,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=O(e);await Pe(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.delete("/api/admin/trails/:id",L,async e=>{try{const t=parseInt(e.req.param("id")),r=O(e);return await Pe(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/trails/:id/lessons",L,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=O(e);await Pe(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});N.delete("/api/admin/trails/:id/lessons/:lessonId",L,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await O(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/trails/:id/reorder",L,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=O(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.get("/api/admin/trails/search-lessons",L,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=O(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
    ...
    `,o);return e.json({lessons:a})}catch(t){return e.json({error:t.message},500)}});N.get("/api/admin/lessons/find",L,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const n=await O(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return n&&n.length>0?e.json({lesson:n[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});function Xr(e,t){const r=Array.isArray(e.alternatives)?e.alternatives:[],s=Array.isArray(e.tags)?e.tags:String(e.tags||"").split(",").map(n=>n.trim()).filter(Boolean);return{title:e.title||null,statement_html:e.statement_html||"",question_type:e.question_type||"multiple_choice",alternatives:JSON.stringify(r),answer_key:JSON.stringify(e.answer_key||{}),technical_comment_html:e.technical_comment_html||null,difficulty:e.difficulty||"medio",theme:e.theme||null,subtheme:e.subtheme||null,legal_basis:e.legal_basis||null,weight:Number(e.weight||1),estimated_minutes:parseInt(e.estimated_minutes)||5,tags:JSON.stringify(s),status:e.status||"draft",professor:e.professor||null,course_id:e.course_id?parseInt(e.course_id):null,lesson_id:e.lesson_id?parseInt(e.lesson_id):null,source_transcript:e.source_transcript||null,ai_generated:e.ai_generated===!0||e.ai_generated==="true",order_index:parseInt(e.order_index)||0,usage_count:parseInt(e.usage_count)||0,attempts_count:parseInt(e.attempts_count)||0,correct_count:parseInt(e.correct_count)||0,wrong_count:parseInt(e.wrong_count)||0,created_by:t||e.created_by||null}}function Mr(e){if(typeof e=="string")return e.replace(/\b(conforme|segundo|de acordo com|com base na|a partir da)\s+a\s+transcri[cç][aã]o\b/gi,"").replace(/\bna\s+transcri[cç][aã]o\b/gi,"no material de estudo").replace(/\bda\s+transcri[cç][aã]o\b/gi,"do material de estudo").replace(/\btranscri[cç][aã]o\b/gi,"material de estudo").replace(/\s{2,}/g," ").replace(/\s+([,.;:!?])/g,"$1").trim();if(Array.isArray(e))return e.map(Mr).filter(t=>!(typeof t=="string"&&/transcri[cç][aã]o/i.test(t)));if(e&&typeof e=="object"){const t={};for(const[r,s]of Object.entries(e))t[r]=Mr(s);return t}return e}N.get("/api/admin/questions",L,async e=>{try{const t=O(e);await ie(t);const r=[],s=[];let n=1;const i=(h,y)=>{r.push(h.replace("?",`$${n++}`)),s.push(y)},o=e.req.query("q"),a=e.req.query("theme"),c=e.req.query("professor"),u=e.req.query("course_id"),l=e.req.query("difficulty"),f=e.req.query("question_type"),d=e.req.query("status"),p=e.req.query("from"),g=e.req.query("to");o&&(r.push(`(title ILIKE $${n} OR statement_html ILIKE $${n+1} OR legal_basis ILIKE $${n+2})`),s.push(`%${o}%`,`%${o}%`,`%${o}%`),n+=3),a&&i("theme = ?",a),c&&i("professor = ?",c),u&&i("course_id = ?",parseInt(u)),l&&i("difficulty = ?",l),f&&i("question_type = ?",f),d&&i("status = ?",d),p&&i("created_at >= ?",p),g&&i("created_at <= ?",g);const b=r.length?`WHERE ${r.join(" AND ")}`:"",v=await t.sql(`
      ...
      LIMIT 500
    `,s);return e.json({questions:v})}catch(t){return console.error("List questions error:",t),e.json({error:t.message||"Failed to list questions"},500)}});N.get("/api/admin/questions/stats",L,async e=>{try{const t=O(e);await ie(t);const r=await t.sql(`
      SELECT
        COUNT(*)::int AS total,
    ...
    `,[`${i.title||"Questao"} (copia)`,i.statement_html,i.question_type,JSON.stringify(i.alternatives||[]),JSON.stringify(i.answer_key||{}),i.technical_comment_html,i.difficulty,i.theme,i.subtheme,i.legal_basis,i.weight,i.estimated_minutes,JSON.stringify(i.tags||[]),i.professor,i.course_id,i.lesson_id,i.source_transcript,i.ai_generated,i.order_index+1,(t==null?void 0:t.email)||null]);return e.json({success:!0,question:o[0]})}catch(t){return e.json({error:t.message},500)}});N.delete("/api/admin/questions/:id",L,async e=>{try{const t=O(e);return await ie(t),await t.sql("DELETE FROM question_bank WHERE id = $1",[parseInt(e.req.param("id"))]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/questions-reorder",L,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions must be an array"},400);const r=O(e);await ie(r);for(const s of t)await r.sql("UPDATE question_bank SET order_index = $1, updated_at = NOW() WHERE id = $2",[parseInt(s.order_index),parseInt(s.id)]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/questions/import",L,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions array required"},400);const r=e.get("user"),s=O(e);await ie(s);let n=0;for(const i of t){const o=Xr(i,r==null?void 0:r.email);await s.sql(`
      ...
      `,[o.title,o.statement_html,o.question_type,o.alternatives,o.answer_key,o.technical_comment_html,o.difficulty,o.theme,o.subtheme,o.legal_basis,o.weight,o.estimated_minutes,o.tags,o.status,o.professor,o.course_id,o.lesson_id,o.source_transcript,o.ai_generated,o.order_index,o.created_by]),n++}return e.json({success:!0,created:n})}catch(t){return e.json({error:t.message},500)}});N.post("/api/admin/questions/generate-ai",L,async e=>{var t,r,s;try{const{lesson_id:n,transcript:i,count:o=5,types:a=["multiple_choice","true_false","discursive"],difficulty:c="misto",context:u=""}=await e.req.json(),l=O(e);await ie(l);let f=i||"",d=null;if(n&&(d=(await l.sql(`
...
---`,b=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${p}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:"Voce e especialista em direito do trabalho, calculos trabalhistas, PJe-Calc e avaliacao educacional. Responda apenas JSON valido."},{role:"user",content:g}],response_format:{type:"json_object"}})});if(!b.ok)return e.json({error:`OpenRouter: ${await b.text()}`},500);const h=((s=(r=(t=(await b.json()).choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||'{"questions":[]}',x=(JSON.parse(h).questions||[]).map(S=>({...Mr(S),lesson_id:(d==null?void 0:d.id)||n||null,course_id:(d==null?void 0:d.course_id)||null,professor:(d==null?void 0:d.instructor)||null,source_transcript:f,ai_generated:!0,status:"review"}));return e.json({questions:x})}catch(n){return console.error("Generate questions error:",n),e.json({error:n.message||"Erro ao gerar questoes"},500)}});N.post("/api/admin/run-migration-lesson-fields",L,async e=>{try{const t=O(e);return await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS support_text TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS transcript TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'::jsonb"),await zr(t),e.json({success:!0,message:"Migration applied successfully"})}catch(t){return console.error("Migration error:",t),e.json({error:t.message},500)}});N.get("/api/admin/users",L,async e=>{try{const r=await O(e).sql(`
    ...
    `);return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});N.get("/api/admin/users/find",L,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await O(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});N.post("/api/admin/users",L,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await O(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});N.put("/api/admin/users/:id",L,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await O(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});N.delete("/api/admin/users/:id",L,async e=>{try{const t=e.req.param("id");return await O(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});N.get("/api/admin/certificates",L,async e=>{try{const r=await O(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});N.get("/api/admin/certificates/:id",L,async e=>{try{const t=e.req.param("id"),s=await O(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});N.get("/api/admin/certificates/find",L,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await O(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});N.post("/api/admin/certificates",L,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=O(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});N.put("/api/admin/certificates/:id",L,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=O(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});N.delete("/api/admin/certificates/:id",L,async e=>{try{const t=e.req.param("id");return await O(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Io(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const u=s?e.modules.map((f,d)=>`<div class="mod-item"><span class="mod-num">${String(d+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",l=r?`
