...
${JSON.stringify(t.fields,null,2)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function jo(e,t,r,s){switch(e){case"search_users":{const n=`%${t.query}%`,i=Math.min(t.limit||10,50),o=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const n=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,n]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[n])}}default:return{error:`Ferramenta desconhecida: ${e}`}}}async function ko(e,t,r,s){const n=new Set(["nome","telefone","whatsapp","ativo","dt_expiracao"]),i=new Set(["data_expiracao","ativo","detalhe","origem"]),o=new Set(["title","description","teste_gratis","rentable","duration_minutes","order_index"]),l=new Set(["title","description","instructor","duration_hours"]),u=new Set(["user_name","course_title","carga_horaria","generated_at"]);switch(e){case"update_user":{const c={};for(const[p,h]of Object.entries(t.fields||{}))n.has(p)&&(c[p]=h);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const a=Object.keys(c).map((p,h)=>`"${p}" = $${h+2}`).join(", "),f=[t.email,...Object.values(c)],d=await r.sql(`UPDATE users SET ${a} WHERE lower(email) = lower($1) RETURNING *`,f);return{updated:d.length,user:d[0]||null}}case"update_member_subscription":{const c={};for(const[p,h]of Object.entries(t.fields||{}))i.has(p)&&(c[p]=h);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const a=Object.keys(c).map((p,h)=>`"${p}" = $${h+2}`).join(", "),f=[t.email,...Object.values(c)],d=await r.sql(`UPDATE member_subscriptions SET ${a}, updated_at = NOW() WHERE lower(email_membro) = lower($1) RETURNING *`,f);return{updated:d.length,subscription:d[0]||null}}case"create_member_subscription":return{created:!0,subscription:(await r.sql(`INSERT INTO member_subscriptions (email_membro, data_expiracao, detalhe, ativo, origem, created_at, updated_at)
         ...
         WHERE lower(email_membro) = lower($1) RETURNING *`,[t.email])).length,expired:!0};case"update_lesson":{const c={};for(const[p,h]of Object.entries(t.fields||{}))o.has(p)&&(c[p]=h);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const a=Object.keys(c).map((p,h)=>`"${p}" = $${h+2}`).join(", "),f=[t.lesson_id,...Object.values(c)],d=await r.sql(`UPDATE lessons SET ${a} WHERE id = $1 RETURNING id, title`,f);return{updated:d.length,lesson:d[0]||null}}case"update_course":{const c={};for(const[p,h]of Object.entries(t.fields||{}))l.has(p)&&(c[p]=h);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const a=Object.keys(c).map((p,h)=>`"${p}" = $${h+2}`).join(", "),f=[t.course_id,...Object.values(c)],d=await r.sql(`UPDATE courses SET ${a}, updated_at = NOW() WHERE id = $1 RETURNING id, title`,f);return{updated:d.length,course:d[0]||null}}case"update_certificate":{const c={};for(const[p,h]of Object.entries(t.fields||{}))u.has(p)&&(c[p]=h);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const a=Object.keys(c).map((p,h)=>`"${p}" = $${h+2}`).join(", "),f=[t.certificate_id,...Object.values(c)],d=await r.sql(`UPDATE certificates SET ${a}, updated_at = NOW() WHERE id = $1 RETURNING *`,f);return{updated:d.length,certificate:d[0]||null}}default:return{error:`Ferramenta de escrita desconhecida: ${e}`}}}R.post("/api/admin/agent",q,async e=>{var t;try{const r=await e.req.json(),{message:s,history:n=[],pendingAction:i}=r,o=L(e),l=e.env.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);if(i){const a=await ko(i.tool,i.args,o,e);return a.error?e.json({reply:`❌ Erro ao executar: ${a.error}`}):e.json({reply:`✅ Feito! Resultado:
\`\`\`json
${JSON.stringify(a,null,2)}
\`\`\``})}let c=[{role:"system",content:`Você é um assistente administrativo do CCT (Clube de Cálculo Trabalhista).
...
Data/hora atual: ${new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"})}`},...n.slice(-20),{role:"user",content:s}];for(let a=0;a<6;a++){const f=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Admin Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:c,tools:qo,tool_choice:"auto"})});if(!f.ok)return e.json({error:`OpenRouter: ${await f.text()}`},500);const p=(t=(await f.json()).choices)==null?void 0:t[0];if(!p)return e.json({error:"Resposta vazia do modelo"},500);const h=p.message,y=h.tool_calls;if(!y||y.length===0)return e.json({reply:h.content||""});const v=y[0],m=v.function.name;let b={};try{b=JSON.parse(v.function.arguments)}catch{}if(Lo.has(m)){const S=Do(m,b);return e.json({reply:`⚠️ **Confirmação necessária**

${S}`,pendingAction:{tool:m,args:b,description:S}})}const w=await jo(m,b,o,e);c.push({role:"assistant",content:h.content||null,tool_calls:y}),c.push({role:"tool",content:JSON.stringify(w),tool_call_id:v.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(r){return console.error("Agent error:",r),e.json({error:r.message||"Erro no agente"},500)}});R.get("/api/admin/users",q,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
  ...
  <div class="page">
    ...
    <div class="verso-body">
      ...
      <div class="modules-grid">
        ${e.modules.map((c,a)=>`
          <div class="mod-item"><span class="mod-num">${String(a+1).padStart(2,"0")}.</span>${c}</div>`).join("")}
      </div>
    </div>
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.query("modules",{select:"*",filters:{course_id:t},order:"order_index"}),i=await r.sql(`SELECT l.* FROM lessons l
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const l of i){const u=o.get(l.module_id)||[];u.push(l),o.set(l.module_id,u)}for(const l of n)l.lessons=o.get(l.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,n;try{const i=e.req.param("id"),o=W(e,"sb-access-token");let l=null;if(o){const m=await G(o,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);m&&(l=m.email)}const u=L(e);let c=!1,a=!1;if(l)try{if(c=!!((t=(await u.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[l,parseInt(i)]))[0])!=null&&t.has_access),console.log("Has access:",c,"User:",l,"Lesson:",i),!c){const b=e.env.DATABASE_SUITEPLUS,[w,S,O]=await Promise.all([u.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[l,parseInt(i)]),u.sql("SELECT id FROM member_subscriptions WHERE email_membro = $1 AND data_expiracao > NOW() AND ativo = true LIMIT 1",[l]),b?_t(l,b):Promise.resolve(null)]);if((w.length>0||S.length>0||O&&O>new Date)&&(c=!0),!c){const A=await u.sql("SELECT rentable, rental_credits, title FROM lessons WHERE id = $1",[parseInt(i)]);return console.log("❌ Access denied for user:",l,"lesson:",i),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((r=A[0])==null?void 0:r.rentable)||!1,rental_credits:((s=A[0])==null?void 0:s.rental_credits)||0,lesson_title:((n=A[0])==null?void 0:n.title)||""},403)}}console.log("✅ Access granted for user:",l,"lesson:",i)}catch(m){console.error("❌ Error checking access via RPC:",m),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),a=!0,c=!0}if(!l||!c&&!a){const m=await u.query("lessons",{select:"teste_gratis",filters:{id:i},single:!0});if(!(m!=null&&m.teste_gratis))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await Et(u);const f=`
    ...
    `,[t,parseInt(r)]);return e.json({progress:i||[]})}catch(t){return console.error("❌ /api/progress error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch progress"},500)}});R.post("/api/progress/complete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=L(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0?await s.update("user_progress",{id:n[0].id},{completed:!0,completed_at:new Date().toISOString()}):await s.insert("user_progress",{user_email:t,lesson_id:parseInt(r),completed:!0,completed_at:new Date().toISOString()}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/progress/uncomplete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=L(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0&&await s.delete("user_progress",{id:n[0].id}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/admin/certificate-template",q,async e=>{try{const t=await e.req.json(),{course_id:r,image_data:s,verso_data:n}=t;if(!r||!s)return e.json({error:"ID do curso e imagem da frente são obrigatórios"},400);const i=y=>{const v=y.match(/^data:([^;]+);base64,/);return v?v[1]:"image/jpeg"},o=y=>y.includes(",")?y.split(",")[1]:y,l=i(s),u=o(s),c=n?i(n):null,a=n?o(n):null,f=`/api/certificate-template/${r}/image`,d=n?`/api/certificate-template/${r}/verso`:null,p=L(e),h=await p.query("certificate_templates",{select:"*",filters:{course_id:r}});if(h&&h.length>0){const y={template_url:f,template_data:u,template_mime:l,updated_at:new Date().toISOString()};n!==void 0&&(y.verso_data=a,y.verso_mime=c),await p.update("certificate_templates",{id:h[0].id},y)}else await p.insert("certificate_templates",{course_id:parseInt(r),template_url:f,template_data:u,template_mime:l,verso_data:a,verso_mime:c,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});return console.log("✅ Certificate template saved to Postgres"),e.json({success:!0,template_url:f,verso_url:d,message:"Template de certificado salvo com sucesso!"})}catch(t){return console.error("💥 Certificate template error:",t),e.json({error:"Erro ao salvar template de certificado",details:t.message},500)}});R.get("/api/certificate-template/:courseId/image",async e=>{try{const t=e.req.param("courseId"),s=await L(e).query("certificate_templates",{select:"template_data, template_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.template_data))return e.json({error:"Imagem não encontrada"},404);const n=Uint8Array.from(atob(s.template_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.template_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar imagem"},500)}});R.get("/api/certificate-template/:courseId/verso",async e=>{try{const t=e.req.param("courseId"),s=await L(e).query("certificate_templates",{select:"verso_data, verso_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.verso_data))return e.json({error:"Verso não encontrado"},404);const n=Uint8Array.from(atob(s.verso_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.verso_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar verso"},500)}});R.get("/api/certificate-template/:courseId",async e=>{try{const t=e.req.param("courseId"),r=L(e),s=await r.query("certificate_templates",{select:"id, course_id, template_mime, verso_mime, created_at, updated_at",filters:{course_id:t},single:!0});if(s){s.template_url=`/api/certificate-template/${t}/image`;const n=await r.query("certificate_templates",{select:"verso_data",filters:{course_id:t},single:!0});s.verso_url=n!=null&&n.verso_data?`/api/certificate-template/${t}/verso`:null}return e.json({template:s})}catch{return e.json({template:null})}});R.post("/api/certificates/generate",async e=>{var t,r,s;try{const n=W(e,"sb-access-token");if(!n)return e.json({error:"Não autenticado"},401);const i=await G(n,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!i)return e.json({error:"Usuário não encontrado"},401);const{course_id:o}=await e.req.json();if(console.log("📜 Certificate generation request:",{user_email:i.email,course_id:o}),!o)return e.json({error:"ID do curso é obrigatório"},400);const l=L(e),u=await l.query("certificates",{select:"*",filters:{user_email:i.email,course_id:o}});if(u&&u.length>0)return console.log("✅ Certificate already exists"),e.json({success:!0,certificate:u[0],message:"Certificado já existe!"});const c=await l.query("courses",{select:"*",filters:{id:o},single:!0});if(!c)return e.json({error:"Curso não encontrado"},404);const a=await l.query("modules",{select:"*",filters:{course_id:o}});let f=[];if(a)for(const v of a){const m=await l.query("lessons",{select:"id",filters:{module_id:v.id}});m&&(f=[...f,...m.map(b=>b.id)])}if(f.length===0)return e.json({error:"Curso não possui aulas"},400);const p=(await l.query("user_progress",{select:"*",filters:{user_email:i.email}})||[]).filter(v=>v.completed&&f.includes(v.lesson_id)).map(v=>v.lesson_id),h=p.length/f.length*100;if(console.log("📊 Course completion:",{total_lessons:f.length,completed_lessons:p.length,percentage:h}),h<100)return e.json({error:"Você precisa completar 100% do curso para receber o certificado",completion:h},400);const y=await l.insert("certificates",{user_email:i.email,user_name:((t=i.user_metadata)==null?void 0:t.name)||"Aluno",course_id:parseInt(o),course_title:c.title,issued_at:new Date().toISOString(),completion_date:new Date().toISOString()});return console.log("✅ Certificate generated successfully"),e.json({success:!0,certificate:y,message:"Parabéns! Seu certificado foi gerado com sucesso!"})}catch(n){return console.error("💥 Certificate generation error:",n),console.error("Error details:",n.message),(r=n.message)!=null&&r.includes("certificates")||(s=n.message)!=null&&s.includes("relation")?e.json({error:"Tabela de certificados não encontrada. Execute a migração SQL no Supabase.",details:n.message},500):e.json({error:"Erro ao gerar certificado",details:n.message},500)}});R.get("/api/certificates",async e=>{try{const t=W(e,"sb-access-token");if(!t)return e.json({error:"Não autenticado"},401);const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({error:"Usuário não encontrado"},401);const s=L(e),n=await s.query("certificates",{select:"*",filters:{user_email:r.email},order:"issued_at DESC"})||[],i=await Promise.all(n.map(async o=>{const l=await s.query("certificate_templates",{select:"*",filters:{course_id:o.course_id},single:!0});return{...o,template_url:(l==null?void 0:l.template_url)||null}}));return e.json({certificates:i})}catch(t){return console.error("💥 Certificates fetch error:",t),e.json({error:"Erro ao buscar certificados"},500)}});R.get("/api/certificates/:id",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("certificates",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Certificado não encontrado"},404);const n=await r.query("certificate_templates",{select:"*",filters:{course_id:s.course_id},single:!0});return e.json({certificate:{...s,template_url:(n==null?void 0:n.template_url)||null}})}catch{return e.json({error:"Erro ao buscar certificado"},500)}});R.get("/api/plans",async e=>{try{const r=await L(e).query("plans",{select:"*",filters:{is_active:!0},order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.get("/api/subscriptions/current",async e=>{try{const t=W(e,"sb-access-token");if(!t)return e.json({subscription:null});const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({subscription:null});const i=await L(e).sql(`
      ...
      LIMIT 1
    `,[r.email]);return e.json({subscription:i&&i.length>0?i[0]:null})}catch(t){return console.error("Error fetching subscription:",t),e.json({subscription:null})}});R.post("/api/admin/subscriptions",q,async e=>{try{const{user_email:t,plan_id:r,duration_days:s}=await e.req.json();if(!t||!r)return e.json({error:"Email e plano são obrigatórios"},400);const n=L(e),i=await n.query("plans",{select:"*",filters:{id:r},single:!0});if(!i)return e.json({error:"Plano não encontrado"},404);const o=new Date;o.setDate(o.getDate()+(s||i.duration_days));const l=o.toISOString(),u=await n.insert("member_subscriptions",{email_membro:t,data_expiracao:l,detalhe:i.name,origem:"admin",teste_gratis:i.is_free_trial||!1,ativo:!0}),c=await n.query("users",{select:"id",filters:{email:t},single:!0});return c&&await n.update("users",{id:c.id},{dt_expiracao:l,updated_at:new Date().toISOString()}),e.json({success:!0,message:"Assinatura criada com sucesso!",subscription:u[0]})}catch(t){return console.error("Error creating subscription:",t),e.json({error:"Erro ao criar assinatura"},500)}});R.get("/api/lessons/:id/access",async e=>{var t;try{const r=e.req.param("id"),s=W(e,"sb-access-token");if(!s){const c=await L(e).query("lessons",{select:"teste_gratis",filters:{id:r},single:!0});return e.json({hasAccess:(c==null?void 0:c.teste_gratis)||!1,reason:c!=null&&c.teste_gratis?"free_lesson":"not_authenticated"})}const n=await G(s,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!n)return e.json({hasAccess:!1,reason:"invalid_token"});let l=!!((t=(await L(e).sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[n.email,parseInt(r)]))[0])!=null&&t.has_access);if(!l){const u=e.env.DATABASE_SUITEPLUS;if(u){const c=await _t(n.email,u);c&&c>new Date&&(l=!0)}}return e.json({hasAccess:l,reason:l?"active_subscription":"no_active_subscription"})}catch(r){return console.error("Error checking lesson access:",r),e.json({hasAccess:!1,reason:"error"},500)}});R.post("/api/admin/subscriptions/expire",q,async e=>{try{return await L(e).rpc("expire_subscriptions",{}),e.json({success:!0,message:"Assinaturas expiradas com sucesso!"})}catch(t){return console.error("Error expiring subscriptions:",t),e.json({error:"Erro ao expirar assinaturas"},500)}});R.get("/api/admin/plans",q,async e=>{try{const r=await L(e).query("plans",{select:"*",order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.post("/api/admin/plans",q,async e=>{try{const t=await e.req.json(),{id:r,name:s,description:n,price:i,duration_days:o,is_active:l,is_free_trial:u,features:c,display_order:a}=t,f=L(e);if(r)return await f.update("plans",{id:r},{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:l,is_free_trial:u,features:c||[],display_order:parseInt(a||0),updated_at:new Date().toISOString()}),e.json({success:!0,message:"Plano atualizado!"});{const d=await f.insert("plans",{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:l,is_free_trial:u,features:c||[],display_order:parseInt(a||0)});return e.json({success:!0,plan:d[0],message:"Plano criado!"})}}catch(t){return console.error("Error saving plan:",t),e.json({error:"Erro ao salvar plano"},500)}});R.get("/api/admin/subscriptions",q,async e=>{try{const t=L(e),r=await t.query("subscriptions",{select:"*",order:"created_at DESC"})||[],s=await Promise.all(r.map(async n=>{const i=await t.query("plans",{select:"*",filters:{id:n.plan_id},single:!0});return{...n,plan_name:(i==null?void 0:i.name)||"Desconhecido"}}));return e.json({subscriptions:s})}catch{return e.json({error:"Erro ao buscar assinaturas"},500)}});R.get("/recover",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
