...
---
${i}
---`,l=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:u}]})});if(!l.ok)return e.json({error:`OpenRouter: ${await l.text()}`},500);const f=await l.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});I.post("/api/admin/vimeo-transcript",k,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((m,b)=>{const v=h=>{let y=0;return String(h.language||"").toLowerCase().startsWith("pt")&&(y+=40),h.active!==!1&&(y+=20),h.type==="captions"&&(y+=10),y};return v(b)-v(m)}).find(m=>m.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const u=await fetch(c.link);if(!u.ok)return e.json({error:`Download da legenda falhou: ${u.status}`},502);const f=(await u.text()).replace(/^﻿/,"").split(/\r?\n/).map(m=>m.trim()),d=[];for(const m of f){if(!m||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(m)||m.includes("-->"))continue;const b=m.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();b&&d[d.length-1]!==b&&d.push(b)}const p=d.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/lessons/:id/whisper-transcribe",k,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=D(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.OPENAI_API_KEY||u.OPENAI_API_KEY||u["OPENAI_API-KEY"],d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const S=Object.keys(u).filter(N=>N.toLowerCase().includes("openai"));return e.json({error:`OPENAI_API_KEY não configurado. Chaves OPENAI encontradas: [${S.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(S=>S.link&&S.size).sort((S,N)=>S.size-N.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],h=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let x=Buffer.from(await y.arrayBuffer()),A="video.mp4";const O=24*1024*1024;if(x.length>O){try{const S=await import("os"),N=await import("path"),B=await import("fs"),U=await import("child_process"),F=N.join(S.tmpdir(),`cct_w_${i}.mp4`),z=N.join(S.tmpdir(),`cct_w_${i}.mp3`);B.writeFileSync(F,x),U.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&B.existsSync(z)&&(x=B.readFileSync(z),A="audio.mp3",B.unlinkSync(z)),B.unlinkSync(F)}catch{}if(x.length>O)return e.json({error:`Arquivo muito grande (${h} MB). Use o script de transcrição em lote.`},400)}const C="----WhisperBoundary"+Date.now().toString(16),g=`\r
`,E=A.endsWith(".mp3")?"audio/mpeg":"video/mp4",w=Buffer.concat([Buffer.from(`--${C}${g}Content-Disposition: form-data; name="file"; filename="${A}"${g}Content-Type: ${E}${g}${g}`),x,Buffer.from(`${g}--${C}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-1`),Buffer.from(`${g}--${C}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${C}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${C}--${g}`)]),_=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${C}`,"Content-Length":String(w.length)},body:w});if(!_.ok)return e.json({error:`OpenAI Whisper ${_.status}: ${await _.text()}`},502);let P=(await _.text()).trim(),R=null;if(d&&P){const S=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${P}`,N=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(N.ok){const U=((n=(s=(r=(await N.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(U){P=U;const F=U.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(R=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[P,R,i]),e.json({transcript:P,description:R,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});I.delete("/api/admin/lessons/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=D(e);await Mr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=Zn(e),f=await qr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await _o(l,s,a)){const p=await qr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=D(e);await Te(r);const s=await r.sql(`
      SELECT t.*,
...
<body class="bg-gray-50">
  ...
  `));const Ps=new Xn,To=Object.assign({"/src/index.tsx":I});let ei=!1;for(const[,e]of Object.entries(To))e&&(Ps.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ps.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),ei=!0);if(!ei)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var er={exports:{}},tr={exports:{}},Oe={},rr={},Ts;function ti(){if(Ts)return rr;Ts=1,rr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return rr}var sr,Rs;function ri(){if(Rs)return sr;Rs=1;var e=ti();return sr={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},sr}var nr,Is;function Ro(){if(Is)return nr;Is=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;nr=function(u){if(s.test(u))return Number(u.replace("i","I"));var l=e.exec(u);if(!l)return n(u)||null;var f=!!l[8],d=parseInt(l[1],10);f&&(d=o(d));var p=parseInt(l[2],10)-1,m=l[3],b=parseInt(l[4],10),v=parseInt(l[5],10),h=parseInt(l[6],10),y=l[7];y=y?1e3*parseFloat(y):0;var x,A=i(u);return A!=null?(x=new Date(Date.UTC(d,p,m,b,v,h,y)),a(d)&&x.setUTCFullYear(d),A!==0&&x.setTime(x.getTime()-A)):(x=new Date(d,p,m,b,v,h,y),a(d)&&x.setFullYear(d)),x};function n(c){var u=t.exec(c);if(u){var l=parseInt(u[1],10),f=!!u[4];f&&(l=o(l));var d=parseInt(u[2],10)-1,p=u[3],m=new Date(l,d,p);return a(l)&&m.setFullYear(l),m}}function i(c){if(c.endsWith("+00"))return 0;var u=r.exec(c.split(" ")[1]);if(u){var l=u[1];if(l==="Z")return 0;var f=l==="-"?-1:1,d=parseInt(u[2],10)*3600+parseInt(u[3]||0,10)*60+parseInt(u[4]||0,10);return d*f*1e3}}function o(c){return-(c-1)}function a(c){return c>=0&&c<100}return nr}var ir,Ns;function Io(){if(Ns)return ir;Ns=1,ir=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return ir}var or,Os;function No(){if(Os)return or;Os=1;var e=Io();or=t;function t(v){if(!(this instanceof t))return new t(v);e(this,b(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(h){var y=this[h]||0;return h==="seconds"&&this.milliseconds&&(y=(y+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),y+" "+h},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(y,this).join(""),h=i.map(y,this).join("");return"P"+v+"T"+h;function y(x){var A=this[x]||0;return x==="seconds"&&this.milliseconds&&(A=(A+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),A+s[x]}};var o="([+-]?\\d+)",a=o+"\\s+years?",c=o+"\\s+mons?",u=o+"\\s+days?",l="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([a,c,u,l].map(function(v){return"("+v+")?"}).join("\\s*")),d={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function m(v){var h=v+"000000".slice(v.length);return parseInt(h,10)/1e3}function b(v){if(!v)return{};var h=f.exec(v),y=h[8]==="-";return Object.keys(d).reduce(function(x,A){var O=d[A],C=h[O];return!C||(C=A==="milliseconds"?m(C):parseInt(C,10),!C)||(y&&~p.indexOf(A)&&(C*=-1),x[A]=C),x},{})}return or}var ar,Ds;function Oo(){if(Ds)return ar;Ds=1;var e=Buffer.from||Buffer;return ar=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},ar}var lr,qs;function Do(){if(qs)return lr;qs=1;var e=ti(),t=ri(),r=Ro(),s=No(),n=Oo();function i(g){return function(w){return w===null?w:g(w)}}function o(g){return g===null?g:g==="TRUE"||g==="t"||g==="true"||g==="y"||g==="yes"||g==="on"||g==="1"}function a(g){return g?e.parse(g,o):null}function c(g){return parseInt(g,10)}function u(g){return g?e.parse(g,i(c)):null}function l(g){return g?e.parse(g,i(function(E){return y(E).trim()})):null}var f=function(g){if(!g)return null;var E=t.create(g,function(w){return w!==null&&(w=A(w)),w});return E.parse()},d=function(g){if(!g)return null;var E=t.create(g,function(w){return w!==null&&(w=parseFloat(w)),w});return E.parse()},p=function(g){if(!g)return null;var E=t.create(g);return E.parse()},m=function(g){if(!g)return null;var E=t.create(g,function(w){return w!==null&&(w=r(w)),w});return E.parse()},b=function(g){if(!g)return null;var E=t.create(g,function(w){return w!==null&&(w=s(w)),w});return E.parse()},v=function(g){return g?e.parse(g,i(n)):null},h=function(g){return parseInt(g,10)},y=function(g){var E=String(g);return/^\d+$/.test(E)?E:g},x=function(g){return g?e.parse(g,i(JSON.parse)):null},A=function(g){return g[0]!=="("?null:(g=g.substring(1,g.length-1).split(","),{x:parseFloat(g[0]),y:parseFloat(g[1])})},O=function(g){if(g[0]!=="<"&&g[1]!=="(")return null;for(var E="(",w="",_=!1,P=2;P<g.length-1;P++){if(_||(E+=g[P]),g[P]===")"){_=!0;continue}else if(!_)continue;g[P]!==","&&(w+=g[P])}var R=A(E);return R.radius=parseFloat(w),R},C=function(g){g(20,y),g(21,h),g(23,h),g(26,h),g(700,parseFloat),g(701,parseFloat),g(16,o),g(1082,r),g(1114,r),g(1184,r),g(600,A),g(651,p),g(718,O),g(1e3,a),g(1001,v),g(1005,u),g(1007,u),g(1028,u),g(1016,l),g(1017,f),g(1021,d),g(1022,d),g(1231,d),g(1014,p),g(1015,p),g(1008,p),g(1009,p),g(1040,p),g(1041,p),g(1115,m),g(1182,m),g(1185,m),g(1186,s),g(1187,b),g(17,n),g(114,JSON.parse.bind(JSON)),g(3802,JSON.parse.bind(JSON)),g(199,x),g(3807,x),g(3907,p),g(2951,p),g(791,p),g(1183,p),g(1270,p)};return lr={init:C},lr}var cr,ks;function qo(){if(ks)return cr;ks=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",a,c,u,l,f,d;{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}return a=s%e,c=4294967296*a+n,u=""+c%e,i+u+o}return cr=t,cr}var ur,js;function ko(){if(js)return ur;js=1;var e=qo(),t=function(p,m,b,v,h){b=b||0,v=v||!1,h=h||function(_,P,R){return _*Math.pow(2,R)+P};var y=b>>3,x=function(_){return v?~_&255:_},A=255,O=8-b%8;m<O&&(A=255<<8-m&255,O=m),b&&(A=A>>b%8);var C=0;b%8+m>=8&&(C=h(0,x(p[y])&A,O));for(var g=m+b>>3,E=y+1;E<g;E++)C=h(C,x(p[E]),8);var w=(m+b)%8;return w>0&&(C=h(C,x(p[g])>>8-w,w)),C},r=function(p,m,b){var v=Math.pow(2,b-1)-1,h=t(p,1),y=t(p,b,1);if(y===0)return 0;var x=1,A=function(C,g,E){C===0&&(C=1);for(var w=1;w<=E;w++)x/=2,(g&1<<E-w)>0&&(C+=x);return C},O=t(p,m,b+1,!1,A);return y==Math.pow(2,b+1)-1?O===0?h===0?1/0:-1/0:NaN:(h===0?1:-1)*Math.pow(2,y-v)*O},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},a=function(p){var m=t(p,16,32);if(m==49152)return NaN;for(var b=Math.pow(1e4,t(p,16,16)),v=0,h=t(p,16),y=0;y<h;y++)v+=t(p,16,64+16*y)*b,b/=1e4;var x=Math.pow(10,t(p,16,48));return(m===0?1:-1)*Math.round(v*x)/x},c=function(p,m){var b=t(m,1),v=t(m,63,1),h=new Date((b===0?1:-1)*v/1e3+9466848e5);return p||h.setTime(h.getTime()+h.getTimezoneOffset()*6e4),h.usec=v%1e3,h.getMicroSeconds=function(){return this.usec},h.setMicroSeconds=function(y){this.usec=y},h.getUTCMicroSeconds=function(){return this.usec},h},u=function(p){var m=t(p,32);t(p,32,32);for(var b=t(p,32,64),v=96,h=[],y=0;y<m;y++)h[y]=t(p,32,v),v+=32,v+=32;var x=function(O){var C=t(p,32,v);if(v+=32,C==4294967295)return null;var g;if(O==23||O==20)return g=t(p,C*8,v),v+=C*8,g;if(O==25)return g=p.toString(this.encoding,v>>3,(v+=C<<3)>>3),g;console.log("ERROR: ElementType not implemented: "+O)},A=function(O,C){var g=[],E;if(O.length>1){var w=O.shift();for(E=0;E<w;E++)g[E]=A(O,C);O.unshift(w)}else for(E=0;E<O[0];E++)g[E]=x(C);return g};return A(h,b)},l=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},d=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,a),p(700,i),p(701,o),p(16,f),p(1114,c.bind(null,!1)),p(1184,c.bind(null,!0)),p(1e3,u),p(1007,u),p(1016,u),p(1008,u),p(1009,u),p(25,l)};return ur={init:d},ur}var dr,Ls;function jo(){return Ls||(Ls=1,dr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),dr}var Bs;function vt(){if(Bs)return Oe;Bs=1;var e=Do(),t=ko(),r=ri(),s=jo();Oe.getTypeParser=o,Oe.setTypeParser=a,Oe.arrayParser=r,Oe.builtins=s;var n={text:{},binary:{}};function i(c){return String(c)}function o(c,u){return u=u||"text",n[u]&&n[u][c]||i}function a(c,u,l){typeof u=="function"&&(l=u,u="text"),n[u][c]=l}return e.init(function(c,u){n.text[c]=u}),t.init(function(c,u){n.binary[c]=u}),Oe}var Ms;function wt(){return Ms||(Ms=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=vt(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(tr)),tr.exports}var fr,Us;function Ze(){if(Us)return fr;Us=1;const e=wt(),t=He,{isDate:r}=t.types||t;function s(d){return'"'+d.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(d){let p="{";for(let m=0;m<d.length;m++)if(m>0&&(p=p+","),d[m]===null||typeof d[m]>"u")p=p+"NULL";else if(Array.isArray(d[m]))p=p+n(d[m]);else if(ArrayBuffer.isView(d[m])){let b=d[m];if(!(b instanceof Buffer)){const v=Buffer.from(b.buffer,b.byteOffset,b.byteLength);v.length===b.byteLength?b=v:b=v.slice(b.byteOffset,b.byteOffset+b.byteLength)}p+="\\\\x"+b.toString("hex")}else p+=s(i(d[m]));return p=p+"}",p}const i=function(d,p){if(d==null)return null;if(typeof d=="object"){if(d instanceof Buffer)return d;if(ArrayBuffer.isView(d)){const m=Buffer.from(d.buffer,d.byteOffset,d.byteLength);return m.length===d.byteLength?m:m.slice(d.byteOffset,d.byteOffset+d.byteLength)}return r(d)?e.parseInputDatesAsUTC?c(d):a(d):Array.isArray(d)?n(d):o(d,p)}return d.toString()};function o(d,p){if(d&&typeof d.toPostgres=="function"){if(p=p||[],p.indexOf(d)!==-1)throw new Error('circular reference detected while preparing "'+d+'" for query');return p.push(d),i(d.toPostgres(i),p)}return JSON.stringify(d)}function a(d){let p=-d.getTimezoneOffset(),m=d.getFullYear();const b=m<1;b&&(m=Math.abs(m)+1);let v=String(m).padStart(4,"0")+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0")+"T"+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")+":"+String(d.getSeconds()).padStart(2,"0")+"."+String(d.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),b&&(v+=" BC"),v}function c(d){let p=d.getUTCFullYear();const m=p<1;m&&(p=Math.abs(p)+1);let b=String(p).padStart(4,"0")+"-"+String(d.getUTCMonth()+1).padStart(2,"0")+"-"+String(d.getUTCDate()).padStart(2,"0")+"T"+String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")+":"+String(d.getUTCSeconds()).padStart(2,"0")+"."+String(d.getUTCMilliseconds()).padStart(3,"0");return b+="+00:00",m&&(b+=" BC"),b}function u(d,p,m){return d=typeof d=="string"?{text:d}:d,p&&(typeof p=="function"?d.callback=p:d.values=p),m&&(d.callback=m),d}return fr={prepareValue:function(p){return i(p)},normalizeQueryConfig:u,escapeIdentifier:function(d){return'"'+d.replace(/"/g,'""')+'"'},escapeLiteral:function(d){let p=!1,m="'";if(d==null||typeof d!="string")return"''";for(let b=0;b<d.length;b++){const v=d[b];v==="'"?m+=v+v:v==="\\"?(m+=v+v,p=!0):m+=v}return m+="'",p===!0&&(m=" E"+m),m}},fr}var lt={exports:{}},pr,Hs;function Lo(){if(Hs)return pr;Hs=1;const e=Cn;function t(a){return e.createHash("md5").update(a,"utf-8").digest("hex")}function r(a,c,u){const l=t(c+a);return"md5"+t(Buffer.concat([Buffer.from(l),u]))}function s(a){return e.createHash("sha256").update(a).digest()}function n(a,c){return a=a.replace(/(\D)-/,"$1"),e.createHash(a).update(c).digest()}function i(a,c){return e.createHmac("sha256",a).update(c).digest()}async function o(a,c,u){return e.pbkdf2Sync(a,c,u,32,"sha256")}return pr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},pr}var hr,Fs;function Bo(){if(Fs)return hr;Fs=1;const e=Cn;hr={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:l,sha256:a,hashByName:c,hmacSha256:u,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,m=await r.digest("MD5",p);return Array.from(new Uint8Array(m)).map(b=>b.toString(16).padStart(2,"0")).join("")}}async function o(f,d,p){const m=await i(d+f);return"md5"+await i(Buffer.concat([Buffer.from(m),p]))}async function a(f){return await r.digest("SHA-256",f)}async function c(f,d){return await r.digest(f,d)}async function u(f,d){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(d))}async function l(f,d,p){const m=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),b={name:"PBKDF2",hash:"SHA-256",salt:d,iterations:p};return await r.deriveBits(b,m,256,["deriveBits"])}return hr}var zs;function si(){return zs||(zs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?lt.exports=Lo():lt.exports=Bo()),lt.exports}var mr,Qs;function Mo(){if(Qs)return mr;Qs=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let a=i[o++];if(a<128)return{length:a,index:o};const c=a&127;if(c>4)throw e("bad length",i);a=0;for(let u=0;u<c;u++)a=a<<8|i[o++];return{length:a,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:a,index:c}=t(i,o);o=c;const u=o+a,l=i[o++];let f=(l/40>>0)+"."+l%40;for(;o<u;){let d=0;for(;o<u;){const p=i[o++];if(d=d<<7|p&127,p<128)break}f+="."+d}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:a,index:c}=s(i,o);o=c+a,o=s(i,o).index;const{oid:u,index:l}=r(i,o);switch(u){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=l,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+u,i)}return mr={signatureAlgorithmHashFromCertificate:n},mr}var gr,Vs;function Uo(){if(Vs)return gr;Vs=1;const e=si(),{signatureAlgorithmHashFromCertificate:t}=Mo();function r(f,d){const p=["SCRAM-SHA-256"];d&&p.unshift("SCRAM-SHA-256-PLUS");const m=p.find(h=>f.includes(h));if(!m)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(m==="SCRAM-SHA-256-PLUS"&&typeof d.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const b=e.randomBytes(18).toString("base64");return{mechanism:m,clientNonce:b,response:(m==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":d?"y":"n")+",,n=*,r="+b,message:"SASLInitialResponse"}}async function s(f,d,p,m){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(d==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const b=c(p);if(b.nonce.startsWith(f.clientNonce)){if(b.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,h="r="+b.nonce+",s="+b.salt+",i="+b.iteration;let y=m?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const S=m.getPeerCertificate().raw;let N=t(S);(N==="MD5"||N==="SHA-1")&&(N="SHA-256");const B=await e.hashByName(N,S);y=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(B)]).toString("base64")}const x="c="+y+",r="+b.nonce,A=v+","+h+","+x,O=Buffer.from(b.salt,"base64"),C=await e.deriveKey(d,O,b.iteration),g=await e.hmacSha256(C,"Client Key"),E=await e.sha256(g),w=await e.hmacSha256(E,A),_=l(Buffer.from(g),Buffer.from(w)).toString("base64"),P=await e.hmacSha256(C,"Server Key"),R=await e.hmacSha256(P,A);f.message="SASLResponse",f.serverSignature=Buffer.from(R).toString("base64"),f.response=x+",p="+_}function n(f,d){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=u(d);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((d,p)=>f.charCodeAt(p)).every(d=>d>=33&&d<=43||d>=45&&d<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function a(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(d=>{if(!/^.=/.test(d))throw new Error("SASL: Invalid attribute pair entry");const p=d[0],m=d.substring(2);return[p,m]}))}function c(f){const d=a(f),p=d.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const m=d.get("s");if(m){if(!o(m))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const b=d.get("i");if(b){if(!/^[1-9][0-9]*$/.test(b))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(b,10);return{nonce:p,salt:m,iteration:v}}function u(f){const p=a(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function l(f,d){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(d))throw new TypeError("second argument must be a Buffer");if(f.length!==d.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,m)=>f[m]^d[m]))}return gr={startSession:r,continueSession:s,finalizeSession:n},gr}var br,Xs;function Fr(){if(Xs)return br;Xs=1;const e=vt();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},br=t,br}var yr,Ws;function Ho(){if(Ws)return yr;Ws=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const a={};let c,u=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(i,"postgres://base")}catch{c=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),u=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of c.searchParams.entries())a[p[0]]=p[1];if(a.user=a.user||decodeURIComponent(c.username),a.password=a.password||decodeURIComponent(c.password),c.protocol=="socket:")return a.host=decodeURI(c.pathname),a.database=c.searchParams.get("db"),a.client_encoding=c.searchParams.get("encoding"),a;const l=u?"":c.hostname;a.host?l&&/^%2f/i.test(l)&&(c.pathname=l+c.pathname):a.host=decodeURIComponent(l),a.port||(a.port=c.port);const f=c.pathname.slice(1)||null;a.database=f?decodeURI(f):null,(a.ssl==="true"||a.ssl==="1")&&(a.ssl=!0),a.ssl==="0"&&(a.ssl=!1),(a.sslcert||a.sslkey||a.sslrootcert||a.sslmode)&&(a.ssl={});const d=a.sslcert||a.sslkey||a.sslrootcert?Pn:null;if(a.sslcert&&(a.ssl.cert=d.readFileSync(a.sslcert).toString()),a.sslkey&&(a.ssl.key=d.readFileSync(a.sslkey).toString()),a.sslrootcert&&(a.ssl.ca=d.readFileSync(a.sslrootcert).toString()),o.useLibpqCompat&&a.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(a.uselibpqcompat==="true"||o.useLibpqCompat)switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":{a.ssl.rejectUnauthorized=!1;break}case"require":{a.sslrootcert?a.ssl.checkServerIdentity=function(){}:a.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!a.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");a.ssl.checkServerIdentity=function(){};break}}else switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{a.sslmode!=="verify-full"&&n(a.sslmode);break}case"no-verify":{a.ssl.rejectUnauthorized=!1;break}}return a}function t(i){return Object.entries(i).reduce((a,[c,u])=>(u!=null&&(a[c]=u),a),{})}function r(i){return Object.entries(i).reduce((a,[c,u])=>{if(c==="ssl"){const l=u;typeof l=="boolean"&&(a[c]=l),typeof l=="object"&&(a[c]=t(l))}else if(u!=null)if(c==="port"){if(u!==""){const l=parseInt(u,10);if(isNaN(l))throw new Error(`Invalid ${c}: ${u}`);a[c]=l}}else a[c]=u;return a},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
