...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      `):e.redirect(`/?error=${r}`);const s=t.searchParams.get("access_token")||((c=t.hash.match(/access_token=([^&]+)/))==null?void 0:c[1]),n=t.searchParams.get("refresh_token")||((u=t.hash.match(/refresh_token=([^&]+)/))==null?void 0:u[1]),i=t.searchParams.get("type")||((a=t.hash.match(/type=([^&]+)/))==null?void 0:a[1]);if(!s)return e.redirect("/?error=no_token");try{const p=(f=JSON.parse(atob(s.split(".")[1])).amr)==null?void 0:f.some(m=>m.method==="otp");if(i==="recovery"||p)return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}catch{if(i==="recovery")return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}return ne(e,"sb-access-token",s,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),n&&ne(e,"sb-refresh-token",n,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.redirect("/?auth=success")}I.post("/api/auth/callback",async e=>{try{const{access_token:t,refresh_token:r}=await e.req.json();return t?(ne(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),r&&ne(e,"sb-refresh-token",r,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0})):e.json({error:"No access token"},400)}catch{return e.json({error:"Callback failed"},500)}});I.post("/api/auth/forgot-password",async e=>{try{const{email:t}=await e.req.json();if(!t)return e.json({error:"Email is required"},400);const r=e.req.header("host")||"localhost:3000",n=`${r.includes("localhost")?"http":"https"}://${r}/auth/callback`,i=await fetch(`${e.env.SUPABASE_URL}/auth/v1/recover`,{method:"POST",headers:{"Content-Type":"application/json",apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({email:t,options:{redirectTo:n}})});if(i.ok)return e.json({success:!0,message:"Se o email estiver cadastrado, você receberá um link de recuperação. O link é válido por 1 hora."});const o=await i.json();return e.json({error:o.error_description||"Failed to send reset email"},400)}catch{return e.json({error:"Failed to process request"},500)}});I.post("/api/auth/reset-password",async e=>{try{const t=await e.req.json(),{token:r,password:s}=t;if(console.log("🔐 Password reset attempt"),console.log("   Token present:",!!r),console.log("   Token length:",r==null?void 0:r.length),console.log("   Password length:",s==null?void 0:s.length),!r||!s)return console.error("❌ Missing token or password"),e.json({error:"Token e senha são obrigatórios"},400);if(s.length<6)return console.error("❌ Password too short"),e.json({error:"A senha deve ter pelo menos 6 caracteres"},400);console.log("📨 Calling Supabase to update password...");const n=await fetch(`${e.env.SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({password:s})});if(console.log("📨 Supabase response:",{status:n.status,ok:n.ok}),!n.ok){const o=await n.json();console.error("❌ Password reset failed:",o);let l="Falha ao redefinir senha";return o.error_code==="same_password"?l="A nova senha deve ser diferente da senha atual":o.msg?l=o.msg:o.error_description?l=o.error_description:o.message&&(l=o.message),e.json({error:l},400)}const i=await n.json();return console.log("✅ Password reset successful"),i.access_token&&ne(e,"sb-access-token",i.access_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),i.refresh_token&&ne(e,"sb-refresh-token",i.refresh_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0,message:"Senha alterada com sucesso!"})}catch{return e.json({error:"Failed to reset password"},500)}});async function zr(e,t,r,s){try{return await new $i(t,r).query("users",{select:"id, email, isadmin",filters:{email:e,isadmin:!0},single:!0},s)!==null}catch(n){return console.error("Error checking admin access in Supabase users:",n),!1}}async function k(e,t){const r=G(e,"sb-access-token");if(!r)return e.json({error:"Unauthorized"},401);const s=await Y(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!s)return e.json({error:"Invalid token"},401);if(!await zr(s.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))return e.json({error:"Forbidden - Admin only"},403);e.set("user",s),await t()}I.get("/api/admin/check",async e=>{const t=G(e,"sb-access-token");if(!t)return e.json({isAdmin:!1});const r=await Y(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({isAdmin:!1});const s=await zr(r.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,t);return e.json({isAdmin:s})});I.post("/api/admin/impersonate",k,async e=>{try{const{user_email:t}=await e.req.json();if(!t)return e.json({error:"user_email is required"},400);console.log(`🎭 Admin impersonating user: ${t}`);const s=await D(e).query("users",{select:"*",filters:{email:t}});if(!s||s.length===0)return e.json({error:"User not found"},404);const n=s[0],i={email:t,nome:n.nome||"Usuário",impersonated:!0,impersonated_at:new Date().toISOString(),user_id:n.id,signature:Buffer.from(`${t}:${e.env.SUPABASE_ANON_KEY}`).toString("base64")},o=`IMPERSONATE:${Buffer.from(JSON.stringify(i)).toString("base64")}`;return console.log(`✅ Impersonation token created for ${t}`),e.json({token:o,user_email:t,user_name:n.nome})}catch(t){return console.error("Impersonation error:",t),e.json({error:t.message||"Failed to impersonate user"},500)}});I.post("/api/admin/courses",k,async e=>{try{const{title:t,description:r,duration_hours:s,instructor:n,offers_certificate:i,is_published:o}=await e.req.json(),c=await D(e).insert("courses",{title:t,description:r||null,duration_hours:s||0,instructor:n||"Vicelmo",offers_certificate:i!==void 0?i:!0,is_published:o!==void 0?o:!0});return e.json({success:!0,course_id:c[0].id})}catch(t){return console.error("Create course error:",t),e.json({error:t.message||"Failed to create course"},500)}});I.put("/api/admin/courses/:id",k,async e=>{try{const t=e.req.param("id"),{title:r,description:s,duration_hours:n,instructor:i,offers_certificate:o,is_published:l}=await e.req.json();return await D(e).update("courses",{id:t},{title:r,description:s||null,duration_hours:n||0,instructor:i||"Vicelmo",offers_certificate:o!==void 0?o:!0,is_published:l!==void 0?l:!0}),e.json({success:!0})}catch{return e.json({error:"Failed to update course"},500)}});I.delete("/api/admin/courses/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("courses",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete course"},500)}});I.get("/api/admin/courses/find",k,async e=>{try{const t=e.req.query("title");if(!t)return e.json({error:"Title is required"},400);const s=await D(e).query("courses",{select:"*",filters:{title:t},limit:1});return s&&s.length>0?e.json({course:s[0]}):e.json({course:null})}catch(t){return console.error("Find course error:",t),e.json({error:t.message||"Failed to find course"},500)}});I.post("/api/admin/modules",k,async e=>{try{const{course_id:t,title:r,description:s,order_index:n}=await e.req.json(),o=await D(e).insert("modules",{course_id:t,title:r,description:s||null,order_index:n||0});return e.json({success:!0,module_id:o[0].id})}catch(t){return console.error("Create module error:",t),e.json({error:t.message||"Failed to create module"},500)}});I.put("/api/admin/modules/:id",k,async e=>{try{const t=e.req.param("id"),{title:r,description:s,order_index:n}=await e.req.json();return await D(e).update("modules",{id:t},{title:r,description:s||null,order_index:n}),e.json({success:!0})}catch{return e.json({error:"Failed to update module"},500)}});I.delete("/api/admin/modules/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("modules",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete module"},500)}});I.get("/api/admin/modules/find",k,async e=>{try{const t=e.req.query("course_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"course_id and title are required"},400);const n=await D(e).query("modules",{select:"*",filters:{course_id:t,title:r},limit:1});return n&&n.length>0?e.json({module:n[0]}):e.json({module:null})}catch(t){return console.error("Find module error:",t),e.json({error:t.message||"Failed to find module"},500)}});I.post("/api/admin/lessons",k,async e=>{try{const{module_id:t,title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:l,free_trial:c,support_text:u,transcript:a,attachments:f,rentable:d,rental_credits:p}=await e.req.json();let m=null;n&&i&&(n==="youtube"?m=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?m=`https://vimeo.com/${i}`:m=i);const v=await D(e).insert("lessons",{module_id:t,title:r,description:s||null,video_url:m,video_provider:n||null,video_id:i||null,duration_minutes:o||0,order_index:l||0,teste_gratis:c||!1,support_text:u||null,transcript:a||null,attachments:JSON.stringify(f||[]),rentable:d||!1,rental_credits:p||0});return e.json({success:!0,lesson_id:v[0].id})}catch(t){return console.error("Create lesson error:",t),e.json({error:t.message||"Failed to create lesson"},500)}});I.post("/api/admin/lessons-reorder",k,async e=>{try{const{lessons:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"lessons must be an array"},400);const r=D(e);for(const{id:s,order_index:n}of t)await r.update("lessons",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder lessons error:",t),e.json({error:t.message||"Failed to reorder lessons"},500)}});I.put("/api/admin/lessons/:id",k,async e=>{try{const t=e.req.param("id"),{title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:l,free_trial:c,support_text:u,transcript:a,attachments:f,rentable:d,rental_credits:p}=await e.req.json();let m=null;n&&i&&(n==="youtube"?m=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?m=`https://vimeo.com/${i}`:m=i);const b=D(e),v=parseInt(t);await b.update("lessons",{id:v},{title:r,description:s||null,video_url:m,video_provider:n||null,video_id:i||null,duration_minutes:parseInt(o)||0,order_index:parseInt(l)||0,teste_gratis:c===!0||c==="true"});try{await b.sql("UPDATE lessons SET support_text = $1, transcript = $2, attachments = $3::jsonb WHERE id = $4",[u||null,a||null,JSON.stringify(f||[]),v])}catch(h){console.warn("support_text/transcript/attachments columns may not exist:",h.message)}return await b.sql("UPDATE lessons SET rentable = $1, rental_credits = $2 WHERE id = $3",[d===!0||d==="true",parseInt(p)||0,v]),e.json({success:!0})}catch(t){return console.error("Update lesson error:",t),e.json({error:t.message||"Failed to update lesson"},500)}});I.post("/api/admin/lessons/:id/generate-transcript",k,async e=>{var t,r,s;try{const n=parseInt(e.req.param("id")),{context:i}=await e.req.json(),l=await D(e).sql("SELECT title, transcript FROM lessons WHERE id = $1",[n]);if(!l.length)return e.json({error:"Aula não encontrada"},404);const c=l[0];if(!c.transcript)return e.json({error:"Esta aula não possui transcrição para estruturar."},400);const u=e.env.VITE_OPENROUTER_API_KEY;if(!u)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada no ambiente"},500);const a="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",f=`Abaixo está a transcrição bruta de uma aula chamada "${c.title}".${i?`
...
---
${i}
---`,a=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:u}]})});if(!a.ok)return e.json({error:`OpenRouter: ${await a.text()}`},500);const f=await a.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});I.post("/api/admin/vimeo-transcript",k,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((m,b)=>{const v=h=>{let y=0;return String(h.language||"").toLowerCase().startsWith("pt")&&(y+=40),h.active!==!1&&(y+=20),h.type==="captions"&&(y+=10),y};return v(b)-v(m)}).find(m=>m.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const u=await fetch(c.link);if(!u.ok)return e.json({error:`Download da legenda falhou: ${u.status}`},502);const f=(await u.text()).replace(/^﻿/,"").split(/\r?\n/).map(m=>m.trim()),d=[];for(const m of f){if(!m||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(m)||m.includes("-->"))continue;const b=m.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();b&&d[d.length-1]!==b&&d.push(b)}const p=d.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/lessons/:id/whisper-transcribe",k,async e=>{var t,r,s,n,i,o,l,c,u;try{const a=parseInt(e.req.param("id")),f=D(e),d=await f.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[a]);if(!d.length)return e.json({error:"Aula não encontrada"},404);const p=d[0];if(p.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!p.video_id||!/^\d+$/.test(p.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const m=e.env.VIMEO_ACCESS_TOKEN||((r=(t=globalThis.process)==null?void 0:t.env)==null?void 0:r.VIMEO_ACCESS_TOKEN),b=e.env.OPENAI_API_KEY||((n=(s=globalThis.process)==null?void 0:s.env)==null?void 0:n.OPENAI_API_KEY),v=e.env.VITE_OPENROUTER_API_KEY||((o=(i=globalThis.process)==null?void 0:i.env)==null?void 0:o.VITE_OPENROUTER_API_KEY);if(!m)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!b)return e.json({error:"OPENAI_API_KEY não configurado"},500);const h=await fetch(`https://api.vimeo.com/videos/${p.video_id}?fields=download`,{headers:{Authorization:`Bearer ${m}`}});if(!h.ok)return e.json({error:`Vimeo API ${h.status}`},502);const E=((await h.json()).download||[]).filter(H=>H.link&&H.size).sort((H,z)=>H.size-z.size);if(!E.length)return e.json({error:"Download não disponível para este vídeo"},400);const C=E[0],N=(C.size/1024/1024).toFixed(1),T=await fetch(C.link);if(!T.ok)return e.json({error:`Download falhou: HTTP ${T.status}`},502);let g=Buffer.from(await T.arrayBuffer()),x="video.mp4";const w=24*1024*1024;if(g.length>w){try{const H=await import("os"),z=await import("path"),he=await import("fs"),oe=await import("child_process"),ee=z.join(H.tmpdir(),`cct_w_${a}.mp4`),Ee=z.join(H.tmpdir(),`cct_w_${a}.mp3`);he.writeFileSync(ee,g),oe.spawnSync("ffmpeg",["-y","-i",ee,"-vn","-ar","16000","-ac","1","-b:a","32k",Ee],{stdio:"pipe"}).status===0&&he.existsSync(Ee)&&(g=he.readFileSync(Ee),x="audio.mp3",he.unlinkSync(Ee)),he.unlinkSync(ee)}catch{}if(g.length>w)return e.json({error:`Arquivo muito grande (${N} MB). Use o script de transcrição em lote.`},400)}const _="----WhisperBoundary"+Date.now().toString(16),A=`\r
`,R=x.endsWith(".mp3")?"audio/mpeg":"video/mp4",S=Buffer.concat([Buffer.from(`--${_}${A}Content-Disposition: form-data; name="file"; filename="${x}"${A}Content-Type: ${R}${A}${A}`),g,Buffer.from(`${A}--${_}${A}Content-Disposition: form-data; name="model"${A}${A}whisper-1`),Buffer.from(`${A}--${_}${A}Content-Disposition: form-data; name="language"${A}${A}pt`),Buffer.from(`${A}--${_}${A}Content-Disposition: form-data; name="response_format"${A}${A}text`),Buffer.from(`${A}--${_}--${A}`)]),O=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${b}`,"Content-Type":`multipart/form-data; boundary=${_}`,"Content-Length":String(S.length)},body:S});if(!O.ok)return e.json({error:`OpenAI Whisper ${O.status}: ${await O.text()}`},502);let B=(await O.text()).trim(),U=null;if(v&&B){const H=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).

Recebi a transcrição bruta de uma aula chamada "${p.title}". Organize esta transcrição em Markdown estruturado com:
...
TRANSCRIÇÃO:
${B}`,z=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${v}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:H}]})});if(z.ok){const oe=((u=(c=(l=(await z.json()).choices)==null?void 0:l[0])==null?void 0:c.message)==null?void 0:u.content)||"";if(oe){B=oe;const ee=oe.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);ee&&(U=ee[1].trim())}}}return await f.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[B,U,a]),e.json({transcript:B,description:U,sizeMB:N})}catch(a){return e.json({error:a.message},500)}});I.delete("/api/admin/lessons/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=D(e);await Hr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],l=parseInt(o.rental_credits);if(!Number.isFinite(l)||l<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const a=Zn(e),f=await jr(a,s);if(f<l)return e.json({error:"Créditos insuficientes",available:f,required:l},400);if(!await _o(a,s,l)){const p=await jr(a,s);return e.json({error:"Créditos insuficientes",available:p,required:l},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       ...
       ORDER BY lr.expires_at DESC`,[t.email]);return e.json({rentals:s})}catch(t){return console.error("Get rentals error:",t),e.json({error:t.message||"Erro ao buscar aluguéis"},500)}});async function Oe(e){await e.sql(`
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=D(e);await Oe(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});I.get("/api/admin/trails",k,async e=>{try{const t=D(e);await Oe(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails",k,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=D(e);await Oe(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});I.put("/api/admin/trails/:id",k,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=D(e);await Oe(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.delete("/api/admin/trails/:id",k,async e=>{try{const t=parseInt(e.req.param("id")),r=D(e);return await Oe(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/lessons",k,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=D(e);await Oe(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});I.delete("/api/admin/trails/:id/lessons/:lessonId",k,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await D(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/reorder",k,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=D(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/trails/search-lessons",k,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=D(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],l=await s.sql(`
      ...
      LIMIT 50
    `,o);return e.json({lessons:l})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/lessons/find",k,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const n=await D(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return n&&n.length>0?e.json({lesson:n[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});I.post("/api/admin/run-migration-lesson-fields",k,async e=>{try{const t=D(e);return await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS support_text TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS transcript TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'::jsonb"),await Hr(t),e.json({success:!0,message:"Migration applied successfully"})}catch(t){return console.error("Migration error:",t),e.json({error:t.message},500)}});I.get("/api/admin/users",k,async e=>{try{const r=await D(e).query("users",{select:"*",order:"created_at DESC"});return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});I.get("/api/admin/users/find",k,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await D(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});I.post("/api/admin/users",k,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await D(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});I.put("/api/admin/users/:id",k,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await D(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});I.delete("/api/admin/users/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});I.get("/api/admin/certificates",k,async e=>{try{const r=await D(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});I.get("/api/admin/certificates/:id",k,async e=>{try{const t=e.req.param("id"),s=await D(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});I.get("/api/admin/certificates/find",k,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await D(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});I.post("/api/admin/certificates",k,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=D(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});I.put("/api/admin/certificates/:id",k,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=D(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});I.delete("/api/admin/certificates/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function To(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const u=s?e.modules.map((f,d)=>`<div class="mod-item"><span class="mod-num">${String(d+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=r?`
...
<body>
  ...
  <div class="page">
    ...
    <div class="logo-section">
      <img class="logo-img" src="${Zr}" alt="Ensino Plus"/>
    </div>
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `)}catch(t){return console.error("Verify certificate error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});I.get("/api/verify/:code",async e=>{try{const t=e.req.param("code"),r=D(e),s=await r.query("certificates",{select:"*",filters:{certificate_code:t}});if(!s||s.length===0)return e.json({valid:!1,message:"Certificate not found"},404);const n=s[0];try{await r.update("certificates",{id:n.id},{verification_count:(n.verification_count||0)+1})}catch{}return e.json({valid:!0,certificate:{student_name:n.user_name,course_title:n.course_title,workload:n.carga_horaria,completion_date:n.completion_date,issued_at:n.issued_at,certificate_code:n.certificate_code,verification_count:(n.verification_count||0)+1}})}catch(t){return console.error("Verify certificate API error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});I.get("/api/admin/member-subscriptions",k,async e=>{try{const r=await D(e).query("member_subscriptions",{select:"*",order:"created_at DESC"});return e.json({subscriptions:r||[]})}catch(t){return console.error("List member subscriptions error:",t),e.json({error:t.message||"Failed to list member subscriptions"},500)}});I.get("/api/admin/member-subscriptions/find",k,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email parameter is required"},400);const s=await D(e).query("member_subscriptions",{select:"*",filters:{email_membro:t}});return e.json({subscriptions:s||[]})}catch(t){return console.error("Find member subscription error:",t),e.json({error:t.message||"Failed to find member subscription"},500)}});I.post("/api/admin/member-subscriptions",k,async e=>{try{const t=await e.req.json();if(!t.email_membro)return e.json({error:"Email is required"},400);const s=await D(e).insert("member_subscriptions",{email_membro:t.email_membro,data_expiracao:t.data_expiracao||null,detalhe:t.detalhe||null,origem:t.origem||null,teste_gratis:t.teste_gratis||!1,ativo:t.ativo!==void 0?t.ativo:!0});return e.json({success:!0,subscription_id:s&&s.length>0?s[0].id:null})}catch(t){return console.error("Create member subscription error:",t),e.json({error:t.message||"Failed to create member subscription"},500)}});I.put("/api/admin/member-subscriptions/:id",k,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await D(e).update("member_subscriptions",{id:t},{email_membro:r.email_membro,data_expiracao:r.data_expiracao,detalhe:r.detalhe,origem:r.origem,teste_gratis:r.teste_gratis,ativo:r.ativo,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update member subscription error:",t),e.json({error:t.message||"Failed to update member subscription"},500)}});I.delete("/api/admin/member-subscriptions/:id",k,async e=>{try{const t=e.req.param("id");return await D(e).delete("member_subscriptions",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});I.get("/api/courses",async e=>{try{const t=D(e),r=G(e,"sb-access-token");let s=!1;if(r){const l=await Y(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);l&&(s=await zr(l.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))}const n=s?{}:{is_published:!0},i=s?"":"WHERE c.is_published = true",o=await t.sql(`
      SELECT c.*,
...
<body class="bg-gray-50">
  ...
  `));const Ts=new Xn,Po=Object.assign({"/src/index.tsx":I});let ei=!1;for(const[,e]of Object.entries(Po))e&&(Ts.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ts.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),ei=!0);if(!ei)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var rr={exports:{}},sr={exports:{}},je={},nr={},Ps;function ti(){if(Ps)return nr;Ps=1,nr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return nr}var ir,Rs;function ri(){if(Rs)return ir;Rs=1;var e=ti();return ir={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},ir}var or,Is;function Ro(){if(Is)return or;Is=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;or=function(u){if(s.test(u))return Number(u.replace("i","I"));var a=e.exec(u);if(!a)return n(u)||null;var f=!!a[8],d=parseInt(a[1],10);f&&(d=o(d));var p=parseInt(a[2],10)-1,m=a[3],b=parseInt(a[4],10),v=parseInt(a[5],10),h=parseInt(a[6],10),y=a[7];y=y?1e3*parseFloat(y):0;var E,C=i(u);return C!=null?(E=new Date(Date.UTC(d,p,m,b,v,h,y)),l(d)&&E.setUTCFullYear(d),C!==0&&E.setTime(E.getTime()-C)):(E=new Date(d,p,m,b,v,h,y),l(d)&&E.setFullYear(d)),E};function n(c){var u=t.exec(c);if(u){var a=parseInt(u[1],10),f=!!u[4];f&&(a=o(a));var d=parseInt(u[2],10)-1,p=u[3],m=new Date(a,d,p);return l(a)&&m.setFullYear(a),m}}function i(c){if(c.endsWith("+00"))return 0;var u=r.exec(c.split(" ")[1]);if(u){var a=u[1];if(a==="Z")return 0;var f=a==="-"?-1:1,d=parseInt(u[2],10)*3600+parseInt(u[3]||0,10)*60+parseInt(u[4]||0,10);return d*f*1e3}}function o(c){return-(c-1)}function l(c){return c>=0&&c<100}return or}var ar,Ns;function Io(){if(Ns)return ar;Ns=1,ar=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return ar}var lr,Os;function No(){if(Os)return lr;Os=1;var e=Io();lr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,b(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(h){var y=this[h]||0;return h==="seconds"&&this.milliseconds&&(y=(y+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),y+" "+h},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(y,this).join(""),h=i.map(y,this).join("");return"P"+v+"T"+h;function y(E){var C=this[E]||0;return E==="seconds"&&this.milliseconds&&(C=(C+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),C+s[E]}};var o="([+-]?\\d+)",l=o+"\\s+years?",c=o+"\\s+mons?",u=o+"\\s+days?",a="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([l,c,u,a].map(function(v){return"("+v+")?"}).join("\\s*")),d={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function m(v){var h=v+"000000".slice(v.length);return parseInt(h,10)/1e3}function b(v){if(!v)return{};var h=f.exec(v),y=h[8]==="-";return Object.keys(d).reduce(function(E,C){var N=d[C],T=h[N];return!T||(T=C==="milliseconds"?m(T):parseInt(T,10),!T)||(y&&~p.indexOf(C)&&(T*=-1),E[C]=T),E},{})}return lr}var cr,Ds;function Oo(){if(Ds)return cr;Ds=1;var e=Buffer.from||Buffer;return cr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},cr}var ur,qs;function Do(){if(qs)return ur;qs=1;var e=ti(),t=ri(),r=Ro(),s=No(),n=Oo();function i(g){return function(w){return w===null?w:g(w)}}function o(g){return g===null?g:g==="TRUE"||g==="t"||g==="true"||g==="y"||g==="yes"||g==="on"||g==="1"}function l(g){return g?e.parse(g,o):null}function c(g){return parseInt(g,10)}function u(g){return g?e.parse(g,i(c)):null}function a(g){return g?e.parse(g,i(function(x){return y(x).trim()})):null}var f=function(g){if(!g)return null;var x=t.create(g,function(w){return w!==null&&(w=C(w)),w});return x.parse()},d=function(g){if(!g)return null;var x=t.create(g,function(w){return w!==null&&(w=parseFloat(w)),w});return x.parse()},p=function(g){if(!g)return null;var x=t.create(g);return x.parse()},m=function(g){if(!g)return null;var x=t.create(g,function(w){return w!==null&&(w=r(w)),w});return x.parse()},b=function(g){if(!g)return null;var x=t.create(g,function(w){return w!==null&&(w=s(w)),w});return x.parse()},v=function(g){return g?e.parse(g,i(n)):null},h=function(g){return parseInt(g,10)},y=function(g){var x=String(g);return/^\d+$/.test(x)?x:g},E=function(g){return g?e.parse(g,i(JSON.parse)):null},C=function(g){return g[0]!=="("?null:(g=g.substring(1,g.length-1).split(","),{x:parseFloat(g[0]),y:parseFloat(g[1])})},N=function(g){if(g[0]!=="<"&&g[1]!=="(")return null;for(var x="(",w="",_=!1,A=2;A<g.length-1;A++){if(_||(x+=g[A]),g[A]===")"){_=!0;continue}else if(!_)continue;g[A]!==","&&(w+=g[A])}var R=C(x);return R.radius=parseFloat(w),R},T=function(g){g(20,y),g(21,h),g(23,h),g(26,h),g(700,parseFloat),g(701,parseFloat),g(16,o),g(1082,r),g(1114,r),g(1184,r),g(600,C),g(651,p),g(718,N),g(1e3,l),g(1001,v),g(1005,u),g(1007,u),g(1028,u),g(1016,a),g(1017,f),g(1021,d),g(1022,d),g(1231,d),g(1014,p),g(1015,p),g(1008,p),g(1009,p),g(1040,p),g(1041,p),g(1115,m),g(1182,m),g(1185,m),g(1186,s),g(1187,b),g(17,n),g(114,JSON.parse.bind(JSON)),g(3802,JSON.parse.bind(JSON)),g(199,E),g(3807,E),g(3907,p),g(2951,p),g(791,p),g(1183,p),g(1270,p)};return ur={init:T},ur}var dr,ks;function qo(){if(ks)return dr;ks=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",l,c,u,a,f,d;{if(l=s%e,s=s/e>>>0,c=4294967296*l+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(a="",f=6-u.length,d=0;d<f;d++)a+="0";o=a+u+o}{if(l=s%e,s=s/e>>>0,c=4294967296*l+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(a="",f=6-u.length,d=0;d<f;d++)a+="0";o=a+u+o}{if(l=s%e,s=s/e>>>0,c=4294967296*l+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(a="",f=6-u.length,d=0;d<f;d++)a+="0";o=a+u+o}return l=s%e,c=4294967296*l+n,u=""+c%e,i+u+o}return dr=t,dr}var fr,js;function ko(){if(js)return fr;js=1;var e=qo(),t=function(p,m,b,v,h){b=b||0,v=v||!1,h=h||function(_,A,R){return _*Math.pow(2,R)+A};var y=b>>3,E=function(_){return v?~_&255:_},C=255,N=8-b%8;m<N&&(C=255<<8-m&255,N=m),b&&(C=C>>b%8);var T=0;b%8+m>=8&&(T=h(0,E(p[y])&C,N));for(var g=m+b>>3,x=y+1;x<g;x++)T=h(T,E(p[x]),8);var w=(m+b)%8;return w>0&&(T=h(T,E(p[g])>>8-w,w)),T},r=function(p,m,b){var v=Math.pow(2,b-1)-1,h=t(p,1),y=t(p,b,1);if(y===0)return 0;var E=1,C=function(T,g,x){T===0&&(T=1);for(var w=1;w<=x;w++)E/=2,(g&1<<x-w)>0&&(T+=E);return T},N=t(p,m,b+1,!1,C);return y==Math.pow(2,b+1)-1?N===0?h===0?1/0:-1/0:NaN:(h===0?1:-1)*Math.pow(2,y-v)*N},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},l=function(p){var m=t(p,16,32);if(m==49152)return NaN;for(var b=Math.pow(1e4,t(p,16,16)),v=0,h=t(p,16),y=0;y<h;y++)v+=t(p,16,64+16*y)*b,b/=1e4;var E=Math.pow(10,t(p,16,48));return(m===0?1:-1)*Math.round(v*E)/E},c=function(p,m){var b=t(m,1),v=t(m,63,1),h=new Date((b===0?1:-1)*v/1e3+9466848e5);return p||h.setTime(h.getTime()+h.getTimezoneOffset()*6e4),h.usec=v%1e3,h.getMicroSeconds=function(){return this.usec},h.setMicroSeconds=function(y){this.usec=y},h.getUTCMicroSeconds=function(){return this.usec},h},u=function(p){var m=t(p,32);t(p,32,32);for(var b=t(p,32,64),v=96,h=[],y=0;y<m;y++)h[y]=t(p,32,v),v+=32,v+=32;var E=function(N){var T=t(p,32,v);if(v+=32,T==4294967295)return null;var g;if(N==23||N==20)return g=t(p,T*8,v),v+=T*8,g;if(N==25)return g=p.toString(this.encoding,v>>3,(v+=T<<3)>>3),g;console.log("ERROR: ElementType not implemented: "+N)},C=function(N,T){var g=[],x;if(N.length>1){var w=N.shift();for(x=0;x<w;x++)g[x]=C(N,T);N.unshift(w)}else for(x=0;x<N[0];x++)g[x]=E(T);return g};return C(h,b)},a=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},d=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,l),p(700,i),p(701,o),p(16,f),p(1114,c.bind(null,!1)),p(1184,c.bind(null,!0)),p(1e3,u),p(1007,u),p(1016,u),p(1008,u),p(1009,u),p(25,a)};return fr={init:d},fr}var pr,Ls;function jo(){return Ls||(Ls=1,pr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),pr}var Bs;function St(){if(Bs)return je;Bs=1;var e=Do(),t=ko(),r=ri(),s=jo();je.getTypeParser=o,je.setTypeParser=l,je.arrayParser=r,je.builtins=s;var n={text:{},binary:{}};function i(c){return String(c)}function o(c,u){return u=u||"text",n[u]&&n[u][c]||i}function l(c,u,a){typeof u=="function"&&(a=u,u="text"),n[u][c]=a}return e.init(function(c,u){n.text[c]=u}),t.init(function(c,u){n.binary[c]=u}),je}var Ms;function At(){return Ms||(Ms=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=St(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(sr)),sr.exports}var hr,Us;function rt(){if(Us)return hr;Us=1;const e=At(),t=Ve,{isDate:r}=t.types||t;function s(d){return'"'+d.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(d){let p="{";for(let m=0;m<d.length;m++)if(m>0&&(p=p+","),d[m]===null||typeof d[m]>"u")p=p+"NULL";else if(Array.isArray(d[m]))p=p+n(d[m]);else if(ArrayBuffer.isView(d[m])){let b=d[m];if(!(b instanceof Buffer)){const v=Buffer.from(b.buffer,b.byteOffset,b.byteLength);v.length===b.byteLength?b=v:b=v.slice(b.byteOffset,b.byteOffset+b.byteLength)}p+="\\\\x"+b.toString("hex")}else p+=s(i(d[m]));return p=p+"}",p}const i=function(d,p){if(d==null)return null;if(typeof d=="object"){if(d instanceof Buffer)return d;if(ArrayBuffer.isView(d)){const m=Buffer.from(d.buffer,d.byteOffset,d.byteLength);return m.length===d.byteLength?m:m.slice(d.byteOffset,d.byteOffset+d.byteLength)}return r(d)?e.parseInputDatesAsUTC?c(d):l(d):Array.isArray(d)?n(d):o(d,p)}return d.toString()};function o(d,p){if(d&&typeof d.toPostgres=="function"){if(p=p||[],p.indexOf(d)!==-1)throw new Error('circular reference detected while preparing "'+d+'" for query');return p.push(d),i(d.toPostgres(i),p)}return JSON.stringify(d)}function l(d){let p=-d.getTimezoneOffset(),m=d.getFullYear();const b=m<1;b&&(m=Math.abs(m)+1);let v=String(m).padStart(4,"0")+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0")+"T"+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")+":"+String(d.getSeconds()).padStart(2,"0")+"."+String(d.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),b&&(v+=" BC"),v}function c(d){let p=d.getUTCFullYear();const m=p<1;m&&(p=Math.abs(p)+1);let b=String(p).padStart(4,"0")+"-"+String(d.getUTCMonth()+1).padStart(2,"0")+"-"+String(d.getUTCDate()).padStart(2,"0")+"T"+String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")+":"+String(d.getUTCSeconds()).padStart(2,"0")+"."+String(d.getUTCMilliseconds()).padStart(3,"0");return b+="+00:00",m&&(b+=" BC"),b}function u(d,p,m){return d=typeof d=="string"?{text:d}:d,p&&(typeof p=="function"?d.callback=p:d.values=p),m&&(d.callback=m),d}return hr={prepareValue:function(p){return i(p)},normalizeQueryConfig:u,escapeIdentifier:function(d){return'"'+d.replace(/"/g,'""')+'"'},escapeLiteral:function(d){let p=!1,m="'";if(d==null||typeof d!="string")return"''";for(let b=0;b<d.length;b++){const v=d[b];v==="'"?m+=v+v:v==="\\"?(m+=v+v,p=!0):m+=v}return m+="'",p===!0&&(m=" E"+m),m}},hr}var ft={exports:{}},mr,Hs;function Lo(){if(Hs)return mr;Hs=1;const e=Cn;function t(l){return e.createHash("md5").update(l,"utf-8").digest("hex")}function r(l,c,u){const a=t(c+l);return"md5"+t(Buffer.concat([Buffer.from(a),u]))}function s(l){return e.createHash("sha256").update(l).digest()}function n(l,c){return l=l.replace(/(\D)-/,"$1"),e.createHash(l).update(c).digest()}function i(l,c){return e.createHmac("sha256",l).update(c).digest()}async function o(l,c,u){return e.pbkdf2Sync(l,c,u,32,"sha256")}return mr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},mr}var gr,Fs;function Bo(){if(Fs)return gr;Fs=1;const e=Cn;gr={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:a,sha256:l,hashByName:c,hmacSha256:u,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,m=await r.digest("MD5",p);return Array.from(new Uint8Array(m)).map(b=>b.toString(16).padStart(2,"0")).join("")}}async function o(f,d,p){const m=await i(d+f);return"md5"+await i(Buffer.concat([Buffer.from(m),p]))}async function l(f){return await r.digest("SHA-256",f)}async function c(f,d){return await r.digest(f,d)}async function u(f,d){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(d))}async function a(f,d,p){const m=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),b={name:"PBKDF2",hash:"SHA-256",salt:d,iterations:p};return await r.deriveBits(b,m,256,["deriveBits"])}return gr}var zs;function si(){return zs||(zs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?ft.exports=Lo():ft.exports=Bo()),ft.exports}var br,Qs;function Mo(){if(Qs)return br;Qs=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let l=i[o++];if(l<128)return{length:l,index:o};const c=l&127;if(c>4)throw e("bad length",i);l=0;for(let u=0;u<c;u++)l=l<<8|i[o++];return{length:l,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:l,index:c}=t(i,o);o=c;const u=o+l,a=i[o++];let f=(a/40>>0)+"."+a%40;for(;o<u;){let d=0;for(;o<u;){const p=i[o++];if(d=d<<7|p&127,p<128)break}f+="."+d}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:l,index:c}=s(i,o);o=c+l,o=s(i,o).index;const{oid:u,index:a}=r(i,o);switch(u){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=a,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+u,i)}return br={signatureAlgorithmHashFromCertificate:n},br}var yr,Vs;function Uo(){if(Vs)return yr;Vs=1;const e=si(),{signatureAlgorithmHashFromCertificate:t}=Mo();function r(f,d){const p=["SCRAM-SHA-256"];d&&p.unshift("SCRAM-SHA-256-PLUS");const m=p.find(h=>f.includes(h));if(!m)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(m==="SCRAM-SHA-256-PLUS"&&typeof d.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const b=e.randomBytes(18).toString("base64");return{mechanism:m,clientNonce:b,response:(m==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":d?"y":"n")+",,n=*,r="+b,message:"SASLInitialResponse"}}async function s(f,d,p,m){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(d==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const b=c(p);if(b.nonce.startsWith(f.clientNonce)){if(b.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,h="r="+b.nonce+",s="+b.salt+",i="+b.iteration;let y=m?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const S=m.getPeerCertificate().raw;let O=t(S);(O==="MD5"||O==="SHA-1")&&(O="SHA-256");const B=await e.hashByName(O,S);y=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(B)]).toString("base64")}const E="c="+y+",r="+b.nonce,C=v+","+h+","+E,N=Buffer.from(b.salt,"base64"),T=await e.deriveKey(d,N,b.iteration),g=await e.hmacSha256(T,"Client Key"),x=await e.sha256(g),w=await e.hmacSha256(x,C),_=a(Buffer.from(g),Buffer.from(w)).toString("base64"),A=await e.hmacSha256(T,"Server Key"),R=await e.hmacSha256(A,C);f.message="SASLResponse",f.serverSignature=Buffer.from(R).toString("base64"),f.response=E+",p="+_}function n(f,d){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=u(d);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((d,p)=>f.charCodeAt(p)).every(d=>d>=33&&d<=43||d>=45&&d<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function l(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(d=>{if(!/^.=/.test(d))throw new Error("SASL: Invalid attribute pair entry");const p=d[0],m=d.substring(2);return[p,m]}))}function c(f){const d=l(f),p=d.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const m=d.get("s");if(m){if(!o(m))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const b=d.get("i");if(b){if(!/^[1-9][0-9]*$/.test(b))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(b,10);return{nonce:p,salt:m,iteration:v}}function u(f){const p=l(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function a(f,d){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(d))throw new TypeError("second argument must be a Buffer");if(f.length!==d.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,m)=>f[m]^d[m]))}return yr={startSession:r,continueSession:s,finalizeSession:n},yr}var vr,Xs;function Qr(){if(Xs)return vr;Xs=1;const e=St();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},vr=t,vr}var wr,Ws;function Ho(){if(Ws)return wr;Ws=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const l={};let c,u=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(i,"postgres://base")}catch{c=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),u=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of c.searchParams.entries())l[p[0]]=p[1];if(l.user=l.user||decodeURIComponent(c.username),l.password=l.password||decodeURIComponent(c.password),c.protocol=="socket:")return l.host=decodeURI(c.pathname),l.database=c.searchParams.get("db"),l.client_encoding=c.searchParams.get("encoding"),l;const a=u?"":c.hostname;l.host?a&&/^%2f/i.test(a)&&(c.pathname=a+c.pathname):l.host=decodeURIComponent(a),l.port||(l.port=c.port);const f=c.pathname.slice(1)||null;l.database=f?decodeURI(f):null,(l.ssl==="true"||l.ssl==="1")&&(l.ssl=!0),l.ssl==="0"&&(l.ssl=!1),(l.sslcert||l.sslkey||l.sslrootcert||l.sslmode)&&(l.ssl={});const d=l.sslcert||l.sslkey||l.sslrootcert?Tn:null;if(l.sslcert&&(l.ssl.cert=d.readFileSync(l.sslcert).toString()),l.sslkey&&(l.ssl.key=d.readFileSync(l.sslkey).toString()),l.sslrootcert&&(l.ssl.ca=d.readFileSync(l.sslrootcert).toString()),o.useLibpqCompat&&l.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(l.uselibpqcompat==="true"||o.useLibpqCompat)switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":{l.ssl.rejectUnauthorized=!1;break}case"require":{l.sslrootcert?l.ssl.checkServerIdentity=function(){}:l.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!l.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");l.ssl.checkServerIdentity=function(){};break}}else switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{l.sslmode!=="verify-full"&&n(l.sslmode);break}case"no-verify":{l.ssl.rejectUnauthorized=!1;break}}return l}function t(i){return Object.entries(i).reduce((l,[c,u])=>(u!=null&&(l[c]=u),l),{})}function r(i){return Object.entries(i).reduce((l,[c,u])=>{if(c==="ssl"){const a=u;typeof a=="boolean"&&(l[c]=a),typeof a=="object"&&(l[c]=t(a))}else if(u!=null)if(c==="port"){if(u!==""){const a=parseInt(u,10);if(isNaN(a))throw new Error(`Invalid ${c}: ${u}`);l[c]=a}}else l[c]=u;return l},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
...
See https://www.postgresql.org/docs/current/libpq-ssl.html for libpq SSL mode definitions.`))}return wr=e,e.parse=e,e.toClientConfig=r,e.parseIntoClientConfig=s,wr}var xr,Gs;function ni(){if(Gs)return xr;Gs=1;var e={};const t=pi,r=At(),s=Ho().parse,n=function(u,a,f){return a[u]?a[u]:(f===void 0?f=e["PG"+u.toUpperCase()]:f===!1||(f=e[f]),f||r[u])},i=function(){switch(e.PGSSLMODE){case"disable":return!1;case"prefer":case"require":case"verify-ca":case"verify-full":return!0;case"no-verify":return{rejectUnauthorized:!1}}return r.ssl},o=function(u){return"'"+(""+u).replace(/\\/g,"\\\\").replace(/'/g,"\\'")+"'"},l=function(u,a,f){const d=a[f];d!=null&&u.push(f+"="+o(d))};class c{constructor(a){a=typeof a=="string"?s(a):a||{},a.connectionString&&(a=Object.assign({},a,s(a.connectionString))),this.user=n("user",a),this.database=n("database",a),this.database===void 0&&(this.database=this.user),this.port=parseInt(n("port",a),10),this.host=n("host",a),Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:n("password",a)}),this.binary=n("binary",a),this.options=n("options",a),this.ssl=typeof a.ssl>"u"?i():a.ssl,typeof this.ssl=="string"&&this.ssl==="true"&&(this.ssl=!0),this.ssl==="no-verify"&&(this.ssl={rejectUnauthorized:!1}),this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this.client_encoding=n("client_encoding",a),this.replication=n("replication",a),this.isDomainSocket=!(this.host||"").indexOf("/"),this.application_name=n("application_name",a,"PGAPPNAME"),this.fallback_application_name=n("fallback_application_name",a,!1),this.statement_timeout=n("statement_timeout",a,!1),this.lock_timeout=n("lock_timeout",a,!1),this.idle_in_transaction_session_timeout=n("idle_in_transaction_session_timeout",a,!1),this.query_timeout=n("query_timeout",a,!1),a.connectionTimeoutMillis===void 0?this.connect_timeout=e.PGCONNECT_TIMEOUT||0:this.connect_timeout=Math.floor(a.connectionTimeoutMillis/1e3),a.keepAlive===!1?this.keepalives=0:a.keepAlive===!0&&(this.keepalives=1),typeof a.keepAliveInitialDelayMillis=="number"&&(this.keepalives_idle=Math.floor(a.keepAliveInitialDelayMillis/1e3))}getLibpqConnectionString(a){const f=[];l(f,this,"user"),l(f,this,"password"),l(f,this,"port"),l(f,this,"application_name"),l(f,this,"fallback_application_name"),l(f,this,"connect_timeout"),l(f,this,"options");const d=typeof this.ssl=="object"?this.ssl:this.ssl?{sslmode:this.ssl}:{};if(l(f,d,"sslmode"),l(f,d,"sslca"),l(f,d,"sslkey"),l(f,d,"sslcert"),l(f,d,"sslrootcert"),this.database&&f.push("dbname="+o(this.database)),this.replication&&f.push("replication="+o(this.replication)),this.host&&f.push("host="+o(this.host)),this.isDomainSocket)return a(null,f.join(" "));this.client_encoding&&f.push("client_encoding="+o(this.client_encoding)),t.lookup(this.host,function(p,m){return p?a(p,null):(f.push("hostaddr="+o(m)),a(null,f.join(" ")))})}}return xr=c,xr}var Er,Js;function ii(){if(Js)return Er;Js=1;const e=St(),t=/^([A-Za-z]+)(?: (\d+))?(?: (\d+))?/;class r{constructor(n,i){this.command=null,this.rowCount=null,this.oid=null,this.rows=[],this.fields=[],this._parsers=void 0,this._types=i,this.RowCtor=null,this.rowAsArray=n==="array",this.rowAsArray&&(this.parseRow=this._parseRowAsArray),this._prebuiltEmptyResultObject=null}addCommandComplete(n){let i;n.text?i=t.exec(n.text):i=t.exec(n.command),i&&(this.command=i[1],i[3]?(this.oid=parseInt(i[2],10),this.rowCount=parseInt(i[3],10)):i[2]&&(this.rowCount=parseInt(i[2],10)))}_parseRowAsArray(n){const i=new Array(n.length);for(let o=0,l=n.length;o<l;o++){const c=n[o];c!==null?i[o]=this._parsers[o](c):i[o]=null}return i}parseRow(n){const i={...this._prebuiltEmptyResultObject};for(let o=0,l=n.length;o<l;o++){const c=n[o],u=this.fields[o].name;if(c!==null){const a=this.fields[o].format==="binary"?Buffer.from(c):c;i[u]=this._parsers[o](a)}else i[u]=null}return i}addRow(n){this.rows.push(n)}addFields(n){this.fields=n,this.fields.length&&(this._parsers=new Array(n.length));const i={};for(let o=0;o<n.length;o++){const l=n[o];i[l.name]=null,this._types?this._parsers[o]=this._types.getTypeParser(l.dataTypeID,l.format||"text"):this._parsers[o]=e.getTypeParser(l.dataTypeID,l.format||"text")}this._prebuiltEmptyResultObject={...i}}}return Er=r,Er}var Sr,Ys;function Fo(){if(Ys)return Sr;Ys=1;const{EventEmitter:e}=Xe,t=ii(),r=rt();class s extends e{constructor(i,o,l){super(),i=r.normalizeQueryConfig(i,o,l),this.text=i.text,this.values=i.values,this.rows=i.rows,this.types=i.types,this.name=i.name,this.queryMode=i.queryMode,this.binary=i.binary,this.portal=i.portal||"",this.callback=i.callback,this._rowMode=i.rowMode,process.domain&&i.callback&&(this.callback=process.domain.bind(i.callback)),this._result=new t(this._rowMode,this.types),this._results=this._result,this._canceledDueToError=!1}requiresPreparation(){return this.queryMode==="extended"||this.name||this.rows?!0:!this.text||!this.values?!1:this.values.length>0}_checkForMultirow(){this._result.command&&(Array.isArray(this._results)||(this._results=[this._result]),this._result=new t(this._rowMode,this._result._types),this._results.push(this._result))}handleRowDescription(i){this._checkForMultirow(),this._result.addFields(i.fields),this._accumulateRows=this.callback||!this.listeners("row").length}handleDataRow(i){let o;if(!this._canceledDueToError){try{o=this._result.parseRow(i.fields)}catch(l){this._canceledDueToError=l;return}this.emit("row",o,this._result),this._accumulateRows&&this._result.addRow(o)}}handleCommandComplete(i,o){this._checkForMultirow(),this._result.addCommandComplete(i),this.rows&&o.sync()}handleEmptyQuery(i){this.rows&&i.sync()}handleError(i,o){if(this._canceledDueToError&&(i=this._canceledDueToError,this._canceledDueToError=!1),this.callback)return this.callback(i);this.emit("error",i)}handleReadyForQuery(i){if(this._canceledDueToError)return this.handleError(this._canceledDueToError,i);if(this.callback)try{this.callback(null,this._results)}catch(o){process.nextTick(()=>{throw o})}this.emit("end",this._results)}submit(i){if(typeof this.text!="string"&&typeof this.name!="string")return new Error("A query must have either text or a name. Supplying neither is unsupported.");const o=i.parsedStatements[this.name];if(this.text&&o&&this.text!==o)return new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);if(this.values&&!Array.isArray(this.values))return new Error("Query values must be an array");if(this.requiresPreparation()){i.stream.cork&&i.stream.cork();try{this.prepare(i)}finally{i.stream.uncork&&i.stream.uncork()}}else i.query(this.text);return null}hasBeenParsed(i){return this.name&&i.parsedStatements[this.name]}handlePortalSuspended(i){this._getRows(i,this.rows)}_getRows(i,o){i.execute({portal:this.portal,rows:o}),o?i.flush():i.sync()}prepare(i){this.hasBeenParsed(i)||i.parse({text:this.text,name:this.name,types:this.types});try{i.bind({portal:this.portal,statement:this.name,values:this.values,binary:this.binary,valueMapper:r.prepareValue})}catch(o){this.handleError(o,i);return}i.describe({type:"P",name:this.portal||""}),this._getRows(i,this.rows)}handleCopyInResponse(i){i.sendCopyFail("No source stream defined")}handleCopyData(i,o){}}return Sr=s,Sr}var Ar={},L={},Ks;function oi(){if(Ks)return L;Ks=1,Object.defineProperty(L,"__esModule",{value:!0}),L.NoticeMessage=L.DataRowMessage=L.CommandCompleteMessage=L.ReadyForQueryMessage=L.NotificationResponseMessage=L.BackendKeyDataMessage=L.AuthenticationMD5Password=L.ParameterStatusMessage=L.ParameterDescriptionMessage=L.RowDescriptionMessage=L.Field=L.CopyResponse=L.CopyDataMessage=L.DatabaseError=L.copyDone=L.emptyQuery=L.replicationStart=L.portalSuspended=L.noData=L.closeComplete=L.bindComplete=L.parseComplete=void 0,L.parseComplete={name:"parseComplete",length:5},L.bindComplete={name:"bindComplete",length:5},L.closeComplete={name:"closeComplete",length:5},L.noData={name:"noData",length:5},L.portalSuspended={name:"portalSuspended",length:5},L.replicationStart={name:"replicationStart",length:4},L.emptyQuery={name:"emptyQuery",length:4},L.copyDone={name:"copyDone",length:4};class e extends Error{constructor(b,v,h){super(b),this.length=v,this.name=h}}L.DatabaseError=e;class t{constructor(b,v){this.length=b,this.chunk=v,this.name="copyData"}}L.CopyDataMessage=t;class r{constructor(b,v,h,y){this.length=b,this.name=v,this.binary=h,this.columnTypes=new Array(y)}}L.CopyResponse=r;class s{constructor(b,v,h,y,E,C,N){this.name=b,this.tableID=v,this.columnID=h,this.dataTypeID=y,this.dataTypeSize=E,this.dataTypeModifier=C,this.format=N}}L.Field=s;class n{constructor(b,v){this.length=b,this.fieldCount=v,this.name="rowDescription",this.fields=new Array(this.fieldCount)}}L.RowDescriptionMessage=n;class i{constructor(b,v){this.length=b,this.parameterCount=v,this.name="parameterDescription",this.dataTypeIDs=new Array(this.parameterCount)}}L.ParameterDescriptionMessage=i;class o{constructor(b,v,h){this.length=b,this.parameterName=v,this.parameterValue=h,this.name="parameterStatus"}}L.ParameterStatusMessage=o;class l{constructor(b,v){this.length=b,this.salt=v,this.name="authenticationMD5Password"}}L.AuthenticationMD5Password=l;class c{constructor(b,v,h){this.length=b,this.processID=v,this.secretKey=h,this.name="backendKeyData"}}L.BackendKeyDataMessage=c;class u{constructor(b,v,h,y){this.length=b,this.processId=v,this.channel=h,this.payload=y,this.name="notification"}}L.NotificationResponseMessage=u;class a{constructor(b,v){this.length=b,this.status=v,this.name="readyForQuery"}}L.ReadyForQueryMessage=a;class f{constructor(b,v){this.length=b,this.text=v,this.name="commandComplete"}}L.CommandCompleteMessage=f;class d{constructor(b,v){this.length=b,this.fields=v,this.name="dataRow",this.fieldCount=v.length}}L.DataRowMessage=d;class p{constructor(b,v){this.length=b,this.message=v,this.name="notice"}}return L.NoticeMessage=p,L}var Ye={},Ke={},Zs;function zo(){if(Zs)return Ke;Zs=1,Object.defineProperty(Ke,"__esModule",{value:!0}),Ke.Writer=void 0;class e{constructor(r=256){this.size=r,this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(r)}ensure(r){if(this.buffer.length-this.offset<r){const n=this.buffer,i=n.length+(n.length>>1)+r;this.buffer=Buffer.allocUnsafe(i),n.copy(this.buffer)}}addInt32(r){return this.ensure(4),this.buffer[this.offset++]=r>>>24&255,this.buffer[this.offset++]=r>>>16&255,this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addInt16(r){return this.ensure(2),this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addCString(r){if(!r)this.ensure(1);else{const s=Buffer.byteLength(r);this.ensure(s+1),this.buffer.write(r,this.offset,"utf-8"),this.offset+=s}return this.buffer[this.offset++]=0,this}addString(r=""){const s=Buffer.byteLength(r);return this.ensure(s),this.buffer.write(r,this.offset),this.offset+=s,this}add(r){return this.ensure(r.length),r.copy(this.buffer,this.offset),this.offset+=r.length,this}join(r){if(r){this.buffer[this.headerPosition]=r;const s=this.offset-(this.headerPosition+1);this.buffer.writeInt32BE(s,this.headerPosition+1)}return this.buffer.slice(r?0:5,this.offset)}flush(r){const s=this.join(r);return this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(this.size),s}}return Ke.Writer=e,Ke}var $s;function Qo(){if($s)return Ye;$s=1,Object.defineProperty(Ye,"__esModule",{value:!0}),Ye.serialize=void 0;const e=zo(),t=new e.Writer,r=S=>{t.addInt16(3).addInt16(0);for(const U of Object.keys(S))t.addCString(U).addCString(S[U]);t.addCString("client_encoding").addCString("UTF8");const O=t.addCString("").flush(),B=O.length+4;return new e.Writer().addInt32(B).add(O).flush()},s=()=>{const S=Buffer.allocUnsafe(8);return S.writeInt32BE(8,0),S.writeInt32BE(80877103,4),S},n=S=>t.addCString(S).flush(112),i=function(S,O){return t.addCString(S).addInt32(Buffer.byteLength(O)).addString(O),t.flush(112)},o=function(S){return t.addString(S).flush(112)},l=S=>t.addCString(S).flush(81),c=[],u=S=>{const O=S.name||"";O.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",O,O.length),console.error("This can cause conflicts and silent errors executing queries"));const B=S.types||c,U=B.length,H=t.addCString(O).addCString(S.text).addInt16(U);for(let z=0;z<U;z++)H.addInt32(B[z]);return t.flush(80)},a=new e.Writer,f=function(S,O){for(let B=0;B<S.length;B++){const U=O?O(S[B],B):S[B];U==null?(t.addInt16(0),a.addInt32(-1)):U instanceof Buffer?(t.addInt16(1),a.addInt32(U.length),a.add(U)):(t.addInt16(0),a.addInt32(Buffer.byteLength(U)),a.addString(U))}},d=(S={})=>{const O=S.portal||"",B=S.statement||"",U=S.binary||!1,H=S.values||c,z=H.length;return t.addCString(O).addCString(B),t.addInt16(z),f(H,S.valueMapper),t.addInt16(z),t.add(a.flush()),t.addInt16(1),t.addInt16(U?1:0),t.flush(66)},p=Buffer.from([69,0,0,0,9,0,0,0,0,0]),m=S=>{if(!S||!S.portal&&!S.rows)return p;const O=S.portal||"",B=S.rows||0,U=Buffer.byteLength(O),H=4+U+1+4,z=Buffer.allocUnsafe(1+H);return z[0]=69,z.writeInt32BE(H,1),z.write(O,5,"utf-8"),z[U+5]=0,z.writeUInt32BE(B,z.length-4),z},b=(S,O)=>{const B=Buffer.allocUnsafe(16);return B.writeInt32BE(16,0),B.writeInt16BE(1234,4),B.writeInt16BE(5678,6),B.writeInt32BE(S,8),B.writeInt32BE(O,12),B},v=(S,O)=>{const U=4+Buffer.byteLength(O)+1,H=Buffer.allocUnsafe(1+U);return H[0]=S,H.writeInt32BE(U,1),H.write(O,5,"utf-8"),H[U]=0,H},h=t.addCString("P").flush(68),y=t.addCString("S").flush(68),E=S=>S.name?v(68,`${S.type}${S.name||""}`):S.type==="P"?h:y,C=S=>{const O=`${S.type}${S.name||""}`;return v(67,O)},N=S=>t.add(S).flush(100),T=S=>v(102,S),g=S=>Buffer.from([S,0,0,0,4]),x=g(72),w=g(83),_=g(88),A=g(99),R={startup:r,password:n,requestSsl:s,sendSASLInitialResponseMessage:i,sendSCRAMClientFinalMessage:o,query:l,parse:u,bind:d,execute:m,describe:E,close:C,flush:()=>x,sync:()=>w,end:()=>_,copyData:N,copyDone:()=>A,copyFail:T,cancel:b};return Ye.serialize=R,Ye}var Ze={},$e={},en;function Vo(){if(en)return $e;en=1,Object.defineProperty($e,"__esModule",{value:!0}),$e.BufferReader=void 0;class e{constructor(r=0){this.offset=r,this.buffer=Buffer.allocUnsafe(0),this.encoding="utf-8"}setBuffer(r,s){this.offset=r,this.buffer=s}int16(){const r=this.buffer.readInt16BE(this.offset);return this.offset+=2,r}byte(){const r=this.buffer[this.offset];return this.offset++,r}int32(){const r=this.buffer.readInt32BE(this.offset);return this.offset+=4,r}uint32(){const r=this.buffer.readUInt32BE(this.offset);return this.offset+=4,r}string(r){const s=this.buffer.toString(this.encoding,this.offset,this.offset+r);return this.offset+=r,s}cstring(){const r=this.offset;let s=r;for(;this.buffer[s++]!==0;);return this.offset=s,this.buffer.toString(this.encoding,r,s-1)}bytes(r){const s=this.buffer.slice(this.offset,this.offset+r);return this.offset+=r,s}}return $e.BufferReader=e,$e}var tn;function Xo(){if(tn)return Ze;tn=1,Object.defineProperty(Ze,"__esModule",{value:!0}),Ze.Parser=void 0;const e=oi(),t=Vo(),r=1,n=r+4,i=-1,o=Buffer.allocUnsafe(0);class l{constructor(x){if(this.buffer=o,this.bufferLength=0,this.bufferOffset=0,this.reader=new t.BufferReader,(x==null?void 0:x.mode)==="binary")throw new Error("Binary mode not supported yet");this.mode=(x==null?void 0:x.mode)||"text"}parse(x,w){this.mergeBuffer(x);const _=this.bufferOffset+this.bufferLength;let A=this.bufferOffset;for(;A+n<=_;){const R=this.buffer[A],S=this.buffer.readUInt32BE(A+r),O=r+S;if(O+A<=_){const B=this.handlePacket(A+n,R,S,this.buffer);w(B),A+=O}else break}A===_?(this.buffer=o,this.bufferLength=0,this.bufferOffset=0):(this.bufferLength=_-A,this.bufferOffset=A)}mergeBuffer(x){if(this.bufferLength>0){const w=this.bufferLength+x.byteLength;if(w+this.bufferOffset>this.buffer.byteLength){let A;if(w<=this.buffer.byteLength&&this.bufferOffset>=this.bufferLength)A=this.buffer;else{let R=this.buffer.byteLength*2;for(;w>=R;)R*=2;A=Buffer.allocUnsafe(R)}this.buffer.copy(A,0,this.bufferOffset,this.bufferOffset+this.bufferLength),this.buffer=A,this.bufferOffset=0}x.copy(this.buffer,this.bufferOffset+this.bufferLength),this.bufferLength=w}else this.buffer=x,this.bufferOffset=0,this.bufferLength=x.byteLength}handlePacket(x,w,_,A){const{reader:R}=this;R.setBuffer(x,A);let S;switch(w){case 50:S=e.bindComplete;break;case 49:S=e.parseComplete;break;case 51:S=e.closeComplete;break;case 110:S=e.noData;break;case 115:S=e.portalSuspended;break;case 99:S=e.copyDone;break;case 87:S=e.replicationStart;break;case 73:S=e.emptyQuery;break;case 68:S=y(R);break;case 67:S=u(R);break;case 90:S=c(R);break;case 65:S=m(R);break;case 82:S=N(R,_);break;case 83:S=E(R);break;case 75:S=C(R);break;case 69:S=T(R,"error");break;case 78:S=T(R,"notice");break;case 84:S=b(R);break;case 116:S=h(R);break;case 71:S=f(R);break;case 72:S=d(R);break;case 100:S=a(R,_);break;default:return new e.DatabaseError("received invalid response: "+w.toString(16),_,"error")}return R.setBuffer(0,o),S.length=_,S}}Ze.Parser=l;const c=g=>{const x=g.string(1);return new e.ReadyForQueryMessage(i,x)},u=g=>{const x=g.cstring();return new e.CommandCompleteMessage(i,x)},a=(g,x)=>{const w=g.bytes(x-4);return new e.CopyDataMessage(i,w)},f=g=>p(g,"copyInResponse"),d=g=>p(g,"copyOutResponse"),p=(g,x)=>{const w=g.byte()!==0,_=g.int16(),A=new e.CopyResponse(i,x,w,_);for(let R=0;R<_;R++)A.columnTypes[R]=g.int16();return A},m=g=>{const x=g.int32(),w=g.cstring(),_=g.cstring();return new e.NotificationResponseMessage(i,x,w,_)},b=g=>{const x=g.int16(),w=new e.RowDescriptionMessage(i,x);for(let _=0;_<x;_++)w.fields[_]=v(g);return w},v=g=>{const x=g.cstring(),w=g.uint32(),_=g.int16(),A=g.uint32(),R=g.int16(),S=g.int32(),O=g.int16()===0?"text":"binary";return new e.Field(x,w,_,A,R,S,O)},h=g=>{const x=g.int16(),w=new e.ParameterDescriptionMessage(i,x);for(let _=0;_<x;_++)w.dataTypeIDs[_]=g.int32();return w},y=g=>{const x=g.int16(),w=new Array(x);for(let _=0;_<x;_++){const A=g.int32();w[_]=A===-1?null:g.string(A)}return new e.DataRowMessage(i,w)},E=g=>{const x=g.cstring(),w=g.cstring();return new e.ParameterStatusMessage(i,x,w)},C=g=>{const x=g.int32(),w=g.int32();return new e.BackendKeyDataMessage(i,x,w)},N=(g,x)=>{const w=g.int32(),_={name:"authenticationOk",length:x};switch(w){case 0:break;case 3:_.length===8&&(_.name="authenticationCleartextPassword");break;case 5:if(_.length===12){_.name="authenticationMD5Password";const A=g.bytes(4);return new e.AuthenticationMD5Password(i,A)}break;case 10:{_.name="authenticationSASL",_.mechanisms=[];let A;do A=g.cstring(),A&&_.mechanisms.push(A);while(A)}break;case 11:_.name="authenticationSASLContinue",_.data=g.string(x-8);break;case 12:_.name="authenticationSASLFinal",_.data=g.string(x-8);break;default:throw new Error("Unknown authenticationOk message type "+w)}return _},T=(g,x)=>{const w={};let _=g.string(1);for(;_!=="\0";)w[_]=g.cstring(),_=g.string(1);const A=w.M,R=x==="notice"?new e.NoticeMessage(i,A):new e.DatabaseError(A,i,x);return R.severity=w.S,R.code=w.C,R.detail=w.D,R.hint=w.H,R.position=w.P,R.internalPosition=w.p,R.internalQuery=w.q,R.where=w.W,R.schema=w.s,R.table=w.t,R.column=w.c,R.dataType=w.d,R.constraint=w.n,R.file=w.F,R.line=w.L,R.routine=w.R,R};return Ze}var rn;function ai(){return rn||(rn=1,(function(e){Object.defineProperty(e,"__esModule",{value:!0}),e.DatabaseError=e.serialize=e.parse=void 0;const t=oi();Object.defineProperty(e,"DatabaseError",{enumerable:!0,get:function(){return t.DatabaseError}});const r=Qo();Object.defineProperty(e,"serialize",{enumerable:!0,get:function(){return r.serialize}});const s=Xo();function n(i,o){const l=new s.Parser;return i.on("data",c=>l.parse(c,o)),new Promise(c=>i.on("end",()=>c()))}e.parse=n})(Ar)),Ar}var pt={},sn;function Wo(){return sn||(sn=1,Object.defineProperty(pt,"__esModule",{value:!0}),pt.default={}),pt}var _r,nn;function Go(){if(nn)return _r;nn=1;const{getStream:e,getSecureStream:t}=i();_r={getStream:e,getSecureStream:t};function r(){function o(c){const u=Pn;return new u.Socket}function l(c){return hi.connect(c)}return{getStream:o,getSecureStream:l}}function s(){function o(c){const{CloudflareSocket:u}=Wo();return new u(c)}function l(c){return c.socket.startTls(c),c.socket}return{getStream:o,getSecureStream:l}}function n(){if(typeof navigator=="object"&&navigator!==null&&typeof navigator.userAgent=="string")return navigator.userAgent==="Cloudflare-Workers";if(typeof Response=="function"){const o=new Response(null,{cf:{thing:!0}});if(typeof o.cf=="object"&&o.cf!==null&&o.cf.thing)return!0}return!1}function i(){return n()?s():r()}return _r}var Cr,on;function li(){if(on)return Cr;on=1;const e=Xe.EventEmitter,{parse:t,serialize:r}=ai(),{getStream:s,getSecureStream:n}=Go(),i=r.flush(),o=r.sync(),l=r.end();class c extends e{constructor(a){super(),a=a||{},this.stream=a.stream||s(a.ssl),typeof this.stream=="function"&&(this.stream=this.stream(a)),this._keepAlive=a.keepAlive,this._keepAliveInitialDelayMillis=a.keepAliveInitialDelayMillis,this.parsedStatements={},this.ssl=a.ssl||!1,this._ending=!1,this._emitMessage=!1;const f=this;this.on("newListener",function(d){d==="message"&&(f._emitMessage=!0)})}connect(a,f){const d=this;this._connecting=!0,this.stream.setNoDelay(!0),this.stream.connect(a,f),this.stream.once("connect",function(){d._keepAlive&&d.stream.setKeepAlive(!0,d._keepAliveInitialDelayMillis),d.emit("connect")});const p=function(m){d._ending&&(m.code==="ECONNRESET"||m.code==="EPIPE")||d.emit("error",m)};if(this.stream.on("error",p),this.stream.on("close",function(){d.emit("end")}),!this.ssl)return this.attachListeners(this.stream);this.stream.once("data",function(m){switch(m.toString("utf8")){case"S":break;case"N":return d.stream.end(),d.emit("error",new Error("The server does not support SSL connections"));default:return d.stream.end(),d.emit("error",new Error("There was an error establishing an SSL connection"))}const v={socket:d.stream};d.ssl!==!0&&(Object.assign(v,d.ssl),"key"in d.ssl&&(v.key=d.ssl.key));const h=Pn;h.isIP&&h.isIP(f)===0&&(v.servername=f);try{d.stream=n(v)}catch(y){return d.emit("error",y)}d.attachListeners(d.stream),d.stream.on("error",p),d.emit("sslconnect")})}attachListeners(a){t(a,f=>{const d=f.name==="error"?"errorMessage":f.name;this._emitMessage&&this.emit("message",f),this.emit(d,f)})}requestSsl(){this.stream.write(r.requestSsl())}startup(a){this.stream.write(r.startup(a))}cancel(a,f){this._send(r.cancel(a,f))}password(a){this._send(r.password(a))}sendSASLInitialResponseMessage(a,f){this._send(r.sendSASLInitialResponseMessage(a,f))}sendSCRAMClientFinalMessage(a){this._send(r.sendSCRAMClientFinalMessage(a))}_send(a){return this.stream.writable?this.stream.write(a):!1}query(a){this._send(r.query(a))}parse(a){this._send(r.parse(a))}bind(a){this._send(r.bind(a))}execute(a){this._send(r.execute(a))}flush(){this.stream.writable&&this.stream.write(i)}sync(){this._ending=!0,this._send(o)}ref(){this.stream.ref()}unref(){this.stream.unref()}end(){if(this._ending=!0,!this._connecting||!this.stream.writable){this.stream.end();return}return this.stream.write(l,()=>{this.stream.end()})}close(a){this._send(r.close(a))}describe(a){this._send(r.describe(a))}sendCopyFromChunk(a){this._send(r.copyData(a))}endCopyFrom(){this._send(r.copyDone())}sendCopyFail(a){this._send(r.copyFail(a))}}return Cr=c,Cr}var ht={exports:{}},Tr={exports:{}},Pr,an;function Jo(){if(an)return Pr;an=1;const{Transform:e}=Rn,{StringDecoder:t}=gi,r=Symbol("last"),s=Symbol("decoder");function n(u,a,f){let d;if(this.overflow){if(d=this[s].write(u).split(this.matcher),d.length===1)return f();d.shift(),this.overflow=!1}else this[r]+=this[s].write(u),d=this[r].split(this.matcher);this[r]=d.pop();for(let p=0;p<d.length;p++)try{o(this,this.mapper(d[p]))}catch(m){return f(m)}if(this.overflow=this[r].length>this.maxLength,this.overflow&&!this.skipOverflow){f(new Error("maximum buffer reached"));return}f()}function i(u){if(this[r]+=this[s].end(),this[r])try{o(this,this.mapper(this[r]))}catch(a){return u(a)}u()}function o(u,a){a!==void 0&&u.push(a)}function l(u){return u}function c(u,a,f){switch(u=u||/\r?\n/,a=a||l,f=f||{},arguments.length){case 1:typeof u=="function"?(a=u,u=/\r?\n/):typeof u=="object"&&!(u instanceof RegExp)&&!u[Symbol.split]&&(f=u,u=/\r?\n/);break;case 2:typeof u=="function"?(f=a,a=u,u=/\r?\n/):typeof a=="object"&&(f=a,a=l)}f=Object.assign({},f),f.autoDestroy=!0,f.transform=n,f.flush=i,f.readableObjectMode=!0;const d=new e(f);return d[r]="",d[s]=new t("utf8"),d.matcher=u,d.mapper=a,d.maxLength=f.maxLength,d.skipOverflow=f.skipOverflow||!1,d.overflow=!1,d._destroy=function(p,m){this._writableState.errorEmitted=!1,m(p)},d}return Pr=c,Pr}var ln;function Yo(){return ln||(ln=1,(function(e){var t={},r=mi,s=Rn.Stream,n=Jo(),i=Ve,o=5432,l=process.platform==="win32",c=process.stderr,u=56,a=7,f=61440,d=32768;function p(N){return(N&f)==d}var m=["host","port","database","user","password"],b=m.length,v=m[b-1];function h(){var N=c instanceof s&&c.writable===!0;if(N){var T=Array.prototype.slice.call(arguments).concat(`
`);c.write(i.format.apply(i,T))}}Object.defineProperty(e.exports,"isWin",{get:function(){return l},set:function(N){l=N}}),e.exports.warnTo=function(N){var T=c;return c=N,T},e.exports.getFileName=function(N){var T=N||t,g=T.PGPASSFILE||(l?r.join(T.APPDATA||"./","postgresql","pgpass.conf"):r.join(T.HOME||"./",".pgpass"));return g},e.exports.usePgPass=function(N,T){return Object.prototype.hasOwnProperty.call(t,"PGPASSWORD")?!1:l?!0:(T=T||"<unkn>",p(N.mode)?N.mode&(u|a)?(h('WARNING: password file "%s" has group or world access; permissions should be u=rw (0600) or less',T),!1):!0:(h('WARNING: password file "%s" is not a plain file',T),!1))};var y=e.exports.match=function(N,T){return m.slice(0,-1).reduce(function(g,x,w){return w==1&&Number(N[x]||o)===Number(T[x])?g&&!0:g&&(T[x]==="*"||T[x]===N[x])},!0)};e.exports.getPassword=function(N,T,g){var x,w=T.pipe(n());function _(S){var O=E(S);O&&C(O)&&y(N,O)&&(x=O[v],w.end())}var A=function(){T.destroy(),g(x)},R=function(S){T.destroy(),h("WARNING: error on reading file: %s",S),g(void 0)};T.on("error",R),w.on("data",_).on("end",A).on("error",R)};var E=e.exports.parseLine=function(N){if(N.length<11||N.match(/^\s+#/))return null;for(var T="",g="",x=0,w=0,_={},A=!1,R=function(O,B,U){var H=N.substring(B,U);Object.hasOwnProperty.call(t,"PGPASS_NO_DEESCAPE")||(H=H.replace(/\\([:\\])/g,"$1")),_[m[O]]=H},S=0;S<N.length-1;S+=1){if(T=N.charAt(S+1),g=N.charAt(S),A=x==b-1,A){R(x,w);break}S>=0&&T==":"&&g!=="\\"&&(R(x,w,S+1),w=S+2,x+=1)}return _=Object.keys(_).length===b?_:null,_},C=e.exports.isValidEntry=function(N){for(var T={0:function(A){return A.length>0},1:function(A){return A==="*"?!0:(A=Number(A),isFinite(A)&&A>0&&A<9007199254740992&&Math.floor(A)===A)},2:function(A){return A.length>0},3:function(A){return A.length>0},4:function(A){return A.length>0}},g=0;g<m.length;g+=1){var x=T[g],w=N[m[g]]||"",_=x(w);if(!_)return!1}return!0}})(Tr)),Tr.exports}var cn;function Ko(){if(cn)return ht.exports;cn=1;var e=Tn,t=Yo();return ht.exports=function(r,s){var n=t.getFileName();e.stat(n,function(i,o){if(i||!t.usePgPass(o,n))return s(void 0);var l=e.createReadStream(n);t.getPassword(r,l,s)})},ht.exports.warnTo=t.warnTo,ht.exports}var Rr,un;function Zo(){if(un)return Rr;un=1;const e=Xe.EventEmitter,t=rt(),r=Ve,s=Uo(),n=Qr(),i=ni(),o=Fo(),l=At(),c=li(),u=si(),a=r.deprecate(()=>{},"Client.activeQuery is deprecated and will be removed in pg@9.0"),f=r.deprecate(()=>{},"Client.queryQueue is deprecated and will be removed in pg@9.0."),d=r.deprecate(()=>{},"pgpass support is deprecated and will be removed in pg@9.0. You can provide an async function as the password property to the Client/Pool constructor that returns a password instead. Within this function you can call the pgpass module in your own code."),p=r.deprecate(()=>{},"Passing a custom Promise implementation to the Client/Pool constructor is deprecated and will be removed in pg@9.0."),m=r.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead.");class b extends e{constructor(h){super(),this.connectionParameters=new i(h),this.user=this.connectionParameters.user,this.database=this.connectionParameters.database,this.port=this.connectionParameters.port,this.host=this.connectionParameters.host,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:this.connectionParameters.password}),this.replication=this.connectionParameters.replication;const y=h||{};y.Promise&&p(),this._Promise=y.Promise||Mr.Promise,this._types=new n(y.types),this._ending=!1,this._ended=!1,this._connecting=!1,this._connected=!1,this._connectionError=!1,this._queryable=!0,this._activeQuery=null,this.enableChannelBinding=!!y.enableChannelBinding,this.connection=y.connection||new c({stream:y.stream,ssl:this.connectionParameters.ssl,keepAlive:y.keepAlive||!1,keepAliveInitialDelayMillis:y.keepAliveInitialDelayMillis||0,encoding:this.connectionParameters.client_encoding||"utf8"}),this._queryQueue=[],this.binary=y.binary||l.binary,this.processID=null,this.secretKey=null,this.ssl=this.connectionParameters.ssl||!1,this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this._connectionTimeoutMillis=y.connectionTimeoutMillis||0}get activeQuery(){return a(),this._activeQuery}set activeQuery(h){a(),this._activeQuery=h}_getActiveQuery(){return this._activeQuery}_errorAllQueries(h){const y=C=>{process.nextTick(()=>{C.handleError(h,this.connection)})},E=this._getActiveQuery();E&&(y(E),this._activeQuery=null),this._queryQueue.forEach(y),this._queryQueue.length=0}_connect(h){const y=this,E=this.connection;if(this._connectionCallback=h,this._connecting||this._connected){const C=new Error("Client has already been connected. You cannot reuse a client.");process.nextTick(()=>{h(C)});return}this._connecting=!0,this._connectionTimeoutMillis>0&&(this.connectionTimeoutHandle=setTimeout(()=>{E._ending=!0,E.stream.destroy(new Error("timeout expired"))},this._connectionTimeoutMillis),this.connectionTimeoutHandle.unref&&this.connectionTimeoutHandle.unref()),this.host&&this.host.indexOf("/")===0?E.connect(this.host+"/.s.PGSQL."+this.port):E.connect(this.port,this.host),E.on("connect",function(){y.ssl?E.requestSsl():E.startup(y.getStartupConf())}),E.on("sslconnect",function(){E.startup(y.getStartupConf())}),this._attachListeners(E),E.once("end",()=>{const C=this._ending?new Error("Connection terminated"):new Error("Connection terminated unexpectedly");clearTimeout(this.connectionTimeoutHandle),this._errorAllQueries(C),this._ended=!0,this._ending||(this._connecting&&!this._connectionError?this._connectionCallback?this._connectionCallback(C):this._handleErrorEvent(C):this._connectionError||this._handleErrorEvent(C)),process.nextTick(()=>{this.emit("end")})})}connect(h){if(h){this._connect(h);return}return new this._Promise((y,E)=>{this._connect(C=>{C?E(C):y(this)})})}_attachListeners(h){h.on("authenticationCleartextPassword",this._handleAuthCleartextPassword.bind(this)),h.on("authenticationMD5Password",this._handleAuthMD5Password.bind(this)),h.on("authenticationSASL",this._handleAuthSASL.bind(this)),h.on("authenticationSASLContinue",this._handleAuthSASLContinue.bind(this)),h.on("authenticationSASLFinal",this._handleAuthSASLFinal.bind(this)),h.on("backendKeyData",this._handleBackendKeyData.bind(this)),h.on("error",this._handleErrorEvent.bind(this)),h.on("errorMessage",this._handleErrorMessage.bind(this)),h.on("readyForQuery",this._handleReadyForQuery.bind(this)),h.on("notice",this._handleNotice.bind(this)),h.on("rowDescription",this._handleRowDescription.bind(this)),h.on("dataRow",this._handleDataRow.bind(this)),h.on("portalSuspended",this._handlePortalSuspended.bind(this)),h.on("emptyQuery",this._handleEmptyQuery.bind(this)),h.on("commandComplete",this._handleCommandComplete.bind(this)),h.on("parseComplete",this._handleParseComplete.bind(this)),h.on("copyInResponse",this._handleCopyInResponse.bind(this)),h.on("copyData",this._handleCopyData.bind(this)),h.on("notification",this._handleNotification.bind(this))}_getPassword(h){const y=this.connection;if(typeof this.password=="function")this._Promise.resolve().then(()=>this.password(this.connectionParameters)).then(E=>{if(E!==void 0){if(typeof E!="string"){y.emit("error",new TypeError("Password must be a string"));return}this.connectionParameters.password=this.password=E}else this.connectionParameters.password=this.password=null;h()}).catch(E=>{y.emit("error",E)});else if(this.password!==null)h();else try{Ko()(this.connectionParameters,C=>{C!==void 0&&(d(),this.connectionParameters.password=this.password=C),h()})}catch(E){this.emit("error",E)}}_handleAuthCleartextPassword(h){this._getPassword(()=>{this.connection.password(this.password)})}_handleAuthMD5Password(h){this._getPassword(async()=>{try{const y=await u.postgresMd5PasswordHash(this.user,this.password,h.salt);this.connection.password(y)}catch(y){this.emit("error",y)}})}_handleAuthSASL(h){this._getPassword(()=>{try{this.saslSession=s.startSession(h.mechanisms,this.enableChannelBinding&&this.connection.stream),this.connection.sendSASLInitialResponseMessage(this.saslSession.mechanism,this.saslSession.response)}catch(y){this.connection.emit("error",y)}})}async _handleAuthSASLContinue(h){try{await s.continueSession(this.saslSession,this.password,h.data,this.enableChannelBinding&&this.connection.stream),this.connection.sendSCRAMClientFinalMessage(this.saslSession.response)}catch(y){this.connection.emit("error",y)}}_handleAuthSASLFinal(h){try{s.finalizeSession(this.saslSession,h.data),this.saslSession=null}catch(y){this.connection.emit("error",y)}}_handleBackendKeyData(h){this.processID=h.processID,this.secretKey=h.secretKey}_handleReadyForQuery(h){this._connecting&&(this._connecting=!1,this._connected=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback&&(this._connectionCallback(null,this),this._connectionCallback=null),this.emit("connect"));const y=this._getActiveQuery();this._activeQuery=null,this.readyForQuery=!0,y&&y.handleReadyForQuery(this.connection),this._pulseQueryQueue()}_handleErrorWhileConnecting(h){if(!this._connectionError){if(this._connectionError=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback)return this._connectionCallback(h);this.emit("error",h)}}_handleErrorEvent(h){if(this._connecting)return this._handleErrorWhileConnecting(h);this._queryable=!1,this._errorAllQueries(h),this.emit("error",h)}_handleErrorMessage(h){if(this._connecting)return this._handleErrorWhileConnecting(h);const y=this._getActiveQuery();if(!y){this._handleErrorEvent(h);return}this._activeQuery=null,y.handleError(h,this.connection)}_handleRowDescription(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected rowDescription message from backend.");this._handleErrorEvent(E);return}y.handleRowDescription(h)}_handleDataRow(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected dataRow message from backend.");this._handleErrorEvent(E);return}y.handleDataRow(h)}_handlePortalSuspended(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected portalSuspended message from backend.");this._handleErrorEvent(E);return}y.handlePortalSuspended(this.connection)}_handleEmptyQuery(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected emptyQuery message from backend.");this._handleErrorEvent(E);return}y.handleEmptyQuery(this.connection)}_handleCommandComplete(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected commandComplete message from backend.");this._handleErrorEvent(E);return}y.handleCommandComplete(h,this.connection)}_handleParseComplete(){const h=this._getActiveQuery();if(h==null){const y=new Error("Received unexpected parseComplete message from backend.");this._handleErrorEvent(y);return}h.name&&(this.connection.parsedStatements[h.name]=h.text)}_handleCopyInResponse(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected copyInResponse message from backend.");this._handleErrorEvent(E);return}y.handleCopyInResponse(this.connection)}_handleCopyData(h){const y=this._getActiveQuery();if(y==null){const E=new Error("Received unexpected copyData message from backend.");this._handleErrorEvent(E);return}y.handleCopyData(h,this.connection)}_handleNotification(h){this.emit("notification",h)}_handleNotice(h){this.emit("notice",h)}getStartupConf(){const h=this.connectionParameters,y={user:h.user,database:h.database},E=h.application_name||h.fallback_application_name;return E&&(y.application_name=E),h.replication&&(y.replication=""+h.replication),h.statement_timeout&&(y.statement_timeout=String(parseInt(h.statement_timeout,10))),h.lock_timeout&&(y.lock_timeout=String(parseInt(h.lock_timeout,10))),h.idle_in_transaction_session_timeout&&(y.idle_in_transaction_session_timeout=String(parseInt(h.idle_in_transaction_session_timeout,10))),h.options&&(y.options=h.options),y}cancel(h,y){if(h.activeQuery===y){const E=this.connection;this.host&&this.host.indexOf("/")===0?E.connect(this.host+"/.s.PGSQL."+this.port):E.connect(this.port,this.host),E.on("connect",function(){E.cancel(h.processID,h.secretKey)})}else h._queryQueue.indexOf(y)!==-1&&h._queryQueue.splice(h._queryQueue.indexOf(y),1)}setTypeParser(h,y,E){return this._types.setTypeParser(h,y,E)}getTypeParser(h,y){return this._types.getTypeParser(h,y)}escapeIdentifier(h){return t.escapeIdentifier(h)}escapeLiteral(h){return t.escapeLiteral(h)}_pulseQueryQueue(){if(this.readyForQuery===!0){this._activeQuery=this._queryQueue.shift();const h=this._getActiveQuery();if(h){this.readyForQuery=!1,this.hasExecuted=!0;const y=h.submit(this.connection);y&&process.nextTick(()=>{h.handleError(y,this.connection),this.readyForQuery=!0,this._pulseQueryQueue()})}else this.hasExecuted&&(this._activeQuery=null,this.emit("drain"))}}query(h,y,E){let C,N,T,g,x;if(h==null)throw new TypeError("Client was passed a null or undefined query");return typeof h.submit=="function"?(T=h.query_timeout||this.connectionParameters.query_timeout,N=C=h,C.callback||(typeof y=="function"?C.callback=y:E&&(C.callback=E))):(T=h.query_timeout||this.connectionParameters.query_timeout,C=new o(h,y,E),C.callback||(N=new this._Promise((w,_)=>{C.callback=(A,R)=>A?_(A):w(R)}).catch(w=>{throw Error.captureStackTrace(w),w}))),T&&(x=C.callback||(()=>{}),g=setTimeout(()=>{const w=new Error("Query read timeout");process.nextTick(()=>{C.handleError(w,this.connection)}),x(w),C.callback=()=>{};const _=this._queryQueue.indexOf(C);_>-1&&this._queryQueue.splice(_,1),this._pulseQueryQueue()},T),C.callback=(w,_)=>{clearTimeout(g),x(w,_)}),this.binary&&!C.binary&&(C.binary=!0),C._result&&!C._result._types&&(C._result._types=this._types),this._queryable?this._ending?(process.nextTick(()=>{C.handleError(new Error("Client was closed and is not queryable"),this.connection)}),N):(this._queryQueue.length>0&&m(),this._queryQueue.push(C),this._pulseQueryQueue(),N):(process.nextTick(()=>{C.handleError(new Error("Client has encountered a connection error and is not queryable"),this.connection)}),N)}ref(){this.connection.ref()}unref(){this.connection.unref()}end(h){if(this._ending=!0,!this.connection._connecting||this._ended)if(h)h();else return this._Promise.resolve();if(this._getActiveQuery()||!this._queryable?this.connection.stream.destroy():this.connection.end(),h)this.connection.once("end",h);else return new this._Promise(y=>{this.connection.once("end",y)})}get queryQueue(){return f(),this._queryQueue}}return b.Query=o,Rr=b,Rr}var Ir,dn;function $o(){if(dn)return Ir;dn=1;const e=Xe.EventEmitter,t=function(){},r=(u,a)=>{const f=u.findIndex(a);return f===-1?void 0:u.splice(f,1)[0]};class s{constructor(a,f,d){this.client=a,this.idleListener=f,this.timeoutId=d}}class n{constructor(a){this.callback=a}}function i(){throw new Error("Release called on client which has already been released to the pool.")}function o(u,a){if(a)return{callback:a,result:void 0};let f,d;const p=function(b,v){b?f(b):d(v)},m=new u(function(b,v){d=b,f=v}).catch(b=>{throw Error.captureStackTrace(b),b});return{callback:p,result:m}}function l(u,a){return function f(d){d.client=a,a.removeListener("error",f),a.on("error",()=>{u.log("additional client error after disconnection due to error",d)}),u._remove(a),u.emit("error",d,a)}}class c extends e{constructor(a,f){super(),this.options=Object.assign({},a),a!=null&&"password"in a&&Object.defineProperty(this.options,"password",{configurable:!0,enumerable:!1,writable:!0,value:a.password}),a!=null&&a.ssl&&a.ssl.key&&Object.defineProperty(this.options.ssl,"key",{enumerable:!1}),this.options.max=this.options.max||this.options.poolSize||10,this.options.min=this.options.min||0,this.options.maxUses=this.options.maxUses||1/0,this.options.allowExitOnIdle=this.options.allowExitOnIdle||!1,this.options.maxLifetimeSeconds=this.options.maxLifetimeSeconds||0,this.log=this.options.log||function(){},this.Client=this.options.Client||f||ci().Client,this.Promise=this.options.Promise||Mr.Promise,typeof this.options.idleTimeoutMillis>"u"&&(this.options.idleTimeoutMillis=1e4),this._clients=[],this._idle=[],this._expired=new WeakSet,this._pendingQueue=[],this._endCallback=void 0,this.ending=!1,this.ended=!1}_promiseTry(a){const f=this.Promise;return typeof f.try=="function"?f.try(a):new f(d=>d(a()))}_isFull(){return this._clients.length>=this.options.max}_isAboveMin(){return this._clients.length>this.options.min}_pulseQueue(){if(this.log("pulse queue"),this.ended){this.log("pulse queue ended");return}if(this.ending){this.log("pulse queue on ending"),this._idle.length&&this._idle.slice().map(f=>{this._remove(f.client)}),this._clients.length||(this.ended=!0,this._endCallback());return}if(!this._pendingQueue.length){this.log("no queued requests");return}if(!this._idle.length&&this._isFull())return;const a=this._pendingQueue.shift();if(this._idle.length){const f=this._idle.pop();clearTimeout(f.timeoutId);const d=f.client;d.ref&&d.ref();const p=f.idleListener;return this._acquireClient(d,a,p,!1)}if(!this._isFull())return this.newClient(a);throw new Error("unexpected condition")}_remove(a,f){const d=r(this._idle,m=>m.client===a);d!==void 0&&clearTimeout(d.timeoutId),this._clients=this._clients.filter(m=>m!==a);const p=this;a.end(()=>{p.emit("remove",a),typeof f=="function"&&f()})}connect(a){if(this.ending){const p=new Error("Cannot use a pool after calling end on the pool");return a?a(p):this.Promise.reject(p)}const f=o(this.Promise,a),d=f.result;if(this._isFull()||this._idle.length){if(this._idle.length&&process.nextTick(()=>this._pulseQueue()),!this.options.connectionTimeoutMillis)return this._pendingQueue.push(new n(f.callback)),d;const p=(v,h,y)=>{clearTimeout(b),f.callback(v,h,y)},m=new n(p),b=setTimeout(()=>{r(this._pendingQueue,v=>v.callback===p),m.timedOut=!0,f.callback(new Error("timeout exceeded when trying to connect"))},this.options.connectionTimeoutMillis);return b.unref&&b.unref(),this._pendingQueue.push(m),d}return this.newClient(new n(f.callback)),d}newClient(a){const f=new this.Client(this.options);this._clients.push(f);const d=l(this,f);this.log("checking client timeout");let p,m=!1;this.options.connectionTimeoutMillis&&(p=setTimeout(()=>{f.connection?(this.log("ending client due to timeout"),m=!0,f.connection.stream.destroy()):f.isConnected()||(this.log("ending client due to timeout"),m=!0,f.end())},this.options.connectionTimeoutMillis)),this.log("connecting new client"),f.connect(b=>{if(p&&clearTimeout(p),f.on("error",d),b)this.log("client failed to connect",b),this._clients=this._clients.filter(v=>v!==f),m&&(b=new Error("Connection terminated due to connection timeout",{cause:b})),this._pulseQueue(),a.timedOut||a.callback(b,void 0,t);else{if(this.log("new client connected"),this.options.onConnect){this._promiseTry(()=>this.options.onConnect(f)).then(()=>{this._afterConnect(f,a,d)},v=>{this._clients=this._clients.filter(h=>h!==f),f.end(()=>{this._pulseQueue(),a.timedOut||a.callback(v,void 0,t)})});return}return this._afterConnect(f,a,d)}})}_afterConnect(a,f,d){if(this.options.maxLifetimeSeconds!==0){const p=setTimeout(()=>{this.log("ending client due to expired lifetime"),this._expired.add(a),this._idle.findIndex(b=>b.client===a)!==-1&&this._acquireClient(a,new n((b,v,h)=>h()),d,!1)},this.options.maxLifetimeSeconds*1e3);p.unref(),a.once("end",()=>clearTimeout(p))}return this._acquireClient(a,f,d,!0)}_acquireClient(a,f,d,p){p&&this.emit("connect",a),this.emit("acquire",a),a.release=this._releaseOnce(a,d),a.removeListener("error",d),f.timedOut?p&&this.options.verify?this.options.verify(a,a.release):a.release():p&&this.options.verify?this.options.verify(a,m=>{if(m)return a.release(m),f.callback(m,void 0,t);f.callback(void 0,a,a.release)}):f.callback(void 0,a,a.release)}_releaseOnce(a,f){let d=!1;return p=>{d&&i(),d=!0,this._release(a,f,p)}}_release(a,f,d){if(a.on("error",f),a._poolUseCount=(a._poolUseCount||0)+1,this.emit("release",d,a),d||this.ending||!a._queryable||a._ending||a._poolUseCount>=this.options.maxUses)return a._poolUseCount>=this.options.maxUses&&this.log("remove expended client"),this._remove(a,this._pulseQueue.bind(this));if(this._expired.has(a))return this.log("remove expired client"),this._expired.delete(a),this._remove(a,this._pulseQueue.bind(this));let m;this.options.idleTimeoutMillis&&this._isAboveMin()&&(m=setTimeout(()=>{this._isAboveMin()&&(this.log("remove idle client"),this._remove(a,this._pulseQueue.bind(this)))},this.options.idleTimeoutMillis),this.options.allowExitOnIdle&&m.unref()),this.options.allowExitOnIdle&&a.unref(),this._idle.push(new s(a,f,m)),this._pulseQueue()}query(a,f,d){if(typeof a=="function"){const m=o(this.Promise,a);return setImmediate(function(){return m.callback(new Error("Passing a function as the first parameter to pool.query is not supported"))}),m.result}typeof f=="function"&&(d=f,f=void 0);const p=o(this.Promise,d);return d=p.callback,this.connect((m,b)=>{if(m)return d(m);let v=!1;const h=y=>{v||(v=!0,b.release(y),d(y))};b.once("error",h),this.log("dispatching query");try{b.query(a,f,(y,E)=>{if(this.log("query dispatched"),b.removeListener("error",h),!v)return v=!0,b.release(y),y?d(y):d(void 0,E)})}catch(y){return b.release(y),d(y)}}),p.result}end(a){if(this.log("ending"),this.ending){const d=new Error("Called end on pool more than once");return a?a(d):this.Promise.reject(d)}this.ending=!0;const f=o(this.Promise,a);return this._endCallback=f.callback,this._pulseQueue(),f.result}get waitingCount(){return this._pendingQueue.length}get idleCount(){return this._idle.length}get expiredCount(){return this._clients.reduce((a,f)=>a+(this._expired.has(f)?1:0),0)}get totalCount(){return this._clients.length}}return Ir=c,Ir}var Nr={exports:{}};const ea={},ta=Object.freeze(Object.defineProperty({__proto__:null,default:ea},Symbol.toStringTag,{value:"Module"})),ra=eo(ta);var Or={exports:{}},fn;function sa(){if(fn)return Or.exports;fn=1;const e=Xe.EventEmitter,t=Ve,r=rt(),s=Or.exports=function(i,o,l){e.call(this),i=r.normalizeQueryConfig(i,o,l),this.text=i.text,this.values=i.values,this.name=i.name,this.queryMode=i.queryMode,this.callback=i.callback,this.state="new",this._arrayMode=i.rowMode==="array",this._emitRowEvents=!1,this.on("newListener",(function(c){c==="row"&&(this._emitRowEvents=!0)}).bind(this))};t.inherits(s,e);const n={sqlState:"code",statementPosition:"position",messagePrimary:"message",context:"where",schemaName:"schema",tableName:"table",columnName:"column",dataTypeName:"dataType",constraintName:"constraint",sourceFile:"file",sourceLine:"line",sourceFunction:"routine"};return s.prototype.handleError=function(i){const o=this.native.pq.resultErrorFields();if(o)for(const l in o){const c=n[l]||l;i[c]=o[l]}this.callback?this.callback(i):this.emit("error",i),this.state="error"},s.prototype.then=function(i,o){return this._getPromise().then(i,o)},s.prototype.catch=function(i){return this._getPromise().catch(i)},s.prototype._getPromise=function(){return this._promise?this._promise:(this._promise=new Promise((function(i,o){this._once("end",i),this._once("error",o)}).bind(this)),this._promise)},s.prototype.submit=function(i){this.state="running";const o=this;this.native=i.native,i.native.arrayMode=this._arrayMode;let l=function(c,u,a){if(i.native.arrayMode=!1,setImmediate(function(){o.emit("_done")}),c)return o.handleError(c);o._emitRowEvents&&(a.length>1?u.forEach((f,d)=>{f.forEach(p=>{o.emit("row",p,a[d])})}):u.forEach(function(f){o.emit("row",f,a)})),o.state="end",o.emit("end",a),o.callback&&o.callback(null,a)};if(process.domain&&(l=process.domain.bind(l)),this.name){this.name.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",this.name,this.name.length),console.error("This can cause conflicts and silent errors executing queries"));const c=(this.values||[]).map(r.prepareValue);if(i.namedQueries[this.name]){if(this.text&&i.namedQueries[this.name]!==this.text){const u=new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);return l(u)}return i.native.execute(this.name,c,l)}return i.native.prepare(this.name,this.text,c.length,function(u){return u?l(u):(i.namedQueries[o.name]=o.text,o.native.execute(o.name,c,l))})}else if(this.values){if(!Array.isArray(this.values)){const u=new Error("Query values must be an array");return l(u)}const c=this.values.map(r.prepareValue);i.native.query(this.text,c,l)}else this.queryMode==="extended"?i.native.query(this.text,[],l):i.native.query(this.text,l)},Or.exports}var pn;function na(){if(pn)return Nr.exports;pn=1;const e=Ve;var t;try{t=ra}catch(u){throw u}const r=Qr(),s=Xe.EventEmitter,n=Ve,i=ni(),o=sa(),l=e.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead."),c=Nr.exports=function(u){s.call(this),u=u||{},this._Promise=u.Promise||Mr.Promise,this._types=new r(u.types),this.native=new t({types:this._types}),this._queryQueue=[],this._ending=!1,this._connecting=!1,this._connected=!1,this._queryable=!0;const a=this.connectionParameters=new i(u);u.nativeConnectionString&&(a.nativeConnectionString=u.nativeConnectionString),this.user=a.user,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:a.password}),this.database=a.database,this.host=a.host,this.port=a.port,this.namedQueries={}};return c.Query=o,n.inherits(c,s),c.prototype._errorAllQueries=function(u){const a=f=>{process.nextTick(()=>{f.native=this.native,f.handleError(u)})};this._hasActiveQuery()&&(a(this._activeQuery),this._activeQuery=null),this._queryQueue.forEach(a),this._queryQueue.length=0},c.prototype._connect=function(u){const a=this;if(this._connecting){process.nextTick(()=>u(new Error("Client has already been connected. You cannot reuse a client.")));return}this._connecting=!0,this.connectionParameters.getLibpqConnectionString(function(f,d){if(a.connectionParameters.nativeConnectionString&&(d=a.connectionParameters.nativeConnectionString),f)return u(f);a.native.connect(d,function(p){if(p)return a.native.end(),u(p);a._connected=!0,a.native.on("error",function(m){a._queryable=!1,a._errorAllQueries(m),a.emit("error",m)}),a.native.on("notification",function(m){a.emit("notification",{channel:m.relname,payload:m.extra})}),a.emit("connect"),a._pulseQueryQueue(!0),u(null,this)})})},c.prototype.connect=function(u){if(u){this._connect(u);return}return new this._Promise((a,f)=>{this._connect(d=>{d?f(d):a(this)})})},c.prototype.query=function(u,a,f){let d,p,m,b,v;if(u==null)throw new TypeError("Client was passed a null or undefined query");if(typeof u.submit=="function")m=u.query_timeout||this.connectionParameters.query_timeout,p=d=u,typeof a=="function"&&(u.callback=a);else if(m=u.query_timeout||this.connectionParameters.query_timeout,d=new o(u,a,f),!d.callback){let h,y;p=new this._Promise((E,C)=>{h=E,y=C}).catch(E=>{throw Error.captureStackTrace(E),E}),d.callback=(E,C)=>E?y(E):h(C)}return m&&(v=d.callback||(()=>{}),b=setTimeout(()=>{const h=new Error("Query read timeout");process.nextTick(()=>{d.handleError(h,this.connection)}),v(h),d.callback=()=>{};const y=this._queryQueue.indexOf(d);y>-1&&this._queryQueue.splice(y,1),this._pulseQueryQueue()},m),d.callback=(h,y)=>{clearTimeout(b),v(h,y)}),this._queryable?this._ending?(d.native=this.native,process.nextTick(()=>{d.handleError(new Error("Client was closed and is not queryable"))}),p):(this._queryQueue.length>0&&l(),this._queryQueue.push(d),this._pulseQueryQueue(),p):(d.native=this.native,process.nextTick(()=>{d.handleError(new Error("Client has encountered a connection error and is not queryable"))}),p)},c.prototype.end=function(u){const a=this;this._ending=!0,this._connected||this.once("connect",this.end.bind(this,u));let f;return u||(f=new this._Promise(function(d,p){u=m=>m?p(m):d()})),this.native.end(function(){a._connected=!1,a._errorAllQueries(new Error("Connection terminated")),process.nextTick(()=>{a.emit("end"),u&&u()})}),f},c.prototype._hasActiveQuery=function(){return this._activeQuery&&this._activeQuery.state!=="error"&&this._activeQuery.state!=="end"},c.prototype._pulseQueryQueue=function(u){if(!this._connected||this._hasActiveQuery())return;const a=this._queryQueue.shift();if(!a){u||this.emit("drain");return}this._activeQuery=a,a.submit(this);const f=this;a.once("_done",function(){f._pulseQueryQueue()})},c.prototype.cancel=function(u){this._activeQuery===u?this.native.cancel(function(){}):this._queryQueue.indexOf(u)!==-1&&this._queryQueue.splice(this._queryQueue.indexOf(u),1)},c.prototype.ref=function(){},c.prototype.unref=function(){},c.prototype.setTypeParser=function(u,a,f){return this._types.setTypeParser(u,a,f)},c.prototype.getTypeParser=function(u,a){return this._types.getTypeParser(u,a)},c.prototype.isConnected=function(){return this._connected},Nr.exports}var Dr,hn;function mn(){return hn||(hn=1,Dr=na()),Dr}var gn;function ci(){return gn||(gn=1,(function(e){var t={};const r=Zo(),s=At(),n=li(),i=ii(),o=rt(),l=$o(),c=Qr(),{DatabaseError:u}=ai(),{escapeIdentifier:a,escapeLiteral:f}=rt(),d=v=>class extends l{constructor(y){super(y,v)}},p=function(v){this.defaults=s,this.Client=v,this.Query=this.Client.Query,this.Pool=d(this.Client),this._pools=[],this.Connection=n,this.types=St(),this.DatabaseError=u,this.TypeOverrides=c,this.escapeIdentifier=a,this.escapeLiteral=f,this.Result=i,this.utils=o};let m=r,b=!1;try{b=!!t.NODE_PG_FORCE_NATIVE}catch{}b&&(m=mn()),e.exports=new p(m),Object.defineProperty(e.exports,"native",{configurable:!0,enumerable:!1,get(){let v=null;try{v=new p(mn())}catch(h){if(h.code!=="MODULE_NOT_FOUND")throw h}return Object.defineProperty(e.exports,"native",{value:v}),v}})})(rr)),rr.exports}var ia=ci();const $=Wn(ia);$.Client;const oa=$.Pool;$.Connection;$.types;$.Query;$.DatabaseError;$.escapeIdentifier;$.escapeLiteral;$.Result;$.TypeOverrides;$.defaults;const aa=Object.freeze(Object.defineProperty({__proto__:null,Pool:oa,default:$},Symbol.toStringTag,{value:"Module"}));export{Ts as default};
