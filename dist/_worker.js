...
---
${i}
---`,l=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:u}]})});if(!l.ok)return e.json({error:`OpenRouter: ${await l.text()}`},500);const f=await l.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});I.post("/api/admin/vimeo-transcript",k,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((m,b)=>{const v=h=>{let y=0;return String(h.language||"").toLowerCase().startsWith("pt")&&(y+=40),h.active!==!1&&(y+=20),h.type==="captions"&&(y+=10),y};return v(b)-v(m)}).find(m=>m.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const u=await fetch(c.link);if(!u.ok)return e.json({error:`Download da legenda falhou: ${u.status}`},502);const f=(await u.text()).replace(/^﻿/,"").split(/\r?\n/).map(m=>m.trim()),d=[];for(const m of f){if(!m||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(m)||m.includes("-->"))continue;const b=m.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();b&&d[d.length-1]!==b&&d.push(b)}const p=d.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/lessons/:id/whisper-transcribe",k,async e=>{var t,r,s;try{const n=parseInt(e.req.param("id")),i=O(e),{rows:o}=await i.pool.query("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[n]);if(!o.length)return e.json({error:"Aula não encontrada"},404);const a=o[0];if(a.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!a.video_id||!/^\d+$/.test(a.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const c=e.env.VIMEO_ACCESS_TOKEN,u=e.env.OPENAI_API_KEY,l=e.env.VITE_OPENROUTER_API_KEY;if(!c)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!u)return e.json({error:"OPENAI_API_KEY não configurado"},500);const f=await fetch(`https://api.vimeo.com/videos/${a.video_id}?fields=download`,{headers:{Authorization:`Bearer ${c}`}});if(!f.ok)return e.json({error:`Vimeo API ${f.status}`},502);const p=((await f.json()).download||[]).filter(_=>_.link&&_.size).sort((_,R)=>_.size-R.size);if(!p.length)return e.json({error:"Download não disponível para este vídeo"},400);const m=p[0],b=(m.size/1024/1024).toFixed(1),v=await fetch(m.link);if(!v.ok)return e.json({error:`Download falhou: HTTP ${v.status}`},502);let h=Buffer.from(await v.arrayBuffer()),y="video.mp4";const E=24*1024*1024;if(h.length>E){try{const _=await import("os"),R=await import("path"),S=await import("fs"),D=await import("child_process"),B=R.join(_.tmpdir(),`cct_w_${n}.mp4`),U=R.join(_.tmpdir(),`cct_w_${n}.mp3`);S.writeFileSync(B,h),D.spawnSync("ffmpeg",["-y","-i",B,"-vn","-ar","16000","-ac","1","-b:a","32k",U],{stdio:"pipe"}).status===0&&S.existsSync(U)&&(h=S.readFileSync(U),y="audio.mp3",S.unlinkSync(U)),S.unlinkSync(B)}catch{}if(h.length>E)return e.json({error:`Arquivo muito grande (${b} MB). Use o script de transcrição em lote.`},400)}const A="----WhisperBoundary"+Date.now().toString(16),N=`\r
`,P=y.endsWith(".mp3")?"audio/mpeg":"video/mp4",g=Buffer.concat([Buffer.from(`--${A}${N}Content-Disposition: form-data; name="file"; filename="${y}"${N}Content-Type: ${P}${N}${N}`),h,Buffer.from(`${N}--${A}${N}Content-Disposition: form-data; name="model"${N}${N}whisper-1`),Buffer.from(`${N}--${A}${N}Content-Disposition: form-data; name="language"${N}${N}pt`),Buffer.from(`${N}--${A}${N}Content-Disposition: form-data; name="response_format"${N}${N}text`),Buffer.from(`${N}--${A}--${N}`)]),x=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${u}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(g.length)},body:g});if(!x.ok)return e.json({error:`OpenAI Whisper ${x.status}: ${await x.text()}`},502);let w=(await x.text()).trim(),C=null;if(l&&w){const _=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).

Recebi a transcrição bruta de uma aula chamada "${a.title}". Organize esta transcrição em Markdown estruturado com:
...
TRANSCRIÇÃO:
${w}`,R=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${l}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:_}]})});if(R.ok){const D=((s=(r=(t=(await R.json()).choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||"";if(D){w=D;const B=D.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);B&&(C=B[1].trim())}}}return await i.pool.query("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[w,C,n]),e.json({transcript:w,description:C,sizeMB:b})}catch(n){return e.json({error:n.message},500)}});I.delete("/api/admin/lessons/:id",k,async e=>{try{const t=e.req.param("id");return await O(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});I.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=O(e);await Mr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=Kn(e),f=await qr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await _o(l,s,a)){const p=await qr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[s,t,a]),e.json({success:!0,message:"Aula alugada com sucesso! Acesso liberado por 30 dias."})}catch(t){return console.error("Rent lesson error:",t),e.json({error:t.message||"Erro ao processar aluguel"},500)}});I.get("/api/user/rentals",K,async e=>{try{const t=e.get("user"),r=O(e);await Mr(r);const s=await r.sql(`SELECT lr.id, lr.lesson_id, lr.credits_paid, lr.rented_at, lr.expires_at,
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}I.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=O(e);await Te(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});I.get("/api/admin/trails",k,async e=>{try{const t=O(e);await Te(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails",k,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=O(e);await Te(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});I.put("/api/admin/trails/:id",k,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=O(e);await Te(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.delete("/api/admin/trails/:id",k,async e=>{try{const t=parseInt(e.req.param("id")),r=O(e);return await Te(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/lessons",k,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=O(e);await Te(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});I.delete("/api/admin/trails/:id/lessons/:lessonId",k,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await O(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.post("/api/admin/trails/:id/reorder",k,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=O(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});I.get("/api/admin/trails/search-lessons",k,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=O(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
...
</html>`)});I.get("/api/favorites",K,async e=>{const t=e.get("user"),r=O(e);try{const s=await r.sql(`
