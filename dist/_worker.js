...
`,x=E.endsWith(".mp3")?"audio/mpeg":"video/mp4",w=Buffer.concat([Buffer.from(`--${A}${g}Content-Disposition: form-data; name="file"; filename="${E}"${g}Content-Type: ${x}${g}${g}`),_,Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-1`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${A}--${g}`)]),C=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(w.length)},body:w});if(!C.ok)return e.json({error:`OpenAI Whisper ${C.status}: ${await C.text()}`},502);let R=(await C.text()).trim(),N=null;if(c&&R){const S=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${R}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const B=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(B){R=B;const F=B.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(N=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[R,N,n]),e.json({transcript:R,description:N,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});T.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});T.post("/api/lessons/:id/rent",X,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,i=L(e);await xt(i);const n=await i.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!n.length||!n[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=n[0],l=parseInt(o.rental_credits);if(!Number.isFinite(l)||l<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const u=await i.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(u.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:u[0].expires_at},400);const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const a=Xr(e),f=await zr(a,s);if(f<l)return e.json({error:"Créditos insuficientes",available:f,required:l},400);if(!await Po(a,s,l)){const h=await zr(a,s);return e.json({error:"Créditos insuficientes",available:h,required:l},400)}await i.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[s,t,l]);const p=new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"});return await qo(e.env,["🎬 *Aula alugada!*","",`📧 *Email:* ${s}`,`📖 *Aula:* ${o.title}`,`💳 *Créditos usados:* ${l}`,`🕐 *Data:* ${p}`].join(`
...
`)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function Bo(e,t,r,s){var i,n,o,l,u,d,a;switch(e){case"search_users":{const f=`%${t.query}%`,c=Math.min(t.limit||10,50),p=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const f=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,f]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[f])}}case"dashboard_stats":{const[f,c,p,h,b,v,m]=await Promise.all([r.sql("SELECT COUNT(*)::int as total FROM users"),r.sql("SELECT COUNT(*)::int as total FROM member_subscriptions WHERE data_expiracao > NOW() AND ativo = true"),r.sql("SELECT COUNT(*)::int as total FROM certificates"),r.sql("SELECT COUNT(*)::int as total FROM comments WHERE admin_reply IS NULL"),r.sql("SELECT COUNT(*)::int as total FROM courses"),r.sql("SELECT COUNT(*)::int as total FROM lessons"),(()=>{const y=s.env.DATABASE_SUITEPLUS;return y?new Re(y).sql("SELECT COUNT(*)::int as total FROM user_subscriptions WHERE status = 'active' AND product_id = 4 AND expires_at > NOW()").catch(()=>[{total:null}]):Promise.resolve([{total:null}])})()]);return{total_users:(i=f[0])==null?void 0:i.total,active_subscriptions_cct:(n=c[0])==null?void 0:n.total,active_subscriptions_suiteplus:(o=m[0])==null?void 0:o.total,total_certificates:(l=p[0])==null?void 0:l.total,pending_comments:(u=h[0])==null?void 0:u.total,total_courses:(d=b[0])==null?void 0:d.total,total_lessons:(a=v[0])==null?void 0:a.total}}case"get_user_progress":{const c=(await r.sql(`SELECT c.id as course_id, c.title as course_title,
         ...
         ${b} ORDER BY ${m} LIMIT $${h}`,p);return{lessons:y,count:y.length}}case"search_suiteplus_subscriptions":{const f=s.env.DATABASE_SUITEPLUS;if(!f)return{error:"DATABASE_SUITEPLUS não configurada"};const c=new Re(f),p=Math.min(t.limit||20,100);let h;return t.email?h=await c.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
...
- Se o aluno perguntar sobre cancelamento, reembolso, cobrança ou alteração de plano, explique gentilmente que você ainda não pode resolver isso diretamente e oriente a falar com o suporte pelo WhatsApp.`},...l.slice(-20),{role:"user",content:o}];for(let p=0;p<5;p++){const h=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Student Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:c,tools:Fo,tool_choice:"auto"})});if(!h.ok)return e.json({error:`OpenRouter: ${await h.text()}`},500);const v=(r=(await h.json()).choices)==null?void 0:r[0];if(!v)return e.json({error:"Resposta vazia do modelo"},500);const m=v.message,y=m.tool_calls;if(!y||y.length===0)return e.json({reply:m.content||""});const _=y[0],E=_.function.name;let I={};try{I=JSON.parse(_.function.arguments)}catch{}const A=await Ho(E,I,u,i,d);c.push({role:"assistant",content:m.content||null,tool_calls:y}),c.push({role:"tool",content:JSON.stringify(A),tool_call_id:_.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(s){return console.error("Student agent error:",s),e.json({error:s.message||"Erro no assistente"},500)}});T.get("/api/external/v1/users",oe,async e=>{try{const t=L(e),r=e.req.query("email"),s=Math.min(parseInt(e.req.query("limit")||"50"),200),i=r?await t.sql("SELECT * FROM users WHERE email ILIKE $1 OR nome ILIKE $1 ORDER BY created_at DESC LIMIT $2",[`%${r}%`,s]):await t.sql("SELECT * FROM users ORDER BY created_at DESC LIMIT $1",[s]);return e.json({users:i,count:i.length})}catch(t){return e.json({error:t.message||"Failed to fetch users"},500)}});T.get("/api/external/v1/users/:email",oe,async e=>{try{const t=e.req.param("email"),r=L(e),[s,i,n]=await Promise.all([r.sql("SELECT * FROM users WHERE lower(email) = lower($1) LIMIT 1",[t]),r.sql("SELECT * FROM member_subscriptions WHERE lower(email_membro) = lower($1) ORDER BY data_expiracao DESC",[t]),r.sql("SELECT * FROM certificates WHERE lower(user_email) = lower($1) ORDER BY created_at DESC",[t])]);if(!s.length)return e.json({error:"User not found"},404);let o=[];const l=e.env.DATABASE_SUITEPLUS;if(l)try{o=await new Re(l).sql(`SELECT id, product_id, started_at, expires_at, status, payment_source, recurring_enabled
    ...
    `,[r,t]))[0]||{};return e.json({start_date:n.start_date||null,completion_date:n.completion_date||null})}catch(t){return console.error("Suggest certificate dates error:",t),e.json({error:t.message||"Failed to suggest dates"},500)}});T.post("/api/admin/certificates",P,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),i=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,start_date:t.start_date||null,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:i&&i.length>0?i[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});T.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],i=L(e),n=[];for(const l of s){const u=m=>{const y=String(m??"").trim();return!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined"?"":y},d=u(l.user_email||l.email).toLowerCase(),a=u(l.user_name||l.nome)||null,f=u(l.course_title||l.curso);let c=l.carga_horaria?parseInt(String(l.carga_horaria)):null;if(c!==null&&!Number.isFinite(c)&&(c=null),!d.includes("@")||!f){n.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const y=String(m).trim();if(!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(y)){const[_,E,I]=y.split("/");return`${I}-${E}-${_}T12:00:00Z`}return isNaN(new Date(y).getTime())?null:new Date(y).toISOString()},h=p(l.data_final||l.completion_date),b=p(l.data_inicio||l.data_inicial||l.start_date);if(!c){const m=await i.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)c=parseInt(m[0].duration_hours);else{const y=E=>E.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),_=y(f);if(_.length>0){const I=(await i.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=y(A.title).join(" ");return _.every(x=>g.includes(x))});I&&(c=parseInt(I.duration_hours))}}}const v=await i.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await i.sql(`UPDATE certificates
           ...
           RETURNING id`,[d,a||"Aluno",f,c,y,_,h,b]);n.push({user_email:d,course_title:f,status:"created",certificate_id:(t=E[0])==null?void 0:t.id,verification_code:y})}}const o=n.filter(l=>l.status==="error").length;return e.json({success:o===0,total:s.length,results:n})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});T.put("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let i=r.course_title;if(r.course_id){const n=await s.query("courses",{select:"title",filters:{id:r.course_id}});n&&n.length>0&&(i=n[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:i,carga_horaria:r.carga_horaria,start_date:r.start_date||null,completion_date:r.completion_date||null,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});T.delete("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function $o(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,c)=>`<div class="mod-item"><span class="mod-num">${String(c+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=s?`
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});T.get("/api/courses/:id/modules",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const i=await r.sql(`SELECT m.*,
      ...
      ORDER BY ${y}
      LIMIT ${m}
    `,a);return e.json({lessons:_,limit:u})}catch(t){return console.error("❌ /api/search/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to search lessons"},500)}});T.get("/api/modules/:id/lessons",async e=>{try{const t=e.req.param("id"),s=await L(e).sql(`SELECT *
       FROM lessons
       WHERE module_id = $1
       ORDER BY order_index`,[t]);return e.json({lessons:s||[]})}catch(t){return console.error("❌ /api/modules/:id/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch module lessons"},500)}});T.get("/api/lessons/:id",async e=>{var t,r,s,i,n,o;try{const l=e.req.param("id"),u=V(e,"sb-access-token");let d=null;if(u){const E=await G(u,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);E&&(d=E.email)}const a=L(e),f=await a.sql("SELECT is_published FROM lessons WHERE id = $1",[parseInt(l)]);if(f.length===0)return e.json({error:"Lesson not found"},404);if(f[0].is_published===!1&&!(d?await ct(d,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,u):!1))return e.json({error:"Lesson not found"},404);let c=!1,p=!1;if(d)try{if(c=!!((t=(await a.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[d,parseInt(l)]))[0])!=null&&t.has_access),console.log("Has access:",c,"User:",d,"Lesson:",l),!c){const I=e.env.DATABASE_SUITEPLUS,[A,g,x]=await Promise.all([a.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[d,parseInt(l)]),a.sql(`SELECT id FROM member_subscriptions
               WHERE lower(email_membro) = lower($1)
                 AND data_expiracao > NOW()
                 AND COALESCE(ativo, true) = true
                 AND COALESCE(teste_gratis, false) = false
               LIMIT 1`,[d]),I?At(d,I):Promise.resolve(null)]);if((A.length>0||g.length>0||x&&x>new Date)&&(c=!0),!c){const w=await a.sql("SELECT rentable, rental_credits, title, COALESCE(teste_gratis, false) AS teste_gratis, COALESCE(free_trial, false) AS free_trial FROM lessons WHERE id = $1",[parseInt(l)]);if((r=w[0])!=null&&r.teste_gratis||(s=w[0])!=null&&s.free_trial)c=!0;else return console.log("❌ Access denied for user:",d,"lesson:",l),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((i=w[0])==null?void 0:i.rentable)||!1,rental_credits:((n=w[0])==null?void 0:n.rental_credits)||0,lesson_title:((o=w[0])==null?void 0:o.title)||""},403)}}console.log("✅ Access granted for user:",d,"lesson:",l)}catch(E){console.error("❌ Error checking access via RPC:",E),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),p=!0,c=!0}if(!d||!c&&!p){const E=await a.query("lessons",{select:"teste_gratis, free_trial",filters:{id:l},single:!0});if(!(E!=null&&E.teste_gratis)&&!(E!=null&&E.free_trial))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await St(a);const h=`
