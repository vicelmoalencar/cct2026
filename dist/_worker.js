...
<body>
  ...
  <div class="page">
    ...
    <div class="logo-section">
      <img class="logo-img" src="${Wr}" alt="Ensino Plus"/>
    </div>
...
<body class="bg-gray-100 p-8">
  ...
  `));async function vt(e,t,r){if(!r)return null;const s=r.replace("Bearer ",""),n=await fetch(`${e}/auth/v1/user`,{headers:{apikey:t,Authorization:`Bearer ${s}`}});return n.ok&&(await n.json()).email||null}R.get("/api/favorites",async e=>{const t=e.env,r=await vt(t.SUPABASE_URL,t.SUPABASE_ANON_KEY,e.req.header("Authorization"));if(!r)return e.json({error:"Unauthorized"},401);const s=new Ue(t.DATABASE_CCT);try{const n=await s.sql`
      ...
      WHERE f.user_email = ${r}
      ORDER BY f.created_at DESC
    `;return e.json(n)}finally{await s.end()}});R.post("/api/favorites",async e=>{const t=e.env,r=await vt(t.SUPABASE_URL,t.SUPABASE_ANON_KEY,e.req.header("Authorization"));if(!r)return e.json({error:"Unauthorized"},401);const s=await e.req.json();if(!s.lesson_id)return e.json({error:"lesson_id required"},400);const n=new Ue(t.DATABASE_CCT);try{return await n.sql`
      INSERT INTO user_favorites (user_email, lesson_id)
      VALUES (${r}, ${s.lesson_id})
      ON CONFLICT DO NOTHING
    `,e.json({ok:!0})}finally{await n.end()}});R.delete("/api/favorites/:lessonId",async e=>{const t=e.env,r=await vt(t.SUPABASE_URL,t.SUPABASE_ANON_KEY,e.req.header("Authorization"));if(!r)return e.json({error:"Unauthorized"},401);const s=parseInt(e.req.param("lessonId")),n=new Ue(t.DATABASE_CCT);try{return await n.sql`
      DELETE FROM user_favorites
      WHERE user_email = ${r} AND lesson_id = ${s}
    `,e.json({ok:!0})}finally{await n.end()}});R.get("/api/favorites/check/:lessonId",async e=>{const t=e.env,r=await vt(t.SUPABASE_URL,t.SUPABASE_ANON_KEY,e.req.header("Authorization"));if(!r)return e.json({favorite:!1});const s=parseInt(e.req.param("lessonId")),n=new Ue(t.DATABASE_CCT);try{const i=await n.sql`
      SELECT id FROM user_favorites
      WHERE user_email = ${r} AND lesson_id = ${s}
      LIMIT 1
    `;return e.json({favorite:i.length>0})}finally{await n.end()}});R.get("/favorites",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
