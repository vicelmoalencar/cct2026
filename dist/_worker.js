...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      `):e.redirect(`/?error=${r}`);const s=t.searchParams.get("access_token")||((c=t.hash.match(/access_token=([^&]+)/))==null?void 0:c[1]),n=t.searchParams.get("refresh_token")||((d=t.hash.match(/refresh_token=([^&]+)/))==null?void 0:d[1]),i=t.searchParams.get("type")||((l=t.hash.match(/type=([^&]+)/))==null?void 0:l[1]);if(!s)return e.redirect("/?error=no_token");try{const p=(f=JSON.parse(atob(s.split(".")[1])).amr)==null?void 0:f.some(h=>h.method==="otp");if(i==="recovery"||p)return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}catch{if(i==="recovery")return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}return J(e,"sb-access-token",s,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),n&&J(e,"sb-refresh-token",n,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.redirect("/?auth=success")}R.post("/api/auth/callback",async e=>{try{const{access_token:t,refresh_token:r}=await e.req.json();return t?(J(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),r&&J(e,"sb-refresh-token",r,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0})):e.json({error:"No access token"},400)}catch{return e.json({error:"Callback failed"},500)}});R.post("/api/auth/forgot-password",async e=>{try{const{email:t}=await e.req.json();if(!t)return e.json({error:"Email is required"},400);const r=e.req.header("host")||"localhost:3000",n=`${r.includes("localhost")?"http":"https"}://${r}/auth/callback`,i=await fetch(`${e.env.SUPABASE_URL}/auth/v1/recover`,{method:"POST",headers:{"Content-Type":"application/json",apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({email:t,options:{redirectTo:n}})});if(i.ok)return e.json({success:!0,message:"Se o email estiver cadastrado, você receberá um link de recuperação. O link é válido por 1 hora."});const o=await i.json();return e.json({error:o.error_description||"Failed to send reset email"},400)}catch{return e.json({error:"Failed to process request"},500)}});R.post("/api/auth/reset-password",async e=>{try{const t=await e.req.json(),{token:r,password:s}=t;if(console.log("🔐 Password reset attempt"),console.log("   Token present:",!!r),console.log("   Token length:",r==null?void 0:r.length),console.log("   Password length:",s==null?void 0:s.length),!r||!s)return console.error("❌ Missing token or password"),e.json({error:"Token e senha são obrigatórios"},400);if(s.length<6)return console.error("❌ Password too short"),e.json({error:"A senha deve ter pelo menos 6 caracteres"},400);console.log("📨 Calling Supabase to update password...");const n=await fetch(`${e.env.SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({password:s})});if(console.log("📨 Supabase response:",{status:n.status,ok:n.ok}),!n.ok){const o=await n.json();console.error("❌ Password reset failed:",o);let a="Falha ao redefinir senha";return o.error_code==="same_password"?a="A nova senha deve ser diferente da senha atual":o.msg?a=o.msg:o.error_description?a=o.error_description:o.message&&(a=o.message),e.json({error:a},400)}const i=await n.json();return console.log("✅ Password reset successful"),i.access_token&&J(e,"sb-access-token",i.access_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),i.refresh_token&&J(e,"sb-refresh-token",i.refresh_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0,message:"Senha alterada com sucesso!"})}catch{return e.json({error:"Failed to reset password"},500)}});async function Yr(e,t,r,s){try{return await new io(t,r).query("users",{select:"id, email, isadmin",filters:{email:e,isadmin:!0},single:!0},s)!==null}catch(n){return console.error("Error checking admin access in Supabase users:",n),!1}}async function q(e,t){const r=V(e,"sb-access-token");if(!r)return e.json({error:"Unauthorized"},401);const s=await G(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!s)return e.json({error:"Invalid token"},401);if(!await Yr(s.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))return e.json({error:"Forbidden - Admin only"},403);e.set("user",s),await t()}R.get("/api/admin/check",async e=>{const t=V(e,"sb-access-token");if(!t)return e.json({isAdmin:!1});const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({isAdmin:!1});const s=await Yr(r.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,t);return e.json({isAdmin:s})});R.post("/api/admin/impersonate",q,async e=>{try{const{user_email:t}=await e.req.json();if(!t)return e.json({error:"user_email is required"},400);console.log(`🎭 Admin impersonating user: ${t}`);const r=P(e);let s=await r.sql("SELECT id, email, nome FROM users WHERE lower(email) = lower($1) LIMIT 1",[t]);if((!s||s.length===0)&&(s=await r.sql(`SELECT NULL::integer AS id, email_membro AS email, NULL::text AS nome
         FROM member_subscriptions
         WHERE lower(email_membro) = lower($1)
         ORDER BY data_expiracao DESC NULLS LAST
         LIMIT 1`,[t])),!s||s.length===0)return e.json({error:"User not found"},404);const n=s[0],i={email:t,nome:n.nome||"Usuário",impersonated:!0,impersonated_at:new Date().toISOString(),user_id:n.id,signature:Buffer.from(`${t}:${e.env.SUPABASE_ANON_KEY}`).toString("base64")},o=`IMPERSONATE:${Buffer.from(JSON.stringify(i)).toString("base64")}`,a=V(e,"sb-access-token");return a&&J(e,"sb-admin-backup-token",a,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),J(e,"sb-access-token",o,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),console.log(`✅ Impersonation session set for ${t}`),e.json({success:!0,user_email:t,user_name:n.nome})}catch(t){return console.error("Impersonation error:",t),e.json({error:t.message||"Failed to impersonate user"},500)}});R.post("/api/admin/exit-impersonation",async e=>{try{const t=V(e,"sb-admin-backup-token");return t?(J(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),et(e,"sb-admin-backup-token"),e.json({success:!0})):e.json({error:"No admin session to restore"},400)}catch(t){return console.error("Exit impersonation error:",t),e.json({error:t.message||"Failed to exit impersonation"},500)}});R.post("/api/admin/courses",q,async e=>{try{const{title:t,description:r,duration_hours:s,instructor:n,offers_certificate:i,is_published:o}=await e.req.json(),c=await P(e).insert("courses",{title:t,description:r||null,duration_hours:s||0,instructor:n||"Vicelmo",offers_certificate:i!==void 0?i:!0,is_published:o!==void 0?o:!0});return e.json({success:!0,course_id:c[0].id})}catch(t){return console.error("Create course error:",t),e.json({error:t.message||"Failed to create course"},500)}});R.put("/api/admin/courses/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,duration_hours:n,instructor:i,offers_certificate:o,is_published:a}=await e.req.json();return await P(e).update("courses",{id:t},{title:r,description:s||null,duration_hours:n||0,instructor:i||"Vicelmo",offers_certificate:o!==void 0?o:!0,is_published:a!==void 0?a:!0}),e.json({success:!0})}catch{return e.json({error:"Failed to update course"},500)}});R.delete("/api/admin/courses/:id",q,async e=>{try{const t=e.req.param("id");return await P(e).delete("courses",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete course"},500)}});R.get("/api/admin/courses/find",q,async e=>{try{const t=e.req.query("title");if(!t)return e.json({error:"Title is required"},400);const r=P(e),s=await r.sql("SELECT * FROM courses WHERE lower(title) = lower($1) LIMIT 1",[t]);if(s&&s.length>0)return e.json({course:s[0]});const n=await r.sql("SELECT * FROM courses WHERE lower(title) ILIKE lower($1) LIMIT 1",[`%${t}%`]);return e.json({course:n&&n.length>0?n[0]:null})}catch(t){return console.error("Find course error:",t),e.json({error:t.message||"Failed to find course"},500)}});R.post("/api/admin/modules",q,async e=>{try{const{course_id:t,title:r,description:s,order_index:n}=await e.req.json(),o=await P(e).insert("modules",{course_id:t,title:r,description:s||null,order_index:n||0});return e.json({success:!0,module_id:o[0].id})}catch(t){return console.error("Create module error:",t),e.json({error:t.message||"Failed to create module"},500)}});R.post("/api/admin/modules-reorder",q,async e=>{try{const{modules:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"modules must be an array"},400);const r=P(e);for(const{id:s,order_index:n}of t)await r.update("modules",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder modules error:",t),e.json({error:t.message||"Failed to reorder modules"},500)}});R.put("/api/admin/modules/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,order_index:n}=await e.req.json();return await P(e).update("modules",{id:t},{title:r,description:s||null,order_index:n}),e.json({success:!0})}catch{return e.json({error:"Failed to update module"},500)}});R.delete("/api/admin/modules/:id",q,async e=>{try{const t=e.req.param("id");return await P(e).delete("modules",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete module"},500)}});R.get("/api/admin/modules/find",q,async e=>{try{const t=e.req.query("course_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"course_id and title are required"},400);const n=await P(e).query("modules",{select:"*",filters:{course_id:t,title:r},limit:1});return n&&n.length>0?e.json({module:n[0]}):e.json({module:null})}catch(t){return console.error("Find module error:",t),e.json({error:t.message||"Failed to find module"},500)}});R.post("/api/admin/lessons",q,async e=>{try{const{module_id:t,title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:a,free_trial:c,support_text:d,transcript:l,attachments:f,rentable:u,rental_credits:p}=await e.req.json();let h=null;n&&i&&(n==="youtube"?h=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?h=`https://vimeo.com/${i}`:h=i);const v=await P(e).insert("lessons",{module_id:t,title:r,description:s||null,video_url:h,video_provider:n||null,video_id:i||null,duration_minutes:o||0,order_index:a||0,teste_gratis:c||!1,support_text:d||null,transcript:l||null,attachments:JSON.stringify(f||[]),rentable:u||!1,rental_credits:p||0});return e.json({success:!0,lesson_id:v[0].id})}catch(t){return console.error("Create lesson error:",t),e.json({error:t.message||"Failed to create lesson"},500)}});R.post("/api/admin/lessons-reorder",q,async e=>{try{const{lessons:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"lessons must be an array"},400);const r=P(e);for(const{id:s,order_index:n}of t)await r.update("lessons",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder lessons error:",t),e.json({error:t.message||"Failed to reorder lessons"},500)}});R.put("/api/admin/lessons/:id",q,async e=>{try{const t=e.req.param("id"),{title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:a,free_trial:c,support_text:d,transcript:l,attachments:f,rentable:u,rental_credits:p}=await e.req.json();let h=null;n&&i&&(n==="youtube"?h=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?h=`https://vimeo.com/${i}`:h=i);const b=P(e),v=parseInt(t);await b.update("lessons",{id:v},{title:r,description:s||null,video_url:h,video_provider:n||null,video_id:i||null,duration_minutes:parseInt(o)||0,order_index:parseInt(a)||0,teste_gratis:c===!0||c==="true"});try{await b.sql("UPDATE lessons SET support_text = $1, transcript = $2, attachments = $3::jsonb WHERE id = $4",[d||null,l||null,JSON.stringify(f||[]),v])}catch(m){console.warn("support_text/transcript/attachments columns may not exist:",m.message)}return await b.sql("UPDATE lessons SET rentable = $1, rental_credits = $2 WHERE id = $3",[u===!0||u==="true",parseInt(p)||0,v]),e.json({success:!0})}catch(t){return console.error("Update lesson error:",t),e.json({error:t.message||"Failed to update lesson"},500)}});R.post("/api/admin/lessons/:id/generate-transcript",q,async e=>{var t,r,s;try{const n=parseInt(e.req.param("id")),{context:i}=await e.req.json(),a=await P(e).sql("SELECT title, transcript FROM lessons WHERE id = $1",[n]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(!c.transcript)return e.json({error:"Esta aula não possui transcrição para estruturar."},400);const d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada no ambiente"},500);const l="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",f=`Abaixo está a transcrição bruta de uma aula chamada "${c.title}".${i?`
...
---
${c.transcript}
---`,u=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:l},{role:"user",content:f}]})});if(!u.ok){const b=await u.text();return e.json({error:`Erro na API OpenRouter: ${b}`},500)}const h=((s=(r=(t=(await u.json()).choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||"";return e.json({transcript:h})}catch(n){return console.error("Generate transcript error:",n),e.json({error:n.message||"Erro ao gerar transcrição"},500)}});R.post("/api/admin/structure-transcript",q,async e=>{var t,r,s;try{const{title:n,transcript:i,context:o}=await e.req.json();if(!(i!=null&&i.trim()))return e.json({error:"Transcrição vazia"},400);const a=e.env.VITE_OPENROUTER_API_KEY;if(!a)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);const c="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",d=`Abaixo está a transcrição bruta de uma aula${n?` chamada "${n}"`:""}.${o?`
...
---
${i}
---`,l=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:d}]})});if(!l.ok)return e.json({error:`OpenRouter: ${await l.text()}`},500);const f=await l.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});R.post("/api/admin/vimeo-transcript",q,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((h,b)=>{const v=m=>{let y=0;return String(m.language||"").toLowerCase().startsWith("pt")&&(y+=40),m.active!==!1&&(y+=20),m.type==="captions"&&(y+=10),y};return v(b)-v(h)}).find(h=>h.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const d=await fetch(c.link);if(!d.ok)return e.json({error:`Download da legenda falhou: ${d.status}`},502);const f=(await d.text()).replace(/^﻿/,"").split(/\r?\n/).map(h=>h.trim()),u=[];for(const h of f){if(!h||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(h)||h.includes("-->"))continue;const b=h.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();b&&u[u.length-1]!==b&&u.push(b)}const p=u.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/lessons/:id/whisper-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=P(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const d=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||d.VIMEO_ACCESS_TOKEN,f=e.env.OPENAI_API_KEY||d.OPENAI_API_KEY||d["OPENAI_API-KEY"],u=e.env.VITE_OPENROUTER_API_KEY||d.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const x=Object.keys(d).filter(L=>L.toLowerCase().includes("openai"));return e.json({error:`OPENAI_API_KEY não configurado. Chaves OPENAI encontradas: [${x.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(x=>x.link&&x.size).sort((x,L)=>x.size-L.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],m=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let w=Buffer.from(await y.arrayBuffer()),S="video.mp4";const I=24*1024*1024;if(w.length>I){try{const x=await import("os"),L=await import("path"),D=await import("fs"),M=await import("child_process"),F=L.join(x.tmpdir(),`cct_w_${i}.mp4`),$=L.join(x.tmpdir(),`cct_w_${i}.mp3`);D.writeFileSync(F,w),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",$],{stdio:"pipe"}).status===0&&D.existsSync($)&&(w=D.readFileSync($),S="audio.mp3",D.unlinkSync($)),D.unlinkSync(F)}catch{}if(w.length>I)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const A="----WhisperBoundary"+Date.now().toString(16),g=`\r
`,_=S.endsWith(".mp3")?"audio/mpeg":"video/mp4",E=Buffer.concat([Buffer.from(`--${A}${g}Content-Disposition: form-data; name="file"; filename="${S}"${g}Content-Type: ${_}${g}${g}`),w,Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-1`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${A}--${g}`)]),T=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(E.length)},body:E});if(!T.ok)return e.json({error:`OpenAI Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),O=null;if(u&&C){const x=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,L=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${u}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:x}]})});if(L.ok){const M=((n=(s=(r=(await L.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,i]),e.json({transcript:C,description:O,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});R.post("/api/admin/lessons/:id/groq-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=P(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const d=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||d.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||d.GROQ_API_KEY,u=e.env.VITE_OPENROUTER_API_KEY||d.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const x=Object.keys(d).filter(L=>L.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${x.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const b=((await p.json()).download||[]).filter(x=>x.link&&x.size).sort((x,L)=>x.size-L.size);if(!b.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=b[0],m=(v.size/1024/1024).toFixed(1),y=await fetch(v.link);if(!y.ok)return e.json({error:`Download falhou: HTTP ${y.status}`},502);let w=Buffer.from(await y.arrayBuffer()),S="video.mp4";const I=24*1024*1024;if(w.length>I){try{const x=await import("os"),L=await import("path"),D=await import("fs"),M=await import("child_process"),F=L.join(x.tmpdir(),`cct_g_${i}.mp4`),$=L.join(x.tmpdir(),`cct_g_${i}.mp3`);D.writeFileSync(F,w),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",$],{stdio:"pipe"}).status===0&&D.existsSync($)&&(w=D.readFileSync($),S="audio.mp3",D.unlinkSync($)),D.unlinkSync(F)}catch{}if(w.length>I)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const A="----GroqBoundary"+Date.now().toString(16),g=`\r
`,_=S.endsWith(".mp3")?"audio/mpeg":"video/mp4",E=Buffer.concat([Buffer.from(`--${A}${g}Content-Disposition: form-data; name="file"; filename="${S}"${g}Content-Type: ${_}${g}${g}`),w,Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-large-v3-turbo`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${A}--${g}`)]),T=await fetch("https://api.groq.com/openai/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(E.length)},body:E});if(!T.ok)return e.json({error:`Groq Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),O=null;if(u&&C){const x=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,L=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${u}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:x}]})});if(L.ok){const M=((n=(s=(r=(await L.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,i]),e.json({transcript:C,description:O,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});R.delete("/api/admin/lessons/:id",q,async e=>{try{const t=e.req.param("id");return await P(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});R.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=P(e);await Xr(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const l=Vr(e),f=await Fr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await Io(l,s,a)){const p=await Fr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
       ON CONFLICT (user_email, lesson_id)
       DO UPDATE SET credits_paid = $3, rented_at = NOW(), expires_at = NOW() + INTERVAL '30 days'`,[s,t,a]),e.json({success:!0,message:"Aula alugada com sucesso! Acesso liberado por 30 dias."})}catch(t){return console.error("Rent lesson error:",t),e.json({error:t.message||"Erro ao processar aluguel"},500)}});R.get("/api/user/rentals",K,async e=>{try{const t=e.get("user"),r=P(e);await Xr(r);const s=await r.sql(`SELECT lr.id, lr.lesson_id, lr.credits_paid, lr.rented_at, lr.expires_at,
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}R.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=P(e);await Ne(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});R.get("/api/admin/trails",q,async e=>{try{const t=P(e);await Ne(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails",q,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=P(e);await Ne(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});R.put("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=P(e);await Ne(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=P(e);return await Ne(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/lessons",q,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=P(e);await Ne(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});R.delete("/api/admin/trails/:id/lessons/:lessonId",q,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await P(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/reorder",q,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=P(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/trails/search-lessons",q,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=P(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
    ...
    `,o);return e.json({lessons:a})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/lessons/find",q,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const n=await P(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return n&&n.length>0?e.json({lesson:n[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});function Jr(e,t){const r=Array.isArray(e.alternatives)?e.alternatives:[],s=Array.isArray(e.tags)?e.tags:String(e.tags||"").split(",").map(n=>n.trim()).filter(Boolean);return{title:e.title||null,statement_html:e.statement_html||"",question_type:e.question_type||"multiple_choice",alternatives:JSON.stringify(r),answer_key:JSON.stringify(e.answer_key||{}),technical_comment_html:e.technical_comment_html||null,difficulty:e.difficulty||"medio",theme:e.theme||null,subtheme:e.subtheme||null,legal_basis:e.legal_basis||null,weight:Number(e.weight||1),estimated_minutes:parseInt(e.estimated_minutes)||5,tags:JSON.stringify(s),status:e.status||"draft",professor:e.professor||null,course_id:e.course_id?parseInt(e.course_id):null,lesson_id:e.lesson_id?parseInt(e.lesson_id):null,source_transcript:e.source_transcript||null,ai_generated:e.ai_generated===!0||e.ai_generated==="true",order_index:parseInt(e.order_index)||0,usage_count:parseInt(e.usage_count)||0,attempts_count:parseInt(e.attempts_count)||0,correct_count:parseInt(e.correct_count)||0,wrong_count:parseInt(e.wrong_count)||0,created_by:t||e.created_by||null}}function Hr(e){if(typeof e=="string")return e.replace(/\b(conforme|segundo|de acordo com|com base na|a partir da)\s+a\s+transcri[cç][aã]o\b/gi,"").replace(/\bna\s+transcri[cç][aã]o\b/gi,"no material de estudo").replace(/\bda\s+transcri[cç][aã]o\b/gi,"do material de estudo").replace(/\btranscri[cç][aã]o\b/gi,"material de estudo").replace(/\s{2,}/g," ").replace(/\s+([,.;:!?])/g,"$1").trim();if(Array.isArray(e))return e.map(Hr).filter(t=>!(typeof t=="string"&&/transcri[cç][aã]o/i.test(t)));if(e&&typeof e=="object"){const t={};for(const[r,s]of Object.entries(e))t[r]=Hr(s);return t}return e}R.get("/api/admin/questions",q,async e=>{try{const t=P(e);await ie(t);const r=[],s=[];let n=1;const i=(m,y)=>{r.push(m.replace("?",`$${n++}`)),s.push(y)},o=e.req.query("q"),a=e.req.query("theme"),c=e.req.query("professor"),d=e.req.query("course_id"),l=e.req.query("difficulty"),f=e.req.query("question_type"),u=e.req.query("status"),p=e.req.query("from"),h=e.req.query("to");o&&(r.push(`(title ILIKE $${n} OR statement_html ILIKE $${n+1} OR legal_basis ILIKE $${n+2})`),s.push(`%${o}%`,`%${o}%`,`%${o}%`),n+=3),a&&i("theme = ?",a),c&&i("professor = ?",c),d&&i("course_id = ?",parseInt(d)),l&&i("difficulty = ?",l),f&&i("question_type = ?",f),u&&i("status = ?",u),p&&i("created_at >= ?",p),h&&i("created_at <= ?",h);const b=r.length?`WHERE ${r.join(" AND ")}`:"",v=await t.sql(`
      ...
      LIMIT 500
    `,s);return e.json({questions:v})}catch(t){return console.error("List questions error:",t),e.json({error:t.message||"Failed to list questions"},500)}});R.get("/api/admin/questions/stats",q,async e=>{try{const t=P(e);await ie(t);const r=await t.sql(`
      SELECT
        COUNT(*)::int AS total,
      ...
      FROM question_bank
    `),s=await t.sql("SELECT id, title, wrong_count, attempts_count FROM question_bank ORDER BY wrong_count DESC, attempts_count DESC LIMIT 5"),n=await t.sql("SELECT id, title, usage_count FROM question_bank ORDER BY usage_count DESC LIMIT 5");return e.json({stats:r[0]||{},mostWrong:s,mostUsed:n})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/questions/:id/versions",q,async e=>{try{const t=P(e);await ie(t);const r=await t.sql("SELECT * FROM question_bank_versions WHERE question_id = $1 ORDER BY version DESC, created_at DESC",[parseInt(e.req.param("id"))]);return e.json({versions:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions",q,async e=>{try{const t=e.get("user"),r=P(e);await ie(r);const s=Jr(await e.req.json(),t==null?void 0:t.email),i=(await r.sql(`
    ...
    `,[o.title,o.statement_html,o.question_type,o.alternatives,o.answer_key,o.technical_comment_html,o.difficulty,o.theme,o.subtheme,o.legal_basis,o.weight,o.estimated_minutes,o.tags,o.status,o.professor,o.course_id,o.lesson_id,o.source_transcript,o.ai_generated,o.order_index,o.usage_count,o.attempts_count,o.correct_count,o.wrong_count,a,r]);return await s.sql("INSERT INTO question_bank_versions (question_id, version, snapshot, changed_by, change_note) VALUES ($1, $2, $3::jsonb, $4, $5)",[r,a,JSON.stringify(c[0]),(t==null?void 0:t.email)||null,"Edicao manual"]),e.json({success:!0,question:c[0]})}catch(t){return console.error("Update question error:",t),e.json({error:t.message||"Failed to update question"},500)}});R.post("/api/admin/questions/:id/duplicate",q,async e=>{try{const t=e.get("user"),r=parseInt(e.req.param("id")),s=P(e);await ie(s);const n=await s.sql("SELECT * FROM question_bank WHERE id = $1",[r]);if(!n.length)return e.json({error:"Question not found"},404);const i=n[0],o=await s.sql(`
      ...
      RETURNING *
    `,[`${i.title||"Questao"} (copia)`,i.statement_html,i.question_type,JSON.stringify(i.alternatives||[]),JSON.stringify(i.answer_key||{}),i.technical_comment_html,i.difficulty,i.theme,i.subtheme,i.legal_basis,i.weight,i.estimated_minutes,JSON.stringify(i.tags||[]),i.professor,i.course_id,i.lesson_id,i.source_transcript,i.ai_generated,i.order_index+1,(t==null?void 0:t.email)||null]);return e.json({success:!0,question:o[0]})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/questions/:id",q,async e=>{try{const t=P(e);return await ie(t),await t.sql("DELETE FROM question_bank WHERE id = $1",[parseInt(e.req.param("id"))]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions-reorder",q,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions must be an array"},400);const r=P(e);await ie(r);for(const s of t)await r.sql("UPDATE question_bank SET order_index = $1, updated_at = NOW() WHERE id = $2",[parseInt(s.order_index),parseInt(s.id)]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/import",q,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions array required"},400);const r=e.get("user"),s=P(e);await ie(s);let n=0;for(const i of t){const o=Jr(i,r==null?void 0:r.email);await s.sql(`
        INSERT INTO question_bank (
          title, statement_html, question_type, alternatives, answer_key, technical_comment_html,
          difficulty, theme, subtheme, legal_basis, weight, estimated_minutes, tags, status,
          professor, course_id, lesson_id, source_transcript, ai_generated, order_index, created_by
...
Crie alternativas plausiveis, gabarito comentado e fundamentacao tecnica. Tipos desejados: ${a.join(", ")}. Dificuldade: ${c}. Quantidade: ${o}.
...
${(t.emails||[]).join(`
`)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function jo(e,t,r,s){var n,i,o,a,c,d,l;switch(e){case"search_users":{const f=`%${t.query}%`,u=Math.min(t.limit||10,50),p=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         ORDER BY u.created_at DESC LIMIT $2`,[f,u]);return{users:p,total:p.length}}case"get_user_details":{const[f,u,p]=await Promise.all([r.sql("SELECT * FROM users WHERE lower(email) = lower($1) LIMIT 1",[t.email]),r.sql("SELECT * FROM member_subscriptions WHERE lower(email_membro) = lower($1) ORDER BY data_expiracao DESC",[t.email]),r.sql("SELECT * FROM certificates WHERE lower(user_email) = lower($1) ORDER BY created_at DESC",[t.email])]);return{user:f[0]||null,subscriptions:u,certificates:p}}case"list_courses":return{courses:await r.sql("SELECT id, title, instructor, duration_hours, created_at FROM courses ORDER BY title")};case"get_course_details":{const[f,u]=await Promise.all([r.sql("SELECT * FROM courses WHERE id = $1",[t.course_id]),r.sql(`SELECT m.id, m.title, m.order_index, COUNT(l.id)::int as lesson_count
           FROM modules m LEFT JOIN lessons l ON l.module_id = m.id
           WHERE m.course_id = $1 GROUP BY m.id ORDER BY m.order_index`,[t.course_id])]);return{course:f[0]||null,modules:u}}case"search_subscriptions":return{subscriptions:await r.sql("SELECT * FROM member_subscriptions WHERE lower(email_membro) = lower($1) ORDER BY data_expiracao DESC",[t.email])};case"list_certificates":{const f=Math.min(t.limit||20,100);return{certificates:t.email?await r.sql("SELECT * FROM certificates WHERE lower(user_email) = lower($1) ORDER BY created_at DESC LIMIT $2",[t.email,f]):await r.sql("SELECT * FROM certificates ORDER BY created_at DESC LIMIT $1",[f])}}case"get_lesson":return{lesson:(await r.sql(`SELECT l.*, m.title as module_title, c.title as course_title
         FROM lessons l LEFT JOIN modules m ON m.id = l.module_id LEFT JOIN courses c ON c.id = m.course_id
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const f=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,f]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[f])}}case"dashboard_stats":{const[f,u,p,h,b,v,m]=await Promise.all([r.sql("SELECT COUNT(*)::int as total FROM users"),r.sql("SELECT COUNT(*)::int as total FROM member_subscriptions WHERE data_expiracao > NOW() AND ativo = true"),r.sql("SELECT COUNT(*)::int as total FROM certificates"),r.sql("SELECT COUNT(*)::int as total FROM comments WHERE admin_reply IS NULL"),r.sql("SELECT COUNT(*)::int as total FROM courses"),r.sql("SELECT COUNT(*)::int as total FROM lessons"),(()=>{const y=s.env.DATABASE_SUITEPLUS;return y?new $e(y).sql("SELECT COUNT(*)::int as total FROM user_subscriptions WHERE status = 'active' AND product_id = 4 AND expires_at > NOW()").catch(()=>[{total:null}]):Promise.resolve([{total:null}])})()]);return{total_users:(n=f[0])==null?void 0:n.total,active_subscriptions_cct:(i=u[0])==null?void 0:i.total,active_subscriptions_suiteplus:(o=m[0])==null?void 0:o.total,total_certificates:(a=p[0])==null?void 0:a.total,pending_comments:(c=h[0])==null?void 0:c.total,total_courses:(d=b[0])==null?void 0:d.total,total_lessons:(l=v[0])==null?void 0:l.total}}case"get_user_progress":{const u=(await r.sql(`SELECT c.id as course_id, c.title as course_title,
         ...
         ORDER BY completed_lessons DESC`,[t.email])).map(p=>({...p,progress_pct:p.total_lessons>0?Math.round(p.completed_lessons/p.total_lessons*100):0}));return{email:t.email,courses_with_progress:u,total_courses:u.length}}case"list_recent_signups":{const f=Math.min(t.days||7,365),u=Math.min(t.limit||20,100),p=await r.sql(`SELECT email, nome, created_at FROM users
         WHERE created_at > NOW() - INTERVAL '${f} days'
         ORDER BY created_at DESC LIMIT $1`,[u]);return{signups:p,count:p.length,days:f}}case"search_lessons":{const f=Math.min(t.limit||20,100),u=[],p=[];let h=1;t.query&&(u.push(`l.title ILIKE $${h++}`),p.push(`%${t.query}%`)),t.course_id&&(u.push(`c.id = $${h++}`),p.push(t.course_id)),t.created_after&&(u.push(`l.created_at >= $${h++}`),p.push(t.created_after)),t.created_before&&(u.push(`l.created_at <= $${h++}`),p.push(t.created_before));const b=u.length>0?`WHERE ${u.join(" AND ")}`:"",m={created_asc:"l.created_at ASC",created_desc:"l.created_at DESC",title:"l.title ASC"}[t.order_by]||"l.created_at DESC";p.push(f);const y=await r.sql(`SELECT l.id, l.title, l.duration_minutes, l.teste_gratis, l.rentable, l.created_at,
         ...
         ${b} ORDER BY ${m} LIMIT $${h}`,p);return{lessons:y,count:y.length}}case"search_suiteplus_subscriptions":{const f=s.env.DATABASE_SUITEPLUS;if(!f)return{error:"DATABASE_SUITEPLUS não configurada"};const u=new $e(f),p=Math.min(t.limit||20,100);let h;return t.email?h=await u.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE lower(user_email) = lower($1) ORDER BY expires_at DESC LIMIT $2`,[t.email,p]):t.status?h=await u.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE status = $1 AND product_id = 4 ORDER BY expires_at DESC LIMIT $2`,[t.status,p]):h=await u.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE product_id = 4 ORDER BY expires_at DESC LIMIT $1`,[p]),{subscriptions:h,count:h.length,source:"SuitePlus"}}case"list_expiring_subscriptions":{const f=Math.min(Math.max(t.days_ahead??7,0),365),u=Math.min(Math.max(t.include_expired_days??0,0),365),h=t.only_active!==!1?"AND ativo = true":"",b=await r.sql(`SELECT ms.email_membro, ms.data_expiracao, ms.ativo, ms.detalhe, ms.origem,
                u.nome
         FROM member_subscriptions ms
         LEFT JOIN users u ON lower(u.email) = lower(ms.email_membro)
         WHERE ms.data_expiracao BETWEEN (NOW() - INTERVAL '${u} days') AND (NOW() + INTERVAL '${f} days')
         ${h.replace("ativo","ms.ativo")}
         ORDER BY ms.data_expiracao ASC`,[]);return{subscriptions:b,count:b.length,query:{days_ahead:f,include_expired_days:u}}}default:return{error:`Ferramenta desconhecida: ${e}`}}}async function ko(e,t,r,s){var d,l,f;const n=new Set(["nome","telefone","whatsapp","ativo","dt_expiracao"]),i=new Set(["data_expiracao","ativo","detalhe","origem"]),o=new Set(["title","description","teste_gratis","rentable","duration_minutes","order_index"]),a=new Set(["title","description","instructor","duration_hours"]),c=new Set(["user_name","course_title","carga_horaria","generated_at"]);switch(e){case"update_user":{const u={};for(const[v,m]of Object.entries(t.fields||{}))n.has(v)&&(u[v]=m);if(Object.keys(u).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(u).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.email,...Object.values(u)],b=await r.sql(`UPDATE users SET ${p} WHERE lower(email) = lower($1) RETURNING *`,h);return{updated:b.length,user:b[0]||null}}case"update_member_subscription":{const u={};for(const[v,m]of Object.entries(t.fields||{}))i.has(v)&&(u[v]=m);if(Object.keys(u).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(u).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.email,...Object.values(u)],b=await r.sql(`UPDATE member_subscriptions SET ${p}, updated_at = NOW() WHERE lower(email_membro) = lower($1) RETURNING *`,h);return{updated:b.length,subscription:b[0]||null}}case"create_member_subscription":return{created:!0,subscription:(await r.sql(`INSERT INTO member_subscriptions (email_membro, data_expiracao, detalhe, ativo, origem, created_at, updated_at)
         ...
         WHERE lower(email_membro) = lower($1) RETURNING *`,[t.email])).length,expired:!0};case"update_lesson":{const u={};for(const[v,m]of Object.entries(t.fields||{}))o.has(v)&&(u[v]=m);if(Object.keys(u).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(u).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.lesson_id,...Object.values(u)],b=await r.sql(`UPDATE lessons SET ${p} WHERE id = $1 RETURNING id, title`,h);return{updated:b.length,lesson:b[0]||null}}case"update_course":{const u={};for(const[v,m]of Object.entries(t.fields||{}))a.has(v)&&(u[v]=m);if(Object.keys(u).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(u).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.course_id,...Object.values(u)],b=await r.sql(`UPDATE courses SET ${p}, updated_at = NOW() WHERE id = $1 RETURNING id, title`,h);return{updated:b.length,course:b[0]||null}}case"update_certificate":{const u={};for(const[v,m]of Object.entries(t.fields||{}))c.has(v)&&(u[v]=m);if(Object.keys(u).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(u).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.certificate_id,...Object.values(u)],b=await r.sql(`UPDATE certificates SET ${p}, updated_at = NOW() WHERE id = $1 RETURNING *`,h);return{updated:b.length,certificate:b[0]||null}}case"reply_comment":{const u=s.get("user"),p=await r.sql("UPDATE comments SET admin_reply = $1, admin_replied_at = NOW(), admin_replied_by = $2 WHERE id = $3 RETURNING id, user_name, comment_text, admin_reply",[t.reply,(u==null?void 0:u.email)||"admin",t.comment_id]);return p.length===0?{error:`Comentário ID ${t.comment_id} não encontrado`}:{replied:!0,comment:p[0]}}case"delete_comment":{const u=await r.sql("DELETE FROM comments WHERE id = $1 RETURNING id, user_name, comment_text",[t.comment_id]);return u.length===0?{error:`Comentário ID ${t.comment_id} não encontrado`}:{deleted:!0,comment:u[0]}}case"generate_certificate":{const u=new Uint8Array(4);crypto.getRandomValues(u);const p="CCT-"+new Date().getFullYear()+"-"+Array.from(u).map(m=>m.toString(16).padStart(2,"0")).join("").toUpperCase(),h=new Date().toISOString(),b=t.completion_date||h.split("T")[0],v=await r.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, created_at)
         VALUES ($1, $2, $3, $4, $5, $5, $6, $7::date, $6)
         RETURNING id, certificate_code`,[t.email,t.user_name,t.course_title,t.carga_horaria,p,h,b]);return{created:!0,certificate_id:(d=v[0])==null?void 0:d.id,certificate_code:(l=v[0])==null?void 0:l.certificate_code}}case"add_credits":{const u=Vr(s);await Qr(u);const p=await u.sql(`INSERT INTO users_credits (user_email, credits_balance, total_credits_used, updated_at)
         VALUES (lower($1), $2, 0, NOW())
         ON CONFLICT (user_email) DO UPDATE
           SET credits_balance = users_credits.credits_balance + $2, updated_at = NOW()
         RETURNING user_email, credits_balance`,[t.email,t.amount]);return{added:t.amount,new_balance:(f=p[0])==null?void 0:f.credits_balance,email:t.email}}case"bulk_extend_subscriptions":{const u=(t.emails||[]).map(b=>b.toLowerCase());if(u.length===0)return{error:"Nenhum email fornecido"};const p=Math.min(Math.max(t.days||0,1),3650),h=await r.sql(`UPDATE member_subscriptions
         SET data_expiracao = data_expiracao + ($1 || ' days')::INTERVAL, updated_at = NOW()
         WHERE lower(email_membro) = ANY($2)
         RETURNING email_membro, data_expiracao`,[String(p),u]);return{updated:h.length,days_added:p,subscriptions:h}}default:return{error:`Ferramenta de escrita desconhecida: ${e}`}}}R.get("/api/admin/agent-settings",q,async e=>{var t;try{const r=P(e);await r.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())");const s=await r.sql("SELECT value FROM agent_settings WHERE key = 'custom_instructions'");return e.json({instructions:((t=s[0])==null?void 0:t.value)||""})}catch(r){return e.json({error:r.message},500)}});R.put("/api/admin/agent-settings",q,async e=>{try{const{instructions:t}=await e.req.json(),r=P(e);return await r.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())"),await r.sql(`INSERT INTO agent_settings (key, value, updated_at) VALUES ('custom_instructions', $1, NOW())
       ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`,[t||""]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/agent",q,async e=>{var t,r,s;try{const n=await e.req.json(),{message:i,history:o=[],pendingAction:a}=n,c=P(e),d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada"},500);if(a){const h=await ko(a.tool,a.args,c,e);return h.error?e.json({reply:`❌ Erro ao executar: ${h.error}`}):e.json({reply:`✅ Feito! Resultado:
\`\`\`json
${JSON.stringify(h,null,2)}
\`\`\``})}let l="";try{await c.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())"),l=((r=(t=(await c.sql("SELECT value FROM agent_settings WHERE key = 'custom_instructions'"))[0])==null?void 0:t.value)==null?void 0:r.trim())||""}catch{}let p=[{role:"system",content:`Você é um assistente administrativo do CCT (Clube de Cálculo Trabalhista).
...
Data/hora atual no Brasil: ${new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo",dateStyle:"full",timeStyle:"short"})}${l?`

INSTRUÇÕES PERSONALIZADAS DO ADMINISTRADOR:
${l}`:""}`},...o.slice(-20),{role:"user",content:i}];for(let h=0;h<6;h++){const b=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Admin Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:p,tools:qo,tool_choice:"auto"})});if(!b.ok)return e.json({error:`OpenRouter: ${await b.text()}`},500);const m=(s=(await b.json()).choices)==null?void 0:s[0];if(!m)return e.json({error:"Resposta vazia do modelo"},500);const y=m.message,w=y.tool_calls;if(!w||w.length===0)return e.json({reply:y.content||""});const S=w[0],I=S.function.name;let A={};try{A=JSON.parse(S.function.arguments)}catch{}if(Po.has(I)){const _=Do(I,A);return e.json({reply:`⚠️ **Confirmação necessária**

${_}`,pendingAction:{tool:I,args:A,description:_}})}const g=await jo(I,A,c,e);p.push({role:"assistant",content:y.content||null,tool_calls:w}),p.push({role:"tool",content:JSON.stringify(g),tool_call_id:S.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(n){return console.error("Agent error:",n),e.json({error:n.message||"Erro no agente"},500)}});R.get("/api/admin/users",q,async e=>{try{const r=await P(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `);return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});R.get("/api/admin/users/find",q,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await P(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});R.post("/api/admin/users",q,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await P(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});R.put("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await P(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});R.delete("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id");return await P(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});R.get("/api/admin/certificates",q,async e=>{try{const r=await P(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});R.get("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),s=await P(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});R.get("/api/admin/certificates/find",q,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await P(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});R.post("/api/admin/certificates",q,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=P(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],n=P(e),i=[];for(const a of s){const c=m=>{const y=String(m??"").trim();return!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined"?"":y},d=c(a.user_email||a.email).toLowerCase(),l=c(a.user_name||a.nome)||null,f=c(a.course_title||a.curso);let u=a.carga_horaria?parseInt(String(a.carga_horaria)):null;if(u!==null&&!Number.isFinite(u)&&(u=null),!d.includes("@")||!f){i.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const y=String(m).trim();if(!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(y)){const[w,S,I]=y.split("/");return`${I}-${S}-${w}T12:00:00Z`}return isNaN(new Date(y).getTime())?null:new Date(y).toISOString()},h=p(a.data_final||a.completion_date),b=p(a.data_inicio||a.data_inicial||a.start_date);if(!u){const m=await n.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)u=parseInt(m[0].duration_hours);else{const y=S=>S.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),w=y(f);if(w.length>0){const I=(await n.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=y(A.title).join(" ");return w.every(_=>g.includes(_))});I&&(u=parseInt(I.duration_hours))}}}const v=await n.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await n.sql(`UPDATE certificates
           SET user_name = COALESCE($1, user_name),
               ...
               updated_at = NOW()
           WHERE id = $5`,[l,u,h,b,v[0].id]),i.push({user_email:d,course_title:f,status:"updated",certificate_id:v[0].id});else{const m=new Uint8Array(4);crypto.getRandomValues(m);const y="CCT-"+new Date().getFullYear()+"-"+Array.from(m).map(I=>I.toString(16).padStart(2,"0")).join("").toUpperCase(),w=new Date().toISOString(),S=await n.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, start_date, created_at)
           VALUES ($1, $2, $3, $4, $5, $5, $6, COALESCE($7, $6::timestamp), $8::date, $6)
           RETURNING id`,[d,l||"Aluno",f,u,y,w,h,b]);i.push({user_email:d,course_title:f,status:"created",certificate_id:(t=S[0])==null?void 0:t.id,verification_code:y})}}const o=i.filter(a=>a.status==="error").length;return e.json({success:o===0,total:s.length,results:i})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=P(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id");return await P(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Bo(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,u)=>`<div class="mod-item"><span class="mod-num">${String(u+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",l=r?`
  ...
  <div class="page">
    ...
    <div class="verso-body">
      ...
      <div class="modules-grid">
        ${e.modules.map((d,l)=>`
          <div class="mod-item"><span class="mod-num">${String(l+1).padStart(2,"0")}.</span>${d}</div>`).join("")}
      </div>
    </div>
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
...
</html>`}R.get("/api/my-certificates",K,async e=>{try{const r=e.get("user").email;if(!r)return e.json({error:"User email not found"},400);const n=await P(e).query("certificates",{select:"*",filters:{user_email:r},order:"completion_date DESC"});return e.json({certificates:n||[]})}catch(t){return console.error("Get my certificates error:",t),e.json({error:t.message||"Failed to get certificates"},500)}});R.get("/api/certificates/:id/html",K,async e=>{try{const t=e.req.param("id"),s=e.get("user").email,n=P(e),i=await n.query("certificates",{select:"*",filters:{id:t}});if(!i||i.length===0)return e.json({error:"Certificate not found"},404);const o=i[0];if(o.user_email!==s)return e.json({error:"Unauthorized"},403);const a=o.start_date?new Date(o.start_date).toLocaleDateString("pt-BR"):void 0,c=o.completion_date?new Date(o.completion_date).toLocaleDateString("pt-BR"):void 0,d=o.generated_at?new Date(o.generated_at).toLocaleDateString("pt-BR"):new Date().toLocaleDateString("pt-BR"),l=new URL(e.req.url).origin,f=o.certificate_code||o.verification_code||"",u=f?`${l}/verificar/${f}`:"";let p=[];if(o.course_modules)try{p=JSON.parse(o.course_modules).map(w=>w.title||w)}catch(y){console.log("Error parsing course_modules:",y)}if(p.length===0&&o.course_id)try{const y=await n.query("modules",{select:"title, order_index",filters:{course_id:o.course_id},order:"order_index ASC"});y&&y.length>0&&(p=y.map(w=>w.title))}catch(y){console.log("Error fetching modules:",y)}let h,b;if(o.course_id)try{const y=await n.query("certificate_templates",{select:"template_data, template_mime, verso_data, verso_mime",filters:{course_id:o.course_id},single:!0});y!=null&&y.template_data&&(h=`data:${y.template_mime||"image/jpeg"};base64,${y.template_data}`),y!=null&&y.verso_data&&(b=`data:${y.verso_mime||"image/jpeg"};base64,${y.verso_data}`)}catch{console.log("No certificate template found for course",o.course_id)}let v;if(u)try{v=await Oo.toString(u,{type:"svg",margin:1,color:{dark:"#1a1a2e",light:"#f8f7f5"}})}catch(y){console.log("QR code generation failed:",y)}const m=Bo({studentName:o.user_name,courseName:o.course_title,workload:o.carga_horaria||"N/A",startDate:a,completionDate:c,issueDate:d,verificationCode:f,verificationUrl:u,qrCodeSVG:v,modules:p.length>0?p:void 0,templateImageUrl:h,versoImageUrl:b});return e.html(m)}catch(t){return console.error("Generate certificate HTML error:",t),e.json({error:t.message||"Failed to generate certificate"},500)}});R.get("/verificar/:code",async e=>{try{const t=e.req.param("code"),r=P(e),s=await r.query("certificates",{select:"*",filters:{certificate_code:t}});if(!s||s.length===0)return e.html(`
        <!DOCTYPE html>
        <html lang="pt-BR">
        <head>
          <meta charset="UTF-8">
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id",async e=>{try{const t=e.req.param("id"),r=P(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.query("modules",{select:"*",filters:{course_id:t},order:"order_index"}),i=await r.sql(`SELECT l.* FROM lessons l
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const a of i){const c=o.get(a.module_id)||[];c.push(a),o.set(a.module_id,c)}for(const a of n)a.lessons=o.get(a.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,n;try{const i=e.req.param("id"),o=V(e,"sb-access-token");let a=null;if(o){const m=await G(o,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);m&&(a=m.email)}const c=P(e);let d=!1,l=!1;if(a)try{if(d=!!((t=(await c.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[a,parseInt(i)]))[0])!=null&&t.has_access),console.log("Has access:",d,"User:",a,"Lesson:",i),!d){const y=e.env.DATABASE_SUITEPLUS,[w,S,I]=await Promise.all([c.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[a,parseInt(i)]),c.sql("SELECT id FROM member_subscriptions WHERE email_membro = $1 AND data_expiracao > NOW() AND ativo = true LIMIT 1",[a]),y?_t(a,y):Promise.resolve(null)]);if((w.length>0||S.length>0||I&&I>new Date)&&(d=!0),!d){const A=await c.sql("SELECT rentable, rental_credits, title FROM lessons WHERE id = $1",[parseInt(i)]);return console.log("❌ Access denied for user:",a,"lesson:",i),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((r=A[0])==null?void 0:r.rentable)||!1,rental_credits:((s=A[0])==null?void 0:s.rental_credits)||0,lesson_title:((n=A[0])==null?void 0:n.title)||""},403)}}console.log("✅ Access granted for user:",a,"lesson:",i)}catch(m){console.error("❌ Error checking access via RPC:",m),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),l=!0,d=!0}if(!a||!d&&!l){const m=await c.query("lessons",{select:"teste_gratis",filters:{id:i},single:!0});if(!(m!=null&&m.teste_gratis))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await Et(c);const f=`
    ...
    `,[p,h,b]=await Promise.all([c.sql(f,[parseInt(i)]),c.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC",[i]),c.sql(u,[parseInt(i)])]);if(!p||p.length===0)return e.json({error:"Lesson not found"},404);const v=p[0];return e.json({lesson:v,comments:h,trails:b})}catch(i){return console.error("Error fetching lesson:",i),e.json({error:"Failed to fetch lesson"},500)}});R.get("/api/admin/comments",q,async e=>{try{const t=e.req.query("status")||"all",r=(e.req.query("search")||"").trim(),s=P(e);await Et(s);const n=[],i=[];t==="pending"?n.push("NULLIF(TRIM(COALESCE(c.admin_reply, '')), '') IS NULL"):t==="answered"&&n.push("NULLIF(TRIM(COALESCE(c.admin_reply, '')), '') IS NOT NULL"),r&&(i.push(`%${r}%`),n.push(`(
      ...
      )`));const o=n.length?`WHERE ${n.join(" AND ")}`:"",a=await s.sql(`
      SELECT
        c.*,
        l.title AS lesson_title,
        l.video_provider,
      ...
      ORDER BY c.created_at DESC
      LIMIT 200
    `,i);return e.json({comments:a})}catch(t){return console.error("List admin comments error:",t),e.json({error:t.message||"Failed to list comments"},500)}});R.put("/api/admin/comments/:id/reply",q,async e=>{try{const t=parseInt(e.req.param("id")),{admin_reply:r}=await e.req.json(),s=e.get("user"),n=String(r||"").trim(),i=P(e);await Et(i);const o=await i.update("comments",{id:t},{admin_reply:n||null,admin_replied_at:n?new Date().toISOString():null,admin_replied_by:n&&(s==null?void 0:s.email)||null});return e.json({success:!0,comment:o[0]||null})}catch(t){return console.error("Reply comment error:",t),e.json({error:t.message||"Failed to reply comment"},500)}});R.delete("/api/admin/comments/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=P(e);return await Et(r),await r.delete("comments",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete comment error:",t),e.json({error:t.message||"Failed to delete comment"},500)}});R.post("/api/lessons/:id/comments",async e=>{var t,r;try{const s=e.req.param("id"),{comment_text:n}=await e.req.json(),i=V(e,"sb-access-token");if(!i)return e.json({error:"Unauthorized"},401);const o=await G(i,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!o)return e.json({error:"Unauthorized"},401);if(!n||!n.trim())return e.json({error:"Comment text is required"},400);const a=((t=o.user_metadata)==null?void 0:t.full_name)||((r=o.email)==null?void 0:r.split("@")[0])||"Usuário",d=await P(e).insert("comments",{lesson_id:parseInt(s),user_name:a,user_email:o.email,comment_text:n.trim()});return e.json({success:!0,comment_id:d[0].id})}catch(s){return console.error("Add comment error:",s),e.json({error:s.message||"Failed to add comment"},500)}});R.get("/api/progress/:email/:courseId",async e=>{try{const t=e.req.param("email"),r=e.req.param("courseId"),i=await P(e).sql(`
      SELECT up.*, l.module_id
      FROM user_progress up
      JOIN lessons l ON up.lesson_id = l.id
      JOIN modules m ON l.module_id = m.id
      WHERE up.user_email = $1 AND m.course_id = $2
    `,[t,parseInt(r)]);return e.json({progress:i||[]})}catch(t){return console.error("❌ /api/progress error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch progress"},500)}});R.post("/api/progress/complete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=P(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0?await s.update("user_progress",{id:n[0].id},{completed:!0,completed_at:new Date().toISOString()}):await s.insert("user_progress",{user_email:t,lesson_id:parseInt(r),completed:!0,completed_at:new Date().toISOString()}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/progress/uncomplete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=P(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0&&await s.delete("user_progress",{id:n[0].id}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/admin/certificate-template",q,async e=>{try{const t=await e.req.json(),{course_id:r,image_data:s,verso_data:n}=t;if(!r||!s)return e.json({error:"ID do curso e imagem da frente são obrigatórios"},400);const i=b=>{const v=b.match(/^data:([^;]+);base64,/);return v?v[1]:"image/jpeg"},o=b=>b.includes(",")?b.split(",")[1]:b,a=i(s),c=o(s),d=n?i(n):null,l=n?o(n):null,f=`/api/certificate-template/${r}/image`,u=n?`/api/certificate-template/${r}/verso`:null,p=P(e),h=await p.query("certificate_templates",{select:"*",filters:{course_id:r}});if(h&&h.length>0){const b={template_url:f,template_data:c,template_mime:a,updated_at:new Date().toISOString()};n!==void 0&&(b.verso_data=l,b.verso_mime=d),await p.update("certificate_templates",{id:h[0].id},b)}else await p.insert("certificate_templates",{course_id:parseInt(r),template_url:f,template_data:c,template_mime:a,verso_data:l,verso_mime:d,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});return console.log("✅ Certificate template saved to Postgres"),e.json({success:!0,template_url:f,verso_url:u,message:"Template de certificado salvo com sucesso!"})}catch(t){return console.error("💥 Certificate template error:",t),e.json({error:"Erro ao salvar template de certificado",details:t.message},500)}});R.get("/api/certificate-template/:courseId/image",async e=>{try{const t=e.req.param("courseId"),s=await P(e).query("certificate_templates",{select:"template_data, template_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.template_data))return e.json({error:"Imagem não encontrada"},404);const n=Uint8Array.from(atob(s.template_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.template_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar imagem"},500)}});R.get("/api/certificate-template/:courseId/verso",async e=>{try{const t=e.req.param("courseId"),s=await P(e).query("certificate_templates",{select:"verso_data, verso_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.verso_data))return e.json({error:"Verso não encontrado"},404);const n=Uint8Array.from(atob(s.verso_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.verso_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar verso"},500)}});R.get("/api/certificate-template/:courseId",async e=>{try{const t=e.req.param("courseId"),r=P(e),s=await r.query("certificate_templates",{select:"id, course_id, template_mime, verso_mime, created_at, updated_at",filters:{course_id:t},single:!0});if(s){s.template_url=`/api/certificate-template/${t}/image`;const n=await r.query("certificate_templates",{select:"verso_data",filters:{course_id:t},single:!0});s.verso_url=n!=null&&n.verso_data?`/api/certificate-template/${t}/verso`:null}return e.json({template:s})}catch{return e.json({template:null})}});R.post("/api/certificates/generate",async e=>{var t,r,s;try{const n=V(e,"sb-access-token");if(!n)return e.json({error:"Não autenticado"},401);const i=await G(n,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!i)return e.json({error:"Usuário não encontrado"},401);const{course_id:o}=await e.req.json();if(console.log("📜 Certificate generation request:",{user_email:i.email,course_id:o}),!o)return e.json({error:"ID do curso é obrigatório"},400);const a=P(e),c=await a.query("certificates",{select:"*",filters:{user_email:i.email,course_id:o}});if(c&&c.length>0)return console.log("✅ Certificate already exists"),e.json({success:!0,certificate:c[0],message:"Certificado já existe!"});const d=await a.query("courses",{select:"*",filters:{id:o},single:!0});if(!d)return e.json({error:"Curso não encontrado"},404);const l=await a.query("modules",{select:"*",filters:{course_id:o}});let f=[];if(l)for(const v of l){const m=await a.query("lessons",{select:"id",filters:{module_id:v.id}});m&&(f=[...f,...m.map(y=>y.id)])}if(f.length===0)return e.json({error:"Curso não possui aulas"},400);const p=(await a.query("user_progress",{select:"*",filters:{user_email:i.email}})||[]).filter(v=>v.completed&&f.includes(v.lesson_id)).map(v=>v.lesson_id),h=p.length/f.length*100;if(console.log("📊 Course completion:",{total_lessons:f.length,completed_lessons:p.length,percentage:h}),h<100)return e.json({error:"Você precisa completar 100% do curso para receber o certificado",completion:h},400);const b=await a.insert("certificates",{user_email:i.email,user_name:((t=i.user_metadata)==null?void 0:t.name)||"Aluno",course_id:parseInt(o),course_title:d.title,issued_at:new Date().toISOString(),completion_date:new Date().toISOString()});return console.log("✅ Certificate generated successfully"),e.json({success:!0,certificate:b,message:"Parabéns! Seu certificado foi gerado com sucesso!"})}catch(n){return console.error("💥 Certificate generation error:",n),console.error("Error details:",n.message),(r=n.message)!=null&&r.includes("certificates")||(s=n.message)!=null&&s.includes("relation")?e.json({error:"Tabela de certificados não encontrada. Execute a migração SQL no Supabase.",details:n.message},500):e.json({error:"Erro ao gerar certificado",details:n.message},500)}});R.get("/api/certificates",async e=>{try{const t=V(e,"sb-access-token");if(!t)return e.json({error:"Não autenticado"},401);const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({error:"Usuário não encontrado"},401);const s=P(e),n=await s.query("certificates",{select:"*",filters:{user_email:r.email},order:"issued_at DESC"})||[],i=await Promise.all(n.map(async o=>{const a=await s.query("certificate_templates",{select:"*",filters:{course_id:o.course_id},single:!0});return{...o,template_url:(a==null?void 0:a.template_url)||null}}));return e.json({certificates:i})}catch(t){return console.error("💥 Certificates fetch error:",t),e.json({error:"Erro ao buscar certificados"},500)}});R.get("/api/certificates/:id",async e=>{try{const t=e.req.param("id"),r=P(e),s=await r.query("certificates",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Certificado não encontrado"},404);const n=await r.query("certificate_templates",{select:"*",filters:{course_id:s.course_id},single:!0});return e.json({certificate:{...s,template_url:(n==null?void 0:n.template_url)||null}})}catch{return e.json({error:"Erro ao buscar certificado"},500)}});R.get("/api/plans",async e=>{try{const r=await P(e).query("plans",{select:"*",filters:{is_active:!0},order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.get("/api/subscriptions/current",async e=>{try{const t=V(e,"sb-access-token");if(!t)return e.json({subscription:null});const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({subscription:null});const i=await P(e).sql(`
      ...
      LIMIT 1
    `,[r.email]);return e.json({subscription:i&&i.length>0?i[0]:null})}catch(t){return console.error("Error fetching subscription:",t),e.json({subscription:null})}});R.post("/api/admin/subscriptions",q,async e=>{try{const{user_email:t,plan_id:r,duration_days:s}=await e.req.json();if(!t||!r)return e.json({error:"Email e plano são obrigatórios"},400);const n=P(e),i=await n.query("plans",{select:"*",filters:{id:r},single:!0});if(!i)return e.json({error:"Plano não encontrado"},404);const o=new Date;o.setDate(o.getDate()+(s||i.duration_days));const a=o.toISOString(),c=await n.insert("member_subscriptions",{email_membro:t,data_expiracao:a,detalhe:i.name,origem:"admin",teste_gratis:i.is_free_trial||!1,ativo:!0}),d=await n.query("users",{select:"id",filters:{email:t},single:!0});return d&&await n.update("users",{id:d.id},{dt_expiracao:a,updated_at:new Date().toISOString()}),e.json({success:!0,message:"Assinatura criada com sucesso!",subscription:c[0]})}catch(t){return console.error("Error creating subscription:",t),e.json({error:"Erro ao criar assinatura"},500)}});R.get("/api/lessons/:id/access",async e=>{var t;try{const r=e.req.param("id"),s=V(e,"sb-access-token");if(!s){const d=await P(e).query("lessons",{select:"teste_gratis",filters:{id:r},single:!0});return e.json({hasAccess:(d==null?void 0:d.teste_gratis)||!1,reason:d!=null&&d.teste_gratis?"free_lesson":"not_authenticated"})}const n=await G(s,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!n)return e.json({hasAccess:!1,reason:"invalid_token"});let a=!!((t=(await P(e).sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[n.email,parseInt(r)]))[0])!=null&&t.has_access);if(!a){const c=e.env.DATABASE_SUITEPLUS;if(c){const d=await _t(n.email,c);d&&d>new Date&&(a=!0)}}return e.json({hasAccess:a,reason:a?"active_subscription":"no_active_subscription"})}catch(r){return console.error("Error checking lesson access:",r),e.json({hasAccess:!1,reason:"error"},500)}});R.post("/api/admin/subscriptions/expire",q,async e=>{try{return await P(e).rpc("expire_subscriptions",{}),e.json({success:!0,message:"Assinaturas expiradas com sucesso!"})}catch(t){return console.error("Error expiring subscriptions:",t),e.json({error:"Erro ao expirar assinaturas"},500)}});R.get("/api/admin/plans",q,async e=>{try{const r=await P(e).query("plans",{select:"*",order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.post("/api/admin/plans",q,async e=>{try{const t=await e.req.json(),{id:r,name:s,description:n,price:i,duration_days:o,is_active:a,is_free_trial:c,features:d,display_order:l}=t,f=P(e);if(r)return await f.update("plans",{id:r},{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:a,is_free_trial:c,features:d||[],display_order:parseInt(l||0),updated_at:new Date().toISOString()}),e.json({success:!0,message:"Plano atualizado!"});{const u=await f.insert("plans",{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:a,is_free_trial:c,features:d||[],display_order:parseInt(l||0)});return e.json({success:!0,plan:u[0],message:"Plano criado!"})}}catch(t){return console.error("Error saving plan:",t),e.json({error:"Erro ao salvar plano"},500)}});R.get("/api/admin/subscriptions",q,async e=>{try{const t=P(e),r=await t.query("subscriptions",{select:"*",order:"created_at DESC"})||[],s=await Promise.all(r.map(async n=>{const i=await t.query("plans",{select:"*",filters:{id:n.plan_id},single:!0});return{...n,plan_name:(i==null?void 0:i.name)||"Desconhecido"}}));return e.json({subscriptions:s})}catch{return e.json({error:"Erro ao buscar assinaturas"},500)}});R.get("/recover",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
...
<p>Abrindo aula...</p>
...
</html>`)});R.get("/api/favorites",K,async e=>{const t=e.get("user"),r=P(e);try{const s=await r.sql(`
    ...
    `,[t.email]);return e.json(s)}finally{await r.end()}});R.post("/api/favorites",K,async e=>{const t=e.get("user"),r=await e.req.json();if(!r.lesson_id)return e.json({error:"lesson_id required"},400);const s=P(e);try{return await s.sql("INSERT INTO user_favorites (user_email, lesson_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",[t.email,r.lesson_id]),e.json({ok:!0})}finally{await s.end()}});R.delete("/api/favorites/:lessonId",K,async e=>{const t=e.get("user"),r=parseInt(e.req.param("lessonId")),s=P(e);try{return await s.sql("DELETE FROM user_favorites WHERE user_email = $1 AND lesson_id = $2",[t.email,r]),e.json({ok:!0})}finally{await s.end()}});R.get("/api/favorites/check/:lessonId",K,async e=>{const t=e.get("user"),r=parseInt(e.req.param("lessonId")),s=P(e);try{const n=await s.sql("SELECT id FROM user_favorites WHERE user_email = $1 AND lesson_id = $2 LIMIT 1",[t.email,r]);return e.json({favorite:n.length>0})}finally{await s.end()}});R.get("/favorites",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-50">
  ...
  `));const Ds=new ei,Mo=Object.assign({"/src/index.tsx":R});let oi=!1;for(const[,e]of Object.entries(Mo))e&&(Ds.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ds.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),oi=!0);if(!oi)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var ar={exports:{}},lr={exports:{}},Pe={},cr={},js;function ai(){if(js)return cr;js=1,cr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return cr}var ur,ks;function li(){if(ks)return ur;ks=1;var e=ai();return ur={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},ur}var dr,Bs;function Uo(){if(Bs)return dr;Bs=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;dr=function(d){if(s.test(d))return Number(d.replace("i","I"));var l=e.exec(d);if(!l)return n(d)||null;var f=!!l[8],u=parseInt(l[1],10);f&&(u=o(u));var p=parseInt(l[2],10)-1,h=l[3],b=parseInt(l[4],10),v=parseInt(l[5],10),m=parseInt(l[6],10),y=l[7];y=y?1e3*parseFloat(y):0;var w,S=i(d);return S!=null?(w=new Date(Date.UTC(u,p,h,b,v,m,y)),a(u)&&w.setUTCFullYear(u),S!==0&&w.setTime(w.getTime()-S)):(w=new Date(u,p,h,b,v,m,y),a(u)&&w.setFullYear(u)),w};function n(c){var d=t.exec(c);if(d){var l=parseInt(d[1],10),f=!!d[4];f&&(l=o(l));var u=parseInt(d[2],10)-1,p=d[3],h=new Date(l,u,p);return a(l)&&h.setFullYear(l),h}}function i(c){if(c.endsWith("+00"))return 0;var d=r.exec(c.split(" ")[1]);if(d){var l=d[1];if(l==="Z")return 0;var f=l==="-"?-1:1,u=parseInt(d[2],10)*3600+parseInt(d[3]||0,10)*60+parseInt(d[4]||0,10);return u*f*1e3}}function o(c){return-(c-1)}function a(c){return c>=0&&c<100}return dr}var fr,Ms;function Fo(){if(Ms)return fr;Ms=1,fr=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return fr}var pr,Us;function Ho(){if(Us)return pr;Us=1;var e=Fo();pr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,b(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(m){var y=this[m]||0;return m==="seconds"&&this.milliseconds&&(y=(y+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),y+" "+m},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(y,this).join(""),m=i.map(y,this).join("");return"P"+v+"T"+m;function y(w){var S=this[w]||0;return w==="seconds"&&this.milliseconds&&(S=(S+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),S+s[w]}};var o="([+-]?\\d+)",a=o+"\\s+years?",c=o+"\\s+mons?",d=o+"\\s+days?",l="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([a,c,d,l].map(function(v){return"("+v+")?"}).join("\\s*")),u={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function h(v){var m=v+"000000".slice(v.length);return parseInt(m,10)/1e3}function b(v){if(!v)return{};var m=f.exec(v),y=m[8]==="-";return Object.keys(u).reduce(function(w,S){var I=u[S],A=m[I];return!A||(A=S==="milliseconds"?h(A):parseInt(A,10),!A)||(y&&~p.indexOf(S)&&(A*=-1),w[S]=A),w},{})}return pr}var mr,Fs;function $o(){if(Fs)return mr;Fs=1;var e=Buffer.from||Buffer;return mr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},mr}var hr,Hs;function zo(){if(Hs)return hr;Hs=1;var e=ai(),t=li(),r=Uo(),s=Ho(),n=$o();function i(g){return function(E){return E===null?E:g(E)}}function o(g){return g===null?g:g==="TRUE"||g==="t"||g==="true"||g==="y"||g==="yes"||g==="on"||g==="1"}function a(g){return g?e.parse(g,o):null}function c(g){return parseInt(g,10)}function d(g){return g?e.parse(g,i(c)):null}function l(g){return g?e.parse(g,i(function(_){return y(_).trim()})):null}var f=function(g){if(!g)return null;var _=t.create(g,function(E){return E!==null&&(E=S(E)),E});return _.parse()},u=function(g){if(!g)return null;var _=t.create(g,function(E){return E!==null&&(E=parseFloat(E)),E});return _.parse()},p=function(g){if(!g)return null;var _=t.create(g);return _.parse()},h=function(g){if(!g)return null;var _=t.create(g,function(E){return E!==null&&(E=r(E)),E});return _.parse()},b=function(g){if(!g)return null;var _=t.create(g,function(E){return E!==null&&(E=s(E)),E});return _.parse()},v=function(g){return g?e.parse(g,i(n)):null},m=function(g){return parseInt(g,10)},y=function(g){var _=String(g);return/^\d+$/.test(_)?_:g},w=function(g){return g?e.parse(g,i(JSON.parse)):null},S=function(g){return g[0]!=="("?null:(g=g.substring(1,g.length-1).split(","),{x:parseFloat(g[0]),y:parseFloat(g[1])})},I=function(g){if(g[0]!=="<"&&g[1]!=="(")return null;for(var _="(",E="",T=!1,C=2;C<g.length-1;C++){if(T||(_+=g[C]),g[C]===")"){T=!0;continue}else if(!T)continue;g[C]!==","&&(E+=g[C])}var O=S(_);return O.radius=parseFloat(E),O},A=function(g){g(20,y),g(21,m),g(23,m),g(26,m),g(700,parseFloat),g(701,parseFloat),g(16,o),g(1082,r),g(1114,r),g(1184,r),g(600,S),g(651,p),g(718,I),g(1e3,a),g(1001,v),g(1005,d),g(1007,d),g(1028,d),g(1016,l),g(1017,f),g(1021,u),g(1022,u),g(1231,u),g(1014,p),g(1015,p),g(1008,p),g(1009,p),g(1040,p),g(1041,p),g(1115,h),g(1182,h),g(1185,h),g(1186,s),g(1187,b),g(17,n),g(114,JSON.parse.bind(JSON)),g(3802,JSON.parse.bind(JSON)),g(199,w),g(3807,w),g(3907,p),g(2951,p),g(791,p),g(1183,p),g(1270,p)};return hr={init:A},hr}var gr,$s;function Wo(){if($s)return gr;$s=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",a,c,d,l,f,u;{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,d=""+(c-e*n),n===0&&s===0)return i+d+o;for(l="",f=6-d.length,u=0;u<f;u++)l+="0";o=l+d+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,d=""+(c-e*n),n===0&&s===0)return i+d+o;for(l="",f=6-d.length,u=0;u<f;u++)l+="0";o=l+d+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,d=""+(c-e*n),n===0&&s===0)return i+d+o;for(l="",f=6-d.length,u=0;u<f;u++)l+="0";o=l+d+o}return a=s%e,c=4294967296*a+n,d=""+c%e,i+d+o}return gr=t,gr}var br,zs;function Vo(){if(zs)return br;zs=1;var e=Wo(),t=function(p,h,b,v,m){b=b||0,v=v||!1,m=m||function(T,C,O){return T*Math.pow(2,O)+C};var y=b>>3,w=function(T){return v?~T&255:T},S=255,I=8-b%8;h<I&&(S=255<<8-h&255,I=h),b&&(S=S>>b%8);var A=0;b%8+h>=8&&(A=m(0,w(p[y])&S,I));for(var g=h+b>>3,_=y+1;_<g;_++)A=m(A,w(p[_]),8);var E=(h+b)%8;return E>0&&(A=m(A,w(p[g])>>8-E,E)),A},r=function(p,h,b){var v=Math.pow(2,b-1)-1,m=t(p,1),y=t(p,b,1);if(y===0)return 0;var w=1,S=function(A,g,_){A===0&&(A=1);for(var E=1;E<=_;E++)w/=2,(g&1<<_-E)>0&&(A+=w);return A},I=t(p,h,b+1,!1,S);return y==Math.pow(2,b+1)-1?I===0?m===0?1/0:-1/0:NaN:(m===0?1:-1)*Math.pow(2,y-v)*I},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},a=function(p){var h=t(p,16,32);if(h==49152)return NaN;for(var b=Math.pow(1e4,t(p,16,16)),v=0,m=t(p,16),y=0;y<m;y++)v+=t(p,16,64+16*y)*b,b/=1e4;var w=Math.pow(10,t(p,16,48));return(h===0?1:-1)*Math.round(v*w)/w},c=function(p,h){var b=t(h,1),v=t(h,63,1),m=new Date((b===0?1:-1)*v/1e3+9466848e5);return p||m.setTime(m.getTime()+m.getTimezoneOffset()*6e4),m.usec=v%1e3,m.getMicroSeconds=function(){return this.usec},m.setMicroSeconds=function(y){this.usec=y},m.getUTCMicroSeconds=function(){return this.usec},m},d=function(p){var h=t(p,32);t(p,32,32);for(var b=t(p,32,64),v=96,m=[],y=0;y<h;y++)m[y]=t(p,32,v),v+=32,v+=32;var w=function(I){var A=t(p,32,v);if(v+=32,A==4294967295)return null;var g;if(I==23||I==20)return g=t(p,A*8,v),v+=A*8,g;if(I==25)return g=p.toString(this.encoding,v>>3,(v+=A<<3)>>3),g;console.log("ERROR: ElementType not implemented: "+I)},S=function(I,A){var g=[],_;if(I.length>1){var E=I.shift();for(_=0;_<E;_++)g[_]=S(I,A);I.unshift(E)}else for(_=0;_<I[0];_++)g[_]=w(A);return g};return S(m,b)},l=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},u=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,a),p(700,i),p(701,o),p(16,f),p(1114,c.bind(null,!1)),p(1184,c.bind(null,!0)),p(1e3,d),p(1007,d),p(1016,d),p(1008,d),p(1009,d),p(25,l)};return br={init:u},br}var yr,Ws;function Qo(){return Ws||(Ws=1,yr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),yr}var Vs;function xt(){if(Vs)return Pe;Vs=1;var e=zo(),t=Vo(),r=li(),s=Qo();Pe.getTypeParser=o,Pe.setTypeParser=a,Pe.arrayParser=r,Pe.builtins=s;var n={text:{},binary:{}};function i(c){return String(c)}function o(c,d){return d=d||"text",n[d]&&n[d][c]||i}function a(c,d,l){typeof d=="function"&&(l=d,d="text"),n[d][c]=l}return e.init(function(c,d){n.text[c]=d}),t.init(function(c,d){n.binary[c]=d}),Pe}var Qs;function St(){return Qs||(Qs=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=xt(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(lr)),lr.exports}var vr,Xs;function tt(){if(Xs)return vr;Xs=1;const e=St(),t=He,{isDate:r}=t.types||t;function s(u){return'"'+u.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(u){let p="{";for(let h=0;h<u.length;h++)if(h>0&&(p=p+","),u[h]===null||typeof u[h]>"u")p=p+"NULL";else if(Array.isArray(u[h]))p=p+n(u[h]);else if(ArrayBuffer.isView(u[h])){let b=u[h];if(!(b instanceof Buffer)){const v=Buffer.from(b.buffer,b.byteOffset,b.byteLength);v.length===b.byteLength?b=v:b=v.slice(b.byteOffset,b.byteOffset+b.byteLength)}p+="\\\\x"+b.toString("hex")}else p+=s(i(u[h]));return p=p+"}",p}const i=function(u,p){if(u==null)return null;if(typeof u=="object"){if(u instanceof Buffer)return u;if(ArrayBuffer.isView(u)){const h=Buffer.from(u.buffer,u.byteOffset,u.byteLength);return h.length===u.byteLength?h:h.slice(u.byteOffset,u.byteOffset+u.byteLength)}return r(u)?e.parseInputDatesAsUTC?c(u):a(u):Array.isArray(u)?n(u):o(u,p)}return u.toString()};function o(u,p){if(u&&typeof u.toPostgres=="function"){if(p=p||[],p.indexOf(u)!==-1)throw new Error('circular reference detected while preparing "'+u+'" for query');return p.push(u),i(u.toPostgres(i),p)}return JSON.stringify(u)}function a(u){let p=-u.getTimezoneOffset(),h=u.getFullYear();const b=h<1;b&&(h=Math.abs(h)+1);let v=String(h).padStart(4,"0")+"-"+String(u.getMonth()+1).padStart(2,"0")+"-"+String(u.getDate()).padStart(2,"0")+"T"+String(u.getHours()).padStart(2,"0")+":"+String(u.getMinutes()).padStart(2,"0")+":"+String(u.getSeconds()).padStart(2,"0")+"."+String(u.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),b&&(v+=" BC"),v}function c(u){let p=u.getUTCFullYear();const h=p<1;h&&(p=Math.abs(p)+1);let b=String(p).padStart(4,"0")+"-"+String(u.getUTCMonth()+1).padStart(2,"0")+"-"+String(u.getUTCDate()).padStart(2,"0")+"T"+String(u.getUTCHours()).padStart(2,"0")+":"+String(u.getUTCMinutes()).padStart(2,"0")+":"+String(u.getUTCSeconds()).padStart(2,"0")+"."+String(u.getUTCMilliseconds()).padStart(3,"0");return b+="+00:00",h&&(b+=" BC"),b}function d(u,p,h){return u=typeof u=="string"?{text:u}:u,p&&(typeof p=="function"?u.callback=p:u.values=p),h&&(u.callback=h),u}return vr={prepareValue:function(p){return i(p)},normalizeQueryConfig:d,escapeIdentifier:function(u){return'"'+u.replace(/"/g,'""')+'"'},escapeLiteral:function(u){let p=!1,h="'";if(u==null||typeof u!="string")return"''";for(let b=0;b<u.length;b++){const v=u[b];v==="'"?h+=v+v:v==="\\"?(h+=v+v,p=!0):h+=v}return h+="'",p===!0&&(h=" E"+h),h}},vr}var dt={exports:{}},wr,Ys;function Xo(){if(Ys)return wr;Ys=1;const e=qn;function t(a){return e.createHash("md5").update(a,"utf-8").digest("hex")}function r(a,c,d){const l=t(c+a);return"md5"+t(Buffer.concat([Buffer.from(l),d]))}function s(a){return e.createHash("sha256").update(a).digest()}function n(a,c){return a=a.replace(/(\D)-/,"$1"),e.createHash(a).update(c).digest()}function i(a,c){return e.createHmac("sha256",a).update(c).digest()}async function o(a,c,d){return e.pbkdf2Sync(a,c,d,32,"sha256")}return wr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},wr}var Er,Js;function Yo(){if(Js)return Er;Js=1;const e=qn;Er={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:l,sha256:a,hashByName:c,hmacSha256:d,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,h=await r.digest("MD5",p);return Array.from(new Uint8Array(h)).map(b=>b.toString(16).padStart(2,"0")).join("")}}async function o(f,u,p){const h=await i(u+f);return"md5"+await i(Buffer.concat([Buffer.from(h),p]))}async function a(f){return await r.digest("SHA-256",f)}async function c(f,u){return await r.digest(f,u)}async function d(f,u){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(u))}async function l(f,u,p){const h=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),b={name:"PBKDF2",hash:"SHA-256",salt:u,iterations:p};return await r.deriveBits(b,h,256,["deriveBits"])}return Er}var Gs;function ci(){return Gs||(Gs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?dt.exports=Xo():dt.exports=Yo()),dt.exports}var _r,Ks;function Jo(){if(Ks)return _r;Ks=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let a=i[o++];if(a<128)return{length:a,index:o};const c=a&127;if(c>4)throw e("bad length",i);a=0;for(let d=0;d<c;d++)a=a<<8|i[o++];return{length:a,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:a,index:c}=t(i,o);o=c;const d=o+a,l=i[o++];let f=(l/40>>0)+"."+l%40;for(;o<d;){let u=0;for(;o<d;){const p=i[o++];if(u=u<<7|p&127,p<128)break}f+="."+u}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:a,index:c}=s(i,o);o=c+a,o=s(i,o).index;const{oid:d,index:l}=r(i,o);switch(d){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=l,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+d,i)}return _r={signatureAlgorithmHashFromCertificate:n},_r}var xr,Zs;function Go(){if(Zs)return xr;Zs=1;const e=ci(),{signatureAlgorithmHashFromCertificate:t}=Jo();function r(f,u){const p=["SCRAM-SHA-256"];u&&p.unshift("SCRAM-SHA-256-PLUS");const h=p.find(m=>f.includes(m));if(!h)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(h==="SCRAM-SHA-256-PLUS"&&typeof u.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const b=e.randomBytes(18).toString("base64");return{mechanism:h,clientNonce:b,response:(h==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":u?"y":"n")+",,n=*,r="+b,message:"SASLInitialResponse"}}async function s(f,u,p,h){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof u!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(u==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const b=c(p);if(b.nonce.startsWith(f.clientNonce)){if(b.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,m="r="+b.nonce+",s="+b.salt+",i="+b.iteration;let y=h?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const x=h.getPeerCertificate().raw;let L=t(x);(L==="MD5"||L==="SHA-1")&&(L="SHA-256");const D=await e.hashByName(L,x);y=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(D)]).toString("base64")}const w="c="+y+",r="+b.nonce,S=v+","+m+","+w,I=Buffer.from(b.salt,"base64"),A=await e.deriveKey(u,I,b.iteration),g=await e.hmacSha256(A,"Client Key"),_=await e.sha256(g),E=await e.hmacSha256(_,S),T=l(Buffer.from(g),Buffer.from(E)).toString("base64"),C=await e.hmacSha256(A,"Server Key"),O=await e.hmacSha256(C,S);f.message="SASLResponse",f.serverSignature=Buffer.from(O).toString("base64"),f.response=w+",p="+T}function n(f,u){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof u!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=d(u);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((u,p)=>f.charCodeAt(p)).every(u=>u>=33&&u<=43||u>=45&&u<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function a(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(u=>{if(!/^.=/.test(u))throw new Error("SASL: Invalid attribute pair entry");const p=u[0],h=u.substring(2);return[p,h]}))}function c(f){const u=a(f),p=u.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const h=u.get("s");if(h){if(!o(h))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const b=u.get("i");if(b){if(!/^[1-9][0-9]*$/.test(b))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(b,10);return{nonce:p,salt:h,iteration:v}}function d(f){const p=a(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function l(f,u){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(u))throw new TypeError("second argument must be a Buffer");if(f.length!==u.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,h)=>f[h]^u[h]))}return xr={startSession:r,continueSession:s,finalizeSession:n},xr}var Sr,en;function Gr(){if(en)return Sr;en=1;const e=xt();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},Sr=t,Sr}var Ar,tn;function Ko(){if(tn)return Ar;tn=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const a={};let c,d=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(i,"postgres://base")}catch{c=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),d=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of c.searchParams.entries())a[p[0]]=p[1];if(a.user=a.user||decodeURIComponent(c.username),a.password=a.password||decodeURIComponent(c.password),c.protocol=="socket:")return a.host=decodeURI(c.pathname),a.database=c.searchParams.get("db"),a.client_encoding=c.searchParams.get("encoding"),a;const l=d?"":c.hostname;a.host?l&&/^%2f/i.test(l)&&(c.pathname=l+c.pathname):a.host=decodeURIComponent(l),a.port||(a.port=c.port);const f=c.pathname.slice(1)||null;a.database=f?decodeURI(f):null,(a.ssl==="true"||a.ssl==="1")&&(a.ssl=!0),a.ssl==="0"&&(a.ssl=!1),(a.sslcert||a.sslkey||a.sslrootcert||a.sslmode)&&(a.ssl={});const u=a.sslcert||a.sslkey||a.sslrootcert?Dn:null;if(a.sslcert&&(a.ssl.cert=u.readFileSync(a.sslcert).toString()),a.sslkey&&(a.ssl.key=u.readFileSync(a.sslkey).toString()),a.sslrootcert&&(a.ssl.ca=u.readFileSync(a.sslrootcert).toString()),o.useLibpqCompat&&a.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(a.uselibpqcompat==="true"||o.useLibpqCompat)switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":{a.ssl.rejectUnauthorized=!1;break}case"require":{a.sslrootcert?a.ssl.checkServerIdentity=function(){}:a.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!a.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");a.ssl.checkServerIdentity=function(){};break}}else switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{a.sslmode!=="verify-full"&&n(a.sslmode);break}case"no-verify":{a.ssl.rejectUnauthorized=!1;break}}return a}function t(i){return Object.entries(i).reduce((a,[c,d])=>(d!=null&&(a[c]=d),a),{})}function r(i){return Object.entries(i).reduce((a,[c,d])=>{if(c==="ssl"){const l=d;typeof l=="boolean"&&(a[c]=l),typeof l=="object"&&(a[c]=t(l))}else if(d!=null)if(c==="port"){if(d!==""){const l=parseInt(d,10);if(isNaN(l))throw new Error(`Invalid ${c}: ${d}`);a[c]=l}}else a[c]=d;return a},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
...
See https://www.postgresql.org/docs/current/libpq-ssl.html for libpq SSL mode definitions.`))}return Ar=e,e.parse=e,e.toClientConfig=r,e.parseIntoClientConfig=s,Ar}var Tr,rn;function ui(){if(rn)return Tr;rn=1;var e={};const t=vi,r=St(),s=Ko().parse,n=function(d,l,f){return l[d]?l[d]:(f===void 0?f=e["PG"+d.toUpperCase()]:f===!1||(f=e[f]),f||r[d])},i=function(){switch(e.PGSSLMODE){case"disable":return!1;case"prefer":case"require":case"verify-ca":case"verify-full":return!0;case"no-verify":return{rejectUnauthorized:!1}}return r.ssl},o=function(d){return"'"+(""+d).replace(/\\/g,"\\\\").replace(/'/g,"\\'")+"'"},a=function(d,l,f){const u=l[f];u!=null&&d.push(f+"="+o(u))};class c{constructor(l){l=typeof l=="string"?s(l):l||{},l.connectionString&&(l=Object.assign({},l,s(l.connectionString))),this.user=n("user",l),this.database=n("database",l),this.database===void 0&&(this.database=this.user),this.port=parseInt(n("port",l),10),this.host=n("host",l),Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:n("password",l)}),this.binary=n("binary",l),this.options=n("options",l),this.ssl=typeof l.ssl>"u"?i():l.ssl,typeof this.ssl=="string"&&this.ssl==="true"&&(this.ssl=!0),this.ssl==="no-verify"&&(this.ssl={rejectUnauthorized:!1}),this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this.client_encoding=n("client_encoding",l),this.replication=n("replication",l),this.isDomainSocket=!(this.host||"").indexOf("/"),this.application_name=n("application_name",l,"PGAPPNAME"),this.fallback_application_name=n("fallback_application_name",l,!1),this.statement_timeout=n("statement_timeout",l,!1),this.lock_timeout=n("lock_timeout",l,!1),this.idle_in_transaction_session_timeout=n("idle_in_transaction_session_timeout",l,!1),this.query_timeout=n("query_timeout",l,!1),l.connectionTimeoutMillis===void 0?this.connect_timeout=e.PGCONNECT_TIMEOUT||0:this.connect_timeout=Math.floor(l.connectionTimeoutMillis/1e3),l.keepAlive===!1?this.keepalives=0:l.keepAlive===!0&&(this.keepalives=1),typeof l.keepAliveInitialDelayMillis=="number"&&(this.keepalives_idle=Math.floor(l.keepAliveInitialDelayMillis/1e3))}getLibpqConnectionString(l){const f=[];a(f,this,"user"),a(f,this,"password"),a(f,this,"port"),a(f,this,"application_name"),a(f,this,"fallback_application_name"),a(f,this,"connect_timeout"),a(f,this,"options");const u=typeof this.ssl=="object"?this.ssl:this.ssl?{sslmode:this.ssl}:{};if(a(f,u,"sslmode"),a(f,u,"sslca"),a(f,u,"sslkey"),a(f,u,"sslcert"),a(f,u,"sslrootcert"),this.database&&f.push("dbname="+o(this.database)),this.replication&&f.push("replication="+o(this.replication)),this.host&&f.push("host="+o(this.host)),this.isDomainSocket)return l(null,f.join(" "));this.client_encoding&&f.push("client_encoding="+o(this.client_encoding)),t.lookup(this.host,function(p,h){return p?l(p,null):(f.push("hostaddr="+o(h)),l(null,f.join(" ")))})}}return Tr=c,Tr}var Cr,sn;function di(){if(sn)return Cr;sn=1;const e=xt(),t=/^([A-Za-z]+)(?: (\d+))?(?: (\d+))?/;class r{constructor(n,i){this.command=null,this.rowCount=null,this.oid=null,this.rows=[],this.fields=[],this._parsers=void 0,this._types=i,this.RowCtor=null,this.rowAsArray=n==="array",this.rowAsArray&&(this.parseRow=this._parseRowAsArray),this._prebuiltEmptyResultObject=null}addCommandComplete(n){let i;n.text?i=t.exec(n.text):i=t.exec(n.command),i&&(this.command=i[1],i[3]?(this.oid=parseInt(i[2],10),this.rowCount=parseInt(i[3],10)):i[2]&&(this.rowCount=parseInt(i[2],10)))}_parseRowAsArray(n){const i=new Array(n.length);for(let o=0,a=n.length;o<a;o++){const c=n[o];c!==null?i[o]=this._parsers[o](c):i[o]=null}return i}parseRow(n){const i={...this._prebuiltEmptyResultObject};for(let o=0,a=n.length;o<a;o++){const c=n[o],d=this.fields[o].name;if(c!==null){const l=this.fields[o].format==="binary"?Buffer.from(c):c;i[d]=this._parsers[o](l)}else i[d]=null}return i}addRow(n){this.rows.push(n)}addFields(n){this.fields=n,this.fields.length&&(this._parsers=new Array(n.length));const i={};for(let o=0;o<n.length;o++){const a=n[o];i[a.name]=null,this._types?this._parsers[o]=this._types.getTypeParser(a.dataTypeID,a.format||"text"):this._parsers[o]=e.getTypeParser(a.dataTypeID,a.format||"text")}this._prebuiltEmptyResultObject={...i}}}return Cr=r,Cr}var Rr,nn;function Zo(){if(nn)return Rr;nn=1;const{EventEmitter:e}=ze,t=di(),r=tt();class s extends e{constructor(i,o,a){super(),i=r.normalizeQueryConfig(i,o,a),this.text=i.text,this.values=i.values,this.rows=i.rows,this.types=i.types,this.name=i.name,this.queryMode=i.queryMode,this.binary=i.binary,this.portal=i.portal||"",this.callback=i.callback,this._rowMode=i.rowMode,process.domain&&i.callback&&(this.callback=process.domain.bind(i.callback)),this._result=new t(this._rowMode,this.types),this._results=this._result,this._canceledDueToError=!1}requiresPreparation(){return this.queryMode==="extended"||this.name||this.rows?!0:!this.text||!this.values?!1:this.values.length>0}_checkForMultirow(){this._result.command&&(Array.isArray(this._results)||(this._results=[this._result]),this._result=new t(this._rowMode,this._result._types),this._results.push(this._result))}handleRowDescription(i){this._checkForMultirow(),this._result.addFields(i.fields),this._accumulateRows=this.callback||!this.listeners("row").length}handleDataRow(i){let o;if(!this._canceledDueToError){try{o=this._result.parseRow(i.fields)}catch(a){this._canceledDueToError=a;return}this.emit("row",o,this._result),this._accumulateRows&&this._result.addRow(o)}}handleCommandComplete(i,o){this._checkForMultirow(),this._result.addCommandComplete(i),this.rows&&o.sync()}handleEmptyQuery(i){this.rows&&i.sync()}handleError(i,o){if(this._canceledDueToError&&(i=this._canceledDueToError,this._canceledDueToError=!1),this.callback)return this.callback(i);this.emit("error",i)}handleReadyForQuery(i){if(this._canceledDueToError)return this.handleError(this._canceledDueToError,i);if(this.callback)try{this.callback(null,this._results)}catch(o){process.nextTick(()=>{throw o})}this.emit("end",this._results)}submit(i){if(typeof this.text!="string"&&typeof this.name!="string")return new Error("A query must have either text or a name. Supplying neither is unsupported.");const o=i.parsedStatements[this.name];if(this.text&&o&&this.text!==o)return new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);if(this.values&&!Array.isArray(this.values))return new Error("Query values must be an array");if(this.requiresPreparation()){i.stream.cork&&i.stream.cork();try{this.prepare(i)}finally{i.stream.uncork&&i.stream.uncork()}}else i.query(this.text);return null}hasBeenParsed(i){return this.name&&i.parsedStatements[this.name]}handlePortalSuspended(i){this._getRows(i,this.rows)}_getRows(i,o){i.execute({portal:this.portal,rows:o}),o?i.flush():i.sync()}prepare(i){this.hasBeenParsed(i)||i.parse({text:this.text,name:this.name,types:this.types});try{i.bind({portal:this.portal,statement:this.name,values:this.values,binary:this.binary,valueMapper:r.prepareValue})}catch(o){this.handleError(o,i);return}i.describe({type:"P",name:this.portal||""}),this._getRows(i,this.rows)}handleCopyInResponse(i){i.sendCopyFail("No source stream defined")}handleCopyData(i,o){}}return Rr=s,Rr}var Nr={},B={},on;function fi(){if(on)return B;on=1,Object.defineProperty(B,"__esModule",{value:!0}),B.NoticeMessage=B.DataRowMessage=B.CommandCompleteMessage=B.ReadyForQueryMessage=B.NotificationResponseMessage=B.BackendKeyDataMessage=B.AuthenticationMD5Password=B.ParameterStatusMessage=B.ParameterDescriptionMessage=B.RowDescriptionMessage=B.Field=B.CopyResponse=B.CopyDataMessage=B.DatabaseError=B.copyDone=B.emptyQuery=B.replicationStart=B.portalSuspended=B.noData=B.closeComplete=B.bindComplete=B.parseComplete=void 0,B.parseComplete={name:"parseComplete",length:5},B.bindComplete={name:"bindComplete",length:5},B.closeComplete={name:"closeComplete",length:5},B.noData={name:"noData",length:5},B.portalSuspended={name:"portalSuspended",length:5},B.replicationStart={name:"replicationStart",length:4},B.emptyQuery={name:"emptyQuery",length:4},B.copyDone={name:"copyDone",length:4};class e extends Error{constructor(b,v,m){super(b),this.length=v,this.name=m}}B.DatabaseError=e;class t{constructor(b,v){this.length=b,this.chunk=v,this.name="copyData"}}B.CopyDataMessage=t;class r{constructor(b,v,m,y){this.length=b,this.name=v,this.binary=m,this.columnTypes=new Array(y)}}B.CopyResponse=r;class s{constructor(b,v,m,y,w,S,I){this.name=b,this.tableID=v,this.columnID=m,this.dataTypeID=y,this.dataTypeSize=w,this.dataTypeModifier=S,this.format=I}}B.Field=s;class n{constructor(b,v){this.length=b,this.fieldCount=v,this.name="rowDescription",this.fields=new Array(this.fieldCount)}}B.RowDescriptionMessage=n;class i{constructor(b,v){this.length=b,this.parameterCount=v,this.name="parameterDescription",this.dataTypeIDs=new Array(this.parameterCount)}}B.ParameterDescriptionMessage=i;class o{constructor(b,v,m){this.length=b,this.parameterName=v,this.parameterValue=m,this.name="parameterStatus"}}B.ParameterStatusMessage=o;class a{constructor(b,v){this.length=b,this.salt=v,this.name="authenticationMD5Password"}}B.AuthenticationMD5Password=a;class c{constructor(b,v,m){this.length=b,this.processID=v,this.secretKey=m,this.name="backendKeyData"}}B.BackendKeyDataMessage=c;class d{constructor(b,v,m,y){this.length=b,this.processId=v,this.channel=m,this.payload=y,this.name="notification"}}B.NotificationResponseMessage=d;class l{constructor(b,v){this.length=b,this.status=v,this.name="readyForQuery"}}B.ReadyForQueryMessage=l;class f{constructor(b,v){this.length=b,this.text=v,this.name="commandComplete"}}B.CommandCompleteMessage=f;class u{constructor(b,v){this.length=b,this.fields=v,this.name="dataRow",this.fieldCount=v.length}}B.DataRowMessage=u;class p{constructor(b,v){this.length=b,this.message=v,this.name="notice"}}return B.NoticeMessage=p,B}var Xe={},Ye={},an;function ea(){if(an)return Ye;an=1,Object.defineProperty(Ye,"__esModule",{value:!0}),Ye.Writer=void 0;class e{constructor(r=256){this.size=r,this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(r)}ensure(r){if(this.buffer.length-this.offset<r){const n=this.buffer,i=n.length+(n.length>>1)+r;this.buffer=Buffer.allocUnsafe(i),n.copy(this.buffer)}}addInt32(r){return this.ensure(4),this.buffer[this.offset++]=r>>>24&255,this.buffer[this.offset++]=r>>>16&255,this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addInt16(r){return this.ensure(2),this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addCString(r){if(!r)this.ensure(1);else{const s=Buffer.byteLength(r);this.ensure(s+1),this.buffer.write(r,this.offset,"utf-8"),this.offset+=s}return this.buffer[this.offset++]=0,this}addString(r=""){const s=Buffer.byteLength(r);return this.ensure(s),this.buffer.write(r,this.offset),this.offset+=s,this}add(r){return this.ensure(r.length),r.copy(this.buffer,this.offset),this.offset+=r.length,this}join(r){if(r){this.buffer[this.headerPosition]=r;const s=this.offset-(this.headerPosition+1);this.buffer.writeInt32BE(s,this.headerPosition+1)}return this.buffer.slice(r?0:5,this.offset)}flush(r){const s=this.join(r);return this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(this.size),s}}return Ye.Writer=e,Ye}var ln;function ta(){if(ln)return Xe;ln=1,Object.defineProperty(Xe,"__esModule",{value:!0}),Xe.serialize=void 0;const e=ea(),t=new e.Writer,r=x=>{t.addInt16(3).addInt16(0);for(const M of Object.keys(x))t.addCString(M).addCString(x[M]);t.addCString("client_encoding").addCString("UTF8");const L=t.addCString("").flush(),D=L.length+4;return new e.Writer().addInt32(D).add(L).flush()},s=()=>{const x=Buffer.allocUnsafe(8);return x.writeInt32BE(8,0),x.writeInt32BE(80877103,4),x},n=x=>t.addCString(x).flush(112),i=function(x,L){return t.addCString(x).addInt32(Buffer.byteLength(L)).addString(L),t.flush(112)},o=function(x){return t.addString(x).flush(112)},a=x=>t.addCString(x).flush(81),c=[],d=x=>{const L=x.name||"";L.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",L,L.length),console.error("This can cause conflicts and silent errors executing queries"));const D=x.types||c,M=D.length,F=t.addCString(L).addCString(x.text).addInt16(M);for(let $=0;$<M;$++)F.addInt32(D[$]);return t.flush(80)},l=new e.Writer,f=function(x,L){for(let D=0;D<x.length;D++){const M=L?L(x[D],D):x[D];M==null?(t.addInt16(0),l.addInt32(-1)):M instanceof Buffer?(t.addInt16(1),l.addInt32(M.length),l.add(M)):(t.addInt16(0),l.addInt32(Buffer.byteLength(M)),l.addString(M))}},u=(x={})=>{const L=x.portal||"",D=x.statement||"",M=x.binary||!1,F=x.values||c,$=F.length;return t.addCString(L).addCString(D),t.addInt16($),f(F,x.valueMapper),t.addInt16($),t.add(l.flush()),t.addInt16(1),t.addInt16(M?1:0),t.flush(66)},p=Buffer.from([69,0,0,0,9,0,0,0,0,0]),h=x=>{if(!x||!x.portal&&!x.rows)return p;const L=x.portal||"",D=x.rows||0,M=Buffer.byteLength(L),F=4+M+1+4,$=Buffer.allocUnsafe(1+F);return $[0]=69,$.writeInt32BE(F,1),$.write(L,5,"utf-8"),$[M+5]=0,$.writeUInt32BE(D,$.length-4),$},b=(x,L)=>{const D=Buffer.allocUnsafe(16);return D.writeInt32BE(16,0),D.writeInt16BE(1234,4),D.writeInt16BE(5678,6),D.writeInt32BE(x,8),D.writeInt32BE(L,12),D},v=(x,L)=>{const M=4+Buffer.byteLength(L)+1,F=Buffer.allocUnsafe(1+M);return F[0]=x,F.writeInt32BE(M,1),F.write(L,5,"utf-8"),F[M]=0,F},m=t.addCString("P").flush(68),y=t.addCString("S").flush(68),w=x=>x.name?v(68,`${x.type}${x.name||""}`):x.type==="P"?m:y,S=x=>{const L=`${x.type}${x.name||""}`;return v(67,L)},I=x=>t.add(x).flush(100),A=x=>v(102,x),g=x=>Buffer.from([x,0,0,0,4]),_=g(72),E=g(83),T=g(88),C=g(99),O={startup:r,password:n,requestSsl:s,sendSASLInitialResponseMessage:i,sendSCRAMClientFinalMessage:o,query:a,parse:d,bind:u,execute:h,describe:w,close:S,flush:()=>_,sync:()=>E,end:()=>T,copyData:I,copyDone:()=>C,copyFail:A,cancel:b};return Xe.serialize=O,Xe}var Je={},Ge={},cn;function ra(){if(cn)return Ge;cn=1,Object.defineProperty(Ge,"__esModule",{value:!0}),Ge.BufferReader=void 0;class e{constructor(r=0){this.offset=r,this.buffer=Buffer.allocUnsafe(0),this.encoding="utf-8"}setBuffer(r,s){this.offset=r,this.buffer=s}int16(){const r=this.buffer.readInt16BE(this.offset);return this.offset+=2,r}byte(){const r=this.buffer[this.offset];return this.offset++,r}int32(){const r=this.buffer.readInt32BE(this.offset);return this.offset+=4,r}uint32(){const r=this.buffer.readUInt32BE(this.offset);return this.offset+=4,r}string(r){const s=this.buffer.toString(this.encoding,this.offset,this.offset+r);return this.offset+=r,s}cstring(){const r=this.offset;let s=r;for(;this.buffer[s++]!==0;);return this.offset=s,this.buffer.toString(this.encoding,r,s-1)}bytes(r){const s=this.buffer.slice(this.offset,this.offset+r);return this.offset+=r,s}}return Ge.BufferReader=e,Ge}var un;function sa(){if(un)return Je;un=1,Object.defineProperty(Je,"__esModule",{value:!0}),Je.Parser=void 0;const e=fi(),t=ra(),r=1,n=r+4,i=-1,o=Buffer.allocUnsafe(0);class a{constructor(_){if(this.buffer=o,this.bufferLength=0,this.bufferOffset=0,this.reader=new t.BufferReader,(_==null?void 0:_.mode)==="binary")throw new Error("Binary mode not supported yet");this.mode=(_==null?void 0:_.mode)||"text"}parse(_,E){this.mergeBuffer(_);const T=this.bufferOffset+this.bufferLength;let C=this.bufferOffset;for(;C+n<=T;){const O=this.buffer[C],x=this.buffer.readUInt32BE(C+r),L=r+x;if(L+C<=T){const D=this.handlePacket(C+n,O,x,this.buffer);E(D),C+=L}else break}C===T?(this.buffer=o,this.bufferLength=0,this.bufferOffset=0):(this.bufferLength=T-C,this.bufferOffset=C)}mergeBuffer(_){if(this.bufferLength>0){const E=this.bufferLength+_.byteLength;if(E+this.bufferOffset>this.buffer.byteLength){let C;if(E<=this.buffer.byteLength&&this.bufferOffset>=this.bufferLength)C=this.buffer;else{let O=this.buffer.byteLength*2;for(;E>=O;)O*=2;C=Buffer.allocUnsafe(O)}this.buffer.copy(C,0,this.bufferOffset,this.bufferOffset+this.bufferLength),this.buffer=C,this.bufferOffset=0}_.copy(this.buffer,this.bufferOffset+this.bufferLength),this.bufferLength=E}else this.buffer=_,this.bufferOffset=0,this.bufferLength=_.byteLength}handlePacket(_,E,T,C){const{reader:O}=this;O.setBuffer(_,C);let x;switch(E){case 50:x=e.bindComplete;break;case 49:x=e.parseComplete;break;case 51:x=e.closeComplete;break;case 110:x=e.noData;break;case 115:x=e.portalSuspended;break;case 99:x=e.copyDone;break;case 87:x=e.replicationStart;break;case 73:x=e.emptyQuery;break;case 68:x=y(O);break;case 67:x=d(O);break;case 90:x=c(O);break;case 65:x=h(O);break;case 82:x=I(O,T);break;case 83:x=w(O);break;case 75:x=S(O);break;case 69:x=A(O,"error");break;case 78:x=A(O,"notice");break;case 84:x=b(O);break;case 116:x=m(O);break;case 71:x=f(O);break;case 72:x=u(O);break;case 100:x=l(O,T);break;default:return new e.DatabaseError("received invalid response: "+E.toString(16),T,"error")}return O.setBuffer(0,o),x.length=T,x}}Je.Parser=a;const c=g=>{const _=g.string(1);return new e.ReadyForQueryMessage(i,_)},d=g=>{const _=g.cstring();return new e.CommandCompleteMessage(i,_)},l=(g,_)=>{const E=g.bytes(_-4);return new e.CopyDataMessage(i,E)},f=g=>p(g,"copyInResponse"),u=g=>p(g,"copyOutResponse"),p=(g,_)=>{const E=g.byte()!==0,T=g.int16(),C=new e.CopyResponse(i,_,E,T);for(let O=0;O<T;O++)C.columnTypes[O]=g.int16();return C},h=g=>{const _=g.int32(),E=g.cstring(),T=g.cstring();return new e.NotificationResponseMessage(i,_,E,T)},b=g=>{const _=g.int16(),E=new e.RowDescriptionMessage(i,_);for(let T=0;T<_;T++)E.fields[T]=v(g);return E},v=g=>{const _=g.cstring(),E=g.uint32(),T=g.int16(),C=g.uint32(),O=g.int16(),x=g.int32(),L=g.int16()===0?"text":"binary";return new e.Field(_,E,T,C,O,x,L)},m=g=>{const _=g.int16(),E=new e.ParameterDescriptionMessage(i,_);for(let T=0;T<_;T++)E.dataTypeIDs[T]=g.int32();return E},y=g=>{const _=g.int16(),E=new Array(_);for(let T=0;T<_;T++){const C=g.int32();E[T]=C===-1?null:g.string(C)}return new e.DataRowMessage(i,E)},w=g=>{const _=g.cstring(),E=g.cstring();return new e.ParameterStatusMessage(i,_,E)},S=g=>{const _=g.int32(),E=g.int32();return new e.BackendKeyDataMessage(i,_,E)},I=(g,_)=>{const E=g.int32(),T={name:"authenticationOk",length:_};switch(E){case 0:break;case 3:T.length===8&&(T.name="authenticationCleartextPassword");break;case 5:if(T.length===12){T.name="authenticationMD5Password";const C=g.bytes(4);return new e.AuthenticationMD5Password(i,C)}break;case 10:{T.name="authenticationSASL",T.mechanisms=[];let C;do C=g.cstring(),C&&T.mechanisms.push(C);while(C)}break;case 11:T.name="authenticationSASLContinue",T.data=g.string(_-8);break;case 12:T.name="authenticationSASLFinal",T.data=g.string(_-8);break;default:throw new Error("Unknown authenticationOk message type "+E)}return T},A=(g,_)=>{const E={};let T=g.string(1);for(;T!=="\0";)E[T]=g.cstring(),T=g.string(1);const C=E.M,O=_==="notice"?new e.NoticeMessage(i,C):new e.DatabaseError(C,i,_);return O.severity=E.S,O.code=E.C,O.detail=E.D,O.hint=E.H,O.position=E.P,O.internalPosition=E.p,O.internalQuery=E.q,O.where=E.W,O.schema=E.s,O.table=E.t,O.column=E.c,O.dataType=E.d,O.constraint=E.n,O.file=E.F,O.line=E.L,O.routine=E.R,O};return Je}var dn;function pi(){return dn||(dn=1,(function(e){Object.defineProperty(e,"__esModule",{value:!0}),e.DatabaseError=e.serialize=e.parse=void 0;const t=fi();Object.defineProperty(e,"DatabaseError",{enumerable:!0,get:function(){return t.DatabaseError}});const r=ta();Object.defineProperty(e,"serialize",{enumerable:!0,get:function(){return r.serialize}});const s=sa();function n(i,o){const a=new s.Parser;return i.on("data",c=>a.parse(c,o)),new Promise(c=>i.on("end",()=>c()))}e.parse=n})(Nr)),Nr}var ft={},fn;function na(){return fn||(fn=1,Object.defineProperty(ft,"__esModule",{value:!0}),ft.default={}),ft}var Or,pn;function ia(){if(pn)return Or;pn=1;const{getStream:e,getSecureStream:t}=i();Or={getStream:e,getSecureStream:t};function r(){function o(c){const d=jn;return new d.Socket}function a(c){return wi.connect(c)}return{getStream:o,getSecureStream:a}}function s(){function o(c){const{CloudflareSocket:d}=na();return new d(c)}function a(c){return c.socket.startTls(c),c.socket}return{getStream:o,getSecureStream:a}}function n(){if(typeof navigator=="object"&&navigator!==null&&typeof navigator.userAgent=="string")return navigator.userAgent==="Cloudflare-Workers";if(typeof Response=="function"){const o=new Response(null,{cf:{thing:!0}});if(typeof o.cf=="object"&&o.cf!==null&&o.cf.thing)return!0}return!1}function i(){return n()?s():r()}return Or}var Ir,mn;function mi(){if(mn)return Ir;mn=1;const e=ze.EventEmitter,{parse:t,serialize:r}=pi(),{getStream:s,getSecureStream:n}=ia(),i=r.flush(),o=r.sync(),a=r.end();class c extends e{constructor(l){super(),l=l||{},this.stream=l.stream||s(l.ssl),typeof this.stream=="function"&&(this.stream=this.stream(l)),this._keepAlive=l.keepAlive,this._keepAliveInitialDelayMillis=l.keepAliveInitialDelayMillis,this.parsedStatements={},this.ssl=l.ssl||!1,this._ending=!1,this._emitMessage=!1;const f=this;this.on("newListener",function(u){u==="message"&&(f._emitMessage=!0)})}connect(l,f){const u=this;this._connecting=!0,this.stream.setNoDelay(!0),this.stream.connect(l,f),this.stream.once("connect",function(){u._keepAlive&&u.stream.setKeepAlive(!0,u._keepAliveInitialDelayMillis),u.emit("connect")});const p=function(h){u._ending&&(h.code==="ECONNRESET"||h.code==="EPIPE")||u.emit("error",h)};if(this.stream.on("error",p),this.stream.on("close",function(){u.emit("end")}),!this.ssl)return this.attachListeners(this.stream);this.stream.once("data",function(h){switch(h.toString("utf8")){case"S":break;case"N":return u.stream.end(),u.emit("error",new Error("The server does not support SSL connections"));default:return u.stream.end(),u.emit("error",new Error("There was an error establishing an SSL connection"))}const v={socket:u.stream};u.ssl!==!0&&(Object.assign(v,u.ssl),"key"in u.ssl&&(v.key=u.ssl.key));const m=jn;m.isIP&&m.isIP(f)===0&&(v.servername=f);try{u.stream=n(v)}catch(y){return u.emit("error",y)}u.attachListeners(u.stream),u.stream.on("error",p),u.emit("sslconnect")})}attachListeners(l){t(l,f=>{const u=f.name==="error"?"errorMessage":f.name;this._emitMessage&&this.emit("message",f),this.emit(u,f)})}requestSsl(){this.stream.write(r.requestSsl())}startup(l){this.stream.write(r.startup(l))}cancel(l,f){this._send(r.cancel(l,f))}password(l){this._send(r.password(l))}sendSASLInitialResponseMessage(l,f){this._send(r.sendSASLInitialResponseMessage(l,f))}sendSCRAMClientFinalMessage(l){this._send(r.sendSCRAMClientFinalMessage(l))}_send(l){return this.stream.writable?this.stream.write(l):!1}query(l){this._send(r.query(l))}parse(l){this._send(r.parse(l))}bind(l){this._send(r.bind(l))}execute(l){this._send(r.execute(l))}flush(){this.stream.writable&&this.stream.write(i)}sync(){this._ending=!0,this._send(o)}ref(){this.stream.ref()}unref(){this.stream.unref()}end(){if(this._ending=!0,!this._connecting||!this.stream.writable){this.stream.end();return}return this.stream.write(a,()=>{this.stream.end()})}close(l){this._send(r.close(l))}describe(l){this._send(r.describe(l))}sendCopyFromChunk(l){this._send(r.copyData(l))}endCopyFrom(){this._send(r.copyDone())}sendCopyFail(l){this._send(r.copyFail(l))}}return Ir=c,Ir}var pt={exports:{}},Lr={exports:{}},Pr,hn;function oa(){if(hn)return Pr;hn=1;const{Transform:e}=kn,{StringDecoder:t}=_i,r=Symbol("last"),s=Symbol("decoder");function n(d,l,f){let u;if(this.overflow){if(u=this[s].write(d).split(this.matcher),u.length===1)return f();u.shift(),this.overflow=!1}else this[r]+=this[s].write(d),u=this[r].split(this.matcher);this[r]=u.pop();for(let p=0;p<u.length;p++)try{o(this,this.mapper(u[p]))}catch(h){return f(h)}if(this.overflow=this[r].length>this.maxLength,this.overflow&&!this.skipOverflow){f(new Error("maximum buffer reached"));return}f()}function i(d){if(this[r]+=this[s].end(),this[r])try{o(this,this.mapper(this[r]))}catch(l){return d(l)}d()}function o(d,l){l!==void 0&&d.push(l)}function a(d){return d}function c(d,l,f){switch(d=d||/\r?\n/,l=l||a,f=f||{},arguments.length){case 1:typeof d=="function"?(l=d,d=/\r?\n/):typeof d=="object"&&!(d instanceof RegExp)&&!d[Symbol.split]&&(f=d,d=/\r?\n/);break;case 2:typeof d=="function"?(f=l,l=d,d=/\r?\n/):typeof l=="object"&&(f=l,l=a)}f=Object.assign({},f),f.autoDestroy=!0,f.transform=n,f.flush=i,f.readableObjectMode=!0;const u=new e(f);return u[r]="",u[s]=new t("utf8"),u.matcher=d,u.mapper=l,u.maxLength=f.maxLength,u.skipOverflow=f.skipOverflow||!1,u.overflow=!1,u._destroy=function(p,h){this._writableState.errorEmitted=!1,h(p)},u}return Pr=c,Pr}var gn;function aa(){return gn||(gn=1,(function(e){var t={},r=Ei,s=kn.Stream,n=oa(),i=He,o=5432,a=process.platform==="win32",c=process.stderr,d=56,l=7,f=61440,u=32768;function p(I){return(I&f)==u}var h=["host","port","database","user","password"],b=h.length,v=h[b-1];function m(){var I=c instanceof s&&c.writable===!0;if(I){var A=Array.prototype.slice.call(arguments).concat(`
`);c.write(i.format.apply(i,A))}}Object.defineProperty(e.exports,"isWin",{get:function(){return a},set:function(I){a=I}}),e.exports.warnTo=function(I){var A=c;return c=I,A},e.exports.getFileName=function(I){var A=I||t,g=A.PGPASSFILE||(a?r.join(A.APPDATA||"./","postgresql","pgpass.conf"):r.join(A.HOME||"./",".pgpass"));return g},e.exports.usePgPass=function(I,A){return Object.prototype.hasOwnProperty.call(t,"PGPASSWORD")?!1:a?!0:(A=A||"<unkn>",p(I.mode)?I.mode&(d|l)?(m('WARNING: password file "%s" has group or world access; permissions should be u=rw (0600) or less',A),!1):!0:(m('WARNING: password file "%s" is not a plain file',A),!1))};var y=e.exports.match=function(I,A){return h.slice(0,-1).reduce(function(g,_,E){return E==1&&Number(I[_]||o)===Number(A[_])?g&&!0:g&&(A[_]==="*"||A[_]===I[_])},!0)};e.exports.getPassword=function(I,A,g){var _,E=A.pipe(n());function T(x){var L=w(x);L&&S(L)&&y(I,L)&&(_=L[v],E.end())}var C=function(){A.destroy(),g(_)},O=function(x){A.destroy(),m("WARNING: error on reading file: %s",x),g(void 0)};A.on("error",O),E.on("data",T).on("end",C).on("error",O)};var w=e.exports.parseLine=function(I){if(I.length<11||I.match(/^\s+#/))return null;for(var A="",g="",_=0,E=0,T={},C=!1,O=function(L,D,M){var F=I.substring(D,M);Object.hasOwnProperty.call(t,"PGPASS_NO_DEESCAPE")||(F=F.replace(/\\([:\\])/g,"$1")),T[h[L]]=F},x=0;x<I.length-1;x+=1){if(A=I.charAt(x+1),g=I.charAt(x),C=_==b-1,C){O(_,E);break}x>=0&&A==":"&&g!=="\\"&&(O(_,E,x+1),E=x+2,_+=1)}return T=Object.keys(T).length===b?T:null,T},S=e.exports.isValidEntry=function(I){for(var A={0:function(C){return C.length>0},1:function(C){return C==="*"?!0:(C=Number(C),isFinite(C)&&C>0&&C<9007199254740992&&Math.floor(C)===C)},2:function(C){return C.length>0},3:function(C){return C.length>0},4:function(C){return C.length>0}},g=0;g<h.length;g+=1){var _=A[g],E=I[h[g]]||"",T=_(E);if(!T)return!1}return!0}})(Lr)),Lr.exports}var bn;function la(){if(bn)return pt.exports;bn=1;var e=Dn,t=aa();return pt.exports=function(r,s){var n=t.getFileName();e.stat(n,function(i,o){if(i||!t.usePgPass(o,n))return s(void 0);var a=e.createReadStream(n);t.getPassword(r,a,s)})},pt.exports.warnTo=t.warnTo,pt.exports}var qr,yn;function ca(){if(yn)return qr;yn=1;const e=ze.EventEmitter,t=tt(),r=He,s=Go(),n=Gr(),i=ui(),o=Zo(),a=St(),c=mi(),d=ci(),l=r.deprecate(()=>{},"Client.activeQuery is deprecated and will be removed in pg@9.0"),f=r.deprecate(()=>{},"Client.queryQueue is deprecated and will be removed in pg@9.0."),u=r.deprecate(()=>{},"pgpass support is deprecated and will be removed in pg@9.0. You can provide an async function as the password property to the Client/Pool constructor that returns a password instead. Within this function you can call the pgpass module in your own code."),p=r.deprecate(()=>{},"Passing a custom Promise implementation to the Client/Pool constructor is deprecated and will be removed in pg@9.0."),h=r.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead.");class b extends e{constructor(m){super(),this.connectionParameters=new i(m),this.user=this.connectionParameters.user,this.database=this.connectionParameters.database,this.port=this.connectionParameters.port,this.host=this.connectionParameters.host,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:this.connectionParameters.password}),this.replication=this.connectionParameters.replication;const y=m||{};y.Promise&&p(),this._Promise=y.Promise||zr.Promise,this._types=new n(y.types),this._ending=!1,this._ended=!1,this._connecting=!1,this._connected=!1,this._connectionError=!1,this._queryable=!0,this._activeQuery=null,this.enableChannelBinding=!!y.enableChannelBinding,this.connection=y.connection||new c({stream:y.stream,ssl:this.connectionParameters.ssl,keepAlive:y.keepAlive||!1,keepAliveInitialDelayMillis:y.keepAliveInitialDelayMillis||0,encoding:this.connectionParameters.client_encoding||"utf8"}),this._queryQueue=[],this.binary=y.binary||a.binary,this.processID=null,this.secretKey=null,this.ssl=this.connectionParameters.ssl||!1,this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this._connectionTimeoutMillis=y.connectionTimeoutMillis||0}get activeQuery(){return l(),this._activeQuery}set activeQuery(m){l(),this._activeQuery=m}_getActiveQuery(){return this._activeQuery}_errorAllQueries(m){const y=S=>{process.nextTick(()=>{S.handleError(m,this.connection)})},w=this._getActiveQuery();w&&(y(w),this._activeQuery=null),this._queryQueue.forEach(y),this._queryQueue.length=0}_connect(m){const y=this,w=this.connection;if(this._connectionCallback=m,this._connecting||this._connected){const S=new Error("Client has already been connected. You cannot reuse a client.");process.nextTick(()=>{m(S)});return}this._connecting=!0,this._connectionTimeoutMillis>0&&(this.connectionTimeoutHandle=setTimeout(()=>{w._ending=!0,w.stream.destroy(new Error("timeout expired"))},this._connectionTimeoutMillis),this.connectionTimeoutHandle.unref&&this.connectionTimeoutHandle.unref()),this.host&&this.host.indexOf("/")===0?w.connect(this.host+"/.s.PGSQL."+this.port):w.connect(this.port,this.host),w.on("connect",function(){y.ssl?w.requestSsl():w.startup(y.getStartupConf())}),w.on("sslconnect",function(){w.startup(y.getStartupConf())}),this._attachListeners(w),w.once("end",()=>{const S=this._ending?new Error("Connection terminated"):new Error("Connection terminated unexpectedly");clearTimeout(this.connectionTimeoutHandle),this._errorAllQueries(S),this._ended=!0,this._ending||(this._connecting&&!this._connectionError?this._connectionCallback?this._connectionCallback(S):this._handleErrorEvent(S):this._connectionError||this._handleErrorEvent(S)),process.nextTick(()=>{this.emit("end")})})}connect(m){if(m){this._connect(m);return}return new this._Promise((y,w)=>{this._connect(S=>{S?w(S):y(this)})})}_attachListeners(m){m.on("authenticationCleartextPassword",this._handleAuthCleartextPassword.bind(this)),m.on("authenticationMD5Password",this._handleAuthMD5Password.bind(this)),m.on("authenticationSASL",this._handleAuthSASL.bind(this)),m.on("authenticationSASLContinue",this._handleAuthSASLContinue.bind(this)),m.on("authenticationSASLFinal",this._handleAuthSASLFinal.bind(this)),m.on("backendKeyData",this._handleBackendKeyData.bind(this)),m.on("error",this._handleErrorEvent.bind(this)),m.on("errorMessage",this._handleErrorMessage.bind(this)),m.on("readyForQuery",this._handleReadyForQuery.bind(this)),m.on("notice",this._handleNotice.bind(this)),m.on("rowDescription",this._handleRowDescription.bind(this)),m.on("dataRow",this._handleDataRow.bind(this)),m.on("portalSuspended",this._handlePortalSuspended.bind(this)),m.on("emptyQuery",this._handleEmptyQuery.bind(this)),m.on("commandComplete",this._handleCommandComplete.bind(this)),m.on("parseComplete",this._handleParseComplete.bind(this)),m.on("copyInResponse",this._handleCopyInResponse.bind(this)),m.on("copyData",this._handleCopyData.bind(this)),m.on("notification",this._handleNotification.bind(this))}_getPassword(m){const y=this.connection;if(typeof this.password=="function")this._Promise.resolve().then(()=>this.password(this.connectionParameters)).then(w=>{if(w!==void 0){if(typeof w!="string"){y.emit("error",new TypeError("Password must be a string"));return}this.connectionParameters.password=this.password=w}else this.connectionParameters.password=this.password=null;m()}).catch(w=>{y.emit("error",w)});else if(this.password!==null)m();else try{la()(this.connectionParameters,S=>{S!==void 0&&(u(),this.connectionParameters.password=this.password=S),m()})}catch(w){this.emit("error",w)}}_handleAuthCleartextPassword(m){this._getPassword(()=>{this.connection.password(this.password)})}_handleAuthMD5Password(m){this._getPassword(async()=>{try{const y=await d.postgresMd5PasswordHash(this.user,this.password,m.salt);this.connection.password(y)}catch(y){this.emit("error",y)}})}_handleAuthSASL(m){this._getPassword(()=>{try{this.saslSession=s.startSession(m.mechanisms,this.enableChannelBinding&&this.connection.stream),this.connection.sendSASLInitialResponseMessage(this.saslSession.mechanism,this.saslSession.response)}catch(y){this.connection.emit("error",y)}})}async _handleAuthSASLContinue(m){try{await s.continueSession(this.saslSession,this.password,m.data,this.enableChannelBinding&&this.connection.stream),this.connection.sendSCRAMClientFinalMessage(this.saslSession.response)}catch(y){this.connection.emit("error",y)}}_handleAuthSASLFinal(m){try{s.finalizeSession(this.saslSession,m.data),this.saslSession=null}catch(y){this.connection.emit("error",y)}}_handleBackendKeyData(m){this.processID=m.processID,this.secretKey=m.secretKey}_handleReadyForQuery(m){this._connecting&&(this._connecting=!1,this._connected=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback&&(this._connectionCallback(null,this),this._connectionCallback=null),this.emit("connect"));const y=this._getActiveQuery();this._activeQuery=null,this.readyForQuery=!0,y&&y.handleReadyForQuery(this.connection),this._pulseQueryQueue()}_handleErrorWhileConnecting(m){if(!this._connectionError){if(this._connectionError=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback)return this._connectionCallback(m);this.emit("error",m)}}_handleErrorEvent(m){if(this._connecting)return this._handleErrorWhileConnecting(m);this._queryable=!1,this._errorAllQueries(m),this.emit("error",m)}_handleErrorMessage(m){if(this._connecting)return this._handleErrorWhileConnecting(m);const y=this._getActiveQuery();if(!y){this._handleErrorEvent(m);return}this._activeQuery=null,y.handleError(m,this.connection)}_handleRowDescription(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected rowDescription message from backend.");this._handleErrorEvent(w);return}y.handleRowDescription(m)}_handleDataRow(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected dataRow message from backend.");this._handleErrorEvent(w);return}y.handleDataRow(m)}_handlePortalSuspended(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected portalSuspended message from backend.");this._handleErrorEvent(w);return}y.handlePortalSuspended(this.connection)}_handleEmptyQuery(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected emptyQuery message from backend.");this._handleErrorEvent(w);return}y.handleEmptyQuery(this.connection)}_handleCommandComplete(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected commandComplete message from backend.");this._handleErrorEvent(w);return}y.handleCommandComplete(m,this.connection)}_handleParseComplete(){const m=this._getActiveQuery();if(m==null){const y=new Error("Received unexpected parseComplete message from backend.");this._handleErrorEvent(y);return}m.name&&(this.connection.parsedStatements[m.name]=m.text)}_handleCopyInResponse(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected copyInResponse message from backend.");this._handleErrorEvent(w);return}y.handleCopyInResponse(this.connection)}_handleCopyData(m){const y=this._getActiveQuery();if(y==null){const w=new Error("Received unexpected copyData message from backend.");this._handleErrorEvent(w);return}y.handleCopyData(m,this.connection)}_handleNotification(m){this.emit("notification",m)}_handleNotice(m){this.emit("notice",m)}getStartupConf(){const m=this.connectionParameters,y={user:m.user,database:m.database},w=m.application_name||m.fallback_application_name;return w&&(y.application_name=w),m.replication&&(y.replication=""+m.replication),m.statement_timeout&&(y.statement_timeout=String(parseInt(m.statement_timeout,10))),m.lock_timeout&&(y.lock_timeout=String(parseInt(m.lock_timeout,10))),m.idle_in_transaction_session_timeout&&(y.idle_in_transaction_session_timeout=String(parseInt(m.idle_in_transaction_session_timeout,10))),m.options&&(y.options=m.options),y}cancel(m,y){if(m.activeQuery===y){const w=this.connection;this.host&&this.host.indexOf("/")===0?w.connect(this.host+"/.s.PGSQL."+this.port):w.connect(this.port,this.host),w.on("connect",function(){w.cancel(m.processID,m.secretKey)})}else m._queryQueue.indexOf(y)!==-1&&m._queryQueue.splice(m._queryQueue.indexOf(y),1)}setTypeParser(m,y,w){return this._types.setTypeParser(m,y,w)}getTypeParser(m,y){return this._types.getTypeParser(m,y)}escapeIdentifier(m){return t.escapeIdentifier(m)}escapeLiteral(m){return t.escapeLiteral(m)}_pulseQueryQueue(){if(this.readyForQuery===!0){this._activeQuery=this._queryQueue.shift();const m=this._getActiveQuery();if(m){this.readyForQuery=!1,this.hasExecuted=!0;const y=m.submit(this.connection);y&&process.nextTick(()=>{m.handleError(y,this.connection),this.readyForQuery=!0,this._pulseQueryQueue()})}else this.hasExecuted&&(this._activeQuery=null,this.emit("drain"))}}query(m,y,w){let S,I,A,g,_;if(m==null)throw new TypeError("Client was passed a null or undefined query");return typeof m.submit=="function"?(A=m.query_timeout||this.connectionParameters.query_timeout,I=S=m,S.callback||(typeof y=="function"?S.callback=y:w&&(S.callback=w))):(A=m.query_timeout||this.connectionParameters.query_timeout,S=new o(m,y,w),S.callback||(I=new this._Promise((E,T)=>{S.callback=(C,O)=>C?T(C):E(O)}).catch(E=>{throw Error.captureStackTrace(E),E}))),A&&(_=S.callback||(()=>{}),g=setTimeout(()=>{const E=new Error("Query read timeout");process.nextTick(()=>{S.handleError(E,this.connection)}),_(E),S.callback=()=>{};const T=this._queryQueue.indexOf(S);T>-1&&this._queryQueue.splice(T,1),this._pulseQueryQueue()},A),S.callback=(E,T)=>{clearTimeout(g),_(E,T)}),this.binary&&!S.binary&&(S.binary=!0),S._result&&!S._result._types&&(S._result._types=this._types),this._queryable?this._ending?(process.nextTick(()=>{S.handleError(new Error("Client was closed and is not queryable"),this.connection)}),I):(this._queryQueue.length>0&&h(),this._queryQueue.push(S),this._pulseQueryQueue(),I):(process.nextTick(()=>{S.handleError(new Error("Client has encountered a connection error and is not queryable"),this.connection)}),I)}ref(){this.connection.ref()}unref(){this.connection.unref()}end(m){if(this._ending=!0,!this.connection._connecting||this._ended)if(m)m();else return this._Promise.resolve();if(this._getActiveQuery()||!this._queryable?this.connection.stream.destroy():this.connection.end(),m)this.connection.once("end",m);else return new this._Promise(y=>{this.connection.once("end",y)})}get queryQueue(){return f(),this._queryQueue}}return b.Query=o,qr=b,qr}var Dr,vn;function ua(){if(vn)return Dr;vn=1;const e=ze.EventEmitter,t=function(){},r=(d,l)=>{const f=d.findIndex(l);return f===-1?void 0:d.splice(f,1)[0]};class s{constructor(l,f,u){this.client=l,this.idleListener=f,this.timeoutId=u}}class n{constructor(l){this.callback=l}}function i(){throw new Error("Release called on client which has already been released to the pool.")}function o(d,l){if(l)return{callback:l,result:void 0};let f,u;const p=function(b,v){b?f(b):u(v)},h=new d(function(b,v){u=b,f=v}).catch(b=>{throw Error.captureStackTrace(b),b});return{callback:p,result:h}}function a(d,l){return function f(u){u.client=l,l.removeListener("error",f),l.on("error",()=>{d.log("additional client error after disconnection due to error",u)}),d._remove(l),d.emit("error",u,l)}}class c extends e{constructor(l,f){super(),this.options=Object.assign({},l),l!=null&&"password"in l&&Object.defineProperty(this.options,"password",{configurable:!0,enumerable:!1,writable:!0,value:l.password}),l!=null&&l.ssl&&l.ssl.key&&Object.defineProperty(this.options.ssl,"key",{enumerable:!1}),this.options.max=this.options.max||this.options.poolSize||10,this.options.min=this.options.min||0,this.options.maxUses=this.options.maxUses||1/0,this.options.allowExitOnIdle=this.options.allowExitOnIdle||!1,this.options.maxLifetimeSeconds=this.options.maxLifetimeSeconds||0,this.log=this.options.log||function(){},this.Client=this.options.Client||f||hi().Client,this.Promise=this.options.Promise||zr.Promise,typeof this.options.idleTimeoutMillis>"u"&&(this.options.idleTimeoutMillis=1e4),this._clients=[],this._idle=[],this._expired=new WeakSet,this._pendingQueue=[],this._endCallback=void 0,this.ending=!1,this.ended=!1}_promiseTry(l){const f=this.Promise;return typeof f.try=="function"?f.try(l):new f(u=>u(l()))}_isFull(){return this._clients.length>=this.options.max}_isAboveMin(){return this._clients.length>this.options.min}_pulseQueue(){if(this.log("pulse queue"),this.ended){this.log("pulse queue ended");return}if(this.ending){this.log("pulse queue on ending"),this._idle.length&&this._idle.slice().map(f=>{this._remove(f.client)}),this._clients.length||(this.ended=!0,this._endCallback());return}if(!this._pendingQueue.length){this.log("no queued requests");return}if(!this._idle.length&&this._isFull())return;const l=this._pendingQueue.shift();if(this._idle.length){const f=this._idle.pop();clearTimeout(f.timeoutId);const u=f.client;u.ref&&u.ref();const p=f.idleListener;return this._acquireClient(u,l,p,!1)}if(!this._isFull())return this.newClient(l);throw new Error("unexpected condition")}_remove(l,f){const u=r(this._idle,h=>h.client===l);u!==void 0&&clearTimeout(u.timeoutId),this._clients=this._clients.filter(h=>h!==l);const p=this;l.end(()=>{p.emit("remove",l),typeof f=="function"&&f()})}connect(l){if(this.ending){const p=new Error("Cannot use a pool after calling end on the pool");return l?l(p):this.Promise.reject(p)}const f=o(this.Promise,l),u=f.result;if(this._isFull()||this._idle.length){if(this._idle.length&&process.nextTick(()=>this._pulseQueue()),!this.options.connectionTimeoutMillis)return this._pendingQueue.push(new n(f.callback)),u;const p=(v,m,y)=>{clearTimeout(b),f.callback(v,m,y)},h=new n(p),b=setTimeout(()=>{r(this._pendingQueue,v=>v.callback===p),h.timedOut=!0,f.callback(new Error("timeout exceeded when trying to connect"))},this.options.connectionTimeoutMillis);return b.unref&&b.unref(),this._pendingQueue.push(h),u}return this.newClient(new n(f.callback)),u}newClient(l){const f=new this.Client(this.options);this._clients.push(f);const u=a(this,f);this.log("checking client timeout");let p,h=!1;this.options.connectionTimeoutMillis&&(p=setTimeout(()=>{f.connection?(this.log("ending client due to timeout"),h=!0,f.connection.stream.destroy()):f.isConnected()||(this.log("ending client due to timeout"),h=!0,f.end())},this.options.connectionTimeoutMillis)),this.log("connecting new client"),f.connect(b=>{if(p&&clearTimeout(p),f.on("error",u),b)this.log("client failed to connect",b),this._clients=this._clients.filter(v=>v!==f),h&&(b=new Error("Connection terminated due to connection timeout",{cause:b})),this._pulseQueue(),l.timedOut||l.callback(b,void 0,t);else{if(this.log("new client connected"),this.options.onConnect){this._promiseTry(()=>this.options.onConnect(f)).then(()=>{this._afterConnect(f,l,u)},v=>{this._clients=this._clients.filter(m=>m!==f),f.end(()=>{this._pulseQueue(),l.timedOut||l.callback(v,void 0,t)})});return}return this._afterConnect(f,l,u)}})}_afterConnect(l,f,u){if(this.options.maxLifetimeSeconds!==0){const p=setTimeout(()=>{this.log("ending client due to expired lifetime"),this._expired.add(l),this._idle.findIndex(b=>b.client===l)!==-1&&this._acquireClient(l,new n((b,v,m)=>m()),u,!1)},this.options.maxLifetimeSeconds*1e3);p.unref(),l.once("end",()=>clearTimeout(p))}return this._acquireClient(l,f,u,!0)}_acquireClient(l,f,u,p){p&&this.emit("connect",l),this.emit("acquire",l),l.release=this._releaseOnce(l,u),l.removeListener("error",u),f.timedOut?p&&this.options.verify?this.options.verify(l,l.release):l.release():p&&this.options.verify?this.options.verify(l,h=>{if(h)return l.release(h),f.callback(h,void 0,t);f.callback(void 0,l,l.release)}):f.callback(void 0,l,l.release)}_releaseOnce(l,f){let u=!1;return p=>{u&&i(),u=!0,this._release(l,f,p)}}_release(l,f,u){if(l.on("error",f),l._poolUseCount=(l._poolUseCount||0)+1,this.emit("release",u,l),u||this.ending||!l._queryable||l._ending||l._poolUseCount>=this.options.maxUses)return l._poolUseCount>=this.options.maxUses&&this.log("remove expended client"),this._remove(l,this._pulseQueue.bind(this));if(this._expired.has(l))return this.log("remove expired client"),this._expired.delete(l),this._remove(l,this._pulseQueue.bind(this));let h;this.options.idleTimeoutMillis&&this._isAboveMin()&&(h=setTimeout(()=>{this._isAboveMin()&&(this.log("remove idle client"),this._remove(l,this._pulseQueue.bind(this)))},this.options.idleTimeoutMillis),this.options.allowExitOnIdle&&h.unref()),this.options.allowExitOnIdle&&l.unref(),this._idle.push(new s(l,f,h)),this._pulseQueue()}query(l,f,u){if(typeof l=="function"){const h=o(this.Promise,l);return setImmediate(function(){return h.callback(new Error("Passing a function as the first parameter to pool.query is not supported"))}),h.result}typeof f=="function"&&(u=f,f=void 0);const p=o(this.Promise,u);return u=p.callback,this.connect((h,b)=>{if(h)return u(h);let v=!1;const m=y=>{v||(v=!0,b.release(y),u(y))};b.once("error",m),this.log("dispatching query");try{b.query(l,f,(y,w)=>{if(this.log("query dispatched"),b.removeListener("error",m),!v)return v=!0,b.release(y),y?u(y):u(void 0,w)})}catch(y){return b.release(y),u(y)}}),p.result}end(l){if(this.log("ending"),this.ending){const u=new Error("Called end on pool more than once");return l?l(u):this.Promise.reject(u)}this.ending=!0;const f=o(this.Promise,l);return this._endCallback=f.callback,this._pulseQueue(),f.result}get waitingCount(){return this._pendingQueue.length}get idleCount(){return this._idle.length}get expiredCount(){return this._clients.reduce((l,f)=>l+(this._expired.has(f)?1:0),0)}get totalCount(){return this._clients.length}}return Dr=c,Dr}var jr={exports:{}};const da={},fa=Object.freeze(Object.defineProperty({__proto__:null,default:da},Symbol.toStringTag,{value:"Module"})),pa=oo(fa);var kr={exports:{}},wn;function ma(){if(wn)return kr.exports;wn=1;const e=ze.EventEmitter,t=He,r=tt(),s=kr.exports=function(i,o,a){e.call(this),i=r.normalizeQueryConfig(i,o,a),this.text=i.text,this.values=i.values,this.name=i.name,this.queryMode=i.queryMode,this.callback=i.callback,this.state="new",this._arrayMode=i.rowMode==="array",this._emitRowEvents=!1,this.on("newListener",(function(c){c==="row"&&(this._emitRowEvents=!0)}).bind(this))};t.inherits(s,e);const n={sqlState:"code",statementPosition:"position",messagePrimary:"message",context:"where",schemaName:"schema",tableName:"table",columnName:"column",dataTypeName:"dataType",constraintName:"constraint",sourceFile:"file",sourceLine:"line",sourceFunction:"routine"};return s.prototype.handleError=function(i){const o=this.native.pq.resultErrorFields();if(o)for(const a in o){const c=n[a]||a;i[c]=o[a]}this.callback?this.callback(i):this.emit("error",i),this.state="error"},s.prototype.then=function(i,o){return this._getPromise().then(i,o)},s.prototype.catch=function(i){return this._getPromise().catch(i)},s.prototype._getPromise=function(){return this._promise?this._promise:(this._promise=new Promise((function(i,o){this._once("end",i),this._once("error",o)}).bind(this)),this._promise)},s.prototype.submit=function(i){this.state="running";const o=this;this.native=i.native,i.native.arrayMode=this._arrayMode;let a=function(c,d,l){if(i.native.arrayMode=!1,setImmediate(function(){o.emit("_done")}),c)return o.handleError(c);o._emitRowEvents&&(l.length>1?d.forEach((f,u)=>{f.forEach(p=>{o.emit("row",p,l[u])})}):d.forEach(function(f){o.emit("row",f,l)})),o.state="end",o.emit("end",l),o.callback&&o.callback(null,l)};if(process.domain&&(a=process.domain.bind(a)),this.name){this.name.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",this.name,this.name.length),console.error("This can cause conflicts and silent errors executing queries"));const c=(this.values||[]).map(r.prepareValue);if(i.namedQueries[this.name]){if(this.text&&i.namedQueries[this.name]!==this.text){const d=new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);return a(d)}return i.native.execute(this.name,c,a)}return i.native.prepare(this.name,this.text,c.length,function(d){return d?a(d):(i.namedQueries[o.name]=o.text,o.native.execute(o.name,c,a))})}else if(this.values){if(!Array.isArray(this.values)){const d=new Error("Query values must be an array");return a(d)}const c=this.values.map(r.prepareValue);i.native.query(this.text,c,a)}else this.queryMode==="extended"?i.native.query(this.text,[],a):i.native.query(this.text,a)},kr.exports}var En;function ha(){if(En)return jr.exports;En=1;const e=He;var t;try{t=pa}catch(d){throw d}const r=Gr(),s=ze.EventEmitter,n=He,i=ui(),o=ma(),a=e.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead."),c=jr.exports=function(d){s.call(this),d=d||{},this._Promise=d.Promise||zr.Promise,this._types=new r(d.types),this.native=new t({types:this._types}),this._queryQueue=[],this._ending=!1,this._connecting=!1,this._connected=!1,this._queryable=!0;const l=this.connectionParameters=new i(d);d.nativeConnectionString&&(l.nativeConnectionString=d.nativeConnectionString),this.user=l.user,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:l.password}),this.database=l.database,this.host=l.host,this.port=l.port,this.namedQueries={}};return c.Query=o,n.inherits(c,s),c.prototype._errorAllQueries=function(d){const l=f=>{process.nextTick(()=>{f.native=this.native,f.handleError(d)})};this._hasActiveQuery()&&(l(this._activeQuery),this._activeQuery=null),this._queryQueue.forEach(l),this._queryQueue.length=0},c.prototype._connect=function(d){const l=this;if(this._connecting){process.nextTick(()=>d(new Error("Client has already been connected. You cannot reuse a client.")));return}this._connecting=!0,this.connectionParameters.getLibpqConnectionString(function(f,u){if(l.connectionParameters.nativeConnectionString&&(u=l.connectionParameters.nativeConnectionString),f)return d(f);l.native.connect(u,function(p){if(p)return l.native.end(),d(p);l._connected=!0,l.native.on("error",function(h){l._queryable=!1,l._errorAllQueries(h),l.emit("error",h)}),l.native.on("notification",function(h){l.emit("notification",{channel:h.relname,payload:h.extra})}),l.emit("connect"),l._pulseQueryQueue(!0),d(null,this)})})},c.prototype.connect=function(d){if(d){this._connect(d);return}return new this._Promise((l,f)=>{this._connect(u=>{u?f(u):l(this)})})},c.prototype.query=function(d,l,f){let u,p,h,b,v;if(d==null)throw new TypeError("Client was passed a null or undefined query");if(typeof d.submit=="function")h=d.query_timeout||this.connectionParameters.query_timeout,p=u=d,typeof l=="function"&&(d.callback=l);else if(h=d.query_timeout||this.connectionParameters.query_timeout,u=new o(d,l,f),!u.callback){let m,y;p=new this._Promise((w,S)=>{m=w,y=S}).catch(w=>{throw Error.captureStackTrace(w),w}),u.callback=(w,S)=>w?y(w):m(S)}return h&&(v=u.callback||(()=>{}),b=setTimeout(()=>{const m=new Error("Query read timeout");process.nextTick(()=>{u.handleError(m,this.connection)}),v(m),u.callback=()=>{};const y=this._queryQueue.indexOf(u);y>-1&&this._queryQueue.splice(y,1),this._pulseQueryQueue()},h),u.callback=(m,y)=>{clearTimeout(b),v(m,y)}),this._queryable?this._ending?(u.native=this.native,process.nextTick(()=>{u.handleError(new Error("Client was closed and is not queryable"))}),p):(this._queryQueue.length>0&&a(),this._queryQueue.push(u),this._pulseQueryQueue(),p):(u.native=this.native,process.nextTick(()=>{u.handleError(new Error("Client has encountered a connection error and is not queryable"))}),p)},c.prototype.end=function(d){const l=this;this._ending=!0,this._connected||this.once("connect",this.end.bind(this,d));let f;return d||(f=new this._Promise(function(u,p){d=h=>h?p(h):u()})),this.native.end(function(){l._connected=!1,l._errorAllQueries(new Error("Connection terminated")),process.nextTick(()=>{l.emit("end"),d&&d()})}),f},c.prototype._hasActiveQuery=function(){return this._activeQuery&&this._activeQuery.state!=="error"&&this._activeQuery.state!=="end"},c.prototype._pulseQueryQueue=function(d){if(!this._connected||this._hasActiveQuery())return;const l=this._queryQueue.shift();if(!l){d||this.emit("drain");return}this._activeQuery=l,l.submit(this);const f=this;l.once("_done",function(){f._pulseQueryQueue()})},c.prototype.cancel=function(d){this._activeQuery===d?this.native.cancel(function(){}):this._queryQueue.indexOf(d)!==-1&&this._queryQueue.splice(this._queryQueue.indexOf(d),1)},c.prototype.ref=function(){},c.prototype.unref=function(){},c.prototype.setTypeParser=function(d,l,f){return this._types.setTypeParser(d,l,f)},c.prototype.getTypeParser=function(d,l){return this._types.getTypeParser(d,l)},c.prototype.isConnected=function(){return this._connected},jr.exports}var Br,_n;function xn(){return _n||(_n=1,Br=ha()),Br}var Sn;function hi(){return Sn||(Sn=1,(function(e){var t={};const r=ca(),s=St(),n=mi(),i=di(),o=tt(),a=ua(),c=Gr(),{DatabaseError:d}=pi(),{escapeIdentifier:l,escapeLiteral:f}=tt(),u=v=>class extends a{constructor(y){super(y,v)}},p=function(v){this.defaults=s,this.Client=v,this.Query=this.Client.Query,this.Pool=u(this.Client),this._pools=[],this.Connection=n,this.types=xt(),this.DatabaseError=d,this.TypeOverrides=c,this.escapeIdentifier=l,this.escapeLiteral=f,this.Result=i,this.utils=o};let h=r,b=!1;try{b=!!t.NODE_PG_FORCE_NATIVE}catch{}b&&(h=xn()),e.exports=new p(h),Object.defineProperty(e.exports,"native",{configurable:!0,enumerable:!1,get(){let v=null;try{v=new p(xn())}catch(m){if(m.code!=="MODULE_NOT_FOUND")throw m}return Object.defineProperty(e.exports,"native",{value:v}),v}})})(ar)),ar.exports}var ga=hi();const ee=ti(ga);ee.Client;const ba=ee.Pool;ee.Connection;ee.types;ee.Query;ee.DatabaseError;ee.escapeIdentifier;ee.escapeLiteral;ee.Result;ee.TypeOverrides;ee.defaults;const ya=Object.freeze(Object.defineProperty({__proto__:null,Pool:ba,default:ee},Symbol.toStringTag,{value:"Module"}));export{Ds as default};
