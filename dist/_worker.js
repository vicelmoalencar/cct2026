...
TRANSCRIÇÃO:
${R}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const B=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(B){R=B;const F=B.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[R,O,n]),e.json({transcript:R,description:O,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});T.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});T.post("/api/lessons/:id/rent",X,async e=>{var t;try{const r=parseInt(e.req.param("id")),i=e.get("user").email,n=L(e);await St(n);const o=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[r]);if(!o.length||!o[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const l=o[0],c=parseInt(l.rental_credits);if(!Number.isFinite(c)||c<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const u=await n.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[i,r]);if(u.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:u[0].expires_at},400);let p=!!((t=(await n.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[i,r]))[0])!=null&&t.has_access);if(!p){const m=e.env.DATABASE_SUITEPLUS,[y,_]=await Promise.all([n.sql(`SELECT id FROM member_subscriptions
           WHERE lower(email_membro) = lower($1)
             AND data_expiracao > NOW()
             AND COALESCE(ativo, true) = true
             AND COALESCE(teste_gratis, false) = false
           LIMIT 1`,[i]),m?ct(i,m):Promise.resolve(null)]);p=y.length>0||_!==null&&_>new Date}if(p)return e.json({success:!0,alreadyHasAccess:!0,message:"Você já tem acesso a esta aula pela sua assinatura ativa — não foi cobrado nenhum crédito."});const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const f=Xr(e),h=await Wr(f,i);if(h<c)return e.json({error:"Créditos insuficientes",available:h,required:c},400);if(!await Po(f,i,c,`aluguel_aula_cct:${r}`)){const m=await Wr(f,i);return e.json({error:"Créditos insuficientes",available:m,required:c},400)}await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
...
${(t.emails||[]).join(`
`)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function Uo(e,t,r,s){var i,n,o,l,c,u,a;switch(e){case"search_users":{const p=`%${t.query}%`,d=Math.min(t.limit||10,50),f=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         ORDER BY u.created_at DESC LIMIT $2`,[p,d]);return{users:f,total:f.length}}case"get_user_details":{const[p,d,f]=await Promise.all([r.sql("SELECT * FROM users WHERE lower(email) = lower($1) LIMIT 1",[t.email]),r.sql("SELECT * FROM member_subscriptions WHERE lower(email_membro) = lower($1) ORDER BY data_expiracao DESC",[t.email]),r.sql("SELECT * FROM certificates WHERE lower(user_email) = lower($1) ORDER BY created_at DESC",[t.email])]);return{user:p[0]||null,subscriptions:d,certificates:f}}case"list_courses":return{courses:await r.sql("SELECT id, title, instructor, duration_hours, offers_certificate, is_published, min_completion_days, created_at FROM courses ORDER BY title")};case"get_course_details":{const[p,d]=await Promise.all([r.sql("SELECT * FROM courses WHERE id = $1",[t.course_id]),r.sql(`SELECT m.id, m.title, m.order_index, COUNT(l.id)::int as lesson_count
           FROM modules m LEFT JOIN lessons l ON l.module_id = m.id
           WHERE m.course_id = $1 GROUP BY m.id ORDER BY m.order_index`,[t.course_id])]);return{course:p[0]||null,modules:d}}case"search_subscriptions":return{subscriptions:await r.sql("SELECT * FROM member_subscriptions WHERE lower(email_membro) = lower($1) ORDER BY data_expiracao DESC",[t.email])};case"list_certificates":{const p=Math.min(t.limit||20,100);return{certificates:t.email?await r.sql("SELECT * FROM certificates WHERE lower(user_email) = lower($1) ORDER BY created_at DESC LIMIT $2",[t.email,p]):await r.sql("SELECT * FROM certificates ORDER BY created_at DESC LIMIT $1",[p])}}case"get_lesson":return{lesson:(await r.sql(`SELECT l.*, m.title as module_title, c.title as course_title
         FROM lessons l LEFT JOIN modules m ON m.id = l.module_id LEFT JOIN courses c ON c.id = m.course_id
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const p=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,p]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[p])}}case"dashboard_stats":{const[p,d,f,h,g,v,m]=await Promise.all([r.sql("SELECT COUNT(*)::int as total FROM users"),r.sql("SELECT COUNT(*)::int as total FROM member_subscriptions WHERE data_expiracao > NOW() AND ativo = true"),r.sql("SELECT COUNT(*)::int as total FROM certificates"),r.sql("SELECT COUNT(*)::int as total FROM comments WHERE admin_reply IS NULL"),r.sql("SELECT COUNT(*)::int as total FROM courses"),r.sql("SELECT COUNT(*)::int as total FROM lessons"),(()=>{const y=s.env.DATABASE_SUITEPLUS;return y?new Re(y).sql("SELECT COUNT(*)::int as total FROM user_subscriptions WHERE status = 'active' AND product_id = 4 AND expires_at > NOW()").catch(()=>[{total:null}]):Promise.resolve([{total:null}])})()]);return{total_users:(i=p[0])==null?void 0:i.total,active_subscriptions_cct:(n=d[0])==null?void 0:n.total,active_subscriptions_suiteplus:(o=m[0])==null?void 0:o.total,total_certificates:(l=f[0])==null?void 0:l.total,pending_comments:(c=h[0])==null?void 0:c.total,total_courses:(u=g[0])==null?void 0:u.total,total_lessons:(a=v[0])==null?void 0:a.total}}case"get_user_progress":{const d=(await r.sql(`SELECT c.id as course_id, c.title as course_title,
         ...
         ORDER BY completed_lessons DESC`,[t.email])).map(f=>({...f,progress_pct:f.total_lessons>0?Math.round(f.completed_lessons/f.total_lessons*100):0}));return{email:t.email,courses_with_progress:d,total_courses:d.length}}case"list_recent_signups":{const p=Math.min(t.days||7,365),d=Math.min(t.limit||20,100),f=await r.sql(`SELECT email, nome, created_at FROM users
         WHERE created_at > NOW() - INTERVAL '${p} days'
         ORDER BY created_at DESC LIMIT $1`,[d]);return{signups:f,count:f.length,days:p}}case"search_lessons":{const p=Math.min(t.limit||20,100),d=[],f=[];let h=1;t.query&&(d.push(`l.title ILIKE $${h++}`),f.push(`%${t.query}%`)),t.course_id&&(d.push(`c.id = $${h++}`),f.push(t.course_id)),t.created_after&&(d.push(`l.created_at >= $${h++}`),f.push(t.created_after)),t.created_before&&(d.push(`l.created_at <= $${h++}`),f.push(t.created_before));const g=d.length>0?`WHERE ${d.join(" AND ")}`:"",m={created_asc:"l.created_at ASC",created_desc:"l.created_at DESC",title:"l.title ASC"}[t.order_by]||"l.created_at DESC";f.push(p);const y=await r.sql(`SELECT l.id, l.title, l.duration_minutes, l.teste_gratis, l.rentable, l.created_at,
         ...
         ${g} ORDER BY ${m} LIMIT $${h}`,f);return{lessons:y,count:y.length}}case"search_suiteplus_subscriptions":{const p=s.env.DATABASE_SUITEPLUS;if(!p)return{error:"DATABASE_SUITEPLUS não configurada"};const d=new Re(p),f=Math.min(t.limit||20,100);let h;return t.email?h=await d.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE lower(user_email) = lower($1) ORDER BY expires_at DESC LIMIT $2`,[t.email,f]):t.status?h=await d.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE status = $1 AND product_id = 4 ORDER BY expires_at DESC LIMIT $2`,[t.status,f]):h=await d.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
           FROM user_subscriptions WHERE product_id = 4 ORDER BY expires_at DESC LIMIT $1`,[f]),{subscriptions:h,count:h.length,source:"SuitePlus"}}case"list_expiring_subscriptions":{const p=Math.min(Math.max(t.days_ahead??7,0),365),d=Math.min(Math.max(t.include_expired_days??0,0),365),h=t.only_active!==!1?"AND ativo = true":"",g=await r.sql(`SELECT ms.email_membro, ms.data_expiracao, ms.ativo, ms.detalhe, ms.origem,
                u.nome
         FROM member_subscriptions ms
         LEFT JOIN users u ON lower(u.email) = lower(ms.email_membro)
         WHERE ms.data_expiracao BETWEEN (NOW() - INTERVAL '${d} days') AND (NOW() + INTERVAL '${p} days')
         ${h.replace("ativo","ms.ativo")}
         ORDER BY ms.data_expiracao ASC`,[]);return{subscriptions:g,count:g.length,query:{days_ahead:p,include_expired_days:d}}}default:return{error:`Ferramenta desconhecida: ${e}`}}}async function Fo(e,t,r,s){var u,a;const i=new Set(["nome","telefone","whatsapp","ativo","dt_expiracao"]),n=new Set(["data_expiracao","ativo","detalhe","origem"]),o=new Set(["title","description","teste_gratis","rentable","duration_minutes","order_index","is_published"]),l=new Set(["title","description","instructor","duration_hours","offers_certificate","is_published","min_completion_days"]),c=new Set(["user_name","course_title","carga_horaria","generated_at"]);switch(e){case"update_user":{const p={};for(const[g,v]of Object.entries(t.fields||{}))i.has(g)&&(p[g]=v);if(Object.keys(p).length===0)return{error:"Nenhum campo válido para atualizar"};const d=Object.keys(p).map((g,v)=>`"${g}" = $${v+2}`).join(", "),f=[t.email,...Object.values(p)],h=await r.sql(`UPDATE users SET ${d} WHERE lower(email) = lower($1) RETURNING *`,f);return{updated:h.length,user:h[0]||null}}case"update_member_subscription":{const p={};for(const[g,v]of Object.entries(t.fields||{}))n.has(g)&&(p[g]=v);if(Object.keys(p).length===0)return{error:"Nenhum campo válido para atualizar"};const d=Object.keys(p).map((g,v)=>`"${g}" = $${v+2}`).join(", "),f=[t.email,...Object.values(p)],h=await r.sql(`UPDATE member_subscriptions SET ${d}, updated_at = NOW() WHERE lower(email_membro) = lower($1) RETURNING *`,f);return{updated:h.length,subscription:h[0]||null}}case"create_member_subscription":return{created:!0,subscription:(await r.sql(`INSERT INTO member_subscriptions (email_membro, data_expiracao, detalhe, ativo, origem, created_at, updated_at)
         ...
         WHERE lower(email_membro) = lower($1) RETURNING *`,[t.email])).length,expired:!0};case"update_lesson":{const p={};for(const[g,v]of Object.entries(t.fields||{}))o.has(g)&&(p[g]=v);if(Object.keys(p).length===0)return{error:"Nenhum campo válido para atualizar"};const d=Object.keys(p).map((g,v)=>`"${g}" = $${v+2}`).join(", "),f=[t.lesson_id,...Object.values(p)],h=await r.sql(`UPDATE lessons SET ${d} WHERE id = $1 RETURNING id, title`,f);return{updated:h.length,lesson:h[0]||null}}case"update_course":{const p={};for(const[g,v]of Object.entries(t.fields||{}))l.has(g)&&(p[g]=v);if(Object.keys(p).length===0)return{error:"Nenhum campo válido para atualizar"};const d=Object.keys(p).map((g,v)=>`"${g}" = $${v+2}`).join(", "),f=[t.course_id,...Object.values(p)],h=await r.sql(`UPDATE courses SET ${d}, updated_at = NOW() WHERE id = $1 RETURNING id, title`,f);return{updated:h.length,course:h[0]||null}}case"update_certificate":{const p={};for(const[g,v]of Object.entries(t.fields||{}))c.has(g)&&(p[g]=v);if(Object.keys(p).length===0)return{error:"Nenhum campo válido para atualizar"};const d=Object.keys(p).map((g,v)=>`"${g}" = $${v+2}`).join(", "),f=[t.certificate_id,...Object.values(p)],h=await r.sql(`UPDATE certificates SET ${d}, updated_at = NOW() WHERE id = $1 RETURNING *`,f);return{updated:h.length,certificate:h[0]||null}}case"reply_comment":{const p=s.get("user"),d=await r.sql("UPDATE comments SET admin_reply = $1, admin_replied_at = NOW(), admin_replied_by = $2 WHERE id = $3 RETURNING id, user_name, comment_text, admin_reply",[t.reply,(p==null?void 0:p.email)||"admin",t.comment_id]);return d.length===0?{error:`Comentário ID ${t.comment_id} não encontrado`}:{replied:!0,comment:d[0]}}case"delete_comment":{const p=await r.sql("DELETE FROM comments WHERE id = $1 RETURNING id, user_name, comment_text",[t.comment_id]);return p.length===0?{error:`Comentário ID ${t.comment_id} não encontrado`}:{deleted:!0,comment:p[0]}}case"generate_certificate":{const p=new Uint8Array(4);crypto.getRandomValues(p);const d="CCT-"+new Date().getFullYear()+"-"+Array.from(p).map(v=>v.toString(16).padStart(2,"0")).join("").toUpperCase(),f=new Date().toISOString(),h=t.completion_date||f.split("T")[0],g=await r.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, created_at)
         VALUES ($1, $2, $3, $4, $5, $5, $6, $7::date, $6)
         RETURNING id, certificate_code`,[t.email,t.user_name,t.course_title,t.carga_horaria,d,f,h]);return{created:!0,certificate_id:(u=g[0])==null?void 0:u.id,certificate_code:(a=g[0])==null?void 0:a.certificate_code}}case"add_credits":{const p=Xr(s),d=await Do(p,t.email,t.amount,"admin_agente_ia");return{added:t.amount,new_balance:d,email:t.email}}case"bulk_extend_subscriptions":{const p=(t.emails||[]).map(h=>h.toLowerCase());if(p.length===0)return{error:"Nenhum email fornecido"};const d=Math.min(Math.max(t.days||0,1),3650),f=await r.sql(`UPDATE member_subscriptions
         SET data_expiracao = data_expiracao + ($1 || ' days')::INTERVAL, updated_at = NOW()
         WHERE lower(email_membro) = ANY($2)
         RETURNING email_membro, data_expiracao`,[String(d),p]);return{updated:f.length,days_added:d,subscriptions:f}}default:return{error:`Ferramenta de escrita desconhecida: ${e}`}}}T.get("/api/admin/agent-settings",P,async e=>{var t;try{const r=L(e);await r.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())");const s=await r.sql("SELECT value FROM agent_settings WHERE key = 'custom_instructions'");return e.json({instructions:((t=s[0])==null?void 0:t.value)||""})}catch(r){return e.json({error:r.message},500)}});T.put("/api/admin/agent-settings",P,async e=>{try{const{instructions:t}=await e.req.json(),r=L(e);return await r.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())"),await r.sql(`INSERT INTO agent_settings (key, value, updated_at) VALUES ('custom_instructions', $1, NOW())
...
\`\`\``})}let a="";try{await c.sql("CREATE TABLE IF NOT EXISTS agent_settings (key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())"),a=((r=(t=(await c.sql("SELECT value FROM agent_settings WHERE key = 'custom_instructions'"))[0])==null?void 0:t.value)==null?void 0:r.trim())||""}catch{}let f=[{role:"system",content:`Você é um assistente administrativo do CCT (Clube de Cálculo Trabalhista).
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});T.get("/api/courses/:id/modules",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const i=await r.sql(`SELECT m.*,
       ...
       ORDER BY m.order_index, l.order_index`,[t]),u=new Map;for(const a of c){const p=u.get(a.module_id)||[];p.push(a),u.set(a.module_id,p)}for(const a of o)a.lessons=u.get(a.id)||[];return e.json({course:n,modules:o})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});T.get("/api/search/lessons",X,async e=>{try{const t=e.get("user"),r=L(e);await St(r);const s=(e.req.query("q")||"").trim(),i=e.req.query("course_id"),n=e.req.query("type")||"all",o=Math.max(parseInt(e.req.query("min_duration")||"0",10),0),l=Math.min(Math.max(parseInt(e.req.query("max_duration")||"999",10),o),999),c=Math.min(Math.max(parseInt(e.req.query("limit")||"80",10),1),120),u=e.req.query("sort")||"relevance",a=[t.email],p=["1=1"];let d=a.length+1;i&&(p.push(`co.id = $${d++}`),a.push(parseInt(i,10))),p.push(`COALESCE(l.duration_minutes, 0) BETWEEN $${d++} AND $${d++}`),a.push(o,l),n==="free"?p.push("(COALESCE(l.teste_gratis, false) = true OR COALESCE(l.free_trial, false) = true)"):n==="premium"?p.push("COALESCE(l.teste_gratis, false) = false AND COALESCE(l.free_trial, false) = false"):n==="rented"&&p.push("lr.lesson_id IS NOT NULL");const f=s.length>=2?s.split(/\s+/).filter(Boolean).slice(0,6):[];for(const E of f)p.push(`(
        l.title ILIKE $${d}
        OR COALESCE(l.description, '') ILIKE $${d}
        OR COALESCE(l.transcript, '') ILIKE $${d}
        OR m.title ILIKE $${d}
        OR co.title ILIKE $${d}
      )`),a.push(`%${E}%`),d++;const h=f[0]||"";a.push(h);const g=`$${d++}`;a.push(`%${s}%`);const v=`$${d++}`;a.push(c);const m=`$${d++}`;let y="relevance_score DESC, co.title ASC, m.order_index ASC, l.order_index ASC";u==="title"&&(y="l.title ASC"),u==="duration"&&(y="COALESCE(l.duration_minutes, 0) ASC, l.title ASC"),u==="date"&&(y="l.created_at DESC NULLS LAST, l.id DESC");const _=await r.sql(`
      SELECT l.id,
             l.title,
             l.description,
             l.duration_minutes,
...
<body class="bg-gray-50 min-h-screen">
  ...
  <script src="/static/auth.js?v=login-barra-escura-20260914b"><\/script>
...
<body class="bg-gray-50">
  ...
  `));const js=new en,zo=Object.assign({"/src/index.tsx":T});let an=!1;for(const[,e]of Object.entries(zo))e&&(js.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),js.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),an=!0);if(!an)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var ur={exports:{}},dr={exports:{}},De={},pr={},ks;function ln(){if(ks)return pr;ks=1,pr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,i){this.source=s,this.transform=i||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var i;(this.recorded.length>0||s)&&(i=this.recorded.join(""),i==="NULL"&&!s&&(i=null),i!==null&&(i=this.transform(i)),this.entries.push(i),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var i,n,o;for(this.consumeDimensions();!this.isEof();)if(i=this.nextCharacter(),i.value==="{"&&!o)this.dimension++,this.dimension>1&&(n=new e(this.source.substr(this.position-1),this.transform),this.entries.push(n.parse(!0)),this.position+=n.position-2);else if(i.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else i.value==='"'&&!i.escaped?(o&&this.newEntry(!0),o=!o):i.value===","&&!o?this.newEntry():this.record(i.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return pr}var fr,Ms;function cn(){if(Ms)return fr;Ms=1;var e=ln();return fr={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},fr}var mr,Bs;function Vo(){if(Bs)return mr;Bs=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;mr=function(u){if(s.test(u))return Number(u.replace("i","I"));var a=e.exec(u);if(!a)return i(u)||null;var p=!!a[8],d=parseInt(a[1],10);p&&(d=o(d));var f=parseInt(a[2],10)-1,h=a[3],g=parseInt(a[4],10),v=parseInt(a[5],10),m=parseInt(a[6],10),y=a[7];y=y?1e3*parseFloat(y):0;var _,E=n(u);return E!=null?(_=new Date(Date.UTC(d,f,h,g,v,m,y)),l(d)&&_.setUTCFullYear(d),E!==0&&_.setTime(_.getTime()-E)):(_=new Date(d,f,h,g,v,m,y),l(d)&&_.setFullYear(d)),_};function i(c){var u=t.exec(c);if(u){var a=parseInt(u[1],10),p=!!u[4];p&&(a=o(a));var d=parseInt(u[2],10)-1,f=u[3],h=new Date(a,d,f);return l(a)&&h.setFullYear(a),h}}function n(c){if(c.endsWith("+00"))return 0;var u=r.exec(c.split(" ")[1]);if(u){var a=u[1];if(a==="Z")return 0;var p=a==="-"?-1:1,d=parseInt(u[2],10)*3600+parseInt(u[3]||0,10)*60+parseInt(u[4]||0,10);return d*p*1e3}}function o(c){return-(c-1)}function l(c){return c>=0&&c<100}return mr}var hr,Us;function Qo(){if(Us)return hr;Us=1,hr=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var i=arguments[s];for(var n in i)e.call(i,n)&&(r[n]=i[n])}return r}return hr}var gr,Fs;function Yo(){if(Fs)return gr;Fs=1;var e=Qo();gr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,g(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(m){var y=this[m]||0;return m==="seconds"&&this.milliseconds&&(y=(y+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),y+" "+m},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},i=["years","months","days"],n=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=i.map(y,this).join(""),m=n.map(y,this).join("");return"P"+v+"T"+m;function y(_){var E=this[_]||0;return _==="seconds"&&this.milliseconds&&(E=(E+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),E+s[_]}};var o="([+-]?\\d+)",l=o+"\\s+years?",c=o+"\\s+mons?",u=o+"\\s+days?",a="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",p=new RegExp([l,c,u,a].map(function(v){return"("+v+")?"}).join("\\s*")),d={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},f=["hours","minutes","seconds","milliseconds"];function h(v){var m=v+"000000".slice(v.length);return parseInt(m,10)/1e3}function g(v){if(!v)return{};var m=p.exec(v),y=m[8]==="-";return Object.keys(d).reduce(function(_,E){var I=d[E],A=m[I];return!A||(A=E==="milliseconds"?h(A):parseInt(A,10),!A)||(y&&~f.indexOf(E)&&(A*=-1),_[E]=A),_},{})}return gr}var br,Hs;function Xo(){if(Hs)return br;Hs=1;var e=Buffer.from||Buffer;return br=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",i=0;i<r.length;)if(r[i]!=="\\")s+=r[i],++i;else if(/[0-7]{3}/.test(r.substr(i+1,3)))s+=String.fromCharCode(parseInt(r.substr(i+1,3),8)),i+=4;else{for(var n=1;i+n<r.length&&r[i+n]==="\\";)n++;for(var o=0;o<Math.floor(n/2);++o)s+="\\";i+=Math.floor(n/2)*2}return e(s,"binary")},br}var yr,$s;function Jo(){if($s)return yr;$s=1;var e=ln(),t=cn(),r=Vo(),s=Yo(),i=Xo();function n(b){return function(w){return w===null?w:b(w)}}function o(b){return b===null?b:b==="TRUE"||b==="t"||b==="true"||b==="y"||b==="yes"||b==="on"||b==="1"}function l(b){return b?e.parse(b,o):null}function c(b){return parseInt(b,10)}function u(b){return b?e.parse(b,n(c)):null}function a(b){return b?e.parse(b,n(function(x){return y(x).trim()})):null}var p=function(b){if(!b)return null;var x=t.create(b,function(w){return w!==null&&(w=E(w)),w});return x.parse()},d=function(b){if(!b)return null;var x=t.create(b,function(w){return w!==null&&(w=parseFloat(w)),w});return x.parse()},f=function(b){if(!b)return null;var x=t.create(b);return x.parse()},h=function(b){if(!b)return null;var x=t.create(b,function(w){return w!==null&&(w=r(w)),w});return x.parse()},g=function(b){if(!b)return null;var x=t.create(b,function(w){return w!==null&&(w=s(w)),w});return x.parse()},v=function(b){return b?e.parse(b,n(i)):null},m=function(b){return parseInt(b,10)},y=function(b){var x=String(b);return/^\d+$/.test(x)?x:b},_=function(b){return b?e.parse(b,n(JSON.parse)):null},E=function(b){return b[0]!=="("?null:(b=b.substring(1,b.length-1).split(","),{x:parseFloat(b[0]),y:parseFloat(b[1])})},I=function(b){if(b[0]!=="<"&&b[1]!=="(")return null;for(var x="(",w="",C=!1,R=2;R<b.length-1;R++){if(C||(x+=b[R]),b[R]===")"){C=!0;continue}else if(!C)continue;b[R]!==","&&(w+=b[R])}var O=E(x);return O.radius=parseFloat(w),O},A=function(b){b(20,y),b(21,m),b(23,m),b(26,m),b(700,parseFloat),b(701,parseFloat),b(16,o),b(1082,r),b(1114,r),b(1184,r),b(600,E),b(651,f),b(718,I),b(1e3,l),b(1001,v),b(1005,u),b(1007,u),b(1028,u),b(1016,a),b(1017,p),b(1021,d),b(1022,d),b(1231,d),b(1014,f),b(1015,f),b(1008,f),b(1009,f),b(1040,f),b(1041,f),b(1115,h),b(1182,h),b(1185,h),b(1186,s),b(1187,g),b(17,i),b(114,JSON.parse.bind(JSON)),b(3802,JSON.parse.bind(JSON)),b(199,_),b(3807,_),b(3907,f),b(2951,f),b(791,f),b(1183,f),b(1270,f)};return yr={init:A},yr}var vr,Ws;function Go(){if(Ws)return vr;Ws=1;var e=1e6;function t(r){var s=r.readInt32BE(0),i=r.readUInt32BE(4),n="";s<0&&(s=~s+(i===0),i=~i+1>>>0,n="-");var o="",l,c,u,a,p,d;{if(l=s%e,s=s/e>>>0,c=4294967296*l+i,i=c/e>>>0,u=""+(c-e*i),i===0&&s===0)return n+u+o;for(a="",p=6-u.length,d=0;d<p;d++)a+="0";o=a+u+o}{if(l=s%e,s=s/e>>>0,c=4294967296*l+i,i=c/e>>>0,u=""+(c-e*i),i===0&&s===0)return n+u+o;for(a="",p=6-u.length,d=0;d<p;d++)a+="0";o=a+u+o}{if(l=s%e,s=s/e>>>0,c=4294967296*l+i,i=c/e>>>0,u=""+(c-e*i),i===0&&s===0)return n+u+o;for(a="",p=6-u.length,d=0;d<p;d++)a+="0";o=a+u+o}return l=s%e,c=4294967296*l+i,u=""+c%e,n+u+o}return vr=t,vr}var _r,zs;function Ko(){if(zs)return _r;zs=1;var e=Go(),t=function(f,h,g,v,m){g=g||0,v=v||!1,m=m||function(C,R,O){return C*Math.pow(2,O)+R};var y=g>>3,_=function(C){return v?~C&255:C},E=255,I=8-g%8;h<I&&(E=255<<8-h&255,I=h),g&&(E=E>>g%8);var A=0;g%8+h>=8&&(A=m(0,_(f[y])&E,I));for(var b=h+g>>3,x=y+1;x<b;x++)A=m(A,_(f[x]),8);var w=(h+g)%8;return w>0&&(A=m(A,_(f[b])>>8-w,w)),A},r=function(f,h,g){var v=Math.pow(2,g-1)-1,m=t(f,1),y=t(f,g,1);if(y===0)return 0;var _=1,E=function(A,b,x){A===0&&(A=1);for(var w=1;w<=x;w++)_/=2,(b&1<<x-w)>0&&(A+=_);return A},I=t(f,h,g+1,!1,E);return y==Math.pow(2,g+1)-1?I===0?m===0?1/0:-1/0:NaN:(m===0?1:-1)*Math.pow(2,y-v)*I},s=function(f){return t(f,1)==1?-1*(t(f,15,1,!0)+1):t(f,15,1)},i=function(f){return t(f,1)==1?-1*(t(f,31,1,!0)+1):t(f,31,1)},n=function(f){return r(f,23,8)},o=function(f){return r(f,52,11)},l=function(f){var h=t(f,16,32);if(h==49152)return NaN;for(var g=Math.pow(1e4,t(f,16,16)),v=0,m=t(f,16),y=0;y<m;y++)v+=t(f,16,64+16*y)*g,g/=1e4;var _=Math.pow(10,t(f,16,48));return(h===0?1:-1)*Math.round(v*_)/_},c=function(f,h){var g=t(h,1),v=t(h,63,1),m=new Date((g===0?1:-1)*v/1e3+9466848e5);return f||m.setTime(m.getTime()+m.getTimezoneOffset()*6e4),m.usec=v%1e3,m.getMicroSeconds=function(){return this.usec},m.setMicroSeconds=function(y){this.usec=y},m.getUTCMicroSeconds=function(){return this.usec},m},u=function(f){var h=t(f,32);t(f,32,32);for(var g=t(f,32,64),v=96,m=[],y=0;y<h;y++)m[y]=t(f,32,v),v+=32,v+=32;var _=function(I){var A=t(f,32,v);if(v+=32,A==4294967295)return null;var b;if(I==23||I==20)return b=t(f,A*8,v),v+=A*8,b;if(I==25)return b=f.toString(this.encoding,v>>3,(v+=A<<3)>>3),b;console.log("ERROR: ElementType not implemented: "+I)},E=function(I,A){var b=[],x;if(I.length>1){var w=I.shift();for(x=0;x<w;x++)b[x]=E(I,A);I.unshift(w)}else for(x=0;x<I[0];x++)b[x]=_(A);return b};return E(m,g)},a=function(f){return f.toString("utf8")},p=function(f){return f===null?null:t(f,8)>0},d=function(f){f(20,e),f(21,s),f(23,i),f(26,i),f(1700,l),f(700,n),f(701,o),f(16,p),f(1114,c.bind(null,!1)),f(1184,c.bind(null,!0)),f(1e3,u),f(1007,u),f(1016,u),f(1008,u),f(1009,u),f(25,a)};return _r={init:d},_r}var Er,Vs;function Zo(){return Vs||(Vs=1,Er={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),Er}var Qs;function Tt(){if(Qs)return De;Qs=1;var e=Jo(),t=Ko(),r=cn(),s=Zo();De.getTypeParser=o,De.setTypeParser=l,De.arrayParser=r,De.builtins=s;var i={text:{},binary:{}};function n(c){return String(c)}function o(c,u){return u=u||"text",i[u]&&i[u][c]||n}function l(c,u,a){typeof u=="function"&&(a=u,u="text"),i[u][c]=a}return e.init(function(c,u){i.text[c]=u}),t.init(function(c,u){i.binary[c]=u}),De}var Ys;function Ct(){return Ys||(Ys=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=Tt(),i=s.getTypeParser(20,"text"),n=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):i),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):n)})})(dr)),dr.exports}var wr,Xs;function rt(){if(Xs)return wr;Xs=1;const e=Ct(),t=We,{isDate:r}=t.types||t;function s(d){return'"'+d.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function i(d){let f="{";for(let h=0;h<d.length;h++)if(h>0&&(f=f+","),d[h]===null||typeof d[h]>"u")f=f+"NULL";else if(Array.isArray(d[h]))f=f+i(d[h]);else if(ArrayBuffer.isView(d[h])){let g=d[h];if(!(g instanceof Buffer)){const v=Buffer.from(g.buffer,g.byteOffset,g.byteLength);v.length===g.byteLength?g=v:g=v.slice(g.byteOffset,g.byteOffset+g.byteLength)}f+="\\\\x"+g.toString("hex")}else f+=s(n(d[h]));return f=f+"}",f}const n=function(d,f){if(d==null)return null;if(typeof d=="object"){if(d instanceof Buffer)return d;if(ArrayBuffer.isView(d)){const h=Buffer.from(d.buffer,d.byteOffset,d.byteLength);return h.length===d.byteLength?h:h.slice(d.byteOffset,d.byteOffset+d.byteLength)}return r(d)?e.parseInputDatesAsUTC?c(d):l(d):Array.isArray(d)?i(d):o(d,f)}return d.toString()};function o(d,f){if(d&&typeof d.toPostgres=="function"){if(f=f||[],f.indexOf(d)!==-1)throw new Error('circular reference detected while preparing "'+d+'" for query');return f.push(d),n(d.toPostgres(n),f)}return JSON.stringify(d)}function l(d){let f=-d.getTimezoneOffset(),h=d.getFullYear();const g=h<1;g&&(h=Math.abs(h)+1);let v=String(h).padStart(4,"0")+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0")+"T"+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")+":"+String(d.getSeconds()).padStart(2,"0")+"."+String(d.getMilliseconds()).padStart(3,"0");return f<0?(v+="-",f*=-1):v+="+",v+=String(Math.floor(f/60)).padStart(2,"0")+":"+String(f%60).padStart(2,"0"),g&&(v+=" BC"),v}function c(d){let f=d.getUTCFullYear();const h=f<1;h&&(f=Math.abs(f)+1);let g=String(f).padStart(4,"0")+"-"+String(d.getUTCMonth()+1).padStart(2,"0")+"-"+String(d.getUTCDate()).padStart(2,"0")+"T"+String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")+":"+String(d.getUTCSeconds()).padStart(2,"0")+"."+String(d.getUTCMilliseconds()).padStart(3,"0");return g+="+00:00",h&&(g+=" BC"),g}function u(d,f,h){return d=typeof d=="string"?{text:d}:d,f&&(typeof f=="function"?d.callback=f:d.values=f),h&&(d.callback=h),d}return wr={prepareValue:function(f){return n(f)},normalizeQueryConfig:u,escapeIdentifier:function(d){return'"'+d.replace(/"/g,'""')+'"'},escapeLiteral:function(d){let f=!1,h="'";if(d==null||typeof d!="string")return"''";for(let g=0;g<d.length;g++){const v=d[g];v==="'"?h+=v+v:v==="\\"?(h+=v+v,f=!0):h+=v}return h+="'",f===!0&&(h=" E"+h),h}},wr}var mt={exports:{}},xr,Js;function ea(){if(Js)return xr;Js=1;const e=Pi;function t(l){return e.createHash("md5").update(l,"utf-8").digest("hex")}function r(l,c,u){const a=t(c+l);return"md5"+t(Buffer.concat([Buffer.from(a),u]))}function s(l){return e.createHash("sha256").update(l).digest()}function i(l,c){return l=l.replace(/(\D)-/,"$1"),e.createHash(l).update(c).digest()}function n(l,c){return e.createHmac("sha256",l).update(c).digest()}async function o(l,c,u){return e.pbkdf2Sync(l,c,u,32,"sha256")}return xr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:i,hmacSha256:n,md5:t},xr}var Sr,Gs;function ta(){if(Gs)return Sr;Gs=1;const e=Pi;Sr={postgresMd5PasswordHash:o,randomBytes:i,deriveKey:a,sha256:l,hashByName:c,hmacSha256:u,md5:n};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function i(p){return t.getRandomValues(Buffer.alloc(p))}async function n(p){try{return e.createHash("md5").update(p,"utf-8").digest("hex")}catch{const f=typeof p=="string"?s.encode(p):p,h=await r.digest("MD5",f);return Array.from(new Uint8Array(h)).map(g=>g.toString(16).padStart(2,"0")).join("")}}async function o(p,d,f){const h=await n(d+p);return"md5"+await n(Buffer.concat([Buffer.from(h),f]))}async function l(p){return await r.digest("SHA-256",p)}async function c(p,d){return await r.digest(p,d)}async function u(p,d){const f=await r.importKey("raw",p,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",f,s.encode(d))}async function a(p,d,f){const h=await r.importKey("raw",s.encode(p),"PBKDF2",!1,["deriveBits"]),g={name:"PBKDF2",hash:"SHA-256",salt:d,iterations:f};return await r.deriveBits(g,h,256,["deriveBits"])}return Sr}var Ks;function un(){return Ks||(Ks=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?mt.exports=ea():mt.exports=ta()),mt.exports}var Ar,Zs;function ra(){if(Zs)return Ar;Zs=1;function e(n,o){return new Error("SASL channel binding: "+n+" when parsing public certificate "+o.toString("base64"))}function t(n,o){let l=n[o++];if(l<128)return{length:l,index:o};const c=l&127;if(c>4)throw e("bad length",n);l=0;for(let u=0;u<c;u++)l=l<<8|n[o++];return{length:l,index:o}}function r(n,o){if(n[o++]!==6)throw e("non-OID data",n);const{length:l,index:c}=t(n,o);o=c;const u=o+l,a=n[o++];let p=(a/40>>0)+"."+a%40;for(;o<u;){let d=0;for(;o<u;){const f=n[o++];if(d=d<<7|f&127,f<128)break}p+="."+d}return{oid:p,index:o}}function s(n,o){if(n[o++]!==48)throw e("non-sequence data",n);return t(n,o)}function i(n,o){o===void 0&&(o=0),o=s(n,o).index;const{length:l,index:c}=s(n,o);o=c+l,o=s(n,o).index;const{oid:u,index:a}=r(n,o);switch(u){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=a,o=s(n,o).index,n[o++]!==160)throw e("non-tag data",n);o=t(n,o).index,o=s(n,o).index;const{oid:p}=r(n,o);switch(p){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+p,n)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+u,n)}return Ar={signatureAlgorithmHashFromCertificate:i},Ar}var Tr,ei;function sa(){if(ei)return Tr;ei=1;const e=un(),{signatureAlgorithmHashFromCertificate:t}=ra();function r(p,d){const f=["SCRAM-SHA-256"];d&&f.unshift("SCRAM-SHA-256-PLUS");const h=f.find(m=>p.includes(m));if(!h)throw new Error("SASL: Only mechanism(s) "+f.join(" and ")+" are supported");if(h==="SCRAM-SHA-256-PLUS"&&typeof d.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const g=e.randomBytes(18).toString("base64");return{mechanism:h,clientNonce:g,response:(h==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":d?"y":"n")+",,n=*,r="+g,message:"SASLInitialResponse"}}async function s(p,d,f,h){if(p.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(d==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof f!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const g=c(f);if(g.nonce.startsWith(p.clientNonce)){if(g.nonce.length===p.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+p.clientNonce,m="r="+g.nonce+",s="+g.salt+",i="+g.iteration;let y=h?"eSws":"biws";if(p.mechanism==="SCRAM-SHA-256-PLUS"){const S=h.getPeerCertificate().raw;let q=t(S);(q==="MD5"||q==="SHA-1")&&(q="SHA-256");const D=await e.hashByName(q,S);y=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(D)]).toString("base64")}const _="c="+y+",r="+g.nonce,E=v+","+m+","+_,I=Buffer.from(g.salt,"base64"),A=await e.deriveKey(d,I,g.iteration),b=await e.hmacSha256(A,"Client Key"),x=await e.sha256(b),w=await e.hmacSha256(x,E),C=a(Buffer.from(b),Buffer.from(w)).toString("base64"),R=await e.hmacSha256(A,"Server Key"),O=await e.hmacSha256(R,E);p.message="SASLResponse",p.serverSignature=Buffer.from(O).toString("base64"),p.response=_+",p="+C}function i(p,d){if(p.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:f}=u(d);if(f!==p.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function n(p){if(typeof p!="string")throw new TypeError("SASL: text must be a string");return p.split("").map((d,f)=>p.charCodeAt(f)).every(d=>d>=33&&d<=43||d>=45&&d<=126)}function o(p){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(p)}function l(p){if(typeof p!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(p.split(",").map(d=>{if(!/^.=/.test(d))throw new Error("SASL: Invalid attribute pair entry");const f=d[0],h=d.substring(2);return[f,h]}))}function c(p){const d=l(p),f=d.get("r");if(f){if(!n(f))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const h=d.get("s");if(h){if(!o(h))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const g=d.get("i");if(g){if(!/^[1-9][0-9]*$/.test(g))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(g,10);return{nonce:f,salt:h,iteration:v}}function u(p){const f=l(p).get("v");if(f){if(!o(f))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:f}}function a(p,d){if(!Buffer.isBuffer(p))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(d))throw new TypeError("second argument must be a Buffer");if(p.length!==d.length)throw new Error("Buffer lengths must match");if(p.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(p.map((f,h)=>p[h]^d[h]))}return Tr={startSession:r,continueSession:s,finalizeSession:i},Tr}var Cr,ti;function Kr(){if(ti)return Cr;ti=1;const e=Tt();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,i){typeof s=="function"&&(i=s,s="text"),this.getOverrides(s)[r]=i},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},Cr=t,Cr}var Rr,ri;function ia(){if(ri)return Rr;ri=1;function e(n,o={}){if(n.charAt(0)==="/"){const f=n.split(" ");return{host:f[0],database:f[1]}}const l={};let c,u=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(n)&&(n=encodeURI(n).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(n,"postgres://base")}catch{c=new URL(n.replace("@/","@___DUMMY___/"),"postgres://base"),u=!0}}catch(f){throw f.input&&(f.input="*****REDACTED*****"),f}for(const f of c.searchParams.entries())l[f[0]]=f[1];if(l.user=l.user||decodeURIComponent(c.username),l.password=l.password||decodeURIComponent(c.password),c.protocol=="socket:")return l.host=decodeURI(c.pathname),l.database=c.searchParams.get("db"),l.client_encoding=c.searchParams.get("encoding"),l;const a=u?"":c.hostname;l.host?a&&/^%2f/i.test(a)&&(c.pathname=a+c.pathname):l.host=decodeURIComponent(a),l.port||(l.port=c.port);const p=c.pathname.slice(1)||null;l.database=p?decodeURI(p):null,(l.ssl==="true"||l.ssl==="1")&&(l.ssl=!0),l.ssl==="0"&&(l.ssl=!1),(l.sslcert||l.sslkey||l.sslrootcert||l.sslmode)&&(l.ssl={});const d=l.sslcert||l.sslkey||l.sslrootcert?Di:null;if(l.sslcert&&(l.ssl.cert=d.readFileSync(l.sslcert).toString()),l.sslkey&&(l.ssl.key=d.readFileSync(l.sslkey).toString()),l.sslrootcert&&(l.ssl.ca=d.readFileSync(l.sslrootcert).toString()),o.useLibpqCompat&&l.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(l.uselibpqcompat==="true"||o.useLibpqCompat)switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":{l.ssl.rejectUnauthorized=!1;break}case"require":{l.sslrootcert?l.ssl.checkServerIdentity=function(){}:l.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!l.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");l.ssl.checkServerIdentity=function(){};break}}else switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{l.sslmode!=="verify-full"&&i(l.sslmode);break}case"no-verify":{l.ssl.rejectUnauthorized=!1;break}}return l}function t(n){return Object.entries(n).reduce((l,[c,u])=>(u!=null&&(l[c]=u),l),{})}function r(n){return Object.entries(n).reduce((l,[c,u])=>{if(c==="ssl"){const a=u;typeof a=="boolean"&&(l[c]=a),typeof a=="object"&&(l[c]=t(a))}else if(u!=null)if(c==="port"){if(u!==""){const a=parseInt(u,10);if(isNaN(a))throw new Error(`Invalid ${c}: ${u}`);l[c]=a}}else l[c]=u;return l},{})}function s(n){return r(e(n))}function i(n){!i.warned&&typeof process<"u"&&process.emitWarning&&(i.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
