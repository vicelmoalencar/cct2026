...
---
${i}
---`,l=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:c},{role:"user",content:u}]})});if(!l.ok)return e.json({error:`OpenRouter: ${await l.text()}`},500);const f=await l.json();return e.json({transcript:((s=(r=(t=f.choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||""})}catch(n){return e.json({error:n.message},500)}});R.post("/api/admin/vimeo-transcript",q,async e=>{try{const{video_id:t}=await e.req.json();if(!t)return e.json({error:"video_id obrigatório"},400);const r=e.env.VIMEO_ACCESS_TOKEN;if(!r)return e.json({error:"VIMEO_ACCESS_TOKEN não configurada"},500);const s=String(t).replace(/^https?:\/\/(?:www\.)?vimeo\.com\//i,"").replace(/[?#].*$/,"").replace(/\/$/,""),n=await fetch(`https://api.vimeo.com/videos/${s}/texttracks?per_page=100`,{headers:{Authorization:`Bearer ${r}`,Accept:"application/vnd.vimeo.*+json;version=3.4"}});if(!n.ok)return e.json({error:`Vimeo API ${n.status}`},502);const o=(await n.json()).data||[];if(!o.length)return e.json({error:"Nenhuma legenda encontrada para este vídeo"},404);const c=[...o].sort((g,y)=>{const v=h=>{let b=0;return String(h.language||"").toLowerCase().startsWith("pt")&&(b+=40),h.active!==!1&&(b+=20),h.type==="captions"&&(b+=10),b};return v(y)-v(g)}).find(g=>g.link)||null;if(!c)return e.json({error:"Nenhuma legenda com link disponível"},404);const u=await fetch(c.link);if(!u.ok)return e.json({error:`Download da legenda falhou: ${u.status}`},502);const f=(await u.text()).replace(/^﻿/,"").split(/\r?\n/).map(g=>g.trim()),d=[];for(const g of f){if(!g||/^(WEBVTT|Kind:|Language:|NOTE|STYLE|REGION|\d+$)/.test(g)||g.includes("-->"))continue;const y=g.replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim();y&&d[d.length-1]!==y&&d.push(y)}const p=d.join(" ").replace(/\s+/g," ").trim();return p?e.json({transcript:p,language:c.language}):e.json({error:"Legenda encontrada mas está vazia"},404)}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/lessons/:id/whisper-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=L(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.OPENAI_API_KEY||u.OPENAI_API_KEY||u["OPENAI_API-KEY"],d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const x=Object.keys(u).filter(O=>O.toLowerCase().includes("openai"));return e.json({error:`OPENAI_API_KEY não configurado. Chaves OPENAI encontradas: [${x.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const y=((await p.json()).download||[]).filter(x=>x.link&&x.size).sort((x,O)=>x.size-O.size);if(!y.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=y[0],h=(v.size/1024/1024).toFixed(1),b=await fetch(v.link);if(!b.ok)return e.json({error:`Download falhou: HTTP ${b.status}`},502);let w=Buffer.from(await b.arrayBuffer()),S="video.mp4";const P=24*1024*1024;if(w.length>P){try{const x=await import("os"),O=await import("path"),D=await import("fs"),M=await import("child_process"),F=O.join(x.tmpdir(),`cct_w_${i}.mp4`),z=O.join(x.tmpdir(),`cct_w_${i}.mp3`);D.writeFileSync(F,w),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&D.existsSync(z)&&(w=D.readFileSync(z),S="audio.mp3",D.unlinkSync(z)),D.unlinkSync(F)}catch{}if(w.length>P)return e.json({error:`Arquivo muito grande (${h} MB). Use o script de transcrição em lote.`},400)}const A="----WhisperBoundary"+Date.now().toString(16),m=`\r
`,_=S.endsWith(".mp3")?"audio/mpeg":"video/mp4",E=Buffer.concat([Buffer.from(`--${A}${m}Content-Disposition: form-data; name="file"; filename="${S}"${m}Content-Type: ${_}${m}${m}`),w,Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="model"${m}${m}whisper-1`),Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="language"${m}${m}pt`),Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="response_format"${m}${m}text`),Buffer.from(`${m}--${A}--${m}`)]),T=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(E.length)},body:E});if(!T.ok)return e.json({error:`OpenAI Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),N=null;if(d&&C){const x=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,O=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:x}]})});if(O.ok){const M=((n=(s=(r=(await O.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(N=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,N,i]),e.json({transcript:C,description:N,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});R.post("/api/admin/lessons/:id/groq-transcribe",q,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=L(e),a=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!a.length)return e.json({error:"Aula não encontrada"},404);const c=a[0];if(c.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!c.video_id||!/^\d+$/.test(c.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const u=((t=globalThis.process)==null?void 0:t.env)||{},l=e.env.VIMEO_ACCESS_TOKEN||u.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||u.GROQ_API_KEY,d=e.env.VITE_OPENROUTER_API_KEY||u.VITE_OPENROUTER_API_KEY;if(!l)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const x=Object.keys(u).filter(O=>O.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${x.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${c.video_id}?fields=download`,{headers:{Authorization:`Bearer ${l}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const y=((await p.json()).download||[]).filter(x=>x.link&&x.size).sort((x,O)=>x.size-O.size);if(!y.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=y[0],h=(v.size/1024/1024).toFixed(1),b=await fetch(v.link);if(!b.ok)return e.json({error:`Download falhou: HTTP ${b.status}`},502);let w=Buffer.from(await b.arrayBuffer()),S="video.mp4";const P=24*1024*1024;if(w.length>P){try{const x=await import("os"),O=await import("path"),D=await import("fs"),M=await import("child_process"),F=O.join(x.tmpdir(),`cct_g_${i}.mp4`),z=O.join(x.tmpdir(),`cct_g_${i}.mp3`);D.writeFileSync(F,w),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",z],{stdio:"pipe"}).status===0&&D.existsSync(z)&&(w=D.readFileSync(z),S="audio.mp3",D.unlinkSync(z)),D.unlinkSync(F)}catch{}if(w.length>P)return e.json({error:`Arquivo muito grande (${h} MB). Use o script de transcrição em lote.`},400)}const A="----GroqBoundary"+Date.now().toString(16),m=`\r
`,_=S.endsWith(".mp3")?"audio/mpeg":"video/mp4",E=Buffer.concat([Buffer.from(`--${A}${m}Content-Disposition: form-data; name="file"; filename="${S}"${m}Content-Type: ${_}${m}${m}`),w,Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="model"${m}${m}whisper-large-v3-turbo`),Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="language"${m}${m}pt`),Buffer.from(`${m}--${A}${m}Content-Disposition: form-data; name="response_format"${m}${m}text`),Buffer.from(`${m}--${A}--${m}`)]),T=await fetch("https://api.groq.com/openai/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(E.length)},body:E});if(!T.ok)return e.json({error:`Groq Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),N=null;if(d&&C){const x=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,O=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:x}]})});if(O.ok){const M=((n=(s=(r=(await O.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(N=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,N,i]),e.json({transcript:C,description:N,sizeMB:h})}catch(i){return e.json({error:i.message},500)}});R.delete("/api/admin/lessons/:id",q,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});R.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=L(e);await $r(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],a=parseInt(o.rental_credits);if(!Number.isFinite(a)||a<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const c=await n.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(c.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:c[0].expires_at},400);const u=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!u,"| prefix:",u==null?void 0:u.substring(0,30));const l=ni(e),f=await Fr(l,s);if(f<a)return e.json({error:"Créditos insuficientes",available:f,required:a},400);if(!await Po(l,s,a)){const p=await Fr(l,s);return e.json({error:"Créditos insuficientes",available:p,required:a},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}R.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=L(e);await Ie(r);const s=await r.sql(`
      SELECT t.*,
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});R.get("/api/admin/trails",q,async e=>{try{const t=L(e);await Ie(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails",q,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=L(e);await Ie(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});R.put("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=L(e);await Ie(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/trails/:id",q,async e=>{try{const t=parseInt(e.req.param("id")),r=L(e);return await Ie(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/lessons",q,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=L(e);await Ie(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});R.delete("/api/admin/trails/:id/lessons/:lessonId",q,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await L(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/reorder",q,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=L(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/trails/search-lessons",q,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=L(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],a=await s.sql(`
    ...
    `,[`${i.title||"Questao"} (copia)`,i.statement_html,i.question_type,JSON.stringify(i.alternatives||[]),JSON.stringify(i.answer_key||{}),i.technical_comment_html,i.difficulty,i.theme,i.subtheme,i.legal_basis,i.weight,i.estimated_minutes,JSON.stringify(i.tags||[]),i.professor,i.course_id,i.lesson_id,i.source_transcript,i.ai_generated,i.order_index+1,(t==null?void 0:t.email)||null]);return e.json({success:!0,question:o[0]})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/questions/:id",q,async e=>{try{const t=L(e);return await ie(t),await t.sql("DELETE FROM question_bank WHERE id = $1",[parseInt(e.req.param("id"))]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions-reorder",q,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions must be an array"},400);const r=L(e);await ie(r);for(const s of t)await r.sql("UPDATE question_bank SET order_index = $1, updated_at = NOW() WHERE id = $2",[parseInt(s.order_index),parseInt(s.id)]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/import",q,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions array required"},400);const r=e.get("user"),s=L(e);await ie(s);let n=0;for(const i of t){const o=Xr(i,r==null?void 0:r.email);await s.sql(`
      ...
      `,[o.title,o.statement_html,o.question_type,o.alternatives,o.answer_key,o.technical_comment_html,o.difficulty,o.theme,o.subtheme,o.legal_basis,o.weight,o.estimated_minutes,o.tags,o.status,o.professor,o.course_id,o.lesson_id,o.source_transcript,o.ai_generated,o.order_index,o.created_by]),n++}return e.json({success:!0,created:n})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/generate-ai",q,async e=>{var t,r,s;try{const{lesson_id:n,transcript:i,count:o=5,types:a=["multiple_choice","true_false","discursive"],difficulty:c="misto",context:u=""}=await e.req.json(),l=L(e);await ie(l);let f=i||"",d=null;if(n&&(d=(await l.sql(`
...
---
${f.slice(0,24e3)}
---`,y=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${p}`,"HTTP-Referer":"https://cct2026.com.br","X-Title":"CCT2026 Admin"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"system",content:"Voce e especialista em direito do trabalho, calculos trabalhistas, PJe-Calc e avaliacao educacional. Responda apenas JSON valido."},{role:"user",content:g}],response_format:{type:"json_object"}})});if(!y.ok)return e.json({error:`OpenRouter: ${await y.text()}`},500);const h=((s=(r=(t=(await y.json()).choices)==null?void 0:t[0])==null?void 0:r.message)==null?void 0:s.content)||'{"questions":[]}',w=(JSON.parse(h).questions||[]).map(S=>({...Hr(S),lesson_id:(d==null?void 0:d.id)||n||null,course_id:(d==null?void 0:d.course_id)||null,professor:(d==null?void 0:d.instructor)||null,source_transcript:f,ai_generated:!0,status:"review"}));return e.json({questions:w})}catch(n){return console.error("Generate questions error:",n),e.json({error:n.message||"Erro ao gerar questoes"},500)}});R.post("/api/admin/run-migration-lesson-fields",q,async e=>{try{const t=L(e);return await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS support_text TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS transcript TEXT"),await t.sql("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'::jsonb"),await $r(t),e.json({success:!0,message:"Migration applied successfully"})}catch(t){return console.error("Migration error:",t),e.json({error:t.message},500)}});R.get("/api/admin/users",q,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `);return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});R.get("/api/admin/users/find",q,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await L(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});R.post("/api/admin/users",q,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await L(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});R.put("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await L(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});R.delete("/api/admin/users/:id",q,async e=>{try{const t=e.req.param("id");return await L(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});R.get("/api/admin/certificates",q,async e=>{try{const r=await L(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});R.get("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),s=await L(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});R.get("/api/admin/certificates/find",q,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await L(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});R.post("/api/admin/certificates",q,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],n=L(e),i=[];for(const a of s){const c=h=>{const b=String(h??"").trim();return!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined"?"":b},u=c(a.user_email||a.email).toLowerCase(),l=c(a.user_name||a.nome)||null,f=c(a.course_title||a.curso);let d=a.carga_horaria?parseInt(String(a.carga_horaria)):null;if(d!==null&&!Number.isFinite(d)&&(d=null),!u.includes("@")||!f){i.push({user_email:u,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=h=>{if(!h)return null;const b=String(h).trim();if(!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(b)){const[w,S,P]=b.split("/");return`${P}-${S}-${w}T12:00:00Z`}return isNaN(new Date(b).getTime())?null:new Date(b).toISOString()},g=p(a.data_final||a.completion_date),y=p(a.data_inicio||a.data_inicial||a.start_date);if(!d){const h=await n.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(h.length>0&&h[0].duration_hours)d=parseInt(h[0].duration_hours);else{const b=S=>S.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(P=>P&&P!=="de"&&P!=="do"&&P!=="da"),w=b(f);if(w.length>0){const P=(await n.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const m=b(A.title).join(" ");return w.every(_=>m.includes(_))});P&&(d=parseInt(P.duration_hours))}}}const v=await n.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[u,f]);if(v.length>0)await n.sql(`UPDATE certificates
           SET user_name = COALESCE($1, user_name),
               ...
               updated_at = NOW()
           WHERE id = $5`,[l,d,g,y,v[0].id]),i.push({user_email:u,course_title:f,status:"updated",certificate_id:v[0].id});else{const h=new Uint8Array(4);crypto.getRandomValues(h);const b="CCT-"+new Date().getFullYear()+"-"+Array.from(h).map(P=>P.toString(16).padStart(2,"0")).join("").toUpperCase(),w=new Date().toISOString(),S=await n.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, start_date, created_at)
           VALUES ($1, $2, $3, $4, $5, $5, $6, COALESCE($7, $6::timestamp), $8::date, $6)
           RETURNING id`,[u,l||"Aluno",f,d,b,w,g,y]);i.push({user_email:u,course_title:f,status:"created",certificate_id:(t=S[0])==null?void 0:t.id,verification_code:b})}}const o=i.filter(a=>a.status==="error").length;return e.json({success:o===0,total:s.length,results:i})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",q,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Lo(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const u=s?e.modules.map((f,d)=>`<div class="mod-item"><span class="mod-num">${String(d+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",l=r?`
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.query("modules",{select:"*",filters:{course_id:t},order:"order_index"}),i=await r.sql(`SELECT l.* FROM lessons l
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const a of i){const c=o.get(a.module_id)||[];c.push(a),o.set(a.module_id,c)}for(const a of n)a.lessons=o.get(a.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,n;try{const i=e.req.param("id"),o=$(e,"sb-access-token");let a=null;if(o){const h=await Y(o,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);h&&(a=h.email)}const c=L(e);let u=!1,l=!1;if(a)try{if(u=!!((t=(await c.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[a,parseInt(i)]))[0])!=null&&t.has_access),console.log("Has access:",u,"User:",a,"Lesson:",i),!u){const b=e.env.DATABASE_SUITEPLUS,[w,S,P]=await Promise.all([c.sql("SELECT expires_at FROM lesson_rentals WHERE user_email = $1 AND lesson_id = $2 AND expires_at > NOW()",[a,parseInt(i)]),c.sql("SELECT id FROM member_subscriptions WHERE email_membro = $1 AND data_expiracao > NOW() AND ativo = true LIMIT 1",[a]),b?xt(a,b):Promise.resolve(null)]);if((w.length>0||S.length>0||P&&P>new Date)&&(u=!0),!u){const A=await c.sql("SELECT rentable, rental_credits, title FROM lessons WHERE id = $1",[parseInt(i)]);return console.log("❌ Access denied for user:",a,"lesson:",i),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((r=A[0])==null?void 0:r.rentable)||!1,rental_credits:((s=A[0])==null?void 0:s.rental_credits)||0,lesson_title:((n=A[0])==null?void 0:n.title)||""},403)}}console.log("✅ Access granted for user:",a,"lesson:",i)}catch(h){console.error("❌ Error checking access via RPC:",h),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),l=!0,u=!0}if(!a||!u&&!l){const h=await c.query("lessons",{select:"teste_gratis",filters:{id:i},single:!0});if(!(h!=null&&h.teste_gratis))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await Et(c);const f=`
...
<body class="bg-gray-50 min-h-screen">
  ...
  <\/script>
    ...
    <body class="bg-gray-50">
        ...
        <script defer src="/static/app.js?v=19"><\/script>
...
<body class="bg-gray-50">
  ...
  `));const Ls=new Kn,qo=Object.assign({"/src/index.tsx":R});let oi=!1;for(const[,e]of Object.entries(qo))e&&(Ls.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ls.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),oi=!0);if(!oi)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var ar={exports:{}},lr={exports:{}},Le={},cr={},qs;function ai(){if(qs)return cr;qs=1,cr.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return cr}var ur,Ds;function li(){if(Ds)return ur;Ds=1;var e=ai();return ur={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},ur}var dr,js;function Do(){if(js)return dr;js=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;dr=function(u){if(s.test(u))return Number(u.replace("i","I"));var l=e.exec(u);if(!l)return n(u)||null;var f=!!l[8],d=parseInt(l[1],10);f&&(d=o(d));var p=parseInt(l[2],10)-1,g=l[3],y=parseInt(l[4],10),v=parseInt(l[5],10),h=parseInt(l[6],10),b=l[7];b=b?1e3*parseFloat(b):0;var w,S=i(u);return S!=null?(w=new Date(Date.UTC(d,p,g,y,v,h,b)),a(d)&&w.setUTCFullYear(d),S!==0&&w.setTime(w.getTime()-S)):(w=new Date(d,p,g,y,v,h,b),a(d)&&w.setFullYear(d)),w};function n(c){var u=t.exec(c);if(u){var l=parseInt(u[1],10),f=!!u[4];f&&(l=o(l));var d=parseInt(u[2],10)-1,p=u[3],g=new Date(l,d,p);return a(l)&&g.setFullYear(l),g}}function i(c){if(c.endsWith("+00"))return 0;var u=r.exec(c.split(" ")[1]);if(u){var l=u[1];if(l==="Z")return 0;var f=l==="-"?-1:1,d=parseInt(u[2],10)*3600+parseInt(u[3]||0,10)*60+parseInt(u[4]||0,10);return d*f*1e3}}function o(c){return-(c-1)}function a(c){return c>=0&&c<100}return dr}var fr,ks;function jo(){if(ks)return fr;ks=1,fr=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return fr}var pr,Bs;function ko(){if(Bs)return pr;Bs=1;var e=jo();pr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,y(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(h){var b=this[h]||0;return h==="seconds"&&this.milliseconds&&(b=(b+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),b+" "+h},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(b,this).join(""),h=i.map(b,this).join("");return"P"+v+"T"+h;function b(w){var S=this[w]||0;return w==="seconds"&&this.milliseconds&&(S=(S+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),S+s[w]}};var o="([+-]?\\d+)",a=o+"\\s+years?",c=o+"\\s+mons?",u=o+"\\s+days?",l="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([a,c,u,l].map(function(v){return"("+v+")?"}).join("\\s*")),d={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function g(v){var h=v+"000000".slice(v.length);return parseInt(h,10)/1e3}function y(v){if(!v)return{};var h=f.exec(v),b=h[8]==="-";return Object.keys(d).reduce(function(w,S){var P=d[S],A=h[P];return!A||(A=S==="milliseconds"?g(A):parseInt(A,10),!A)||(b&&~p.indexOf(S)&&(A*=-1),w[S]=A),w},{})}return pr}var hr,Ms;function Bo(){if(Ms)return hr;Ms=1;var e=Buffer.from||Buffer;return hr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},hr}var mr,Us;function Mo(){if(Us)return mr;Us=1;var e=ai(),t=li(),r=Do(),s=ko(),n=Bo();function i(m){return function(E){return E===null?E:m(E)}}function o(m){return m===null?m:m==="TRUE"||m==="t"||m==="true"||m==="y"||m==="yes"||m==="on"||m==="1"}function a(m){return m?e.parse(m,o):null}function c(m){return parseInt(m,10)}function u(m){return m?e.parse(m,i(c)):null}function l(m){return m?e.parse(m,i(function(_){return b(_).trim()})):null}var f=function(m){if(!m)return null;var _=t.create(m,function(E){return E!==null&&(E=S(E)),E});return _.parse()},d=function(m){if(!m)return null;var _=t.create(m,function(E){return E!==null&&(E=parseFloat(E)),E});return _.parse()},p=function(m){if(!m)return null;var _=t.create(m);return _.parse()},g=function(m){if(!m)return null;var _=t.create(m,function(E){return E!==null&&(E=r(E)),E});return _.parse()},y=function(m){if(!m)return null;var _=t.create(m,function(E){return E!==null&&(E=s(E)),E});return _.parse()},v=function(m){return m?e.parse(m,i(n)):null},h=function(m){return parseInt(m,10)},b=function(m){var _=String(m);return/^\d+$/.test(_)?_:m},w=function(m){return m?e.parse(m,i(JSON.parse)):null},S=function(m){return m[0]!=="("?null:(m=m.substring(1,m.length-1).split(","),{x:parseFloat(m[0]),y:parseFloat(m[1])})},P=function(m){if(m[0]!=="<"&&m[1]!=="(")return null;for(var _="(",E="",T=!1,C=2;C<m.length-1;C++){if(T||(_+=m[C]),m[C]===")"){T=!0;continue}else if(!T)continue;m[C]!==","&&(E+=m[C])}var N=S(_);return N.radius=parseFloat(E),N},A=function(m){m(20,b),m(21,h),m(23,h),m(26,h),m(700,parseFloat),m(701,parseFloat),m(16,o),m(1082,r),m(1114,r),m(1184,r),m(600,S),m(651,p),m(718,P),m(1e3,a),m(1001,v),m(1005,u),m(1007,u),m(1028,u),m(1016,l),m(1017,f),m(1021,d),m(1022,d),m(1231,d),m(1014,p),m(1015,p),m(1008,p),m(1009,p),m(1040,p),m(1041,p),m(1115,g),m(1182,g),m(1185,g),m(1186,s),m(1187,y),m(17,n),m(114,JSON.parse.bind(JSON)),m(3802,JSON.parse.bind(JSON)),m(199,w),m(3807,w),m(3907,p),m(2951,p),m(791,p),m(1183,p),m(1270,p)};return mr={init:A},mr}var gr,Fs;function Uo(){if(Fs)return gr;Fs=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",a,c,u,l,f,d;{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}{if(a=s%e,s=s/e>>>0,c=4294967296*a+n,n=c/e>>>0,u=""+(c-e*n),n===0&&s===0)return i+u+o;for(l="",f=6-u.length,d=0;d<f;d++)l+="0";o=l+u+o}return a=s%e,c=4294967296*a+n,u=""+c%e,i+u+o}return gr=t,gr}var br,Hs;function Fo(){if(Hs)return br;Hs=1;var e=Uo(),t=function(p,g,y,v,h){y=y||0,v=v||!1,h=h||function(T,C,N){return T*Math.pow(2,N)+C};var b=y>>3,w=function(T){return v?~T&255:T},S=255,P=8-y%8;g<P&&(S=255<<8-g&255,P=g),y&&(S=S>>y%8);var A=0;y%8+g>=8&&(A=h(0,w(p[b])&S,P));for(var m=g+y>>3,_=b+1;_<m;_++)A=h(A,w(p[_]),8);var E=(g+y)%8;return E>0&&(A=h(A,w(p[m])>>8-E,E)),A},r=function(p,g,y){var v=Math.pow(2,y-1)-1,h=t(p,1),b=t(p,y,1);if(b===0)return 0;var w=1,S=function(A,m,_){A===0&&(A=1);for(var E=1;E<=_;E++)w/=2,(m&1<<_-E)>0&&(A+=w);return A},P=t(p,g,y+1,!1,S);return b==Math.pow(2,y+1)-1?P===0?h===0?1/0:-1/0:NaN:(h===0?1:-1)*Math.pow(2,b-v)*P},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},a=function(p){var g=t(p,16,32);if(g==49152)return NaN;for(var y=Math.pow(1e4,t(p,16,16)),v=0,h=t(p,16),b=0;b<h;b++)v+=t(p,16,64+16*b)*y,y/=1e4;var w=Math.pow(10,t(p,16,48));return(g===0?1:-1)*Math.round(v*w)/w},c=function(p,g){var y=t(g,1),v=t(g,63,1),h=new Date((y===0?1:-1)*v/1e3+9466848e5);return p||h.setTime(h.getTime()+h.getTimezoneOffset()*6e4),h.usec=v%1e3,h.getMicroSeconds=function(){return this.usec},h.setMicroSeconds=function(b){this.usec=b},h.getUTCMicroSeconds=function(){return this.usec},h},u=function(p){var g=t(p,32);t(p,32,32);for(var y=t(p,32,64),v=96,h=[],b=0;b<g;b++)h[b]=t(p,32,v),v+=32,v+=32;var w=function(P){var A=t(p,32,v);if(v+=32,A==4294967295)return null;var m;if(P==23||P==20)return m=t(p,A*8,v),v+=A*8,m;if(P==25)return m=p.toString(this.encoding,v>>3,(v+=A<<3)>>3),m;console.log("ERROR: ElementType not implemented: "+P)},S=function(P,A){var m=[],_;if(P.length>1){var E=P.shift();for(_=0;_<E;_++)m[_]=S(P,A);P.unshift(E)}else for(_=0;_<P[0];_++)m[_]=w(A);return m};return S(h,y)},l=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},d=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,a),p(700,i),p(701,o),p(16,f),p(1114,c.bind(null,!1)),p(1184,c.bind(null,!0)),p(1e3,u),p(1007,u),p(1016,u),p(1008,u),p(1009,u),p(25,l)};return br={init:d},br}var yr,zs;function Ho(){return zs||(zs=1,yr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),yr}var Vs;function _t(){if(Vs)return Le;Vs=1;var e=Mo(),t=Fo(),r=li(),s=Ho();Le.getTypeParser=o,Le.setTypeParser=a,Le.arrayParser=r,Le.builtins=s;var n={text:{},binary:{}};function i(c){return String(c)}function o(c,u){return u=u||"text",n[u]&&n[u][c]||i}function a(c,u,l){typeof u=="function"&&(l=u,u="text"),n[u][c]=l}return e.init(function(c,u){n.text[c]=u}),t.init(function(c,u){n.binary[c]=u}),Le}var Qs;function St(){return Qs||(Qs=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=_t(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(lr)),lr.exports}var vr,$s;function et(){if($s)return vr;$s=1;const e=St(),t=He,{isDate:r}=t.types||t;function s(d){return'"'+d.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(d){let p="{";for(let g=0;g<d.length;g++)if(g>0&&(p=p+","),d[g]===null||typeof d[g]>"u")p=p+"NULL";else if(Array.isArray(d[g]))p=p+n(d[g]);else if(ArrayBuffer.isView(d[g])){let y=d[g];if(!(y instanceof Buffer)){const v=Buffer.from(y.buffer,y.byteOffset,y.byteLength);v.length===y.byteLength?y=v:y=v.slice(y.byteOffset,y.byteOffset+y.byteLength)}p+="\\\\x"+y.toString("hex")}else p+=s(i(d[g]));return p=p+"}",p}const i=function(d,p){if(d==null)return null;if(typeof d=="object"){if(d instanceof Buffer)return d;if(ArrayBuffer.isView(d)){const g=Buffer.from(d.buffer,d.byteOffset,d.byteLength);return g.length===d.byteLength?g:g.slice(d.byteOffset,d.byteOffset+d.byteLength)}return r(d)?e.parseInputDatesAsUTC?c(d):a(d):Array.isArray(d)?n(d):o(d,p)}return d.toString()};function o(d,p){if(d&&typeof d.toPostgres=="function"){if(p=p||[],p.indexOf(d)!==-1)throw new Error('circular reference detected while preparing "'+d+'" for query');return p.push(d),i(d.toPostgres(i),p)}return JSON.stringify(d)}function a(d){let p=-d.getTimezoneOffset(),g=d.getFullYear();const y=g<1;y&&(g=Math.abs(g)+1);let v=String(g).padStart(4,"0")+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0")+"T"+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")+":"+String(d.getSeconds()).padStart(2,"0")+"."+String(d.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),y&&(v+=" BC"),v}function c(d){let p=d.getUTCFullYear();const g=p<1;g&&(p=Math.abs(p)+1);let y=String(p).padStart(4,"0")+"-"+String(d.getUTCMonth()+1).padStart(2,"0")+"-"+String(d.getUTCDate()).padStart(2,"0")+"T"+String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")+":"+String(d.getUTCSeconds()).padStart(2,"0")+"."+String(d.getUTCMilliseconds()).padStart(3,"0");return y+="+00:00",g&&(y+=" BC"),y}function u(d,p,g){return d=typeof d=="string"?{text:d}:d,p&&(typeof p=="function"?d.callback=p:d.values=p),g&&(d.callback=g),d}return vr={prepareValue:function(p){return i(p)},normalizeQueryConfig:u,escapeIdentifier:function(d){return'"'+d.replace(/"/g,'""')+'"'},escapeLiteral:function(d){let p=!1,g="'";if(d==null||typeof d!="string")return"''";for(let y=0;y<d.length;y++){const v=d[y];v==="'"?g+=v+v:v==="\\"?(g+=v+v,p=!0):g+=v}return g+="'",p===!0&&(g=" E"+g),g}},vr}var ut={exports:{}},wr,Ws;function zo(){if(Ws)return wr;Ws=1;const e=On;function t(a){return e.createHash("md5").update(a,"utf-8").digest("hex")}function r(a,c,u){const l=t(c+a);return"md5"+t(Buffer.concat([Buffer.from(l),u]))}function s(a){return e.createHash("sha256").update(a).digest()}function n(a,c){return a=a.replace(/(\D)-/,"$1"),e.createHash(a).update(c).digest()}function i(a,c){return e.createHmac("sha256",a).update(c).digest()}async function o(a,c,u){return e.pbkdf2Sync(a,c,u,32,"sha256")}return wr={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},wr}var Er,Xs;function Vo(){if(Xs)return Er;Xs=1;const e=On;Er={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:l,sha256:a,hashByName:c,hmacSha256:u,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,g=await r.digest("MD5",p);return Array.from(new Uint8Array(g)).map(y=>y.toString(16).padStart(2,"0")).join("")}}async function o(f,d,p){const g=await i(d+f);return"md5"+await i(Buffer.concat([Buffer.from(g),p]))}async function a(f){return await r.digest("SHA-256",f)}async function c(f,d){return await r.digest(f,d)}async function u(f,d){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(d))}async function l(f,d,p){const g=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),y={name:"PBKDF2",hash:"SHA-256",salt:d,iterations:p};return await r.deriveBits(y,g,256,["deriveBits"])}return Er}var Gs;function ci(){return Gs||(Gs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?ut.exports=zo():ut.exports=Vo()),ut.exports}var xr,Js;function Qo(){if(Js)return xr;Js=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let a=i[o++];if(a<128)return{length:a,index:o};const c=a&127;if(c>4)throw e("bad length",i);a=0;for(let u=0;u<c;u++)a=a<<8|i[o++];return{length:a,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:a,index:c}=t(i,o);o=c;const u=o+a,l=i[o++];let f=(l/40>>0)+"."+l%40;for(;o<u;){let d=0;for(;o<u;){const p=i[o++];if(d=d<<7|p&127,p<128)break}f+="."+d}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:a,index:c}=s(i,o);o=c+a,o=s(i,o).index;const{oid:u,index:l}=r(i,o);switch(u){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=l,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+u,i)}return xr={signatureAlgorithmHashFromCertificate:n},xr}var _r,Ys;function $o(){if(Ys)return _r;Ys=1;const e=ci(),{signatureAlgorithmHashFromCertificate:t}=Qo();function r(f,d){const p=["SCRAM-SHA-256"];d&&p.unshift("SCRAM-SHA-256-PLUS");const g=p.find(h=>f.includes(h));if(!g)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(g==="SCRAM-SHA-256-PLUS"&&typeof d.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const y=e.randomBytes(18).toString("base64");return{mechanism:g,clientNonce:y,response:(g==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":d?"y":"n")+",,n=*,r="+y,message:"SASLInitialResponse"}}async function s(f,d,p,g){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(d==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const y=c(p);if(y.nonce.startsWith(f.clientNonce)){if(y.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,h="r="+y.nonce+",s="+y.salt+",i="+y.iteration;let b=g?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const x=g.getPeerCertificate().raw;let O=t(x);(O==="MD5"||O==="SHA-1")&&(O="SHA-256");const D=await e.hashByName(O,x);b=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(D)]).toString("base64")}const w="c="+b+",r="+y.nonce,S=v+","+h+","+w,P=Buffer.from(y.salt,"base64"),A=await e.deriveKey(d,P,y.iteration),m=await e.hmacSha256(A,"Client Key"),_=await e.sha256(m),E=await e.hmacSha256(_,S),T=l(Buffer.from(m),Buffer.from(E)).toString("base64"),C=await e.hmacSha256(A,"Server Key"),N=await e.hmacSha256(C,S);f.message="SASLResponse",f.serverSignature=Buffer.from(N).toString("base64"),f.response=w+",p="+T}function n(f,d){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof d!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=u(d);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((d,p)=>f.charCodeAt(p)).every(d=>d>=33&&d<=43||d>=45&&d<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function a(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(d=>{if(!/^.=/.test(d))throw new Error("SASL: Invalid attribute pair entry");const p=d[0],g=d.substring(2);return[p,g]}))}function c(f){const d=a(f),p=d.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const g=d.get("s");if(g){if(!o(g))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const y=d.get("i");if(y){if(!/^[1-9][0-9]*$/.test(y))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(y,10);return{nonce:p,salt:g,iteration:v}}function u(f){const p=a(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function l(f,d){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(d))throw new TypeError("second argument must be a Buffer");if(f.length!==d.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,g)=>f[g]^d[g]))}return _r={startSession:r,continueSession:s,finalizeSession:n},_r}var Sr,Ks;function Gr(){if(Ks)return Sr;Ks=1;const e=_t();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},Sr=t,Sr}var Ar,Zs;function Wo(){if(Zs)return Ar;Zs=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const a={};let c,u=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{c=new URL(i,"postgres://base")}catch{c=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),u=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of c.searchParams.entries())a[p[0]]=p[1];if(a.user=a.user||decodeURIComponent(c.username),a.password=a.password||decodeURIComponent(c.password),c.protocol=="socket:")return a.host=decodeURI(c.pathname),a.database=c.searchParams.get("db"),a.client_encoding=c.searchParams.get("encoding"),a;const l=u?"":c.hostname;a.host?l&&/^%2f/i.test(l)&&(c.pathname=l+c.pathname):a.host=decodeURIComponent(l),a.port||(a.port=c.port);const f=c.pathname.slice(1)||null;a.database=f?decodeURI(f):null,(a.ssl==="true"||a.ssl==="1")&&(a.ssl=!0),a.ssl==="0"&&(a.ssl=!1),(a.sslcert||a.sslkey||a.sslrootcert||a.sslmode)&&(a.ssl={});const d=a.sslcert||a.sslkey||a.sslrootcert?Ln:null;if(a.sslcert&&(a.ssl.cert=d.readFileSync(a.sslcert).toString()),a.sslkey&&(a.ssl.key=d.readFileSync(a.sslkey).toString()),a.sslrootcert&&(a.ssl.ca=d.readFileSync(a.sslrootcert).toString()),o.useLibpqCompat&&a.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(a.uselibpqcompat==="true"||o.useLibpqCompat)switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":{a.ssl.rejectUnauthorized=!1;break}case"require":{a.sslrootcert?a.ssl.checkServerIdentity=function(){}:a.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!a.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");a.ssl.checkServerIdentity=function(){};break}}else switch(a.sslmode){case"disable":{a.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{a.sslmode!=="verify-full"&&n(a.sslmode);break}case"no-verify":{a.ssl.rejectUnauthorized=!1;break}}return a}function t(i){return Object.entries(i).reduce((a,[c,u])=>(u!=null&&(a[c]=u),a),{})}function r(i){return Object.entries(i).reduce((a,[c,u])=>{if(c==="ssl"){const l=u;typeof l=="boolean"&&(a[c]=l),typeof l=="object"&&(a[c]=t(l))}else if(u!=null)if(c==="port"){if(u!==""){const l=parseInt(u,10);if(isNaN(l))throw new Error(`Invalid ${c}: ${u}`);a[c]=l}}else a[c]=u;return a},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
...
See https://www.postgresql.org/docs/current/libpq-ssl.html for libpq SSL mode definitions.`))}return Ar=e,e.parse=e,e.toClientConfig=r,e.parseIntoClientConfig=s,Ar}var Tr,en;function ui(){if(en)return Tr;en=1;var e={};const t=vi,r=St(),s=Wo().parse,n=function(u,l,f){return l[u]?l[u]:(f===void 0?f=e["PG"+u.toUpperCase()]:f===!1||(f=e[f]),f||r[u])},i=function(){switch(e.PGSSLMODE){case"disable":return!1;case"prefer":case"require":case"verify-ca":case"verify-full":return!0;case"no-verify":return{rejectUnauthorized:!1}}return r.ssl},o=function(u){return"'"+(""+u).replace(/\\/g,"\\\\").replace(/'/g,"\\'")+"'"},a=function(u,l,f){const d=l[f];d!=null&&u.push(f+"="+o(d))};class c{constructor(l){l=typeof l=="string"?s(l):l||{},l.connectionString&&(l=Object.assign({},l,s(l.connectionString))),this.user=n("user",l),this.database=n("database",l),this.database===void 0&&(this.database=this.user),this.port=parseInt(n("port",l),10),this.host=n("host",l),Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:n("password",l)}),this.binary=n("binary",l),this.options=n("options",l),this.ssl=typeof l.ssl>"u"?i():l.ssl,typeof this.ssl=="string"&&this.ssl==="true"&&(this.ssl=!0),this.ssl==="no-verify"&&(this.ssl={rejectUnauthorized:!1}),this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this.client_encoding=n("client_encoding",l),this.replication=n("replication",l),this.isDomainSocket=!(this.host||"").indexOf("/"),this.application_name=n("application_name",l,"PGAPPNAME"),this.fallback_application_name=n("fallback_application_name",l,!1),this.statement_timeout=n("statement_timeout",l,!1),this.lock_timeout=n("lock_timeout",l,!1),this.idle_in_transaction_session_timeout=n("idle_in_transaction_session_timeout",l,!1),this.query_timeout=n("query_timeout",l,!1),l.connectionTimeoutMillis===void 0?this.connect_timeout=e.PGCONNECT_TIMEOUT||0:this.connect_timeout=Math.floor(l.connectionTimeoutMillis/1e3),l.keepAlive===!1?this.keepalives=0:l.keepAlive===!0&&(this.keepalives=1),typeof l.keepAliveInitialDelayMillis=="number"&&(this.keepalives_idle=Math.floor(l.keepAliveInitialDelayMillis/1e3))}getLibpqConnectionString(l){const f=[];a(f,this,"user"),a(f,this,"password"),a(f,this,"port"),a(f,this,"application_name"),a(f,this,"fallback_application_name"),a(f,this,"connect_timeout"),a(f,this,"options");const d=typeof this.ssl=="object"?this.ssl:this.ssl?{sslmode:this.ssl}:{};if(a(f,d,"sslmode"),a(f,d,"sslca"),a(f,d,"sslkey"),a(f,d,"sslcert"),a(f,d,"sslrootcert"),this.database&&f.push("dbname="+o(this.database)),this.replication&&f.push("replication="+o(this.replication)),this.host&&f.push("host="+o(this.host)),this.isDomainSocket)return l(null,f.join(" "));this.client_encoding&&f.push("client_encoding="+o(this.client_encoding)),t.lookup(this.host,function(p,g){return p?l(p,null):(f.push("hostaddr="+o(g)),l(null,f.join(" ")))})}}return Tr=c,Tr}var Cr,tn;function di(){if(tn)return Cr;tn=1;const e=_t(),t=/^([A-Za-z]+)(?: (\d+))?(?: (\d+))?/;class r{constructor(n,i){this.command=null,this.rowCount=null,this.oid=null,this.rows=[],this.fields=[],this._parsers=void 0,this._types=i,this.RowCtor=null,this.rowAsArray=n==="array",this.rowAsArray&&(this.parseRow=this._parseRowAsArray),this._prebuiltEmptyResultObject=null}addCommandComplete(n){let i;n.text?i=t.exec(n.text):i=t.exec(n.command),i&&(this.command=i[1],i[3]?(this.oid=parseInt(i[2],10),this.rowCount=parseInt(i[3],10)):i[2]&&(this.rowCount=parseInt(i[2],10)))}_parseRowAsArray(n){const i=new Array(n.length);for(let o=0,a=n.length;o<a;o++){const c=n[o];c!==null?i[o]=this._parsers[o](c):i[o]=null}return i}parseRow(n){const i={...this._prebuiltEmptyResultObject};for(let o=0,a=n.length;o<a;o++){const c=n[o],u=this.fields[o].name;if(c!==null){const l=this.fields[o].format==="binary"?Buffer.from(c):c;i[u]=this._parsers[o](l)}else i[u]=null}return i}addRow(n){this.rows.push(n)}addFields(n){this.fields=n,this.fields.length&&(this._parsers=new Array(n.length));const i={};for(let o=0;o<n.length;o++){const a=n[o];i[a.name]=null,this._types?this._parsers[o]=this._types.getTypeParser(a.dataTypeID,a.format||"text"):this._parsers[o]=e.getTypeParser(a.dataTypeID,a.format||"text")}this._prebuiltEmptyResultObject={...i}}}return Cr=r,Cr}var Rr,rn;function Xo(){if(rn)return Rr;rn=1;const{EventEmitter:e}=ze,t=di(),r=et();class s extends e{constructor(i,o,a){super(),i=r.normalizeQueryConfig(i,o,a),this.text=i.text,this.values=i.values,this.rows=i.rows,this.types=i.types,this.name=i.name,this.queryMode=i.queryMode,this.binary=i.binary,this.portal=i.portal||"",this.callback=i.callback,this._rowMode=i.rowMode,process.domain&&i.callback&&(this.callback=process.domain.bind(i.callback)),this._result=new t(this._rowMode,this.types),this._results=this._result,this._canceledDueToError=!1}requiresPreparation(){return this.queryMode==="extended"||this.name||this.rows?!0:!this.text||!this.values?!1:this.values.length>0}_checkForMultirow(){this._result.command&&(Array.isArray(this._results)||(this._results=[this._result]),this._result=new t(this._rowMode,this._result._types),this._results.push(this._result))}handleRowDescription(i){this._checkForMultirow(),this._result.addFields(i.fields),this._accumulateRows=this.callback||!this.listeners("row").length}handleDataRow(i){let o;if(!this._canceledDueToError){try{o=this._result.parseRow(i.fields)}catch(a){this._canceledDueToError=a;return}this.emit("row",o,this._result),this._accumulateRows&&this._result.addRow(o)}}handleCommandComplete(i,o){this._checkForMultirow(),this._result.addCommandComplete(i),this.rows&&o.sync()}handleEmptyQuery(i){this.rows&&i.sync()}handleError(i,o){if(this._canceledDueToError&&(i=this._canceledDueToError,this._canceledDueToError=!1),this.callback)return this.callback(i);this.emit("error",i)}handleReadyForQuery(i){if(this._canceledDueToError)return this.handleError(this._canceledDueToError,i);if(this.callback)try{this.callback(null,this._results)}catch(o){process.nextTick(()=>{throw o})}this.emit("end",this._results)}submit(i){if(typeof this.text!="string"&&typeof this.name!="string")return new Error("A query must have either text or a name. Supplying neither is unsupported.");const o=i.parsedStatements[this.name];if(this.text&&o&&this.text!==o)return new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);if(this.values&&!Array.isArray(this.values))return new Error("Query values must be an array");if(this.requiresPreparation()){i.stream.cork&&i.stream.cork();try{this.prepare(i)}finally{i.stream.uncork&&i.stream.uncork()}}else i.query(this.text);return null}hasBeenParsed(i){return this.name&&i.parsedStatements[this.name]}handlePortalSuspended(i){this._getRows(i,this.rows)}_getRows(i,o){i.execute({portal:this.portal,rows:o}),o?i.flush():i.sync()}prepare(i){this.hasBeenParsed(i)||i.parse({text:this.text,name:this.name,types:this.types});try{i.bind({portal:this.portal,statement:this.name,values:this.values,binary:this.binary,valueMapper:r.prepareValue})}catch(o){this.handleError(o,i);return}i.describe({type:"P",name:this.portal||""}),this._getRows(i,this.rows)}handleCopyInResponse(i){i.sendCopyFail("No source stream defined")}handleCopyData(i,o){}}return Rr=s,Rr}var Ir={},B={},sn;function fi(){if(sn)return B;sn=1,Object.defineProperty(B,"__esModule",{value:!0}),B.NoticeMessage=B.DataRowMessage=B.CommandCompleteMessage=B.ReadyForQueryMessage=B.NotificationResponseMessage=B.BackendKeyDataMessage=B.AuthenticationMD5Password=B.ParameterStatusMessage=B.ParameterDescriptionMessage=B.RowDescriptionMessage=B.Field=B.CopyResponse=B.CopyDataMessage=B.DatabaseError=B.copyDone=B.emptyQuery=B.replicationStart=B.portalSuspended=B.noData=B.closeComplete=B.bindComplete=B.parseComplete=void 0,B.parseComplete={name:"parseComplete",length:5},B.bindComplete={name:"bindComplete",length:5},B.closeComplete={name:"closeComplete",length:5},B.noData={name:"noData",length:5},B.portalSuspended={name:"portalSuspended",length:5},B.replicationStart={name:"replicationStart",length:4},B.emptyQuery={name:"emptyQuery",length:4},B.copyDone={name:"copyDone",length:4};class e extends Error{constructor(y,v,h){super(y),this.length=v,this.name=h}}B.DatabaseError=e;class t{constructor(y,v){this.length=y,this.chunk=v,this.name="copyData"}}B.CopyDataMessage=t;class r{constructor(y,v,h,b){this.length=y,this.name=v,this.binary=h,this.columnTypes=new Array(b)}}B.CopyResponse=r;class s{constructor(y,v,h,b,w,S,P){this.name=y,this.tableID=v,this.columnID=h,this.dataTypeID=b,this.dataTypeSize=w,this.dataTypeModifier=S,this.format=P}}B.Field=s;class n{constructor(y,v){this.length=y,this.fieldCount=v,this.name="rowDescription",this.fields=new Array(this.fieldCount)}}B.RowDescriptionMessage=n;class i{constructor(y,v){this.length=y,this.parameterCount=v,this.name="parameterDescription",this.dataTypeIDs=new Array(this.parameterCount)}}B.ParameterDescriptionMessage=i;class o{constructor(y,v,h){this.length=y,this.parameterName=v,this.parameterValue=h,this.name="parameterStatus"}}B.ParameterStatusMessage=o;class a{constructor(y,v){this.length=y,this.salt=v,this.name="authenticationMD5Password"}}B.AuthenticationMD5Password=a;class c{constructor(y,v,h){this.length=y,this.processID=v,this.secretKey=h,this.name="backendKeyData"}}B.BackendKeyDataMessage=c;class u{constructor(y,v,h,b){this.length=y,this.processId=v,this.channel=h,this.payload=b,this.name="notification"}}B.NotificationResponseMessage=u;class l{constructor(y,v){this.length=y,this.status=v,this.name="readyForQuery"}}B.ReadyForQueryMessage=l;class f{constructor(y,v){this.length=y,this.text=v,this.name="commandComplete"}}B.CommandCompleteMessage=f;class d{constructor(y,v){this.length=y,this.fields=v,this.name="dataRow",this.fieldCount=v.length}}B.DataRowMessage=d;class p{constructor(y,v){this.length=y,this.message=v,this.name="notice"}}return B.NoticeMessage=p,B}var We={},Xe={},nn;function Go(){if(nn)return Xe;nn=1,Object.defineProperty(Xe,"__esModule",{value:!0}),Xe.Writer=void 0;class e{constructor(r=256){this.size=r,this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(r)}ensure(r){if(this.buffer.length-this.offset<r){const n=this.buffer,i=n.length+(n.length>>1)+r;this.buffer=Buffer.allocUnsafe(i),n.copy(this.buffer)}}addInt32(r){return this.ensure(4),this.buffer[this.offset++]=r>>>24&255,this.buffer[this.offset++]=r>>>16&255,this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addInt16(r){return this.ensure(2),this.buffer[this.offset++]=r>>>8&255,this.buffer[this.offset++]=r>>>0&255,this}addCString(r){if(!r)this.ensure(1);else{const s=Buffer.byteLength(r);this.ensure(s+1),this.buffer.write(r,this.offset,"utf-8"),this.offset+=s}return this.buffer[this.offset++]=0,this}addString(r=""){const s=Buffer.byteLength(r);return this.ensure(s),this.buffer.write(r,this.offset),this.offset+=s,this}add(r){return this.ensure(r.length),r.copy(this.buffer,this.offset),this.offset+=r.length,this}join(r){if(r){this.buffer[this.headerPosition]=r;const s=this.offset-(this.headerPosition+1);this.buffer.writeInt32BE(s,this.headerPosition+1)}return this.buffer.slice(r?0:5,this.offset)}flush(r){const s=this.join(r);return this.offset=5,this.headerPosition=0,this.buffer=Buffer.allocUnsafe(this.size),s}}return Xe.Writer=e,Xe}var on;function Jo(){if(on)return We;on=1,Object.defineProperty(We,"__esModule",{value:!0}),We.serialize=void 0;const e=Go(),t=new e.Writer,r=x=>{t.addInt16(3).addInt16(0);for(const M of Object.keys(x))t.addCString(M).addCString(x[M]);t.addCString("client_encoding").addCString("UTF8");const O=t.addCString("").flush(),D=O.length+4;return new e.Writer().addInt32(D).add(O).flush()},s=()=>{const x=Buffer.allocUnsafe(8);return x.writeInt32BE(8,0),x.writeInt32BE(80877103,4),x},n=x=>t.addCString(x).flush(112),i=function(x,O){return t.addCString(x).addInt32(Buffer.byteLength(O)).addString(O),t.flush(112)},o=function(x){return t.addString(x).flush(112)},a=x=>t.addCString(x).flush(81),c=[],u=x=>{const O=x.name||"";O.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",O,O.length),console.error("This can cause conflicts and silent errors executing queries"));const D=x.types||c,M=D.length,F=t.addCString(O).addCString(x.text).addInt16(M);for(let z=0;z<M;z++)F.addInt32(D[z]);return t.flush(80)},l=new e.Writer,f=function(x,O){for(let D=0;D<x.length;D++){const M=O?O(x[D],D):x[D];M==null?(t.addInt16(0),l.addInt32(-1)):M instanceof Buffer?(t.addInt16(1),l.addInt32(M.length),l.add(M)):(t.addInt16(0),l.addInt32(Buffer.byteLength(M)),l.addString(M))}},d=(x={})=>{const O=x.portal||"",D=x.statement||"",M=x.binary||!1,F=x.values||c,z=F.length;return t.addCString(O).addCString(D),t.addInt16(z),f(F,x.valueMapper),t.addInt16(z),t.add(l.flush()),t.addInt16(1),t.addInt16(M?1:0),t.flush(66)},p=Buffer.from([69,0,0,0,9,0,0,0,0,0]),g=x=>{if(!x||!x.portal&&!x.rows)return p;const O=x.portal||"",D=x.rows||0,M=Buffer.byteLength(O),F=4+M+1+4,z=Buffer.allocUnsafe(1+F);return z[0]=69,z.writeInt32BE(F,1),z.write(O,5,"utf-8"),z[M+5]=0,z.writeUInt32BE(D,z.length-4),z},y=(x,O)=>{const D=Buffer.allocUnsafe(16);return D.writeInt32BE(16,0),D.writeInt16BE(1234,4),D.writeInt16BE(5678,6),D.writeInt32BE(x,8),D.writeInt32BE(O,12),D},v=(x,O)=>{const M=4+Buffer.byteLength(O)+1,F=Buffer.allocUnsafe(1+M);return F[0]=x,F.writeInt32BE(M,1),F.write(O,5,"utf-8"),F[M]=0,F},h=t.addCString("P").flush(68),b=t.addCString("S").flush(68),w=x=>x.name?v(68,`${x.type}${x.name||""}`):x.type==="P"?h:b,S=x=>{const O=`${x.type}${x.name||""}`;return v(67,O)},P=x=>t.add(x).flush(100),A=x=>v(102,x),m=x=>Buffer.from([x,0,0,0,4]),_=m(72),E=m(83),T=m(88),C=m(99),N={startup:r,password:n,requestSsl:s,sendSASLInitialResponseMessage:i,sendSCRAMClientFinalMessage:o,query:a,parse:u,bind:d,execute:g,describe:w,close:S,flush:()=>_,sync:()=>E,end:()=>T,copyData:P,copyDone:()=>C,copyFail:A,cancel:y};return We.serialize=N,We}var Ge={},Je={},an;function Yo(){if(an)return Je;an=1,Object.defineProperty(Je,"__esModule",{value:!0}),Je.BufferReader=void 0;class e{constructor(r=0){this.offset=r,this.buffer=Buffer.allocUnsafe(0),this.encoding="utf-8"}setBuffer(r,s){this.offset=r,this.buffer=s}int16(){const r=this.buffer.readInt16BE(this.offset);return this.offset+=2,r}byte(){const r=this.buffer[this.offset];return this.offset++,r}int32(){const r=this.buffer.readInt32BE(this.offset);return this.offset+=4,r}uint32(){const r=this.buffer.readUInt32BE(this.offset);return this.offset+=4,r}string(r){const s=this.buffer.toString(this.encoding,this.offset,this.offset+r);return this.offset+=r,s}cstring(){const r=this.offset;let s=r;for(;this.buffer[s++]!==0;);return this.offset=s,this.buffer.toString(this.encoding,r,s-1)}bytes(r){const s=this.buffer.slice(this.offset,this.offset+r);return this.offset+=r,s}}return Je.BufferReader=e,Je}var ln;function Ko(){if(ln)return Ge;ln=1,Object.defineProperty(Ge,"__esModule",{value:!0}),Ge.Parser=void 0;const e=fi(),t=Yo(),r=1,n=r+4,i=-1,o=Buffer.allocUnsafe(0);class a{constructor(_){if(this.buffer=o,this.bufferLength=0,this.bufferOffset=0,this.reader=new t.BufferReader,(_==null?void 0:_.mode)==="binary")throw new Error("Binary mode not supported yet");this.mode=(_==null?void 0:_.mode)||"text"}parse(_,E){this.mergeBuffer(_);const T=this.bufferOffset+this.bufferLength;let C=this.bufferOffset;for(;C+n<=T;){const N=this.buffer[C],x=this.buffer.readUInt32BE(C+r),O=r+x;if(O+C<=T){const D=this.handlePacket(C+n,N,x,this.buffer);E(D),C+=O}else break}C===T?(this.buffer=o,this.bufferLength=0,this.bufferOffset=0):(this.bufferLength=T-C,this.bufferOffset=C)}mergeBuffer(_){if(this.bufferLength>0){const E=this.bufferLength+_.byteLength;if(E+this.bufferOffset>this.buffer.byteLength){let C;if(E<=this.buffer.byteLength&&this.bufferOffset>=this.bufferLength)C=this.buffer;else{let N=this.buffer.byteLength*2;for(;E>=N;)N*=2;C=Buffer.allocUnsafe(N)}this.buffer.copy(C,0,this.bufferOffset,this.bufferOffset+this.bufferLength),this.buffer=C,this.bufferOffset=0}_.copy(this.buffer,this.bufferOffset+this.bufferLength),this.bufferLength=E}else this.buffer=_,this.bufferOffset=0,this.bufferLength=_.byteLength}handlePacket(_,E,T,C){const{reader:N}=this;N.setBuffer(_,C);let x;switch(E){case 50:x=e.bindComplete;break;case 49:x=e.parseComplete;break;case 51:x=e.closeComplete;break;case 110:x=e.noData;break;case 115:x=e.portalSuspended;break;case 99:x=e.copyDone;break;case 87:x=e.replicationStart;break;case 73:x=e.emptyQuery;break;case 68:x=b(N);break;case 67:x=u(N);break;case 90:x=c(N);break;case 65:x=g(N);break;case 82:x=P(N,T);break;case 83:x=w(N);break;case 75:x=S(N);break;case 69:x=A(N,"error");break;case 78:x=A(N,"notice");break;case 84:x=y(N);break;case 116:x=h(N);break;case 71:x=f(N);break;case 72:x=d(N);break;case 100:x=l(N,T);break;default:return new e.DatabaseError("received invalid response: "+E.toString(16),T,"error")}return N.setBuffer(0,o),x.length=T,x}}Ge.Parser=a;const c=m=>{const _=m.string(1);return new e.ReadyForQueryMessage(i,_)},u=m=>{const _=m.cstring();return new e.CommandCompleteMessage(i,_)},l=(m,_)=>{const E=m.bytes(_-4);return new e.CopyDataMessage(i,E)},f=m=>p(m,"copyInResponse"),d=m=>p(m,"copyOutResponse"),p=(m,_)=>{const E=m.byte()!==0,T=m.int16(),C=new e.CopyResponse(i,_,E,T);for(let N=0;N<T;N++)C.columnTypes[N]=m.int16();return C},g=m=>{const _=m.int32(),E=m.cstring(),T=m.cstring();return new e.NotificationResponseMessage(i,_,E,T)},y=m=>{const _=m.int16(),E=new e.RowDescriptionMessage(i,_);for(let T=0;T<_;T++)E.fields[T]=v(m);return E},v=m=>{const _=m.cstring(),E=m.uint32(),T=m.int16(),C=m.uint32(),N=m.int16(),x=m.int32(),O=m.int16()===0?"text":"binary";return new e.Field(_,E,T,C,N,x,O)},h=m=>{const _=m.int16(),E=new e.ParameterDescriptionMessage(i,_);for(let T=0;T<_;T++)E.dataTypeIDs[T]=m.int32();return E},b=m=>{const _=m.int16(),E=new Array(_);for(let T=0;T<_;T++){const C=m.int32();E[T]=C===-1?null:m.string(C)}return new e.DataRowMessage(i,E)},w=m=>{const _=m.cstring(),E=m.cstring();return new e.ParameterStatusMessage(i,_,E)},S=m=>{const _=m.int32(),E=m.int32();return new e.BackendKeyDataMessage(i,_,E)},P=(m,_)=>{const E=m.int32(),T={name:"authenticationOk",length:_};switch(E){case 0:break;case 3:T.length===8&&(T.name="authenticationCleartextPassword");break;case 5:if(T.length===12){T.name="authenticationMD5Password";const C=m.bytes(4);return new e.AuthenticationMD5Password(i,C)}break;case 10:{T.name="authenticationSASL",T.mechanisms=[];let C;do C=m.cstring(),C&&T.mechanisms.push(C);while(C)}break;case 11:T.name="authenticationSASLContinue",T.data=m.string(_-8);break;case 12:T.name="authenticationSASLFinal",T.data=m.string(_-8);break;default:throw new Error("Unknown authenticationOk message type "+E)}return T},A=(m,_)=>{const E={};let T=m.string(1);for(;T!=="\0";)E[T]=m.cstring(),T=m.string(1);const C=E.M,N=_==="notice"?new e.NoticeMessage(i,C):new e.DatabaseError(C,i,_);return N.severity=E.S,N.code=E.C,N.detail=E.D,N.hint=E.H,N.position=E.P,N.internalPosition=E.p,N.internalQuery=E.q,N.where=E.W,N.schema=E.s,N.table=E.t,N.column=E.c,N.dataType=E.d,N.constraint=E.n,N.file=E.F,N.line=E.L,N.routine=E.R,N};return Ge}var cn;function pi(){return cn||(cn=1,(function(e){Object.defineProperty(e,"__esModule",{value:!0}),e.DatabaseError=e.serialize=e.parse=void 0;const t=fi();Object.defineProperty(e,"DatabaseError",{enumerable:!0,get:function(){return t.DatabaseError}});const r=Jo();Object.defineProperty(e,"serialize",{enumerable:!0,get:function(){return r.serialize}});const s=Ko();function n(i,o){const a=new s.Parser;return i.on("data",c=>a.parse(c,o)),new Promise(c=>i.on("end",()=>c()))}e.parse=n})(Ir)),Ir}var dt={},un;function Zo(){return un||(un=1,Object.defineProperty(dt,"__esModule",{value:!0}),dt.default={}),dt}var Nr,dn;function ea(){if(dn)return Nr;dn=1;const{getStream:e,getSecureStream:t}=i();Nr={getStream:e,getSecureStream:t};function r(){function o(c){const u=qn;return new u.Socket}function a(c){return wi.connect(c)}return{getStream:o,getSecureStream:a}}function s(){function o(c){const{CloudflareSocket:u}=Zo();return new u(c)}function a(c){return c.socket.startTls(c),c.socket}return{getStream:o,getSecureStream:a}}function n(){if(typeof navigator=="object"&&navigator!==null&&typeof navigator.userAgent=="string")return navigator.userAgent==="Cloudflare-Workers";if(typeof Response=="function"){const o=new Response(null,{cf:{thing:!0}});if(typeof o.cf=="object"&&o.cf!==null&&o.cf.thing)return!0}return!1}function i(){return n()?s():r()}return Nr}var Pr,fn;function hi(){if(fn)return Pr;fn=1;const e=ze.EventEmitter,{parse:t,serialize:r}=pi(),{getStream:s,getSecureStream:n}=ea(),i=r.flush(),o=r.sync(),a=r.end();class c extends e{constructor(l){super(),l=l||{},this.stream=l.stream||s(l.ssl),typeof this.stream=="function"&&(this.stream=this.stream(l)),this._keepAlive=l.keepAlive,this._keepAliveInitialDelayMillis=l.keepAliveInitialDelayMillis,this.parsedStatements={},this.ssl=l.ssl||!1,this._ending=!1,this._emitMessage=!1;const f=this;this.on("newListener",function(d){d==="message"&&(f._emitMessage=!0)})}connect(l,f){const d=this;this._connecting=!0,this.stream.setNoDelay(!0),this.stream.connect(l,f),this.stream.once("connect",function(){d._keepAlive&&d.stream.setKeepAlive(!0,d._keepAliveInitialDelayMillis),d.emit("connect")});const p=function(g){d._ending&&(g.code==="ECONNRESET"||g.code==="EPIPE")||d.emit("error",g)};if(this.stream.on("error",p),this.stream.on("close",function(){d.emit("end")}),!this.ssl)return this.attachListeners(this.stream);this.stream.once("data",function(g){switch(g.toString("utf8")){case"S":break;case"N":return d.stream.end(),d.emit("error",new Error("The server does not support SSL connections"));default:return d.stream.end(),d.emit("error",new Error("There was an error establishing an SSL connection"))}const v={socket:d.stream};d.ssl!==!0&&(Object.assign(v,d.ssl),"key"in d.ssl&&(v.key=d.ssl.key));const h=qn;h.isIP&&h.isIP(f)===0&&(v.servername=f);try{d.stream=n(v)}catch(b){return d.emit("error",b)}d.attachListeners(d.stream),d.stream.on("error",p),d.emit("sslconnect")})}attachListeners(l){t(l,f=>{const d=f.name==="error"?"errorMessage":f.name;this._emitMessage&&this.emit("message",f),this.emit(d,f)})}requestSsl(){this.stream.write(r.requestSsl())}startup(l){this.stream.write(r.startup(l))}cancel(l,f){this._send(r.cancel(l,f))}password(l){this._send(r.password(l))}sendSASLInitialResponseMessage(l,f){this._send(r.sendSASLInitialResponseMessage(l,f))}sendSCRAMClientFinalMessage(l){this._send(r.sendSCRAMClientFinalMessage(l))}_send(l){return this.stream.writable?this.stream.write(l):!1}query(l){this._send(r.query(l))}parse(l){this._send(r.parse(l))}bind(l){this._send(r.bind(l))}execute(l){this._send(r.execute(l))}flush(){this.stream.writable&&this.stream.write(i)}sync(){this._ending=!0,this._send(o)}ref(){this.stream.ref()}unref(){this.stream.unref()}end(){if(this._ending=!0,!this._connecting||!this.stream.writable){this.stream.end();return}return this.stream.write(a,()=>{this.stream.end()})}close(l){this._send(r.close(l))}describe(l){this._send(r.describe(l))}sendCopyFromChunk(l){this._send(r.copyData(l))}endCopyFrom(){this._send(r.copyDone())}sendCopyFail(l){this._send(r.copyFail(l))}}return Pr=c,Pr}var ft={exports:{}},Or={exports:{}},Lr,pn;function ta(){if(pn)return Lr;pn=1;const{Transform:e}=Dn,{StringDecoder:t}=xi,r=Symbol("last"),s=Symbol("decoder");function n(u,l,f){let d;if(this.overflow){if(d=this[s].write(u).split(this.matcher),d.length===1)return f();d.shift(),this.overflow=!1}else this[r]+=this[s].write(u),d=this[r].split(this.matcher);this[r]=d.pop();for(let p=0;p<d.length;p++)try{o(this,this.mapper(d[p]))}catch(g){return f(g)}if(this.overflow=this[r].length>this.maxLength,this.overflow&&!this.skipOverflow){f(new Error("maximum buffer reached"));return}f()}function i(u){if(this[r]+=this[s].end(),this[r])try{o(this,this.mapper(this[r]))}catch(l){return u(l)}u()}function o(u,l){l!==void 0&&u.push(l)}function a(u){return u}function c(u,l,f){switch(u=u||/\r?\n/,l=l||a,f=f||{},arguments.length){case 1:typeof u=="function"?(l=u,u=/\r?\n/):typeof u=="object"&&!(u instanceof RegExp)&&!u[Symbol.split]&&(f=u,u=/\r?\n/);break;case 2:typeof u=="function"?(f=l,l=u,u=/\r?\n/):typeof l=="object"&&(f=l,l=a)}f=Object.assign({},f),f.autoDestroy=!0,f.transform=n,f.flush=i,f.readableObjectMode=!0;const d=new e(f);return d[r]="",d[s]=new t("utf8"),d.matcher=u,d.mapper=l,d.maxLength=f.maxLength,d.skipOverflow=f.skipOverflow||!1,d.overflow=!1,d._destroy=function(p,g){this._writableState.errorEmitted=!1,g(p)},d}return Lr=c,Lr}var hn;function ra(){return hn||(hn=1,(function(e){var t={},r=Ei,s=Dn.Stream,n=ta(),i=He,o=5432,a=process.platform==="win32",c=process.stderr,u=56,l=7,f=61440,d=32768;function p(P){return(P&f)==d}var g=["host","port","database","user","password"],y=g.length,v=g[y-1];function h(){var P=c instanceof s&&c.writable===!0;if(P){var A=Array.prototype.slice.call(arguments).concat(`
`);c.write(i.format.apply(i,A))}}Object.defineProperty(e.exports,"isWin",{get:function(){return a},set:function(P){a=P}}),e.exports.warnTo=function(P){var A=c;return c=P,A},e.exports.getFileName=function(P){var A=P||t,m=A.PGPASSFILE||(a?r.join(A.APPDATA||"./","postgresql","pgpass.conf"):r.join(A.HOME||"./",".pgpass"));return m},e.exports.usePgPass=function(P,A){return Object.prototype.hasOwnProperty.call(t,"PGPASSWORD")?!1:a?!0:(A=A||"<unkn>",p(P.mode)?P.mode&(u|l)?(h('WARNING: password file "%s" has group or world access; permissions should be u=rw (0600) or less',A),!1):!0:(h('WARNING: password file "%s" is not a plain file',A),!1))};var b=e.exports.match=function(P,A){return g.slice(0,-1).reduce(function(m,_,E){return E==1&&Number(P[_]||o)===Number(A[_])?m&&!0:m&&(A[_]==="*"||A[_]===P[_])},!0)};e.exports.getPassword=function(P,A,m){var _,E=A.pipe(n());function T(x){var O=w(x);O&&S(O)&&b(P,O)&&(_=O[v],E.end())}var C=function(){A.destroy(),m(_)},N=function(x){A.destroy(),h("WARNING: error on reading file: %s",x),m(void 0)};A.on("error",N),E.on("data",T).on("end",C).on("error",N)};var w=e.exports.parseLine=function(P){if(P.length<11||P.match(/^\s+#/))return null;for(var A="",m="",_=0,E=0,T={},C=!1,N=function(O,D,M){var F=P.substring(D,M);Object.hasOwnProperty.call(t,"PGPASS_NO_DEESCAPE")||(F=F.replace(/\\([:\\])/g,"$1")),T[g[O]]=F},x=0;x<P.length-1;x+=1){if(A=P.charAt(x+1),m=P.charAt(x),C=_==y-1,C){N(_,E);break}x>=0&&A==":"&&m!=="\\"&&(N(_,E,x+1),E=x+2,_+=1)}return T=Object.keys(T).length===y?T:null,T},S=e.exports.isValidEntry=function(P){for(var A={0:function(C){return C.length>0},1:function(C){return C==="*"?!0:(C=Number(C),isFinite(C)&&C>0&&C<9007199254740992&&Math.floor(C)===C)},2:function(C){return C.length>0},3:function(C){return C.length>0},4:function(C){return C.length>0}},m=0;m<g.length;m+=1){var _=A[m],E=P[g[m]]||"",T=_(E);if(!T)return!1}return!0}})(Or)),Or.exports}var mn;function sa(){if(mn)return ft.exports;mn=1;var e=Ln,t=ra();return ft.exports=function(r,s){var n=t.getFileName();e.stat(n,function(i,o){if(i||!t.usePgPass(o,n))return s(void 0);var a=e.createReadStream(n);t.getPassword(r,a,s)})},ft.exports.warnTo=t.warnTo,ft.exports}var qr,gn;function na(){if(gn)return qr;gn=1;const e=ze.EventEmitter,t=et(),r=He,s=$o(),n=Gr(),i=ui(),o=Xo(),a=St(),c=hi(),u=ci(),l=r.deprecate(()=>{},"Client.activeQuery is deprecated and will be removed in pg@9.0"),f=r.deprecate(()=>{},"Client.queryQueue is deprecated and will be removed in pg@9.0."),d=r.deprecate(()=>{},"pgpass support is deprecated and will be removed in pg@9.0. You can provide an async function as the password property to the Client/Pool constructor that returns a password instead. Within this function you can call the pgpass module in your own code."),p=r.deprecate(()=>{},"Passing a custom Promise implementation to the Client/Pool constructor is deprecated and will be removed in pg@9.0."),g=r.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead.");class y extends e{constructor(h){super(),this.connectionParameters=new i(h),this.user=this.connectionParameters.user,this.database=this.connectionParameters.database,this.port=this.connectionParameters.port,this.host=this.connectionParameters.host,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:this.connectionParameters.password}),this.replication=this.connectionParameters.replication;const b=h||{};b.Promise&&p(),this._Promise=b.Promise||Vr.Promise,this._types=new n(b.types),this._ending=!1,this._ended=!1,this._connecting=!1,this._connected=!1,this._connectionError=!1,this._queryable=!0,this._activeQuery=null,this.enableChannelBinding=!!b.enableChannelBinding,this.connection=b.connection||new c({stream:b.stream,ssl:this.connectionParameters.ssl,keepAlive:b.keepAlive||!1,keepAliveInitialDelayMillis:b.keepAliveInitialDelayMillis||0,encoding:this.connectionParameters.client_encoding||"utf8"}),this._queryQueue=[],this.binary=b.binary||a.binary,this.processID=null,this.secretKey=null,this.ssl=this.connectionParameters.ssl||!1,this.ssl&&this.ssl.key&&Object.defineProperty(this.ssl,"key",{enumerable:!1}),this._connectionTimeoutMillis=b.connectionTimeoutMillis||0}get activeQuery(){return l(),this._activeQuery}set activeQuery(h){l(),this._activeQuery=h}_getActiveQuery(){return this._activeQuery}_errorAllQueries(h){const b=S=>{process.nextTick(()=>{S.handleError(h,this.connection)})},w=this._getActiveQuery();w&&(b(w),this._activeQuery=null),this._queryQueue.forEach(b),this._queryQueue.length=0}_connect(h){const b=this,w=this.connection;if(this._connectionCallback=h,this._connecting||this._connected){const S=new Error("Client has already been connected. You cannot reuse a client.");process.nextTick(()=>{h(S)});return}this._connecting=!0,this._connectionTimeoutMillis>0&&(this.connectionTimeoutHandle=setTimeout(()=>{w._ending=!0,w.stream.destroy(new Error("timeout expired"))},this._connectionTimeoutMillis),this.connectionTimeoutHandle.unref&&this.connectionTimeoutHandle.unref()),this.host&&this.host.indexOf("/")===0?w.connect(this.host+"/.s.PGSQL."+this.port):w.connect(this.port,this.host),w.on("connect",function(){b.ssl?w.requestSsl():w.startup(b.getStartupConf())}),w.on("sslconnect",function(){w.startup(b.getStartupConf())}),this._attachListeners(w),w.once("end",()=>{const S=this._ending?new Error("Connection terminated"):new Error("Connection terminated unexpectedly");clearTimeout(this.connectionTimeoutHandle),this._errorAllQueries(S),this._ended=!0,this._ending||(this._connecting&&!this._connectionError?this._connectionCallback?this._connectionCallback(S):this._handleErrorEvent(S):this._connectionError||this._handleErrorEvent(S)),process.nextTick(()=>{this.emit("end")})})}connect(h){if(h){this._connect(h);return}return new this._Promise((b,w)=>{this._connect(S=>{S?w(S):b(this)})})}_attachListeners(h){h.on("authenticationCleartextPassword",this._handleAuthCleartextPassword.bind(this)),h.on("authenticationMD5Password",this._handleAuthMD5Password.bind(this)),h.on("authenticationSASL",this._handleAuthSASL.bind(this)),h.on("authenticationSASLContinue",this._handleAuthSASLContinue.bind(this)),h.on("authenticationSASLFinal",this._handleAuthSASLFinal.bind(this)),h.on("backendKeyData",this._handleBackendKeyData.bind(this)),h.on("error",this._handleErrorEvent.bind(this)),h.on("errorMessage",this._handleErrorMessage.bind(this)),h.on("readyForQuery",this._handleReadyForQuery.bind(this)),h.on("notice",this._handleNotice.bind(this)),h.on("rowDescription",this._handleRowDescription.bind(this)),h.on("dataRow",this._handleDataRow.bind(this)),h.on("portalSuspended",this._handlePortalSuspended.bind(this)),h.on("emptyQuery",this._handleEmptyQuery.bind(this)),h.on("commandComplete",this._handleCommandComplete.bind(this)),h.on("parseComplete",this._handleParseComplete.bind(this)),h.on("copyInResponse",this._handleCopyInResponse.bind(this)),h.on("copyData",this._handleCopyData.bind(this)),h.on("notification",this._handleNotification.bind(this))}_getPassword(h){const b=this.connection;if(typeof this.password=="function")this._Promise.resolve().then(()=>this.password(this.connectionParameters)).then(w=>{if(w!==void 0){if(typeof w!="string"){b.emit("error",new TypeError("Password must be a string"));return}this.connectionParameters.password=this.password=w}else this.connectionParameters.password=this.password=null;h()}).catch(w=>{b.emit("error",w)});else if(this.password!==null)h();else try{sa()(this.connectionParameters,S=>{S!==void 0&&(d(),this.connectionParameters.password=this.password=S),h()})}catch(w){this.emit("error",w)}}_handleAuthCleartextPassword(h){this._getPassword(()=>{this.connection.password(this.password)})}_handleAuthMD5Password(h){this._getPassword(async()=>{try{const b=await u.postgresMd5PasswordHash(this.user,this.password,h.salt);this.connection.password(b)}catch(b){this.emit("error",b)}})}_handleAuthSASL(h){this._getPassword(()=>{try{this.saslSession=s.startSession(h.mechanisms,this.enableChannelBinding&&this.connection.stream),this.connection.sendSASLInitialResponseMessage(this.saslSession.mechanism,this.saslSession.response)}catch(b){this.connection.emit("error",b)}})}async _handleAuthSASLContinue(h){try{await s.continueSession(this.saslSession,this.password,h.data,this.enableChannelBinding&&this.connection.stream),this.connection.sendSCRAMClientFinalMessage(this.saslSession.response)}catch(b){this.connection.emit("error",b)}}_handleAuthSASLFinal(h){try{s.finalizeSession(this.saslSession,h.data),this.saslSession=null}catch(b){this.connection.emit("error",b)}}_handleBackendKeyData(h){this.processID=h.processID,this.secretKey=h.secretKey}_handleReadyForQuery(h){this._connecting&&(this._connecting=!1,this._connected=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback&&(this._connectionCallback(null,this),this._connectionCallback=null),this.emit("connect"));const b=this._getActiveQuery();this._activeQuery=null,this.readyForQuery=!0,b&&b.handleReadyForQuery(this.connection),this._pulseQueryQueue()}_handleErrorWhileConnecting(h){if(!this._connectionError){if(this._connectionError=!0,clearTimeout(this.connectionTimeoutHandle),this._connectionCallback)return this._connectionCallback(h);this.emit("error",h)}}_handleErrorEvent(h){if(this._connecting)return this._handleErrorWhileConnecting(h);this._queryable=!1,this._errorAllQueries(h),this.emit("error",h)}_handleErrorMessage(h){if(this._connecting)return this._handleErrorWhileConnecting(h);const b=this._getActiveQuery();if(!b){this._handleErrorEvent(h);return}this._activeQuery=null,b.handleError(h,this.connection)}_handleRowDescription(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected rowDescription message from backend.");this._handleErrorEvent(w);return}b.handleRowDescription(h)}_handleDataRow(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected dataRow message from backend.");this._handleErrorEvent(w);return}b.handleDataRow(h)}_handlePortalSuspended(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected portalSuspended message from backend.");this._handleErrorEvent(w);return}b.handlePortalSuspended(this.connection)}_handleEmptyQuery(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected emptyQuery message from backend.");this._handleErrorEvent(w);return}b.handleEmptyQuery(this.connection)}_handleCommandComplete(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected commandComplete message from backend.");this._handleErrorEvent(w);return}b.handleCommandComplete(h,this.connection)}_handleParseComplete(){const h=this._getActiveQuery();if(h==null){const b=new Error("Received unexpected parseComplete message from backend.");this._handleErrorEvent(b);return}h.name&&(this.connection.parsedStatements[h.name]=h.text)}_handleCopyInResponse(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected copyInResponse message from backend.");this._handleErrorEvent(w);return}b.handleCopyInResponse(this.connection)}_handleCopyData(h){const b=this._getActiveQuery();if(b==null){const w=new Error("Received unexpected copyData message from backend.");this._handleErrorEvent(w);return}b.handleCopyData(h,this.connection)}_handleNotification(h){this.emit("notification",h)}_handleNotice(h){this.emit("notice",h)}getStartupConf(){const h=this.connectionParameters,b={user:h.user,database:h.database},w=h.application_name||h.fallback_application_name;return w&&(b.application_name=w),h.replication&&(b.replication=""+h.replication),h.statement_timeout&&(b.statement_timeout=String(parseInt(h.statement_timeout,10))),h.lock_timeout&&(b.lock_timeout=String(parseInt(h.lock_timeout,10))),h.idle_in_transaction_session_timeout&&(b.idle_in_transaction_session_timeout=String(parseInt(h.idle_in_transaction_session_timeout,10))),h.options&&(b.options=h.options),b}cancel(h,b){if(h.activeQuery===b){const w=this.connection;this.host&&this.host.indexOf("/")===0?w.connect(this.host+"/.s.PGSQL."+this.port):w.connect(this.port,this.host),w.on("connect",function(){w.cancel(h.processID,h.secretKey)})}else h._queryQueue.indexOf(b)!==-1&&h._queryQueue.splice(h._queryQueue.indexOf(b),1)}setTypeParser(h,b,w){return this._types.setTypeParser(h,b,w)}getTypeParser(h,b){return this._types.getTypeParser(h,b)}escapeIdentifier(h){return t.escapeIdentifier(h)}escapeLiteral(h){return t.escapeLiteral(h)}_pulseQueryQueue(){if(this.readyForQuery===!0){this._activeQuery=this._queryQueue.shift();const h=this._getActiveQuery();if(h){this.readyForQuery=!1,this.hasExecuted=!0;const b=h.submit(this.connection);b&&process.nextTick(()=>{h.handleError(b,this.connection),this.readyForQuery=!0,this._pulseQueryQueue()})}else this.hasExecuted&&(this._activeQuery=null,this.emit("drain"))}}query(h,b,w){let S,P,A,m,_;if(h==null)throw new TypeError("Client was passed a null or undefined query");return typeof h.submit=="function"?(A=h.query_timeout||this.connectionParameters.query_timeout,P=S=h,S.callback||(typeof b=="function"?S.callback=b:w&&(S.callback=w))):(A=h.query_timeout||this.connectionParameters.query_timeout,S=new o(h,b,w),S.callback||(P=new this._Promise((E,T)=>{S.callback=(C,N)=>C?T(C):E(N)}).catch(E=>{throw Error.captureStackTrace(E),E}))),A&&(_=S.callback||(()=>{}),m=setTimeout(()=>{const E=new Error("Query read timeout");process.nextTick(()=>{S.handleError(E,this.connection)}),_(E),S.callback=()=>{};const T=this._queryQueue.indexOf(S);T>-1&&this._queryQueue.splice(T,1),this._pulseQueryQueue()},A),S.callback=(E,T)=>{clearTimeout(m),_(E,T)}),this.binary&&!S.binary&&(S.binary=!0),S._result&&!S._result._types&&(S._result._types=this._types),this._queryable?this._ending?(process.nextTick(()=>{S.handleError(new Error("Client was closed and is not queryable"),this.connection)}),P):(this._queryQueue.length>0&&g(),this._queryQueue.push(S),this._pulseQueryQueue(),P):(process.nextTick(()=>{S.handleError(new Error("Client has encountered a connection error and is not queryable"),this.connection)}),P)}ref(){this.connection.ref()}unref(){this.connection.unref()}end(h){if(this._ending=!0,!this.connection._connecting||this._ended)if(h)h();else return this._Promise.resolve();if(this._getActiveQuery()||!this._queryable?this.connection.stream.destroy():this.connection.end(),h)this.connection.once("end",h);else return new this._Promise(b=>{this.connection.once("end",b)})}get queryQueue(){return f(),this._queryQueue}}return y.Query=o,qr=y,qr}var Dr,bn;function ia(){if(bn)return Dr;bn=1;const e=ze.EventEmitter,t=function(){},r=(u,l)=>{const f=u.findIndex(l);return f===-1?void 0:u.splice(f,1)[0]};class s{constructor(l,f,d){this.client=l,this.idleListener=f,this.timeoutId=d}}class n{constructor(l){this.callback=l}}function i(){throw new Error("Release called on client which has already been released to the pool.")}function o(u,l){if(l)return{callback:l,result:void 0};let f,d;const p=function(y,v){y?f(y):d(v)},g=new u(function(y,v){d=y,f=v}).catch(y=>{throw Error.captureStackTrace(y),y});return{callback:p,result:g}}function a(u,l){return function f(d){d.client=l,l.removeListener("error",f),l.on("error",()=>{u.log("additional client error after disconnection due to error",d)}),u._remove(l),u.emit("error",d,l)}}class c extends e{constructor(l,f){super(),this.options=Object.assign({},l),l!=null&&"password"in l&&Object.defineProperty(this.options,"password",{configurable:!0,enumerable:!1,writable:!0,value:l.password}),l!=null&&l.ssl&&l.ssl.key&&Object.defineProperty(this.options.ssl,"key",{enumerable:!1}),this.options.max=this.options.max||this.options.poolSize||10,this.options.min=this.options.min||0,this.options.maxUses=this.options.maxUses||1/0,this.options.allowExitOnIdle=this.options.allowExitOnIdle||!1,this.options.maxLifetimeSeconds=this.options.maxLifetimeSeconds||0,this.log=this.options.log||function(){},this.Client=this.options.Client||f||mi().Client,this.Promise=this.options.Promise||Vr.Promise,typeof this.options.idleTimeoutMillis>"u"&&(this.options.idleTimeoutMillis=1e4),this._clients=[],this._idle=[],this._expired=new WeakSet,this._pendingQueue=[],this._endCallback=void 0,this.ending=!1,this.ended=!1}_promiseTry(l){const f=this.Promise;return typeof f.try=="function"?f.try(l):new f(d=>d(l()))}_isFull(){return this._clients.length>=this.options.max}_isAboveMin(){return this._clients.length>this.options.min}_pulseQueue(){if(this.log("pulse queue"),this.ended){this.log("pulse queue ended");return}if(this.ending){this.log("pulse queue on ending"),this._idle.length&&this._idle.slice().map(f=>{this._remove(f.client)}),this._clients.length||(this.ended=!0,this._endCallback());return}if(!this._pendingQueue.length){this.log("no queued requests");return}if(!this._idle.length&&this._isFull())return;const l=this._pendingQueue.shift();if(this._idle.length){const f=this._idle.pop();clearTimeout(f.timeoutId);const d=f.client;d.ref&&d.ref();const p=f.idleListener;return this._acquireClient(d,l,p,!1)}if(!this._isFull())return this.newClient(l);throw new Error("unexpected condition")}_remove(l,f){const d=r(this._idle,g=>g.client===l);d!==void 0&&clearTimeout(d.timeoutId),this._clients=this._clients.filter(g=>g!==l);const p=this;l.end(()=>{p.emit("remove",l),typeof f=="function"&&f()})}connect(l){if(this.ending){const p=new Error("Cannot use a pool after calling end on the pool");return l?l(p):this.Promise.reject(p)}const f=o(this.Promise,l),d=f.result;if(this._isFull()||this._idle.length){if(this._idle.length&&process.nextTick(()=>this._pulseQueue()),!this.options.connectionTimeoutMillis)return this._pendingQueue.push(new n(f.callback)),d;const p=(v,h,b)=>{clearTimeout(y),f.callback(v,h,b)},g=new n(p),y=setTimeout(()=>{r(this._pendingQueue,v=>v.callback===p),g.timedOut=!0,f.callback(new Error("timeout exceeded when trying to connect"))},this.options.connectionTimeoutMillis);return y.unref&&y.unref(),this._pendingQueue.push(g),d}return this.newClient(new n(f.callback)),d}newClient(l){const f=new this.Client(this.options);this._clients.push(f);const d=a(this,f);this.log("checking client timeout");let p,g=!1;this.options.connectionTimeoutMillis&&(p=setTimeout(()=>{f.connection?(this.log("ending client due to timeout"),g=!0,f.connection.stream.destroy()):f.isConnected()||(this.log("ending client due to timeout"),g=!0,f.end())},this.options.connectionTimeoutMillis)),this.log("connecting new client"),f.connect(y=>{if(p&&clearTimeout(p),f.on("error",d),y)this.log("client failed to connect",y),this._clients=this._clients.filter(v=>v!==f),g&&(y=new Error("Connection terminated due to connection timeout",{cause:y})),this._pulseQueue(),l.timedOut||l.callback(y,void 0,t);else{if(this.log("new client connected"),this.options.onConnect){this._promiseTry(()=>this.options.onConnect(f)).then(()=>{this._afterConnect(f,l,d)},v=>{this._clients=this._clients.filter(h=>h!==f),f.end(()=>{this._pulseQueue(),l.timedOut||l.callback(v,void 0,t)})});return}return this._afterConnect(f,l,d)}})}_afterConnect(l,f,d){if(this.options.maxLifetimeSeconds!==0){const p=setTimeout(()=>{this.log("ending client due to expired lifetime"),this._expired.add(l),this._idle.findIndex(y=>y.client===l)!==-1&&this._acquireClient(l,new n((y,v,h)=>h()),d,!1)},this.options.maxLifetimeSeconds*1e3);p.unref(),l.once("end",()=>clearTimeout(p))}return this._acquireClient(l,f,d,!0)}_acquireClient(l,f,d,p){p&&this.emit("connect",l),this.emit("acquire",l),l.release=this._releaseOnce(l,d),l.removeListener("error",d),f.timedOut?p&&this.options.verify?this.options.verify(l,l.release):l.release():p&&this.options.verify?this.options.verify(l,g=>{if(g)return l.release(g),f.callback(g,void 0,t);f.callback(void 0,l,l.release)}):f.callback(void 0,l,l.release)}_releaseOnce(l,f){let d=!1;return p=>{d&&i(),d=!0,this._release(l,f,p)}}_release(l,f,d){if(l.on("error",f),l._poolUseCount=(l._poolUseCount||0)+1,this.emit("release",d,l),d||this.ending||!l._queryable||l._ending||l._poolUseCount>=this.options.maxUses)return l._poolUseCount>=this.options.maxUses&&this.log("remove expended client"),this._remove(l,this._pulseQueue.bind(this));if(this._expired.has(l))return this.log("remove expired client"),this._expired.delete(l),this._remove(l,this._pulseQueue.bind(this));let g;this.options.idleTimeoutMillis&&this._isAboveMin()&&(g=setTimeout(()=>{this._isAboveMin()&&(this.log("remove idle client"),this._remove(l,this._pulseQueue.bind(this)))},this.options.idleTimeoutMillis),this.options.allowExitOnIdle&&g.unref()),this.options.allowExitOnIdle&&l.unref(),this._idle.push(new s(l,f,g)),this._pulseQueue()}query(l,f,d){if(typeof l=="function"){const g=o(this.Promise,l);return setImmediate(function(){return g.callback(new Error("Passing a function as the first parameter to pool.query is not supported"))}),g.result}typeof f=="function"&&(d=f,f=void 0);const p=o(this.Promise,d);return d=p.callback,this.connect((g,y)=>{if(g)return d(g);let v=!1;const h=b=>{v||(v=!0,y.release(b),d(b))};y.once("error",h),this.log("dispatching query");try{y.query(l,f,(b,w)=>{if(this.log("query dispatched"),y.removeListener("error",h),!v)return v=!0,y.release(b),b?d(b):d(void 0,w)})}catch(b){return y.release(b),d(b)}}),p.result}end(l){if(this.log("ending"),this.ending){const d=new Error("Called end on pool more than once");return l?l(d):this.Promise.reject(d)}this.ending=!0;const f=o(this.Promise,l);return this._endCallback=f.callback,this._pulseQueue(),f.result}get waitingCount(){return this._pendingQueue.length}get idleCount(){return this._idle.length}get expiredCount(){return this._clients.reduce((l,f)=>l+(this._expired.has(f)?1:0),0)}get totalCount(){return this._clients.length}}return Dr=c,Dr}var jr={exports:{}};const oa={},aa=Object.freeze(Object.defineProperty({__proto__:null,default:oa},Symbol.toStringTag,{value:"Module"})),la=oo(aa);var kr={exports:{}},yn;function ca(){if(yn)return kr.exports;yn=1;const e=ze.EventEmitter,t=He,r=et(),s=kr.exports=function(i,o,a){e.call(this),i=r.normalizeQueryConfig(i,o,a),this.text=i.text,this.values=i.values,this.name=i.name,this.queryMode=i.queryMode,this.callback=i.callback,this.state="new",this._arrayMode=i.rowMode==="array",this._emitRowEvents=!1,this.on("newListener",(function(c){c==="row"&&(this._emitRowEvents=!0)}).bind(this))};t.inherits(s,e);const n={sqlState:"code",statementPosition:"position",messagePrimary:"message",context:"where",schemaName:"schema",tableName:"table",columnName:"column",dataTypeName:"dataType",constraintName:"constraint",sourceFile:"file",sourceLine:"line",sourceFunction:"routine"};return s.prototype.handleError=function(i){const o=this.native.pq.resultErrorFields();if(o)for(const a in o){const c=n[a]||a;i[c]=o[a]}this.callback?this.callback(i):this.emit("error",i),this.state="error"},s.prototype.then=function(i,o){return this._getPromise().then(i,o)},s.prototype.catch=function(i){return this._getPromise().catch(i)},s.prototype._getPromise=function(){return this._promise?this._promise:(this._promise=new Promise((function(i,o){this._once("end",i),this._once("error",o)}).bind(this)),this._promise)},s.prototype.submit=function(i){this.state="running";const o=this;this.native=i.native,i.native.arrayMode=this._arrayMode;let a=function(c,u,l){if(i.native.arrayMode=!1,setImmediate(function(){o.emit("_done")}),c)return o.handleError(c);o._emitRowEvents&&(l.length>1?u.forEach((f,d)=>{f.forEach(p=>{o.emit("row",p,l[d])})}):u.forEach(function(f){o.emit("row",f,l)})),o.state="end",o.emit("end",l),o.callback&&o.callback(null,l)};if(process.domain&&(a=process.domain.bind(a)),this.name){this.name.length>63&&(console.error("Warning! Postgres only supports 63 characters for query names."),console.error("You supplied %s (%s)",this.name,this.name.length),console.error("This can cause conflicts and silent errors executing queries"));const c=(this.values||[]).map(r.prepareValue);if(i.namedQueries[this.name]){if(this.text&&i.namedQueries[this.name]!==this.text){const u=new Error(`Prepared statements must be unique - '${this.name}' was used for a different statement`);return a(u)}return i.native.execute(this.name,c,a)}return i.native.prepare(this.name,this.text,c.length,function(u){return u?a(u):(i.namedQueries[o.name]=o.text,o.native.execute(o.name,c,a))})}else if(this.values){if(!Array.isArray(this.values)){const u=new Error("Query values must be an array");return a(u)}const c=this.values.map(r.prepareValue);i.native.query(this.text,c,a)}else this.queryMode==="extended"?i.native.query(this.text,[],a):i.native.query(this.text,a)},kr.exports}var vn;function ua(){if(vn)return jr.exports;vn=1;const e=He;var t;try{t=la}catch(u){throw u}const r=Gr(),s=ze.EventEmitter,n=He,i=ui(),o=ca(),a=e.deprecate(()=>{},"Calling client.query() when the client is already executing a query is deprecated and will be removed in pg@9.0. Use async/await or an external async flow control mechanism instead."),c=jr.exports=function(u){s.call(this),u=u||{},this._Promise=u.Promise||Vr.Promise,this._types=new r(u.types),this.native=new t({types:this._types}),this._queryQueue=[],this._ending=!1,this._connecting=!1,this._connected=!1,this._queryable=!0;const l=this.connectionParameters=new i(u);u.nativeConnectionString&&(l.nativeConnectionString=u.nativeConnectionString),this.user=l.user,Object.defineProperty(this,"password",{configurable:!0,enumerable:!1,writable:!0,value:l.password}),this.database=l.database,this.host=l.host,this.port=l.port,this.namedQueries={}};return c.Query=o,n.inherits(c,s),c.prototype._errorAllQueries=function(u){const l=f=>{process.nextTick(()=>{f.native=this.native,f.handleError(u)})};this._hasActiveQuery()&&(l(this._activeQuery),this._activeQuery=null),this._queryQueue.forEach(l),this._queryQueue.length=0},c.prototype._connect=function(u){const l=this;if(this._connecting){process.nextTick(()=>u(new Error("Client has already been connected. You cannot reuse a client.")));return}this._connecting=!0,this.connectionParameters.getLibpqConnectionString(function(f,d){if(l.connectionParameters.nativeConnectionString&&(d=l.connectionParameters.nativeConnectionString),f)return u(f);l.native.connect(d,function(p){if(p)return l.native.end(),u(p);l._connected=!0,l.native.on("error",function(g){l._queryable=!1,l._errorAllQueries(g),l.emit("error",g)}),l.native.on("notification",function(g){l.emit("notification",{channel:g.relname,payload:g.extra})}),l.emit("connect"),l._pulseQueryQueue(!0),u(null,this)})})},c.prototype.connect=function(u){if(u){this._connect(u);return}return new this._Promise((l,f)=>{this._connect(d=>{d?f(d):l(this)})})},c.prototype.query=function(u,l,f){let d,p,g,y,v;if(u==null)throw new TypeError("Client was passed a null or undefined query");if(typeof u.submit=="function")g=u.query_timeout||this.connectionParameters.query_timeout,p=d=u,typeof l=="function"&&(u.callback=l);else if(g=u.query_timeout||this.connectionParameters.query_timeout,d=new o(u,l,f),!d.callback){let h,b;p=new this._Promise((w,S)=>{h=w,b=S}).catch(w=>{throw Error.captureStackTrace(w),w}),d.callback=(w,S)=>w?b(w):h(S)}return g&&(v=d.callback||(()=>{}),y=setTimeout(()=>{const h=new Error("Query read timeout");process.nextTick(()=>{d.handleError(h,this.connection)}),v(h),d.callback=()=>{};const b=this._queryQueue.indexOf(d);b>-1&&this._queryQueue.splice(b,1),this._pulseQueryQueue()},g),d.callback=(h,b)=>{clearTimeout(y),v(h,b)}),this._queryable?this._ending?(d.native=this.native,process.nextTick(()=>{d.handleError(new Error("Client was closed and is not queryable"))}),p):(this._queryQueue.length>0&&a(),this._queryQueue.push(d),this._pulseQueryQueue(),p):(d.native=this.native,process.nextTick(()=>{d.handleError(new Error("Client has encountered a connection error and is not queryable"))}),p)},c.prototype.end=function(u){const l=this;this._ending=!0,this._connected||this.once("connect",this.end.bind(this,u));let f;return u||(f=new this._Promise(function(d,p){u=g=>g?p(g):d()})),this.native.end(function(){l._connected=!1,l._errorAllQueries(new Error("Connection terminated")),process.nextTick(()=>{l.emit("end"),u&&u()})}),f},c.prototype._hasActiveQuery=function(){return this._activeQuery&&this._activeQuery.state!=="error"&&this._activeQuery.state!=="end"},c.prototype._pulseQueryQueue=function(u){if(!this._connected||this._hasActiveQuery())return;const l=this._queryQueue.shift();if(!l){u||this.emit("drain");return}this._activeQuery=l,l.submit(this);const f=this;l.once("_done",function(){f._pulseQueryQueue()})},c.prototype.cancel=function(u){this._activeQuery===u?this.native.cancel(function(){}):this._queryQueue.indexOf(u)!==-1&&this._queryQueue.splice(this._queryQueue.indexOf(u),1)},c.prototype.ref=function(){},c.prototype.unref=function(){},c.prototype.setTypeParser=function(u,l,f){return this._types.setTypeParser(u,l,f)},c.prototype.getTypeParser=function(u,l){return this._types.getTypeParser(u,l)},c.prototype.isConnected=function(){return this._connected},jr.exports}var Br,wn;function En(){return wn||(wn=1,Br=ua()),Br}var xn;function mi(){return xn||(xn=1,(function(e){var t={};const r=na(),s=St(),n=hi(),i=di(),o=et(),a=ia(),c=Gr(),{DatabaseError:u}=pi(),{escapeIdentifier:l,escapeLiteral:f}=et(),d=v=>class extends a{constructor(b){super(b,v)}},p=function(v){this.defaults=s,this.Client=v,this.Query=this.Client.Query,this.Pool=d(this.Client),this._pools=[],this.Connection=n,this.types=_t(),this.DatabaseError=u,this.TypeOverrides=c,this.escapeIdentifier=l,this.escapeLiteral=f,this.Result=i,this.utils=o};let g=r,y=!1;try{y=!!t.NODE_PG_FORCE_NATIVE}catch{}y&&(g=En()),e.exports=new p(g),Object.defineProperty(e.exports,"native",{configurable:!0,enumerable:!1,get(){let v=null;try{v=new p(En())}catch(h){if(h.code!=="MODULE_NOT_FOUND")throw h}return Object.defineProperty(e.exports,"native",{value:v}),v}})})(ar)),ar.exports}var da=mi();const ee=Zn(da);ee.Client;const fa=ee.Pool;ee.Connection;ee.types;ee.Query;ee.DatabaseError;ee.escapeIdentifier;ee.escapeLiteral;ee.Result;ee.TypeOverrides;ee.defaults;const pa=Object.freeze(Object.defineProperty({__proto__:null,Pool:fa,default:ee},Symbol.toStringTag,{value:"Module"}));export{Ls as default};
