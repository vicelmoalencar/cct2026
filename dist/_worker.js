...
TRANSCRIÇÃO:
${C}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const M=((i=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:i.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,n]),e.json({transcript:C,description:O,sizeMB:m})}catch(n){return e.json({error:n.message},500)}});R.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});R.post("/api/lessons/:id/rent",J,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,i=L(e);await wt(i);const n=await i.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!n.length||!n[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=n[0],l=parseInt(o.rental_credits);if(!Number.isFinite(l)||l<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const u=await i.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(u.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:u[0].expires_at},400);const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const a=Qr(e),f=await Hr(a,s);if(f<l)return e.json({error:"Créditos insuficientes",available:f,required:l},400);if(!await Io(a,s,l)){const p=await Hr(a,s);return e.json({error:"Créditos insuficientes",available:p,required:l},400)}return await i.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
...
${a}`:""}`},...o.slice(-20),{role:"user",content:n}];for(let h=0;h<6;h++){const b=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Admin Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:p,tools:Po,tool_choice:"auto"})});if(!b.ok)return e.json({error:`OpenRouter: ${await b.text()}`},500);const m=(s=(await b.json()).choices)==null?void 0:s[0];if(!m)return e.json({error:"Resposta vazia do modelo"},500);const y=m.message,_=y.tool_calls;if(!_||_.length===0)return e.json({reply:y.content||""});const w=_[0],I=w.function.name;let A={};try{A=JSON.parse(w.function.arguments)}catch{}if(qo.has(I)){const E=Do(I,A);return e.json({reply:`⚠️ **Confirmação necessária**

${E}`,pendingAction:{tool:I,args:A,description:E}})}const g=await jo(I,A,u,e);p.push({role:"assistant",content:y.content||null,tool_calls:_}),p.push({role:"tool",content:JSON.stringify(g),tool_call_id:w.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(i){return console.error("Agent error:",i),e.json({error:i.message||"Erro no agente"},500)}});const Bo=[{type:"function",function:{name:"list_courses",description:"Lista todos os cursos publicados disponíveis na plataforma, com id, título e carga horária.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"get_course_details",description:"Retorna os módulos e a quantidade de aulas de um curso publicado.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso"}},required:["course_id"]}}},{type:"function",function:{name:"get_my_progress",description:"Retorna o progresso do aluno autenticado em todos os cursos que ele já iniciou: % concluído, aulas completadas e aulas totais por curso.",parameters:{type:"object",properties:{}}}},{type:"function",function:{name:"recommend_next_lessons",description:"Recomenda as próximas aulas que o aluno autenticado ainda não assistiu. Se course_id for informado, recomenda dentro daquele curso; caso contrário, considera todos os cursos que o aluno já iniciou.",parameters:{type:"object",properties:{course_id:{type:"number",description:"ID do curso (opcional)"},limit:{type:"number",description:"Máximo de aulas a recomendar (padrão 5)"}}}}},{type:"function",function:{name:"search_lessons",description:"Busca aulas por título/palavra-chave em cursos publicados.",parameters:{type:"object",properties:{query:{type:"string",description:"Termo de busca no título da aula"},limit:{type:"number",description:"Máximo de resultados (padrão 10)"}},required:["query"]}}},{type:"function",function:{name:"get_my_certificates",description:"Lista os certificados já emitidos para o aluno autenticado.",parameters:{type:"object",properties:{}}}}];async function Mo(e,t,r,s,i){switch(e){case"list_courses":return{courses:await r.sql("SELECT id, title, instructor, duration_hours FROM courses WHERE is_published = true ORDER BY title")};case"get_course_details":{const[n,o]=await Promise.all([r.sql("SELECT id, title, description, instructor, duration_hours FROM courses WHERE id = $1 AND is_published = true",[t.course_id]),r.sql(`SELECT m.id, m.title, m.order_index, COUNT(l.id)::int as lesson_count
           FROM modules m LEFT JOIN lessons l ON l.module_id = m.id
...
- Se o aluno perguntar sobre cancelamento, reembolso, cobrança ou alteração de plano, explique gentilmente que você ainda não pode resolver isso diretamente e oriente a falar com o suporte pelo WhatsApp.`},...l.slice(-20),{role:"user",content:o}];for(let p=0;p<5;p++){const h=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Student Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:c,tools:Bo,tool_choice:"auto"})});if(!h.ok)return e.json({error:`OpenRouter: ${await h.text()}`},500);const v=(r=(await h.json()).choices)==null?void 0:r[0];if(!v)return e.json({error:"Resposta vazia do modelo"},500);const m=v.message,y=m.tool_calls;if(!y||y.length===0)return e.json({reply:m.content||""});const _=y[0],w=_.function.name;let I={};try{I=JSON.parse(_.function.arguments)}catch{}const A=await Mo(w,I,u,i,d);c.push({role:"assistant",content:m.content||null,tool_calls:y}),c.push({role:"tool",content:JSON.stringify(A),tool_call_id:_.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(s){return console.error("Student agent error:",s),e.json({error:s.message||"Erro no assistente"},500)}});R.get("/api/admin/users",P,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `,[r,t]))[0]||{};return e.json({start_date:n.start_date||null,completion_date:n.completion_date||null})}catch(t){return console.error("Suggest certificate dates error:",t),e.json({error:t.message||"Failed to suggest dates"},500)}});R.post("/api/admin/certificates",P,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),i=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,start_date:t.start_date||null,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:i&&i.length>0?i[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],i=L(e),n=[];for(const l of s){const u=m=>{const y=String(m??"").trim();return!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined"?"":y},d=u(l.user_email||l.email).toLowerCase(),a=u(l.user_name||l.nome)||null,f=u(l.course_title||l.curso);let c=l.carga_horaria?parseInt(String(l.carga_horaria)):null;if(c!==null&&!Number.isFinite(c)&&(c=null),!d.includes("@")||!f){n.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const y=String(m).trim();if(!y||y.toLowerCase()==="null"||y.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(y)){const[_,w,I]=y.split("/");return`${I}-${w}-${_}T12:00:00Z`}return isNaN(new Date(y).getTime())?null:new Date(y).toISOString()},h=p(l.data_final||l.completion_date),b=p(l.data_inicio||l.data_inicial||l.start_date);if(!c){const m=await i.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)c=parseInt(m[0].duration_hours);else{const y=w=>w.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),_=y(f);if(_.length>0){const I=(await i.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=y(A.title).join(" ");return _.every(E=>g.includes(E))});I&&(c=parseInt(I.duration_hours))}}}const v=await i.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await i.sql(`UPDATE certificates
           SET user_name = COALESCE($1, user_name),
               ...
               updated_at = NOW()
           WHERE id = $5`,[a,c,h,b,v[0].id]),n.push({user_email:d,course_title:f,status:"updated",certificate_id:v[0].id});else{const m=new Uint8Array(4);crypto.getRandomValues(m);const y="CCT-"+new Date().getFullYear()+"-"+Array.from(m).map(I=>I.toString(16).padStart(2,"0")).join("").toUpperCase(),_=new Date().toISOString(),w=await i.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, start_date, created_at)
           VALUES ($1, $2, $3, $4, $5, $5, $6, COALESCE($7, $6::timestamp), $8::date, $6)
           RETURNING id`,[d,a||"Aluno",f,c,y,_,h,b]);n.push({user_email:d,course_title:f,status:"created",certificate_id:(t=w[0])==null?void 0:t.id,verification_code:y})}}const o=n.filter(l=>l.status==="error").length;return e.json({success:o===0,total:s.length,results:n})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let i=r.course_title;if(r.course_id){const n=await s.query("courses",{select:"title",filters:{id:r.course_id}});n&&n.length>0&&(i=n[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:i,carga_horaria:r.carga_horaria,start_date:r.start_date||null,completion_date:r.completion_date||null,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Uo(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,c)=>`<div class="mod-item"><span class="mod-num">${String(c+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=s?`
