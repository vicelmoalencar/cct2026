...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      `):e.redirect(`/?error=${r}`);const s=t.searchParams.get("access_token")||((u=t.hash.match(/access_token=([^&]+)/))==null?void 0:u[1]),n=t.searchParams.get("refresh_token")||((d=t.hash.match(/refresh_token=([^&]+)/))==null?void 0:d[1]),i=t.searchParams.get("type")||((a=t.hash.match(/type=([^&]+)/))==null?void 0:a[1]);if(!s)return e.redirect("/?error=no_token");try{const p=(f=JSON.parse(atob(s.split(".")[1])).amr)==null?void 0:f.some(h=>h.method==="otp");if(i==="recovery"||p)return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}catch{if(i==="recovery")return e.redirect(`/reset-password#access_token=${s}&refresh_token=${n||""}&type=recovery`)}return J(e,"sb-access-token",s,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),n&&J(e,"sb-refresh-token",n,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.redirect("/?auth=success")}R.post("/api/auth/callback",async e=>{try{const{access_token:t,refresh_token:r}=await e.req.json();return t?(J(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),r&&J(e,"sb-refresh-token",r,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0})):e.json({error:"No access token"},400)}catch{return e.json({error:"Callback failed"},500)}});R.post("/api/auth/forgot-password",async e=>{try{const{email:t}=await e.req.json();if(!t)return e.json({error:"Email is required"},400);const r=e.req.header("host")||"localhost:3000",n=`${r.includes("localhost")?"http":"https"}://${r}/auth/callback`,i=await fetch(`${e.env.SUPABASE_URL}/auth/v1/recover`,{method:"POST",headers:{"Content-Type":"application/json",apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({email:t,options:{redirectTo:n}})});if(i.ok)return e.json({success:!0,message:"Se o email estiver cadastrado, você receberá um link de recuperação. O link é válido por 1 hora."});const o=await i.json();return e.json({error:o.error_description||"Failed to send reset email"},400)}catch{return e.json({error:"Failed to process request"},500)}});R.post("/api/auth/reset-password",async e=>{try{const t=await e.req.json(),{token:r,password:s}=t;if(console.log("🔐 Password reset attempt"),console.log("   Token present:",!!r),console.log("   Token length:",r==null?void 0:r.length),console.log("   Password length:",s==null?void 0:s.length),!r||!s)return console.error("❌ Missing token or password"),e.json({error:"Token e senha são obrigatórios"},400);if(s.length<6)return console.error("❌ Password too short"),e.json({error:"A senha deve ter pelo menos 6 caracteres"},400);console.log("📨 Calling Supabase to update password...");const n=await fetch(`${e.env.SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:e.env.SUPABASE_ANON_KEY},body:JSON.stringify({password:s})});if(console.log("📨 Supabase response:",{status:n.status,ok:n.ok}),!n.ok){const o=await n.json();console.error("❌ Password reset failed:",o);let l="Falha ao redefinir senha";return o.error_code==="same_password"?l="A nova senha deve ser diferente da senha atual":o.msg?l=o.msg:o.error_description?l=o.error_description:o.message&&(l=o.message),e.json({error:l},400)}const i=await n.json();return console.log("✅ Password reset successful"),i.access_token&&J(e,"sb-access-token",i.access_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),i.refresh_token&&J(e,"sb-refresh-token",i.refresh_token,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:604800}),e.json({success:!0,message:"Senha alterada com sucesso!"})}catch{return e.json({error:"Failed to reset password"},500)}});async function Yr(e,t,r,s){try{return await new io(t,r).query("users",{select:"id, email, isadmin",filters:{email:e,isadmin:!0},single:!0},s)!==null}catch(n){return console.error("Error checking admin access in Supabase users:",n),!1}}async function P(e,t){const r=V(e,"sb-access-token");if(!r)return e.json({error:"Unauthorized"},401);const s=await G(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!s)return e.json({error:"Invalid token"},401);if(!await Yr(s.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))return e.json({error:"Forbidden - Admin only"},403);e.set("user",s),await t()}R.get("/api/admin/check",async e=>{const t=V(e,"sb-access-token");if(!t)return e.json({isAdmin:!1});const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({isAdmin:!1});const s=await Yr(r.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,t);return e.json({isAdmin:s})});R.post("/api/admin/impersonate",P,async e=>{try{const{user_email:t}=await e.req.json();if(!t)return e.json({error:"user_email is required"},400);console.log(`🎭 Admin impersonating user: ${t}`);const r=L(e);let s=await r.sql("SELECT id, email, nome FROM users WHERE lower(email) = lower($1) LIMIT 1",[t]);if((!s||s.length===0)&&(s=await r.sql(`SELECT NULL::integer AS id, email_membro AS email, NULL::text AS nome
         FROM member_subscriptions
         WHERE lower(email_membro) = lower($1)
         ORDER BY data_expiracao DESC NULLS LAST
         LIMIT 1`,[t])),!s||s.length===0)return e.json({error:"User not found"},404);const n=s[0],i={email:t,nome:n.nome||"Usuário",impersonated:!0,impersonated_at:new Date().toISOString(),user_id:n.id,signature:Buffer.from(`${t}:${e.env.SUPABASE_ANON_KEY}`).toString("base64")},o=`IMPERSONATE:${Buffer.from(JSON.stringify(i)).toString("base64")}`,l=V(e,"sb-access-token");return l&&J(e,"sb-admin-backup-token",l,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),J(e,"sb-access-token",o,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:86400}),console.log(`✅ Impersonation session set for ${t}`),e.json({success:!0,user_email:t,user_name:n.nome})}catch(t){return console.error("Impersonation error:",t),e.json({error:t.message||"Failed to impersonate user"},500)}});R.post("/api/admin/exit-impersonation",async e=>{try{const t=V(e,"sb-admin-backup-token");return t?(J(e,"sb-access-token",t,{httpOnly:!0,secure:!0,sameSite:"Lax",maxAge:3600}),et(e,"sb-admin-backup-token"),e.json({success:!0})):e.json({error:"No admin session to restore"},400)}catch(t){return console.error("Exit impersonation error:",t),e.json({error:t.message||"Failed to exit impersonation"},500)}});R.post("/api/admin/courses",P,async e=>{try{const{title:t,description:r,duration_hours:s,instructor:n,offers_certificate:i,is_published:o,min_completion_days:l}=await e.req.json(),d=await L(e).insert("courses",{title:t,description:r||null,duration_hours:s||0,instructor:n||"Vicelmo",offers_certificate:i!==void 0?i:!0,is_published:o!==void 0?o:!0,min_completion_days:l||null});return e.json({success:!0,course_id:d[0].id})}catch(t){return console.error("Create course error:",t),e.json({error:t.message||"Failed to create course"},500)}});R.put("/api/admin/courses/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,duration_hours:n,instructor:i,offers_certificate:o,is_published:l,min_completion_days:u}=await e.req.json();return await L(e).update("courses",{id:t},{title:r,description:s||null,duration_hours:n||0,instructor:i||"Vicelmo",offers_certificate:o!==void 0?o:!0,is_published:l!==void 0?l:!0,min_completion_days:u||null}),e.json({success:!0})}catch{return e.json({error:"Failed to update course"},500)}});R.delete("/api/admin/courses/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("courses",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete course"},500)}});R.get("/api/admin/courses/find",P,async e=>{try{const t=e.req.query("title");if(!t)return e.json({error:"Title is required"},400);const r=L(e),s=await r.sql("SELECT * FROM courses WHERE lower(title) = lower($1) LIMIT 1",[t]);if(s&&s.length>0)return e.json({course:s[0]});const n=await r.sql("SELECT * FROM courses WHERE lower(title) ILIKE lower($1) LIMIT 1",[`%${t}%`]);return e.json({course:n&&n.length>0?n[0]:null})}catch(t){return console.error("Find course error:",t),e.json({error:t.message||"Failed to find course"},500)}});R.post("/api/admin/modules",P,async e=>{try{const{course_id:t,title:r,description:s,order_index:n}=await e.req.json(),o=await L(e).insert("modules",{course_id:t,title:r,description:s||null,order_index:n||0});return e.json({success:!0,module_id:o[0].id})}catch(t){return console.error("Create module error:",t),e.json({error:t.message||"Failed to create module"},500)}});R.post("/api/admin/modules-reorder",P,async e=>{try{const{modules:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"modules must be an array"},400);const r=L(e);for(const{id:s,order_index:n}of t)await r.update("modules",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder modules error:",t),e.json({error:t.message||"Failed to reorder modules"},500)}});R.put("/api/admin/modules/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,order_index:n}=await e.req.json();return await L(e).update("modules",{id:t},{title:r,description:s||null,order_index:n}),e.json({success:!0})}catch{return e.json({error:"Failed to update module"},500)}});R.delete("/api/admin/modules/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("modules",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete module"},500)}});R.get("/api/admin/modules/find",P,async e=>{try{const t=e.req.query("course_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"course_id and title are required"},400);const n=await L(e).query("modules",{select:"*",filters:{course_id:t,title:r},limit:1});return n&&n.length>0?e.json({module:n[0]}):e.json({module:null})}catch(t){return console.error("Find module error:",t),e.json({error:t.message||"Failed to find module"},500)}});R.post("/api/admin/lessons",P,async e=>{try{const{module_id:t,title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:l,free_trial:u,support_text:d,transcript:a,attachments:f,rentable:c,rental_credits:p}=await e.req.json();let h=null;n&&i&&(n==="youtube"?h=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?h=`https://vimeo.com/${i}`:h=i);const v=await L(e).insert("lessons",{module_id:t,title:r,description:s||null,video_url:h,video_provider:n||null,video_id:i||null,duration_minutes:o||0,order_index:l||0,teste_gratis:u||!1,support_text:d||null,transcript:a||null,attachments:JSON.stringify(f||[]),rentable:c||!1,rental_credits:p||0});return e.json({success:!0,lesson_id:v[0].id})}catch(t){return console.error("Create lesson error:",t),e.json({error:t.message||"Failed to create lesson"},500)}});R.post("/api/admin/lessons-reorder",P,async e=>{try{const{lessons:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"lessons must be an array"},400);const r=L(e);for(const{id:s,order_index:n}of t)await r.update("lessons",{id:Number(s)},{order_index:Number(n)});return e.json({success:!0})}catch(t){return console.error("Reorder lessons error:",t),e.json({error:t.message||"Failed to reorder lessons"},500)}});R.put("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id"),{title:r,description:s,video_provider:n,video_id:i,duration_minutes:o,order_index:l,free_trial:u,support_text:d,transcript:a,attachments:f,rentable:c,rental_credits:p}=await e.req.json();let h=null;n&&i&&(n==="youtube"?h=`https://www.youtube.com/watch?v=${i}`:n==="vimeo"?h=`https://vimeo.com/${i}`:h=i);const y=L(e),v=parseInt(t);await y.update("lessons",{id:v},{title:r,description:s||null,video_url:h,video_provider:n||null,video_id:i||null,duration_minutes:parseInt(o)||0,order_index:parseInt(l)||0,teste_gratis:u===!0||u==="true"});try{await y.sql("UPDATE lessons SET support_text = $1, transcript = $2, attachments = $3::jsonb WHERE id = $4",[d||null,a||null,JSON.stringify(f||[]),v])}catch(m){console.warn("support_text/transcript/attachments columns may not exist:",m.message)}return await y.sql("UPDATE lessons SET rentable = $1, rental_credits = $2 WHERE id = $3",[c===!0||c==="true",parseInt(p)||0,v]),e.json({success:!0})}catch(t){return console.error("Update lesson error:",t),e.json({error:t.message||"Failed to update lesson"},500)}});R.post("/api/admin/lessons/:id/generate-transcript",P,async e=>{var t,r,s;try{const n=parseInt(e.req.param("id")),{context:i}=await e.req.json(),l=await L(e).sql("SELECT title, transcript FROM lessons WHERE id = $1",[n]);if(!l.length)return e.json({error:"Aula não encontrada"},404);const u=l[0];if(!u.transcript)return e.json({error:"Esta aula não possui transcrição para estruturar."},400);const d=e.env.VITE_OPENROUTER_API_KEY;if(!d)return e.json({error:"VITE_OPENROUTER_API_KEY não configurada no ambiente"},500);const a="Você é um especialista em direito trabalhista, liquidação de sentença judicial e uso do software PJe-Calc. Sua tarefa é pegar transcrições brutas de aulas e organizá-las em Markdown estruturado, claro e didático.",f=`Abaixo está a transcrição bruta de uma aula chamada "${u.title}".${i?`
...
TRANSCRIÇÃO:
${C}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const M=((n=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,i]),e.json({transcript:C,description:O,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});R.post("/api/admin/lessons/:id/groq-transcribe",P,async e=>{var t,r,s,n;try{const i=parseInt(e.req.param("id")),o=L(e),l=await o.sql("SELECT id, title, video_provider, video_id FROM lessons WHERE id = $1",[i]);if(!l.length)return e.json({error:"Aula não encontrada"},404);const u=l[0];if(u.video_provider!=="vimeo")return e.json({error:"Esta aula não usa Vimeo como provedor"},400);if(!u.video_id||!/^\d+$/.test(u.video_id))return e.json({error:"ID do vídeo Vimeo inválido"},400);const d=((t=globalThis.process)==null?void 0:t.env)||{},a=e.env.VIMEO_ACCESS_TOKEN||d.VIMEO_ACCESS_TOKEN,f=e.env.GROQ_API_KEY||d.GROQ_API_KEY,c=e.env.VITE_OPENROUTER_API_KEY||d.VITE_OPENROUTER_API_KEY;if(!a)return e.json({error:"VIMEO_ACCESS_TOKEN não configurado"},500);if(!f){const S=Object.keys(d).filter(q=>q.toLowerCase().includes("groq"));return e.json({error:`GROQ_API_KEY não configurado. Chaves GROQ encontradas: [${S.join(", ")}]`},500)}const p=await fetch(`https://api.vimeo.com/videos/${u.video_id}?fields=download`,{headers:{Authorization:`Bearer ${a}`}});if(!p.ok)return e.json({error:`Vimeo API ${p.status}`},502);const y=((await p.json()).download||[]).filter(S=>S.link&&S.size).sort((S,q)=>S.size-q.size);if(!y.length)return e.json({error:"Download não disponível para este vídeo"},400);const v=y[0],m=(v.size/1024/1024).toFixed(1),b=await fetch(v.link);if(!b.ok)return e.json({error:`Download falhou: HTTP ${b.status}`},502);let w=Buffer.from(await b.arrayBuffer()),x="video.mp4";const I=24*1024*1024;if(w.length>I){try{const S=await import("os"),q=await import("path"),D=await import("fs"),M=await import("child_process"),F=q.join(S.tmpdir(),`cct_g_${i}.mp4`),$=q.join(S.tmpdir(),`cct_g_${i}.mp3`);D.writeFileSync(F,w),M.spawnSync("ffmpeg",["-y","-i",F,"-vn","-ar","16000","-ac","1","-b:a","32k",$],{stdio:"pipe"}).status===0&&D.existsSync($)&&(w=D.readFileSync($),x="audio.mp3",D.unlinkSync($)),D.unlinkSync(F)}catch{}if(w.length>I)return e.json({error:`Arquivo muito grande (${m} MB). Use o script de transcrição em lote.`},400)}const A="----GroqBoundary"+Date.now().toString(16),g=`\r
`,E=x.endsWith(".mp3")?"audio/mpeg":"video/mp4",_=Buffer.concat([Buffer.from(`--${A}${g}Content-Disposition: form-data; name="file"; filename="${x}"${g}Content-Type: ${E}${g}${g}`),w,Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="model"${g}${g}whisper-large-v3-turbo`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="language"${g}${g}pt`),Buffer.from(`${g}--${A}${g}Content-Disposition: form-data; name="response_format"${g}${g}text`),Buffer.from(`${g}--${A}--${g}`)]),T=await fetch("https://api.groq.com/openai/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${f}`,"Content-Type":`multipart/form-data; boundary=${A}`,"Content-Length":String(_.length)},body:_});if(!T.ok)return e.json({error:`Groq Whisper ${T.status}: ${await T.text()}`},502);let C=(await T.text()).trim(),O=null;if(c&&C){const S=`Você é um assistente especializado em cursos de PJe-Calc (cálculos trabalhistas no sistema judicial brasileiro).
...
TRANSCRIÇÃO:
${C}`,q=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:[{role:"user",content:S}]})});if(q.ok){const M=((n=(s=(r=(await q.json()).choices)==null?void 0:r[0])==null?void 0:s.message)==null?void 0:n.content)||"";if(M){C=M;const F=M.match(/##\s*Resumo\s*\n([\s\S]*?)(?=\n##\s|\n*$)/);F&&(O=F[1].trim())}}}return await o.sql("UPDATE lessons SET transcript = $1, description = CASE WHEN $2::text IS NOT NULL THEN $2 ELSE description END WHERE id = $3",[C,O,i]),e.json({transcript:C,description:O,sizeMB:m})}catch(i){return e.json({error:i.message},500)}});R.delete("/api/admin/lessons/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("lessons",{id:t}),e.json({success:!0})}catch{return e.json({error:"Failed to delete lesson"},500)}});R.post("/api/lessons/:id/rent",K,async e=>{try{const t=parseInt(e.req.param("id")),s=e.get("user").email,n=L(e);await Et(n);const i=await n.sql("SELECT id, title, rentable, rental_credits FROM lessons WHERE id = $1",[t]);if(!i.length||!i[0].rentable)return e.json({error:"Esta aula não está disponível para aluguel"},400);const o=i[0],l=parseInt(o.rental_credits);if(!Number.isFinite(l)||l<=0)return e.json({error:"Créditos de aluguel inválidos para esta aula"},400);const u=await n.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[s,t]);if(u.length>0)return e.json({error:"Você já possui acesso ativo a esta aula",expires_at:u[0].expires_at},400);const d=e.env.DATABASE_URL_CREDITOS||e.env.DATABASE_SUITEPLUS;console.log("Credits DB configured:",!!d,"| prefix:",d==null?void 0:d.substring(0,30));const a=Qr(e),f=await Hr(a,s);if(f<l)return e.json({error:"Créditos insuficientes",available:f,required:l},400);if(!await Io(a,s,l)){const p=await Hr(a,s);return e.json({error:"Créditos insuficientes",available:p,required:l},400)}return await n.sql(`INSERT INTO lesson_rentals (user_email, lesson_id, credits_paid, rented_at, expires_at)
  ...
  `),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_trail ON trail_lessons(trail_id)"),await e.sql("CREATE INDEX IF NOT EXISTS idx_trail_lessons_lesson ON trail_lessons(lesson_id)")}R.get("/api/trails",K,async e=>{try{const t=e.get("user"),r=L(e);await Ne(r);const s=await r.sql(`
    ...
    `,[t,r.email]);return e.json({trail:i,lessons:o})}catch(t){return console.error("Get trail error:",t),e.json({error:t.message||"Erro ao buscar trilha"},500)}});R.get("/api/admin/trails",P,async e=>{try{const t=L(e);await Ne(t);const r=await t.sql(`
      SELECT t.*, COUNT(tl.id)::int AS lessons_count
      FROM trails t
      LEFT JOIN trail_lessons tl ON tl.trail_id = t.id
      GROUP BY t.id
      ORDER BY t.order_index ASC, t.created_at DESC
    `);return e.json({trails:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails",P,async e=>{try{const{title:t,description:r,is_published:s,order_index:n}=await e.req.json();if(!t)return e.json({error:"title is required"},400);const i=L(e);await Ne(i);const o=await i.insert("trails",{title:t,description:r||null,is_published:s??!1,order_index:n??0});return e.json({success:!0,trail_id:o[0].id,trail:o[0]})}catch(t){return e.json({error:t.message},500)}});R.put("/api/admin/trails/:id",P,async e=>{try{const t=parseInt(e.req.param("id")),r=await e.req.json(),s=L(e);await Ne(s);const n=["title","description","is_published","order_index"],i={updated_at:new Date().toISOString()};for(const o of n)o in r&&(i[o]=r[o]);return await s.update("trails",{id:t},i),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/trails/:id",P,async e=>{try{const t=parseInt(e.req.param("id")),r=L(e);return await Ne(r),await r.delete("trails",{id:t}),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/lessons",P,async e=>{var t;try{const r=parseInt(e.req.param("id")),{lesson_id:s}=await e.req.json();if(!s)return e.json({error:"lesson_id required"},400);const n=L(e);await Ne(n);const o=(((t=(await n.sql("SELECT COALESCE(MAX(order_index), -1) AS max_idx FROM trail_lessons WHERE trail_id = $1",[r]))[0])==null?void 0:t.max_idx)??-1)+1;return await n.sql("INSERT INTO trail_lessons (trail_id, lesson_id, order_index) VALUES ($1, $2, $3) ON CONFLICT (trail_id, lesson_id) DO NOTHING",[r,s,o]),e.json({success:!0})}catch(r){return e.json({error:r.message},500)}});R.delete("/api/admin/trails/:id/lessons/:lessonId",P,async e=>{try{const t=parseInt(e.req.param("id")),r=parseInt(e.req.param("lessonId"));return await L(e).sql("DELETE FROM trail_lessons WHERE trail_id = $1 AND lesson_id = $2",[t,r]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/trails/:id/reorder",P,async e=>{try{const t=parseInt(e.req.param("id")),{lessons:r}=await e.req.json();if(!Array.isArray(r))return e.json({error:"lessons array required"},400);const s=L(e);for(const n of r)await s.sql("UPDATE trail_lessons SET order_index = $1 WHERE trail_id = $2 AND lesson_id = $3",[n.order_index,t,n.lesson_id]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/trails/search-lessons",P,async e=>{try{const t=e.req.query("q")||"",r=e.req.query("course_id"),s=L(e),n=r?`AND co.id = ${parseInt(r)}`:"",i=t?"AND (l.title ILIKE $1)":"",o=t?[`%${t}%`]:[],l=await s.sql(`
    ...
    `,o);return e.json({lessons:l})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/lessons/find",P,async e=>{try{const t=e.req.query("module_id"),r=e.req.query("title");if(!t||!r)return e.json({error:"module_id and title are required"},400);const n=await L(e).query("lessons",{select:"*",filters:{module_id:t,title:r},limit:1});return n&&n.length>0?e.json({lesson:n[0]}):e.json({lesson:null})}catch(t){return console.error("Find lesson error:",t),e.json({error:t.message||"Failed to find lesson"},500)}});function Jr(e,t){const r=Array.isArray(e.alternatives)?e.alternatives:[],s=Array.isArray(e.tags)?e.tags:String(e.tags||"").split(",").map(n=>n.trim()).filter(Boolean);return{title:e.title||null,statement_html:e.statement_html||"",question_type:e.question_type||"multiple_choice",alternatives:JSON.stringify(r),answer_key:JSON.stringify(e.answer_key||{}),technical_comment_html:e.technical_comment_html||null,difficulty:e.difficulty||"medio",theme:e.theme||null,subtheme:e.subtheme||null,legal_basis:e.legal_basis||null,weight:Number(e.weight||1),estimated_minutes:parseInt(e.estimated_minutes)||5,tags:JSON.stringify(s),status:e.status||"draft",professor:e.professor||null,course_id:e.course_id?parseInt(e.course_id):null,lesson_id:e.lesson_id?parseInt(e.lesson_id):null,source_transcript:e.source_transcript||null,ai_generated:e.ai_generated===!0||e.ai_generated==="true",order_index:parseInt(e.order_index)||0,usage_count:parseInt(e.usage_count)||0,attempts_count:parseInt(e.attempts_count)||0,correct_count:parseInt(e.correct_count)||0,wrong_count:parseInt(e.wrong_count)||0,created_by:t||e.created_by||null}}function $r(e){if(typeof e=="string")return e.replace(/\b(conforme|segundo|de acordo com|com base na|a partir da)\s+a\s+transcri[cç][aã]o\b/gi,"").replace(/\bna\s+transcri[cç][aã]o\b/gi,"no material de estudo").replace(/\bda\s+transcri[cç][aã]o\b/gi,"do material de estudo").replace(/\btranscri[cç][aã]o\b/gi,"material de estudo").replace(/\s{2,}/g," ").replace(/\s+([,.;:!?])/g,"$1").trim();if(Array.isArray(e))return e.map($r).filter(t=>!(typeof t=="string"&&/transcri[cç][aã]o/i.test(t)));if(e&&typeof e=="object"){const t={};for(const[r,s]of Object.entries(e))t[r]=$r(s);return t}return e}R.get("/api/admin/questions",P,async e=>{try{const t=L(e);await ie(t);const r=[],s=[];let n=1;const i=(m,b)=>{r.push(m.replace("?",`$${n++}`)),s.push(b)},o=e.req.query("q"),l=e.req.query("theme"),u=e.req.query("professor"),d=e.req.query("course_id"),a=e.req.query("difficulty"),f=e.req.query("question_type"),c=e.req.query("status"),p=e.req.query("from"),h=e.req.query("to");o&&(r.push(`(title ILIKE $${n} OR statement_html ILIKE $${n+1} OR legal_basis ILIKE $${n+2})`),s.push(`%${o}%`,`%${o}%`,`%${o}%`),n+=3),l&&i("theme = ?",l),u&&i("professor = ?",u),d&&i("course_id = ?",parseInt(d)),a&&i("difficulty = ?",a),f&&i("question_type = ?",f),c&&i("status = ?",c),p&&i("created_at >= ?",p),h&&i("created_at <= ?",h);const y=r.length?`WHERE ${r.join(" AND ")}`:"",v=await t.sql(`
      ...
      LIMIT 500
    `,s);return e.json({questions:v})}catch(t){return console.error("List questions error:",t),e.json({error:t.message||"Failed to list questions"},500)}});R.get("/api/admin/questions/stats",P,async e=>{try{const t=L(e);await ie(t);const r=await t.sql(`
      SELECT
        COUNT(*)::int AS total,
      ...
      FROM question_bank
    `),s=await t.sql("SELECT id, title, wrong_count, attempts_count FROM question_bank ORDER BY wrong_count DESC, attempts_count DESC LIMIT 5"),n=await t.sql("SELECT id, title, usage_count FROM question_bank ORDER BY usage_count DESC LIMIT 5");return e.json({stats:r[0]||{},mostWrong:s,mostUsed:n})}catch(t){return e.json({error:t.message},500)}});R.get("/api/admin/questions/:id/versions",P,async e=>{try{const t=L(e);await ie(t);const r=await t.sql("SELECT * FROM question_bank_versions WHERE question_id = $1 ORDER BY version DESC, created_at DESC",[parseInt(e.req.param("id"))]);return e.json({versions:r})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions",P,async e=>{try{const t=e.get("user"),r=L(e);await ie(r);const s=Jr(await e.req.json(),t==null?void 0:t.email),i=(await r.sql(`
    ...
    `,[`${i.title||"Questao"} (copia)`,i.statement_html,i.question_type,JSON.stringify(i.alternatives||[]),JSON.stringify(i.answer_key||{}),i.technical_comment_html,i.difficulty,i.theme,i.subtheme,i.legal_basis,i.weight,i.estimated_minutes,JSON.stringify(i.tags||[]),i.professor,i.course_id,i.lesson_id,i.source_transcript,i.ai_generated,i.order_index+1,(t==null?void 0:t.email)||null]);return e.json({success:!0,question:o[0]})}catch(t){return e.json({error:t.message},500)}});R.delete("/api/admin/questions/:id",P,async e=>{try{const t=L(e);return await ie(t),await t.sql("DELETE FROM question_bank WHERE id = $1",[parseInt(e.req.param("id"))]),e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions-reorder",P,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions must be an array"},400);const r=L(e);await ie(r);for(const s of t)await r.sql("UPDATE question_bank SET order_index = $1, updated_at = NOW() WHERE id = $2",[parseInt(s.order_index),parseInt(s.id)]);return e.json({success:!0})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/import",P,async e=>{try{const{questions:t}=await e.req.json();if(!Array.isArray(t))return e.json({error:"questions array required"},400);const r=e.get("user"),s=L(e);await ie(s);let n=0;for(const i of t){const o=Jr(i,r==null?void 0:r.email);await s.sql(`
      ...
      `,[o.title,o.statement_html,o.question_type,o.alternatives,o.answer_key,o.technical_comment_html,o.difficulty,o.theme,o.subtheme,o.legal_basis,o.weight,o.estimated_minutes,o.tags,o.status,o.professor,o.course_id,o.lesson_id,o.source_transcript,o.ai_generated,o.order_index,o.created_by]),n++}return e.json({success:!0,created:n})}catch(t){return e.json({error:t.message},500)}});R.post("/api/admin/questions/generate-ai",P,async e=>{var t,r,s;try{const{lesson_id:n,transcript:i,count:o=5,types:l=["multiple_choice","true_false","discursive"],difficulty:u="misto",context:d=""}=await e.req.json(),a=L(e);await ie(a);let f=i||"",c=null;if(n&&(c=(await a.sql(`
...
${(t.emails||[]).join(`
`)}`;default:return`Executar ação **${e}** com args: ${JSON.stringify(t)}`}}async function jo(e,t,r,s){var n,i,o,l,u,d,a;switch(e){case"search_users":{const f=`%${t.query}%`,c=Math.min(t.limit||10,50),p=await r.sql(`SELECT u.id, u.email, u.nome, u.ativo, u.dt_expiracao,
         ...
         WHERE l.id = $1`,[t.lesson_id]))[0]||null};case"list_comments":{const f=Math.min(t.limit||20,100);return{comments:t.lesson_id?await r.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC LIMIT $2",[t.lesson_id,f]):await r.sql("SELECT * FROM comments ORDER BY created_at DESC LIMIT $1",[f])}}case"dashboard_stats":{const[f,c,p,h,y,v,m]=await Promise.all([r.sql("SELECT COUNT(*)::int as total FROM users"),r.sql("SELECT COUNT(*)::int as total FROM member_subscriptions WHERE data_expiracao > NOW() AND ativo = true"),r.sql("SELECT COUNT(*)::int as total FROM certificates"),r.sql("SELECT COUNT(*)::int as total FROM comments WHERE admin_reply IS NULL"),r.sql("SELECT COUNT(*)::int as total FROM courses"),r.sql("SELECT COUNT(*)::int as total FROM lessons"),(()=>{const b=s.env.DATABASE_SUITEPLUS;return b?new $e(b).sql("SELECT COUNT(*)::int as total FROM user_subscriptions WHERE status = 'active' AND product_id = 4 AND expires_at > NOW()").catch(()=>[{total:null}]):Promise.resolve([{total:null}])})()]);return{total_users:(n=f[0])==null?void 0:n.total,active_subscriptions_cct:(i=c[0])==null?void 0:i.total,active_subscriptions_suiteplus:(o=m[0])==null?void 0:o.total,total_certificates:(l=p[0])==null?void 0:l.total,pending_comments:(u=h[0])==null?void 0:u.total,total_courses:(d=y[0])==null?void 0:d.total,total_lessons:(a=v[0])==null?void 0:a.total}}case"get_user_progress":{const c=(await r.sql(`SELECT c.id as course_id, c.title as course_title,
         ...
         ${y} ORDER BY ${m} LIMIT $${h}`,p);return{lessons:b,count:b.length}}case"search_suiteplus_subscriptions":{const f=s.env.DATABASE_SUITEPLUS;if(!f)return{error:"DATABASE_SUITEPLUS não configurada"};const c=new $e(f),p=Math.min(t.limit||20,100);let h;return t.email?h=await c.sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
         ...
         ORDER BY ms.data_expiracao ASC`,[]);return{subscriptions:y,count:y.length,query:{days_ahead:f,include_expired_days:c}}}default:return{error:`Ferramenta desconhecida: ${e}`}}}async function ko(e,t,r,s){var d,a,f;const n=new Set(["nome","telefone","whatsapp","ativo","dt_expiracao"]),i=new Set(["data_expiracao","ativo","detalhe","origem"]),o=new Set(["title","description","teste_gratis","rentable","duration_minutes","order_index"]),l=new Set(["title","description","instructor","duration_hours"]),u=new Set(["user_name","course_title","carga_horaria","generated_at"]);switch(e){case"update_user":{const c={};for(const[v,m]of Object.entries(t.fields||{}))n.has(v)&&(c[v]=m);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(c).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.email,...Object.values(c)],y=await r.sql(`UPDATE users SET ${p} WHERE lower(email) = lower($1) RETURNING *`,h);return{updated:y.length,user:y[0]||null}}case"update_member_subscription":{const c={};for(const[v,m]of Object.entries(t.fields||{}))i.has(v)&&(c[v]=m);if(Object.keys(c).length===0)return{error:"Nenhum campo válido para atualizar"};const p=Object.keys(c).map((v,m)=>`"${v}" = $${m+2}`).join(", "),h=[t.email,...Object.values(c)],y=await r.sql(`UPDATE member_subscriptions SET ${p}, updated_at = NOW() WHERE lower(email_membro) = lower($1) RETURNING *`,h);return{updated:y.length,subscription:y[0]||null}}case"create_member_subscription":return{created:!0,subscription:(await r.sql(`INSERT INTO member_subscriptions (email_membro, data_expiracao, detalhe, ativo, origem, created_at, updated_at)
...
${a}`:""}`},...o.slice(-20),{role:"user",content:i}];for(let h=0;h<6;h++){const y=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`,"HTTP-Referer":"https://novocct.ensinoplus.com.br","X-Title":"CCT Admin Agent"},body:JSON.stringify({model:"google/gemini-2.5-flash",messages:p,tools:Po,tool_choice:"auto"})});if(!y.ok)return e.json({error:`OpenRouter: ${await y.text()}`},500);const m=(s=(await y.json()).choices)==null?void 0:s[0];if(!m)return e.json({error:"Resposta vazia do modelo"},500);const b=m.message,w=b.tool_calls;if(!w||w.length===0)return e.json({reply:b.content||""});const x=w[0],I=x.function.name;let A={};try{A=JSON.parse(x.function.arguments)}catch{}if(qo.has(I)){const E=Do(I,A);return e.json({reply:`⚠️ **Confirmação necessária**

${E}`,pendingAction:{tool:I,args:A,description:E}})}const g=await jo(I,A,u,e);p.push({role:"assistant",content:b.content||null,tool_calls:w}),p.push({role:"tool",content:JSON.stringify(g),tool_call_id:x.id})}return e.json({reply:"Não consegui completar a consulta. Tente reformular sua pergunta."})}catch(n){return console.error("Agent error:",n),e.json({error:n.message||"Erro no agente"},500)}});R.get("/api/admin/users",P,async e=>{try{const r=await L(e).sql(`
      SELECT *
      FROM (
        SELECT
          id,
    ...
    `);return e.json({users:r})}catch(t){return console.error("Get users error:",t),e.json({error:t.message||"Failed to fetch users"},500)}});R.get("/api/admin/users/find",P,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email is required"},400);const s=await L(e).query("users",{select:"*",filters:{email:t},limit:1});return s&&s.length>0?e.json({user:s[0]}):e.json({user:null})}catch(t){return console.error("Find user error:",t),e.json({error:t.message||"Failed to find user"},500)}});R.post("/api/admin/users",P,async e=>{try{const t=await e.req.json();if(!t.email)return e.json({error:"Email is required"},400);const s=await L(e).insert("users",{email:t.email,nome:t.nome||null,first_name:t.first_name||null,last_name:t.last_name||null,cpf:t.cpf||null,telefone:t.telefone||null,whatsapp:t.whatsapp||null,foto:t.foto||null,end_cep:t.end_cep||null,end_logradouro:t.end_logradouro||null,end_numero:t.end_numero||null,end_cidade:t.end_cidade||null,end_estado:t.end_estado||null,ativo:t.ativo!==void 0?t.ativo:!0,teste_gratis:t.teste_gratis||!1,dt_expiracao:t.dt_expiracao||null});return e.json({success:!0,user_id:s[0].id})}catch(t){return console.error("Create user error:",t),e.json({error:t.message||"Failed to create user"},500)}});R.put("/api/admin/users/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await L(e).update("users",{id:t},{nome:r.nome,first_name:r.first_name,last_name:r.last_name,cpf:r.cpf,telefone:r.telefone,whatsapp:r.whatsapp,foto:r.foto,end_cep:r.end_cep,end_logradouro:r.end_logradouro,end_numero:r.end_numero,end_cidade:r.end_cidade,end_estado:r.end_estado,ativo:r.ativo,teste_gratis:r.teste_gratis,dt_expiracao:r.dt_expiracao,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update user error:",t),e.json({error:t.message||"Failed to update user"},500)}});R.delete("/api/admin/users/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("users",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});R.get("/api/admin/certificates",P,async e=>{try{const r=await L(e).query("certificates",{select:"*",order:"created_at DESC"});return e.json({certificates:r||[]})}catch(t){return console.error("List certificates error:",t),e.json({error:t.message||"Failed to list certificates"},500)}});R.get("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id"),s=await L(e).query("certificates",{select:"*",filters:{id:t}});return!s||s.length===0?e.json({error:"Certificate not found"},404):e.json({certificate:s[0]})}catch(t){return console.error("Get certificate error:",t),e.json({error:t.message||"Failed to get certificate"},500)}});R.get("/api/admin/certificates/find",P,async e=>{try{const t=e.req.query("email"),r=e.req.query("course");if(!t||!r)return e.json({error:"Email and course parameters are required"},400);const n=await L(e).query("certificates",{select:"*",filters:{user_email:t,course_title:r}});return e.json({certificates:n||[]})}catch(t){return console.error("Find certificate error:",t),e.json({error:t.message||"Failed to find certificate"},500)}});R.get("/api/admin/certificates/suggested-dates",P,async e=>{try{const t=e.req.query("email"),r=e.req.query("course_id");if(!t||!r)return e.json({error:"email e course_id são obrigatórios"},400);const i=(await L(e).sql(`
    ...
    `,[r,t]))[0]||{};return e.json({start_date:i.start_date||null,completion_date:i.completion_date||null})}catch(t){return console.error("Suggest certificate dates error:",t),e.json({error:t.message||"Failed to suggest dates"},500)}});R.post("/api/admin/certificates",P,async e=>{try{const t=await e.req.json();if(!t.user_email||!t.course_title)return e.json({error:"Email and course title are required"},400);const r=L(e),s=new Date().toISOString(),n=await r.insert("certificates",{user_email:t.user_email,user_name:t.user_name||"Aluno",course_id:t.course_id||null,course_title:t.course_title,issued_at:t.issued_at||s,start_date:t.start_date||null,completion_date:t.completion_date||s,carga_horaria:t.carga_horaria||null,certificate_code:t.certificate_code||null,generated_at:t.generated_at||null});return e.json({success:!0,certificate_id:n&&n.length>0?n[0].id:null})}catch(t){return console.error("Create certificate error:",t),e.json({error:t.message||"Failed to create certificate"},500)}});R.post("/api_certificado",async e=>{var t;try{const r=await e.req.json(),s=Array.isArray(r)?r:[r],n=L(e),i=[];for(const l of s){const u=m=>{const b=String(m??"").trim();return!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined"?"":b},d=u(l.user_email||l.email).toLowerCase(),a=u(l.user_name||l.nome)||null,f=u(l.course_title||l.curso);let c=l.carga_horaria?parseInt(String(l.carga_horaria)):null;if(c!==null&&!Number.isFinite(c)&&(c=null),!d.includes("@")||!f){i.push({user_email:d,course_title:f,status:"error",error:"user_email e course_title são obrigatórios"});continue}const p=m=>{if(!m)return null;const b=String(m).trim();if(!b||b.toLowerCase()==="null"||b.toLowerCase()==="undefined")return null;if(/^\d{2}\/\d{2}\/\d{4}$/.test(b)){const[w,x,I]=b.split("/");return`${I}-${x}-${w}T12:00:00Z`}return isNaN(new Date(b).getTime())?null:new Date(b).toISOString()},h=p(l.data_final||l.completion_date),y=p(l.data_inicio||l.data_inicial||l.start_date);if(!c){const m=await n.sql("SELECT duration_hours FROM courses WHERE lower(title) = lower($1) OR lower(title) ILIKE lower($2) LIMIT 1",[f,`%${f}%`]);if(m.length>0&&m[0].duration_hours)c=parseInt(m[0].duration_hours);else{const b=x=>x.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9\s]/g,"").split(/\s+/).filter(I=>I&&I!=="de"&&I!=="do"&&I!=="da"),w=b(f);if(w.length>0){const I=(await n.sql("SELECT title, duration_hours FROM courses WHERE duration_hours > 0")).find(A=>{const g=b(A.title).join(" ");return w.every(E=>g.includes(E))});I&&(c=parseInt(I.duration_hours))}}}const v=await n.sql("SELECT id FROM certificates WHERE lower(user_email) = lower($1) AND lower(course_title) = lower($2) LIMIT 1",[d,f]);if(v.length>0)await n.sql(`UPDATE certificates
           SET user_name = COALESCE($1, user_name),
               ...
               updated_at = NOW()
           WHERE id = $5`,[a,c,h,y,v[0].id]),i.push({user_email:d,course_title:f,status:"updated",certificate_id:v[0].id});else{const m=new Uint8Array(4);crypto.getRandomValues(m);const b="CCT-"+new Date().getFullYear()+"-"+Array.from(m).map(I=>I.toString(16).padStart(2,"0")).join("").toUpperCase(),w=new Date().toISOString(),x=await n.sql(`INSERT INTO certificates (user_email, user_name, course_title, carga_horaria, certificate_code, verification_code, issued_at, completion_date, start_date, created_at)
           VALUES ($1, $2, $3, $4, $5, $5, $6, COALESCE($7, $6::timestamp), $8::date, $6)
           RETURNING id`,[d,a||"Aluno",f,c,b,w,h,y]);i.push({user_email:d,course_title:f,status:"created",certificate_id:(t=x[0])==null?void 0:t.id,verification_code:b})}}const o=i.filter(l=>l.status==="error").length;return e.json({success:o===0,total:s.length,results:i})}catch(r){return console.error("Webhook api_certificado error:",r),e.json({error:r.message||"Falha ao processar certificado"},500)}});R.put("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json(),s=L(e);let n=r.course_title;if(r.course_id){const i=await s.query("courses",{select:"title",filters:{id:r.course_id}});i&&i.length>0&&(n=i[0].title)}return await s.update("certificates",{id:t},{user_email:r.user_email,user_name:r.user_name,course_id:r.course_id,course_title:n,carga_horaria:r.carga_horaria,start_date:r.start_date||null,completion_date:r.completion_date||null,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update certificate error:",t),e.json({error:t.message||"Failed to update certificate"},500)}});R.delete("/api/admin/certificates/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("certificates",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete certificate error:",t),e.json({error:t.message||"Failed to delete certificate"},500)}});function Bo(e){const t=!!e.templateImageUrl,r=!!e.versoImageUrl,s=e.modules&&e.modules.length>0;if(t){const d=s?e.modules.map((f,c)=>`<div class="mod-item"><span class="mod-num">${String(c+1).padStart(2,"0")}.</span> ${f}</div>`).join(""):"",a=r?`
...
<body>
  ...
  <script>window.onload=function(){document.querySelectorAll('.qr-wrap svg').forEach(function(s){s.setAttribute('width','60');s.setAttribute('height','60');})}<\/script>
    ...
    `)}catch(t){return console.error("Verify certificate error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});R.get("/api/verify/:code",async e=>{try{const t=e.req.param("code"),r=L(e),s=await r.query("certificates",{select:"*",filters:{certificate_code:t}});if(!s||s.length===0)return e.json({valid:!1,message:"Certificate not found"},404);const n=s[0];try{await r.update("certificates",{id:n.id},{verification_count:(n.verification_count||0)+1})}catch{}return e.json({valid:!0,certificate:{student_name:n.user_name,course_title:n.course_title,workload:n.carga_horaria,completion_date:n.completion_date,issued_at:n.issued_at,certificate_code:n.certificate_code,verification_count:(n.verification_count||0)+1}})}catch(t){return console.error("Verify certificate API error:",t),e.json({error:t.message||"Failed to verify certificate"},500)}});R.get("/api/admin/member-subscriptions",P,async e=>{try{const r=await L(e).query("member_subscriptions",{select:"*",order:"created_at DESC"});return e.json({subscriptions:r||[]})}catch(t){return console.error("List member subscriptions error:",t),e.json({error:t.message||"Failed to list member subscriptions"},500)}});R.get("/api/admin/suiteplus-subscriptions",P,async e=>{try{const t=e.env.DATABASE_SUITEPLUS;if(!t)return e.json({error:"DATABASE_SUITEPLUS não configurada"},500);const s=await new $e(t).sql(`SELECT id, user_email, product_id, started_at, expires_at, status, payment_source, recurring_enabled
       FROM user_subscriptions
       ORDER BY expires_at DESC`);return e.json({subscriptions:s})}catch(t){return console.error("SuitePlus subscriptions error:",t),e.json({error:t.message||"Erro ao buscar assinaturas SuitePlus"},500)}});R.get("/api/admin/member-subscriptions/find",P,async e=>{try{const t=e.req.query("email");if(!t)return e.json({error:"Email parameter is required"},400);const s=await L(e).query("member_subscriptions",{select:"*",filters:{email_membro:t}});return e.json({subscriptions:s||[]})}catch(t){return console.error("Find member subscription error:",t),e.json({error:t.message||"Failed to find member subscription"},500)}});R.post("/api/admin/member-subscriptions",P,async e=>{try{const t=await e.req.json();if(!t.email_membro)return e.json({error:"Email is required"},400);const s=await L(e).insert("member_subscriptions",{email_membro:t.email_membro,data_expiracao:t.data_expiracao||null,detalhe:t.detalhe||null,origem:t.origem||null,teste_gratis:t.teste_gratis||!1,ativo:t.ativo!==void 0?t.ativo:!0});return e.json({success:!0,subscription_id:s&&s.length>0?s[0].id:null})}catch(t){return console.error("Create member subscription error:",t),e.json({error:t.message||"Failed to create member subscription"},500)}});R.put("/api/admin/member-subscriptions/:id",P,async e=>{try{const t=e.req.param("id"),r=await e.req.json();return await L(e).update("member_subscriptions",{id:t},{email_membro:r.email_membro,data_expiracao:r.data_expiracao,detalhe:r.detalhe,origem:r.origem,teste_gratis:r.teste_gratis,ativo:r.ativo,updated_at:new Date().toISOString()}),e.json({success:!0})}catch(t){return console.error("Update member subscription error:",t),e.json({error:t.message||"Failed to update member subscription"},500)}});R.delete("/api/admin/member-subscriptions/:id",P,async e=>{try{const t=e.req.param("id");return await L(e).delete("member_subscriptions",{id:t}),e.json({success:!0})}catch(t){return console.error("Delete user error:",t),e.json({error:t.message||"Failed to delete user"},500)}});R.get("/api/courses",async e=>{try{const t=L(e),r=V(e,"sb-access-token");let s=!1;if(r){const l=await G(r,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);l&&(s=await Yr(l.email,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY,r))}const n=s?{}:{is_published:!0},i=s?"":"WHERE c.is_published = true",o=await t.sql(`
      SELECT c.*,
    ...
    `);return e.json({courses:o})}catch(t){return console.error("❌ /api/courses error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch courses"},500)}});R.get("/api/courses/:id/modules",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.sql(`SELECT m.*,
       ...
       WHERE m.course_id = $1
       GROUP BY m.id
       ORDER BY m.order_index`,[t]);return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id/modules error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course modules"},500)}});R.get("/api/courses/:id",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("courses",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Course not found"},404);const n=await r.query("modules",{select:"*",filters:{course_id:t},order:"order_index"}),i=await r.sql(`SELECT l.* FROM lessons l
       JOIN modules m ON m.id = l.module_id
       WHERE m.course_id = $1
       ORDER BY m.order_index, l.order_index`,[t]),o=new Map;for(const l of i){const u=o.get(l.module_id)||[];u.push(l),o.set(l.module_id,u)}for(const l of n)l.lessons=o.get(l.id)||[];return e.json({course:s,modules:n})}catch(t){return console.error("❌ /api/courses/:id error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch course"},500)}});R.get("/api/search/lessons",K,async e=>{try{const t=e.get("user"),r=L(e);await Et(r);const s=(e.req.query("q")||"").trim(),n=e.req.query("course_id"),i=e.req.query("type")||"all",o=Math.max(parseInt(e.req.query("min_duration")||"0",10),0),l=Math.min(Math.max(parseInt(e.req.query("max_duration")||"999",10),o),999),u=Math.min(Math.max(parseInt(e.req.query("limit")||"80",10),1),120),d=e.req.query("sort")||"relevance",a=[t.email],f=["1=1"];let c=a.length+1;n&&(f.push(`co.id = $${c++}`),a.push(parseInt(n,10))),f.push(`COALESCE(l.duration_minutes, 0) BETWEEN $${c++} AND $${c++}`),a.push(o,l),i==="free"?f.push("(COALESCE(l.teste_gratis, false) = true OR COALESCE(l.free_trial, false) = true)"):i==="premium"?f.push("COALESCE(l.teste_gratis, false) = false AND COALESCE(l.free_trial, false) = false"):i==="rented"&&f.push("lr.lesson_id IS NOT NULL");const p=s.length>=2?s.split(/\s+/).filter(Boolean).slice(0,6):[];for(const x of p)f.push(`(
        l.title ILIKE $${c}
        OR COALESCE(l.description, '') ILIKE $${c}
        OR COALESCE(l.transcript, '') ILIKE $${c}
        OR m.title ILIKE $${c}
        OR co.title ILIKE $${c}
      )`),a.push(`%${x}%`),c++;const h=p[0]||"";a.push(h);const y=`$${c++}`;a.push(`%${s}%`);const v=`$${c++}`;a.push(u);const m=`$${c++}`;let b="relevance_score DESC, co.title ASC, m.order_index ASC, l.order_index ASC";d==="title"&&(b="l.title ASC"),d==="duration"&&(b="COALESCE(l.duration_minutes, 0) ASC, l.title ASC"),d==="date"&&(b="l.created_at DESC NULLS LAST, l.id DESC");const w=await r.sql(`
      SELECT l.id,
             l.title,
             l.description,
             l.duration_minutes,
             ...
             CASE
               WHEN ${y} <> ''
                    AND COALESCE(l.transcript, '') ILIKE ('%' || ${y} || '%')
               THEN substring(
                 l.transcript
                 FROM GREATEST(1, strpos(lower(l.transcript), lower(${y})) - 80)
                 FOR 280
               )
               ELSE NULL
    ...
    `,a);return e.json({lessons:w,limit:u})}catch(t){return console.error("❌ /api/search/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to search lessons"},500)}});R.get("/api/modules/:id/lessons",async e=>{try{const t=e.req.param("id"),s=await L(e).sql(`SELECT *
       FROM lessons
       WHERE module_id = $1
       ORDER BY order_index`,[t]);return e.json({lessons:s||[]})}catch(t){return console.error("❌ /api/modules/:id/lessons error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch module lessons"},500)}});R.get("/api/lessons/:id",async e=>{var t,r,s,n,i,o;try{const l=e.req.param("id"),u=V(e,"sb-access-token");let d=null;if(u){const w=await G(u,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);w&&(d=w.email)}const a=L(e);let f=!1,c=!1;if(d)try{if(f=!!((t=(await a.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[d,parseInt(l)]))[0])!=null&&t.has_access),console.log("Has access:",f,"User:",d,"Lesson:",l),!f){const x=e.env.DATABASE_SUITEPLUS,[I,A,g]=await Promise.all([a.sql("SELECT expires_at FROM lesson_rentals WHERE lower(user_email) = lower($1) AND lesson_id = $2 AND expires_at > NOW()",[d,parseInt(l)]),a.sql(`SELECT id FROM member_subscriptions
               WHERE lower(email_membro) = lower($1)
                 AND data_expiracao > NOW()
                 AND COALESCE(ativo, true) = true
                 AND COALESCE(teste_gratis, false) = false
               LIMIT 1`,[d]),x?xt(d,x):Promise.resolve(null)]);if((I.length>0||A.length>0||g&&g>new Date)&&(f=!0),!f){const E=await a.sql("SELECT rentable, rental_credits, title, COALESCE(teste_gratis, false) AS teste_gratis, COALESCE(free_trial, false) AS free_trial FROM lessons WHERE id = $1",[parseInt(l)]);if((r=E[0])!=null&&r.teste_gratis||(s=E[0])!=null&&s.free_trial)f=!0;else return console.log("❌ Access denied for user:",d,"lesson:",l),e.json({error:"Access denied",message:"Você não tem permissão para acessar esta aula.",needsUpgrade:!0,rentable:((n=E[0])==null?void 0:n.rentable)||!1,rental_credits:((i=E[0])==null?void 0:i.rental_credits)||0,lesson_title:((o=E[0])==null?void 0:o.title)||""},403)}}console.log("✅ Access granted for user:",d,"lesson:",l)}catch(w){console.error("❌ Error checking access via RPC:",w),console.log("⚠️ Allowing access due to RPC error (fallback mode)"),c=!0,f=!0}if(!d||!f&&!c){const w=await a.query("lessons",{select:"teste_gratis, free_trial",filters:{id:l},single:!0});if(!(w!=null&&w.teste_gratis)&&!(w!=null&&w.free_trial))return e.json({error:"Access denied",message:"Esta é uma aula premium. Faça login e tenha um plano ativo para acessar.",needsLogin:!0},403)}await _t(a);const p=`
    ...
    `,[y,v,m]=await Promise.all([a.sql(p,[parseInt(l)]),a.sql("SELECT * FROM comments WHERE lesson_id = $1 ORDER BY created_at DESC",[l]),a.sql(h,[parseInt(l)])]);if(!y||y.length===0)return e.json({error:"Lesson not found"},404);const b=y[0];return e.json({lesson:b,comments:v,trails:m})}catch(l){return console.error("Error fetching lesson:",l),e.json({error:"Failed to fetch lesson"},500)}});R.get("/api/admin/comments",P,async e=>{try{const t=e.req.query("status")||"all",r=(e.req.query("search")||"").trim(),s=L(e);await _t(s);const n=[],i=[];t==="pending"?n.push("NULLIF(TRIM(COALESCE(c.admin_reply, '')), '') IS NULL"):t==="answered"&&n.push("NULLIF(TRIM(COALESCE(c.admin_reply, '')), '') IS NOT NULL"),r&&(i.push(`%${r}%`),n.push(`(
    ...
    `,[t,parseInt(r)]);return e.json({progress:i||[]})}catch(t){return console.error("❌ /api/progress error:",(t==null?void 0:t.message)||t),e.json({error:(t==null?void 0:t.message)||"Failed to fetch progress"},500)}});R.post("/api/progress/complete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=L(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0?await s.update("user_progress",{id:n[0].id},{completed:!0,completed_at:new Date().toISOString()}):await s.insert("user_progress",{user_email:t,lesson_id:parseInt(r),completed:!0,completed_at:new Date().toISOString()}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/progress/uncomplete",async e=>{try{const{user_email:t,lesson_id:r}=await e.req.json();if(!t||!r)return e.json({error:"Missing required fields"},400);const s=L(e),n=await s.query("user_progress",{select:"*",filters:{user_email:t,lesson_id:r}});return n&&n.length>0&&await s.delete("user_progress",{id:n[0].id}),e.json({success:!0})}catch{return e.json({error:"Failed to update progress"},500)}});R.post("/api/admin/certificate-template",P,async e=>{try{const t=await e.req.json(),{course_id:r,image_data:s,verso_data:n}=t;if(!r||!s)return e.json({error:"ID do curso e imagem da frente são obrigatórios"},400);const i=y=>{const v=y.match(/^data:([^;]+);base64,/);return v?v[1]:"image/jpeg"},o=y=>y.includes(",")?y.split(",")[1]:y,l=i(s),u=o(s),d=n?i(n):null,a=n?o(n):null,f=`/api/certificate-template/${r}/image`,c=n?`/api/certificate-template/${r}/verso`:null,p=L(e),h=await p.query("certificate_templates",{select:"*",filters:{course_id:r}});if(h&&h.length>0){const y={template_url:f,template_data:u,template_mime:l,updated_at:new Date().toISOString()};n!==void 0&&(y.verso_data=a,y.verso_mime=d),await p.update("certificate_templates",{id:h[0].id},y)}else await p.insert("certificate_templates",{course_id:parseInt(r),template_url:f,template_data:u,template_mime:l,verso_data:a,verso_mime:d,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});return console.log("✅ Certificate template saved to Postgres"),e.json({success:!0,template_url:f,verso_url:c,message:"Template de certificado salvo com sucesso!"})}catch(t){return console.error("💥 Certificate template error:",t),e.json({error:"Erro ao salvar template de certificado",details:t.message},500)}});R.get("/api/certificate-template/:courseId/image",async e=>{try{const t=e.req.param("courseId"),s=await L(e).query("certificate_templates",{select:"template_data, template_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.template_data))return e.json({error:"Imagem não encontrada"},404);const n=Uint8Array.from(atob(s.template_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.template_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar imagem"},500)}});R.get("/api/certificate-template/:courseId/verso",async e=>{try{const t=e.req.param("courseId"),s=await L(e).query("certificate_templates",{select:"verso_data, verso_mime",filters:{course_id:t},single:!0});if(!(s!=null&&s.verso_data))return e.json({error:"Verso não encontrado"},404);const n=Uint8Array.from(atob(s.verso_data),i=>i.charCodeAt(0));return new Response(n,{headers:{"Content-Type":s.verso_mime||"image/jpeg","Cache-Control":"public, max-age=86400"}})}catch{return e.json({error:"Erro ao buscar verso"},500)}});R.get("/api/certificate-template/:courseId",async e=>{try{const t=e.req.param("courseId"),r=L(e),s=await r.query("certificate_templates",{select:"id, course_id, template_mime, verso_mime, created_at, updated_at",filters:{course_id:t},single:!0});if(s){s.template_url=`/api/certificate-template/${t}/image`;const n=await r.query("certificate_templates",{select:"verso_data",filters:{course_id:t},single:!0});s.verso_url=n!=null&&n.verso_data?`/api/certificate-template/${t}/verso`:null}return e.json({template:s})}catch{return e.json({template:null})}});R.post("/api/certificates/generate",async e=>{var t,r,s;try{const n=V(e,"sb-access-token");if(!n)return e.json({error:"Não autenticado"},401);const i=await G(n,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!i)return e.json({error:"Usuário não encontrado"},401);const{course_id:o}=await e.req.json();if(console.log("📜 Certificate generation request:",{user_email:i.email,course_id:o}),!o)return e.json({error:"ID do curso é obrigatório"},400);const l=L(e),u=await l.query("certificates",{select:"*",filters:{user_email:i.email,course_id:o}});if(u&&u.length>0)return console.log("✅ Certificate already exists"),e.json({success:!0,certificate:u[0],message:"Certificado já existe!"});const d=await l.query("courses",{select:"*",filters:{id:o},single:!0});if(!d)return e.json({error:"Curso não encontrado"},404);const a=await l.query("modules",{select:"*",filters:{course_id:o}});let f=[];if(a)for(const x of a){const I=await l.query("lessons",{select:"id",filters:{module_id:x.id}});I&&(f=[...f,...I.map(A=>A.id)])}if(f.length===0)return e.json({error:"Curso não possui aulas"},400);const p=(await l.query("user_progress",{select:"*",filters:{user_email:i.email}})||[]).filter(x=>x.completed&&f.includes(x.lesson_id)),h=p.map(x=>x.lesson_id),y=h.length/f.length*100;if(console.log("📊 Course completion:",{total_lessons:f.length,completed_lessons:h.length,percentage:y}),y<100)return e.json({error:"Você precisa completar 100% do curso para receber o certificado",completion:y},400);const v=p.map(x=>x.completed_at).filter(x=>!!x).sort(),m=v[0]||new Date().toISOString(),b=v[v.length-1]||new Date().toISOString();if(d.min_completion_days){const x=(new Date(b).getTime()-new Date(m).getTime())/864e5;if(x<d.min_completion_days)return console.log("⏳ Certificate blocked: minimum completion period not met",{days_elapsed:x,min_days_required:d.min_completion_days}),e.json({error:`Este curso exige um prazo mínimo de ${d.min_completion_days} dia(s) entre a primeira e a última aula concluída. Você concluiu em ${x.toFixed(1)} dia(s). Continue acessando o curso normalmente — o certificado ficará disponível assim que o prazo mínimo for atingido.`,days_elapsed:x,min_days_required:d.min_completion_days},400)}const w=await l.insert("certificates",{user_email:i.email,user_name:((t=i.user_metadata)==null?void 0:t.name)||"Aluno",course_id:parseInt(o),course_title:d.title,issued_at:new Date().toISOString(),start_date:m,completion_date:b});return console.log("✅ Certificate generated successfully"),e.json({success:!0,certificate:w,message:"Parabéns! Seu certificado foi gerado com sucesso!"})}catch(n){return console.error("💥 Certificate generation error:",n),console.error("Error details:",n.message),(r=n.message)!=null&&r.includes("certificates")||(s=n.message)!=null&&s.includes("relation")?e.json({error:"Tabela de certificados não encontrada. Execute a migração SQL no Supabase.",details:n.message},500):e.json({error:"Erro ao gerar certificado",details:n.message},500)}});R.get("/api/certificates",async e=>{try{const t=V(e,"sb-access-token");if(!t)return e.json({error:"Não autenticado"},401);const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({error:"Usuário não encontrado"},401);const s=L(e),n=await s.query("certificates",{select:"*",filters:{user_email:r.email},order:"issued_at DESC"})||[],i=await Promise.all(n.map(async o=>{const l=await s.query("certificate_templates",{select:"*",filters:{course_id:o.course_id},single:!0});return{...o,template_url:(l==null?void 0:l.template_url)||null}}));return e.json({certificates:i})}catch(t){return console.error("💥 Certificates fetch error:",t),e.json({error:"Erro ao buscar certificados"},500)}});R.get("/api/certificates/:id",async e=>{try{const t=e.req.param("id"),r=L(e),s=await r.query("certificates",{select:"*",filters:{id:t},single:!0});if(!s)return e.json({error:"Certificado não encontrado"},404);const n=await r.query("certificate_templates",{select:"*",filters:{course_id:s.course_id},single:!0});return e.json({certificate:{...s,template_url:(n==null?void 0:n.template_url)||null}})}catch{return e.json({error:"Erro ao buscar certificado"},500)}});R.get("/api/plans",async e=>{try{const r=await L(e).query("plans",{select:"*",filters:{is_active:!0},order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.get("/api/subscriptions/current",async e=>{try{const t=V(e,"sb-access-token");if(!t)return e.json({subscription:null});const r=await G(t,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!r)return e.json({subscription:null});const i=await L(e).sql(`
      ...
      LIMIT 1
    `,[r.email]);return e.json({subscription:i&&i.length>0?i[0]:null})}catch(t){return console.error("Error fetching subscription:",t),e.json({subscription:null})}});R.post("/api/admin/subscriptions",P,async e=>{try{const{user_email:t,plan_id:r,duration_days:s}=await e.req.json();if(!t||!r)return e.json({error:"Email e plano são obrigatórios"},400);const n=L(e),i=await n.query("plans",{select:"*",filters:{id:r},single:!0});if(!i)return e.json({error:"Plano não encontrado"},404);const o=new Date;o.setDate(o.getDate()+(s||i.duration_days));const l=o.toISOString(),u=await n.insert("member_subscriptions",{email_membro:t,data_expiracao:l,detalhe:i.name,origem:"admin",teste_gratis:i.is_free_trial||!1,ativo:!0}),d=await n.query("users",{select:"id",filters:{email:t},single:!0});return d&&await n.update("users",{id:d.id},{dt_expiracao:l,updated_at:new Date().toISOString()}),e.json({success:!0,message:"Assinatura criada com sucesso!",subscription:u[0]})}catch(t){return console.error("Error creating subscription:",t),e.json({error:"Erro ao criar assinatura"},500)}});R.get("/api/lessons/:id/access",async e=>{var t;try{const r=e.req.param("id"),s=V(e,"sb-access-token");if(!s){const d=await L(e).query("lessons",{select:"teste_gratis, free_trial",filters:{id:r},single:!0}),a=!!(d!=null&&d.teste_gratis||d!=null&&d.free_trial);return e.json({hasAccess:a,reason:a?"free_lesson":"not_authenticated"})}const n=await G(s,e.env.SUPABASE_URL,e.env.SUPABASE_ANON_KEY);if(!n)return e.json({hasAccess:!1,reason:"invalid_token"});const i=L(e);let l=!!((t=(await i.sql("SELECT user_has_lesson_access($1::text, $2::integer) AS has_access",[n.email,parseInt(r)]))[0])!=null&&t.has_access);if(!l){const[u,d]=await Promise.all([i.sql(`SELECT id FROM member_subscriptions
           WHERE lower(email_membro) = lower($1)
             AND data_expiracao > NOW()
             AND COALESCE(ativo, true) = true
             AND COALESCE(teste_gratis, false) = false
           ...
           WHERE lower(user_email) = lower($1)
             AND lesson_id = $2
             AND expires_at > NOW()
           LIMIT 1`,[n.email,parseInt(r)])]);(u.length>0||d.length>0)&&(l=!0)}if(!l){const u=e.env.DATABASE_SUITEPLUS;if(u){const d=await xt(n.email,u);d&&d>new Date&&(l=!0)}}if(!l){const u=await i.query("lessons",{select:"teste_gratis, free_trial",filters:{id:r},single:!0});if(u!=null&&u.teste_gratis||u!=null&&u.free_trial)return e.json({hasAccess:!0,reason:"free_lesson"})}return e.json({hasAccess:l,reason:l?"active_subscription":"no_active_subscription"})}catch(r){return console.error("Error checking lesson access:",r),e.json({hasAccess:!1,reason:"error"},500)}});R.post("/api/admin/subscriptions/expire",P,async e=>{try{return await L(e).rpc("expire_subscriptions",{}),e.json({success:!0,message:"Assinaturas expiradas com sucesso!"})}catch(t){return console.error("Error expiring subscriptions:",t),e.json({error:"Erro ao expirar assinaturas"},500)}});R.get("/api/admin/plans",P,async e=>{try{const r=await L(e).query("plans",{select:"*",order:"display_order"})||[];return e.json({plans:r})}catch{return e.json({error:"Erro ao buscar planos"},500)}});R.post("/api/admin/plans",P,async e=>{try{const t=await e.req.json(),{id:r,name:s,description:n,price:i,duration_days:o,is_active:l,is_free_trial:u,features:d,display_order:a}=t,f=L(e);if(r)return await f.update("plans",{id:r},{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:l,is_free_trial:u,features:d||[],display_order:parseInt(a||0),updated_at:new Date().toISOString()}),e.json({success:!0,message:"Plano atualizado!"});{const c=await f.insert("plans",{name:s,description:n,price:parseFloat(i),duration_days:parseInt(o),is_active:l,is_free_trial:u,features:d||[],display_order:parseInt(a||0)});return e.json({success:!0,plan:c[0],message:"Plano criado!"})}}catch(t){return console.error("Error saving plan:",t),e.json({error:"Erro ao salvar plano"},500)}});R.get("/api/admin/subscriptions",P,async e=>{try{const t=L(e),r=await t.query("subscriptions",{select:"*",order:"created_at DESC"})||[],s=await Promise.all(r.map(async n=>{const i=await t.query("plans",{select:"*",filters:{id:n.plan_id},single:!0});return{...n,plan_name:(i==null?void 0:i.name)||"Desconhecido"}}));return e.json({subscriptions:s})}catch{return e.json({error:"Erro ao buscar assinaturas"},500)}});R.get("/recover",e=>e.html(`
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-50">
  ...
  `));const Ds=new ei,Mo=Object.assign({"/src/index.tsx":R});let oi=!1;for(const[,e]of Object.entries(Mo))e&&(Ds.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),Ds.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),oi=!0);if(!oi)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");var lr={exports:{}},cr={exports:{}},qe={},ur={},js;function ai(){if(js)return ur;js=1,ur.parse=function(r,s){return new e(r,s).parse()};class e{constructor(s,n){this.source=s,this.transform=n||t,this.position=0,this.entries=[],this.recorded=[],this.dimension=0}isEof(){return this.position>=this.source.length}nextCharacter(){var s=this.source[this.position++];return s==="\\"?{value:this.source[this.position++],escaped:!0}:{value:s,escaped:!1}}record(s){this.recorded.push(s)}newEntry(s){var n;(this.recorded.length>0||s)&&(n=this.recorded.join(""),n==="NULL"&&!s&&(n=null),n!==null&&(n=this.transform(n)),this.entries.push(n),this.recorded=[])}consumeDimensions(){if(this.source[0]==="[")for(;!this.isEof();){var s=this.nextCharacter();if(s.value==="=")break}}parse(s){var n,i,o;for(this.consumeDimensions();!this.isEof();)if(n=this.nextCharacter(),n.value==="{"&&!o)this.dimension++,this.dimension>1&&(i=new e(this.source.substr(this.position-1),this.transform),this.entries.push(i.parse(!0)),this.position+=i.position-2);else if(n.value==="}"&&!o){if(this.dimension--,!this.dimension&&(this.newEntry(),s))return this.entries}else n.value==='"'&&!n.escaped?(o&&this.newEntry(!0),o=!o):n.value===","&&!o?this.newEntry():this.record(n.value);if(this.dimension!==0)throw new Error("array dimension not balanced");return this.entries}}function t(r){return r}return ur}var dr,ks;function li(){if(ks)return dr;ks=1;var e=ai();return dr={create:function(t,r){return{parse:function(){return e.parse(t,r)}}}},dr}var fr,Bs;function Uo(){if(Bs)return fr;Bs=1;var e=/(\d{1,})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})(\.\d{1,})?.*?( BC)?$/,t=/^(\d{1,})-(\d{2})-(\d{2})( BC)?$/,r=/([Z+-])(\d{2})?:?(\d{2})?:?(\d{2})?/,s=/^-?infinity$/;fr=function(d){if(s.test(d))return Number(d.replace("i","I"));var a=e.exec(d);if(!a)return n(d)||null;var f=!!a[8],c=parseInt(a[1],10);f&&(c=o(c));var p=parseInt(a[2],10)-1,h=a[3],y=parseInt(a[4],10),v=parseInt(a[5],10),m=parseInt(a[6],10),b=a[7];b=b?1e3*parseFloat(b):0;var w,x=i(d);return x!=null?(w=new Date(Date.UTC(c,p,h,y,v,m,b)),l(c)&&w.setUTCFullYear(c),x!==0&&w.setTime(w.getTime()-x)):(w=new Date(c,p,h,y,v,m,b),l(c)&&w.setFullYear(c)),w};function n(u){var d=t.exec(u);if(d){var a=parseInt(d[1],10),f=!!d[4];f&&(a=o(a));var c=parseInt(d[2],10)-1,p=d[3],h=new Date(a,c,p);return l(a)&&h.setFullYear(a),h}}function i(u){if(u.endsWith("+00"))return 0;var d=r.exec(u.split(" ")[1]);if(d){var a=d[1];if(a==="Z")return 0;var f=a==="-"?-1:1,c=parseInt(d[2],10)*3600+parseInt(d[3]||0,10)*60+parseInt(d[4]||0,10);return c*f*1e3}}function o(u){return-(u-1)}function l(u){return u>=0&&u<100}return fr}var pr,Ms;function Fo(){if(Ms)return pr;Ms=1,pr=t;var e=Object.prototype.hasOwnProperty;function t(r){for(var s=1;s<arguments.length;s++){var n=arguments[s];for(var i in n)e.call(n,i)&&(r[i]=n[i])}return r}return pr}var mr,Us;function Ho(){if(Us)return mr;Us=1;var e=Fo();mr=t;function t(v){if(!(this instanceof t))return new t(v);e(this,y(v))}var r=["seconds","minutes","hours","days","months","years"];t.prototype.toPostgres=function(){var v=r.filter(this.hasOwnProperty,this);return this.milliseconds&&v.indexOf("seconds")<0&&v.push("seconds"),v.length===0?"0":v.map(function(m){var b=this[m]||0;return m==="seconds"&&this.milliseconds&&(b=(b+this.milliseconds/1e3).toFixed(6).replace(/\.?0+$/,"")),b+" "+m},this).join(" ")};var s={years:"Y",months:"M",days:"D",hours:"H",minutes:"M",seconds:"S"},n=["years","months","days"],i=["hours","minutes","seconds"];t.prototype.toISOString=t.prototype.toISO=function(){var v=n.map(b,this).join(""),m=i.map(b,this).join("");return"P"+v+"T"+m;function b(w){var x=this[w]||0;return w==="seconds"&&this.milliseconds&&(x=(x+this.milliseconds/1e3).toFixed(6).replace(/0+$/,"")),x+s[w]}};var o="([+-]?\\d+)",l=o+"\\s+years?",u=o+"\\s+mons?",d=o+"\\s+days?",a="([+-])?([\\d]*):(\\d\\d):(\\d\\d)\\.?(\\d{1,6})?",f=new RegExp([l,u,d,a].map(function(v){return"("+v+")?"}).join("\\s*")),c={years:2,months:4,days:6,hours:9,minutes:10,seconds:11,milliseconds:12},p=["hours","minutes","seconds","milliseconds"];function h(v){var m=v+"000000".slice(v.length);return parseInt(m,10)/1e3}function y(v){if(!v)return{};var m=f.exec(v),b=m[8]==="-";return Object.keys(c).reduce(function(w,x){var I=c[x],A=m[I];return!A||(A=x==="milliseconds"?h(A):parseInt(A,10),!A)||(b&&~p.indexOf(x)&&(A*=-1),w[x]=A),w},{})}return mr}var hr,Fs;function $o(){if(Fs)return hr;Fs=1;var e=Buffer.from||Buffer;return hr=function(r){if(/^\\x/.test(r))return e(r.substr(2),"hex");for(var s="",n=0;n<r.length;)if(r[n]!=="\\")s+=r[n],++n;else if(/[0-7]{3}/.test(r.substr(n+1,3)))s+=String.fromCharCode(parseInt(r.substr(n+1,3),8)),n+=4;else{for(var i=1;n+i<r.length&&r[n+i]==="\\";)i++;for(var o=0;o<Math.floor(i/2);++o)s+="\\";n+=Math.floor(i/2)*2}return e(s,"binary")},hr}var gr,Hs;function zo(){if(Hs)return gr;Hs=1;var e=ai(),t=li(),r=Uo(),s=Ho(),n=$o();function i(g){return function(_){return _===null?_:g(_)}}function o(g){return g===null?g:g==="TRUE"||g==="t"||g==="true"||g==="y"||g==="yes"||g==="on"||g==="1"}function l(g){return g?e.parse(g,o):null}function u(g){return parseInt(g,10)}function d(g){return g?e.parse(g,i(u)):null}function a(g){return g?e.parse(g,i(function(E){return b(E).trim()})):null}var f=function(g){if(!g)return null;var E=t.create(g,function(_){return _!==null&&(_=x(_)),_});return E.parse()},c=function(g){if(!g)return null;var E=t.create(g,function(_){return _!==null&&(_=parseFloat(_)),_});return E.parse()},p=function(g){if(!g)return null;var E=t.create(g);return E.parse()},h=function(g){if(!g)return null;var E=t.create(g,function(_){return _!==null&&(_=r(_)),_});return E.parse()},y=function(g){if(!g)return null;var E=t.create(g,function(_){return _!==null&&(_=s(_)),_});return E.parse()},v=function(g){return g?e.parse(g,i(n)):null},m=function(g){return parseInt(g,10)},b=function(g){var E=String(g);return/^\d+$/.test(E)?E:g},w=function(g){return g?e.parse(g,i(JSON.parse)):null},x=function(g){return g[0]!=="("?null:(g=g.substring(1,g.length-1).split(","),{x:parseFloat(g[0]),y:parseFloat(g[1])})},I=function(g){if(g[0]!=="<"&&g[1]!=="(")return null;for(var E="(",_="",T=!1,C=2;C<g.length-1;C++){if(T||(E+=g[C]),g[C]===")"){T=!0;continue}else if(!T)continue;g[C]!==","&&(_+=g[C])}var O=x(E);return O.radius=parseFloat(_),O},A=function(g){g(20,b),g(21,m),g(23,m),g(26,m),g(700,parseFloat),g(701,parseFloat),g(16,o),g(1082,r),g(1114,r),g(1184,r),g(600,x),g(651,p),g(718,I),g(1e3,l),g(1001,v),g(1005,d),g(1007,d),g(1028,d),g(1016,a),g(1017,f),g(1021,c),g(1022,c),g(1231,c),g(1014,p),g(1015,p),g(1008,p),g(1009,p),g(1040,p),g(1041,p),g(1115,h),g(1182,h),g(1185,h),g(1186,s),g(1187,y),g(17,n),g(114,JSON.parse.bind(JSON)),g(3802,JSON.parse.bind(JSON)),g(199,w),g(3807,w),g(3907,p),g(2951,p),g(791,p),g(1183,p),g(1270,p)};return gr={init:A},gr}var yr,$s;function Wo(){if($s)return yr;$s=1;var e=1e6;function t(r){var s=r.readInt32BE(0),n=r.readUInt32BE(4),i="";s<0&&(s=~s+(n===0),n=~n+1>>>0,i="-");var o="",l,u,d,a,f,c;{if(l=s%e,s=s/e>>>0,u=4294967296*l+n,n=u/e>>>0,d=""+(u-e*n),n===0&&s===0)return i+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}{if(l=s%e,s=s/e>>>0,u=4294967296*l+n,n=u/e>>>0,d=""+(u-e*n),n===0&&s===0)return i+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}{if(l=s%e,s=s/e>>>0,u=4294967296*l+n,n=u/e>>>0,d=""+(u-e*n),n===0&&s===0)return i+d+o;for(a="",f=6-d.length,c=0;c<f;c++)a+="0";o=a+d+o}return l=s%e,u=4294967296*l+n,d=""+u%e,i+d+o}return yr=t,yr}var br,zs;function Vo(){if(zs)return br;zs=1;var e=Wo(),t=function(p,h,y,v,m){y=y||0,v=v||!1,m=m||function(T,C,O){return T*Math.pow(2,O)+C};var b=y>>3,w=function(T){return v?~T&255:T},x=255,I=8-y%8;h<I&&(x=255<<8-h&255,I=h),y&&(x=x>>y%8);var A=0;y%8+h>=8&&(A=m(0,w(p[b])&x,I));for(var g=h+y>>3,E=b+1;E<g;E++)A=m(A,w(p[E]),8);var _=(h+y)%8;return _>0&&(A=m(A,w(p[g])>>8-_,_)),A},r=function(p,h,y){var v=Math.pow(2,y-1)-1,m=t(p,1),b=t(p,y,1);if(b===0)return 0;var w=1,x=function(A,g,E){A===0&&(A=1);for(var _=1;_<=E;_++)w/=2,(g&1<<E-_)>0&&(A+=w);return A},I=t(p,h,y+1,!1,x);return b==Math.pow(2,y+1)-1?I===0?m===0?1/0:-1/0:NaN:(m===0?1:-1)*Math.pow(2,b-v)*I},s=function(p){return t(p,1)==1?-1*(t(p,15,1,!0)+1):t(p,15,1)},n=function(p){return t(p,1)==1?-1*(t(p,31,1,!0)+1):t(p,31,1)},i=function(p){return r(p,23,8)},o=function(p){return r(p,52,11)},l=function(p){var h=t(p,16,32);if(h==49152)return NaN;for(var y=Math.pow(1e4,t(p,16,16)),v=0,m=t(p,16),b=0;b<m;b++)v+=t(p,16,64+16*b)*y,y/=1e4;var w=Math.pow(10,t(p,16,48));return(h===0?1:-1)*Math.round(v*w)/w},u=function(p,h){var y=t(h,1),v=t(h,63,1),m=new Date((y===0?1:-1)*v/1e3+9466848e5);return p||m.setTime(m.getTime()+m.getTimezoneOffset()*6e4),m.usec=v%1e3,m.getMicroSeconds=function(){return this.usec},m.setMicroSeconds=function(b){this.usec=b},m.getUTCMicroSeconds=function(){return this.usec},m},d=function(p){var h=t(p,32);t(p,32,32);for(var y=t(p,32,64),v=96,m=[],b=0;b<h;b++)m[b]=t(p,32,v),v+=32,v+=32;var w=function(I){var A=t(p,32,v);if(v+=32,A==4294967295)return null;var g;if(I==23||I==20)return g=t(p,A*8,v),v+=A*8,g;if(I==25)return g=p.toString(this.encoding,v>>3,(v+=A<<3)>>3),g;console.log("ERROR: ElementType not implemented: "+I)},x=function(I,A){var g=[],E;if(I.length>1){var _=I.shift();for(E=0;E<_;E++)g[E]=x(I,A);I.unshift(_)}else for(E=0;E<I[0];E++)g[E]=w(A);return g};return x(m,y)},a=function(p){return p.toString("utf8")},f=function(p){return p===null?null:t(p,8)>0},c=function(p){p(20,e),p(21,s),p(23,n),p(26,n),p(1700,l),p(700,i),p(701,o),p(16,f),p(1114,u.bind(null,!1)),p(1184,u.bind(null,!0)),p(1e3,d),p(1007,d),p(1016,d),p(1008,d),p(1009,d),p(25,a)};return br={init:c},br}var vr,Ws;function Qo(){return Ws||(Ws=1,vr={BOOL:16,BYTEA:17,CHAR:18,INT8:20,INT2:21,INT4:23,REGPROC:24,TEXT:25,OID:26,TID:27,XID:28,CID:29,JSON:114,XML:142,PG_NODE_TREE:194,SMGR:210,PATH:602,POLYGON:604,CIDR:650,FLOAT4:700,FLOAT8:701,ABSTIME:702,RELTIME:703,TINTERVAL:704,CIRCLE:718,MACADDR8:774,MONEY:790,MACADDR:829,INET:869,ACLITEM:1033,BPCHAR:1042,VARCHAR:1043,DATE:1082,TIME:1083,TIMESTAMP:1114,TIMESTAMPTZ:1184,INTERVAL:1186,TIMETZ:1266,BIT:1560,VARBIT:1562,NUMERIC:1700,REFCURSOR:1790,REGPROCEDURE:2202,REGOPER:2203,REGOPERATOR:2204,REGCLASS:2205,REGTYPE:2206,UUID:2950,TXID_SNAPSHOT:2970,PG_LSN:3220,PG_NDISTINCT:3361,PG_DEPENDENCIES:3402,TSVECTOR:3614,TSQUERY:3615,GTSVECTOR:3642,REGCONFIG:3734,REGDICTIONARY:3769,JSONB:3802,REGNAMESPACE:4089,REGROLE:4096}),vr}var Vs;function St(){if(Vs)return qe;Vs=1;var e=zo(),t=Vo(),r=li(),s=Qo();qe.getTypeParser=o,qe.setTypeParser=l,qe.arrayParser=r,qe.builtins=s;var n={text:{},binary:{}};function i(u){return String(u)}function o(u,d){return d=d||"text",n[d]&&n[d][u]||i}function l(u,d,a){typeof d=="function"&&(a=d,d="text"),n[d][u]=a}return e.init(function(u,d){n.text[u]=d}),t.init(function(u,d){n.binary[u]=d}),qe}var Qs;function At(){return Qs||(Qs=1,(function(e){var t={};let r;try{r=process.platform==="win32"?t.USERNAME:t.USER}catch{}e.exports={host:"localhost",user:r,database:void 0,password:null,connectionString:void 0,port:5432,rows:0,binary:!1,max:10,idleTimeoutMillis:3e4,client_encoding:"",ssl:!1,application_name:void 0,fallback_application_name:void 0,options:void 0,parseInputDatesAsUTC:!1,statement_timeout:!1,lock_timeout:!1,idle_in_transaction_session_timeout:!1,query_timeout:!1,connect_timeout:0,keepalives:1,keepalives_idle:0};const s=St(),n=s.getTypeParser(20,"text"),i=s.getTypeParser(1016,"text");e.exports.__defineSetter__("parseInt8",function(o){s.setTypeParser(20,"text",o?s.getTypeParser(23,"text"):n),s.setTypeParser(1016,"text",o?s.getTypeParser(1007,"text"):i)})})(cr)),cr.exports}var wr,Xs;function tt(){if(Xs)return wr;Xs=1;const e=At(),t=He,{isDate:r}=t.types||t;function s(c){return'"'+c.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function n(c){let p="{";for(let h=0;h<c.length;h++)if(h>0&&(p=p+","),c[h]===null||typeof c[h]>"u")p=p+"NULL";else if(Array.isArray(c[h]))p=p+n(c[h]);else if(ArrayBuffer.isView(c[h])){let y=c[h];if(!(y instanceof Buffer)){const v=Buffer.from(y.buffer,y.byteOffset,y.byteLength);v.length===y.byteLength?y=v:y=v.slice(y.byteOffset,y.byteOffset+y.byteLength)}p+="\\\\x"+y.toString("hex")}else p+=s(i(c[h]));return p=p+"}",p}const i=function(c,p){if(c==null)return null;if(typeof c=="object"){if(c instanceof Buffer)return c;if(ArrayBuffer.isView(c)){const h=Buffer.from(c.buffer,c.byteOffset,c.byteLength);return h.length===c.byteLength?h:h.slice(c.byteOffset,c.byteOffset+c.byteLength)}return r(c)?e.parseInputDatesAsUTC?u(c):l(c):Array.isArray(c)?n(c):o(c,p)}return c.toString()};function o(c,p){if(c&&typeof c.toPostgres=="function"){if(p=p||[],p.indexOf(c)!==-1)throw new Error('circular reference detected while preparing "'+c+'" for query');return p.push(c),i(c.toPostgres(i),p)}return JSON.stringify(c)}function l(c){let p=-c.getTimezoneOffset(),h=c.getFullYear();const y=h<1;y&&(h=Math.abs(h)+1);let v=String(h).padStart(4,"0")+"-"+String(c.getMonth()+1).padStart(2,"0")+"-"+String(c.getDate()).padStart(2,"0")+"T"+String(c.getHours()).padStart(2,"0")+":"+String(c.getMinutes()).padStart(2,"0")+":"+String(c.getSeconds()).padStart(2,"0")+"."+String(c.getMilliseconds()).padStart(3,"0");return p<0?(v+="-",p*=-1):v+="+",v+=String(Math.floor(p/60)).padStart(2,"0")+":"+String(p%60).padStart(2,"0"),y&&(v+=" BC"),v}function u(c){let p=c.getUTCFullYear();const h=p<1;h&&(p=Math.abs(p)+1);let y=String(p).padStart(4,"0")+"-"+String(c.getUTCMonth()+1).padStart(2,"0")+"-"+String(c.getUTCDate()).padStart(2,"0")+"T"+String(c.getUTCHours()).padStart(2,"0")+":"+String(c.getUTCMinutes()).padStart(2,"0")+":"+String(c.getUTCSeconds()).padStart(2,"0")+"."+String(c.getUTCMilliseconds()).padStart(3,"0");return y+="+00:00",h&&(y+=" BC"),y}function d(c,p,h){return c=typeof c=="string"?{text:c}:c,p&&(typeof p=="function"?c.callback=p:c.values=p),h&&(c.callback=h),c}return wr={prepareValue:function(p){return i(p)},normalizeQueryConfig:d,escapeIdentifier:function(c){return'"'+c.replace(/"/g,'""')+'"'},escapeLiteral:function(c){let p=!1,h="'";if(c==null||typeof c!="string")return"''";for(let y=0;y<c.length;y++){const v=c[y];v==="'"?h+=v+v:v==="\\"?(h+=v+v,p=!0):h+=v}return h+="'",p===!0&&(h=" E"+h),h}},wr}var dt={exports:{}},Er,Ys;function Xo(){if(Ys)return Er;Ys=1;const e=Pn;function t(l){return e.createHash("md5").update(l,"utf-8").digest("hex")}function r(l,u,d){const a=t(u+l);return"md5"+t(Buffer.concat([Buffer.from(a),d]))}function s(l){return e.createHash("sha256").update(l).digest()}function n(l,u){return l=l.replace(/(\D)-/,"$1"),e.createHash(l).update(u).digest()}function i(l,u){return e.createHmac("sha256",l).update(u).digest()}async function o(l,u,d){return e.pbkdf2Sync(l,u,d,32,"sha256")}return Er={postgresMd5PasswordHash:r,randomBytes:e.randomBytes,deriveKey:o,sha256:s,hashByName:n,hmacSha256:i,md5:t},Er}var _r,Js;function Yo(){if(Js)return _r;Js=1;const e=Pn;_r={postgresMd5PasswordHash:o,randomBytes:n,deriveKey:a,sha256:l,hashByName:u,hmacSha256:d,md5:i};const t=e.webcrypto||globalThis.crypto,r=t.subtle,s=new TextEncoder;function n(f){return t.getRandomValues(Buffer.alloc(f))}async function i(f){try{return e.createHash("md5").update(f,"utf-8").digest("hex")}catch{const p=typeof f=="string"?s.encode(f):f,h=await r.digest("MD5",p);return Array.from(new Uint8Array(h)).map(y=>y.toString(16).padStart(2,"0")).join("")}}async function o(f,c,p){const h=await i(c+f);return"md5"+await i(Buffer.concat([Buffer.from(h),p]))}async function l(f){return await r.digest("SHA-256",f)}async function u(f,c){return await r.digest(f,c)}async function d(f,c){const p=await r.importKey("raw",f,{name:"HMAC",hash:"SHA-256"},!1,["sign"]);return await r.sign("HMAC",p,s.encode(c))}async function a(f,c,p){const h=await r.importKey("raw",s.encode(f),"PBKDF2",!1,["deriveBits"]),y={name:"PBKDF2",hash:"SHA-256",salt:c,iterations:p};return await r.deriveBits(y,h,256,["deriveBits"])}return _r}var Gs;function ci(){return Gs||(Gs=1,parseInt(process.versions&&process.versions.node&&process.versions.node.split(".")[0])<15?dt.exports=Xo():dt.exports=Yo()),dt.exports}var xr,Ks;function Jo(){if(Ks)return xr;Ks=1;function e(i,o){return new Error("SASL channel binding: "+i+" when parsing public certificate "+o.toString("base64"))}function t(i,o){let l=i[o++];if(l<128)return{length:l,index:o};const u=l&127;if(u>4)throw e("bad length",i);l=0;for(let d=0;d<u;d++)l=l<<8|i[o++];return{length:l,index:o}}function r(i,o){if(i[o++]!==6)throw e("non-OID data",i);const{length:l,index:u}=t(i,o);o=u;const d=o+l,a=i[o++];let f=(a/40>>0)+"."+a%40;for(;o<d;){let c=0;for(;o<d;){const p=i[o++];if(c=c<<7|p&127,p<128)break}f+="."+c}return{oid:f,index:o}}function s(i,o){if(i[o++]!==48)throw e("non-sequence data",i);return t(i,o)}function n(i,o){o===void 0&&(o=0),o=s(i,o).index;const{length:l,index:u}=s(i,o);o=u+l,o=s(i,o).index;const{oid:d,index:a}=r(i,o);switch(d){case"1.2.840.113549.1.1.4":return"MD5";case"1.2.840.113549.1.1.5":return"SHA-1";case"1.2.840.113549.1.1.11":return"SHA-256";case"1.2.840.113549.1.1.12":return"SHA-384";case"1.2.840.113549.1.1.13":return"SHA-512";case"1.2.840.113549.1.1.14":return"SHA-224";case"1.2.840.113549.1.1.15":return"SHA512-224";case"1.2.840.113549.1.1.16":return"SHA512-256";case"1.2.840.10045.4.1":return"SHA-1";case"1.2.840.10045.4.3.1":return"SHA-224";case"1.2.840.10045.4.3.2":return"SHA-256";case"1.2.840.10045.4.3.3":return"SHA-384";case"1.2.840.10045.4.3.4":return"SHA-512";case"1.2.840.113549.1.1.10":{if(o=a,o=s(i,o).index,i[o++]!==160)throw e("non-tag data",i);o=t(i,o).index,o=s(i,o).index;const{oid:f}=r(i,o);switch(f){case"1.2.840.113549.2.5":return"MD5";case"1.3.14.3.2.26":return"SHA-1";case"2.16.840.1.101.3.4.2.1":return"SHA-256";case"2.16.840.1.101.3.4.2.2":return"SHA-384";case"2.16.840.1.101.3.4.2.3":return"SHA-512"}throw e("unknown hash OID "+f,i)}case"1.3.101.110":case"1.3.101.112":return"SHA-512";case"1.3.101.111":case"1.3.101.113":throw e("Ed448 certificate channel binding is not currently supported by Postgres")}throw e("unknown OID "+d,i)}return xr={signatureAlgorithmHashFromCertificate:n},xr}var Sr,Zs;function Go(){if(Zs)return Sr;Zs=1;const e=ci(),{signatureAlgorithmHashFromCertificate:t}=Jo();function r(f,c){const p=["SCRAM-SHA-256"];c&&p.unshift("SCRAM-SHA-256-PLUS");const h=p.find(m=>f.includes(m));if(!h)throw new Error("SASL: Only mechanism(s) "+p.join(" and ")+" are supported");if(h==="SCRAM-SHA-256-PLUS"&&typeof c.getPeerCertificate!="function")throw new Error("SASL: Mechanism SCRAM-SHA-256-PLUS requires a certificate");const y=e.randomBytes(18).toString("base64");return{mechanism:h,clientNonce:y,response:(h==="SCRAM-SHA-256-PLUS"?"p=tls-server-end-point":c?"y":"n")+",,n=*,r="+y,message:"SASLInitialResponse"}}async function s(f,c,p,h){if(f.message!=="SASLInitialResponse")throw new Error("SASL: Last message was not SASLInitialResponse");if(typeof c!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string");if(c==="")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a non-empty string");if(typeof p!="string")throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: serverData must be a string");const y=u(p);if(y.nonce.startsWith(f.clientNonce)){if(y.nonce.length===f.clientNonce.length)throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce is too short")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: server nonce does not start with client nonce");const v="n=*,r="+f.clientNonce,m="r="+y.nonce+",s="+y.salt+",i="+y.iteration;let b=h?"eSws":"biws";if(f.mechanism==="SCRAM-SHA-256-PLUS"){const S=h.getPeerCertificate().raw;let q=t(S);(q==="MD5"||q==="SHA-1")&&(q="SHA-256");const D=await e.hashByName(q,S);b=Buffer.concat([Buffer.from("p=tls-server-end-point,,"),Buffer.from(D)]).toString("base64")}const w="c="+b+",r="+y.nonce,x=v+","+m+","+w,I=Buffer.from(y.salt,"base64"),A=await e.deriveKey(c,I,y.iteration),g=await e.hmacSha256(A,"Client Key"),E=await e.sha256(g),_=await e.hmacSha256(E,x),T=a(Buffer.from(g),Buffer.from(_)).toString("base64"),C=await e.hmacSha256(A,"Server Key"),O=await e.hmacSha256(C,x);f.message="SASLResponse",f.serverSignature=Buffer.from(O).toString("base64"),f.response=w+",p="+T}function n(f,c){if(f.message!=="SASLResponse")throw new Error("SASL: Last message was not SASLResponse");if(typeof c!="string")throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: serverData must be a string");const{serverSignature:p}=d(c);if(p!==f.serverSignature)throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature does not match")}function i(f){if(typeof f!="string")throw new TypeError("SASL: text must be a string");return f.split("").map((c,p)=>f.charCodeAt(p)).every(c=>c>=33&&c<=43||c>=45&&c<=126)}function o(f){return/^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$/.test(f)}function l(f){if(typeof f!="string")throw new TypeError("SASL: attribute pairs text must be a string");return new Map(f.split(",").map(c=>{if(!/^.=/.test(c))throw new Error("SASL: Invalid attribute pair entry");const p=c[0],h=c.substring(2);return[p,h]}))}function u(f){const c=l(f),p=c.get("r");if(p){if(!i(p))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce must only contain printable characters")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: nonce missing");const h=c.get("s");if(h){if(!o(h))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt must be base64")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: salt missing");const y=c.get("i");if(y){if(!/^[1-9][0-9]*$/.test(y))throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: invalid iteration count")}else throw new Error("SASL: SCRAM-SERVER-FIRST-MESSAGE: iteration missing");const v=parseInt(y,10);return{nonce:p,salt:h,iteration:v}}function d(f){const p=l(f).get("v");if(p){if(!o(p))throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature must be base64")}else throw new Error("SASL: SCRAM-SERVER-FINAL-MESSAGE: server signature is missing");return{serverSignature:p}}function a(f,c){if(!Buffer.isBuffer(f))throw new TypeError("first argument must be a Buffer");if(!Buffer.isBuffer(c))throw new TypeError("second argument must be a Buffer");if(f.length!==c.length)throw new Error("Buffer lengths must match");if(f.length===0)throw new Error("Buffers cannot be empty");return Buffer.from(f.map((p,h)=>f[h]^c[h]))}return Sr={startSession:r,continueSession:s,finalizeSession:n},Sr}var Ar,en;function Gr(){if(en)return Ar;en=1;const e=St();function t(r){this._types=r||e,this.text={},this.binary={}}return t.prototype.getOverrides=function(r){switch(r){case"text":return this.text;case"binary":return this.binary;default:return{}}},t.prototype.setTypeParser=function(r,s,n){typeof s=="function"&&(n=s,s="text"),this.getOverrides(s)[r]=n},t.prototype.getTypeParser=function(r,s){return s=s||"text",this.getOverrides(s)[r]||this._types.getTypeParser(r,s)},Ar=t,Ar}var Tr,tn;function Ko(){if(tn)return Tr;tn=1;function e(i,o={}){if(i.charAt(0)==="/"){const p=i.split(" ");return{host:p[0],database:p[1]}}const l={};let u,d=!1;/ |%[^a-f0-9]|%[a-f0-9][^a-f0-9]/i.test(i)&&(i=encodeURI(i).replace(/%25(\d\d)/g,"%$1"));try{try{u=new URL(i,"postgres://base")}catch{u=new URL(i.replace("@/","@___DUMMY___/"),"postgres://base"),d=!0}}catch(p){throw p.input&&(p.input="*****REDACTED*****"),p}for(const p of u.searchParams.entries())l[p[0]]=p[1];if(l.user=l.user||decodeURIComponent(u.username),l.password=l.password||decodeURIComponent(u.password),u.protocol=="socket:")return l.host=decodeURI(u.pathname),l.database=u.searchParams.get("db"),l.client_encoding=u.searchParams.get("encoding"),l;const a=d?"":u.hostname;l.host?a&&/^%2f/i.test(a)&&(u.pathname=a+u.pathname):l.host=decodeURIComponent(a),l.port||(l.port=u.port);const f=u.pathname.slice(1)||null;l.database=f?decodeURI(f):null,(l.ssl==="true"||l.ssl==="1")&&(l.ssl=!0),l.ssl==="0"&&(l.ssl=!1),(l.sslcert||l.sslkey||l.sslrootcert||l.sslmode)&&(l.ssl={});const c=l.sslcert||l.sslkey||l.sslrootcert?Dn:null;if(l.sslcert&&(l.ssl.cert=c.readFileSync(l.sslcert).toString()),l.sslkey&&(l.ssl.key=c.readFileSync(l.sslkey).toString()),l.sslrootcert&&(l.ssl.ca=c.readFileSync(l.sslrootcert).toString()),o.useLibpqCompat&&l.uselibpqcompat)throw new Error("Both useLibpqCompat and uselibpqcompat are set. Please use only one of them.");if(l.uselibpqcompat==="true"||o.useLibpqCompat)switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":{l.ssl.rejectUnauthorized=!1;break}case"require":{l.sslrootcert?l.ssl.checkServerIdentity=function(){}:l.ssl.rejectUnauthorized=!1;break}case"verify-ca":{if(!l.ssl.ca)throw new Error("SECURITY WARNING: Using sslmode=verify-ca requires specifying a CA with sslrootcert. If a public CA is used, verify-ca allows connections to a server that somebody else may have registered with the CA, making you vulnerable to Man-in-the-Middle attacks. Either specify a custom CA certificate with sslrootcert parameter or use sslmode=verify-full for proper security.");l.ssl.checkServerIdentity=function(){};break}}else switch(l.sslmode){case"disable":{l.ssl=!1;break}case"prefer":case"require":case"verify-ca":case"verify-full":{l.sslmode!=="verify-full"&&n(l.sslmode);break}case"no-verify":{l.ssl.rejectUnauthorized=!1;break}}return l}function t(i){return Object.entries(i).reduce((l,[u,d])=>(d!=null&&(l[u]=d),l),{})}function r(i){return Object.entries(i).reduce((l,[u,d])=>{if(u==="ssl"){const a=d;typeof a=="boolean"&&(l[u]=a),typeof a=="object"&&(l[u]=t(a))}else if(d!=null)if(u==="port"){if(d!==""){const a=parseInt(d,10);if(isNaN(a))throw new Error(`Invalid ${u}: ${d}`);l[u]=a}}else l[u]=d;return l},{})}function s(i){return r(e(i))}function n(i){!n.warned&&typeof process<"u"&&process.emitWarning&&(n.warned=!0,process.emitWarning(`SECURITY WARNING: The SSL modes 'prefer', 'require', and 'verify-ca' are treated as aliases for 'verify-full'.
